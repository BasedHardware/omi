...
### PRIORITY 0: Pre-existing Untranslated Keys (~162 keys)
**Status: PENDING - CRITICAL**

~162 keys in non-English ARB files still contain English text instead of proper translations. This affects all non-English users.
...
| `providers/onboarding_provider.dart` | ~10 | Permission error messages (Bluetooth, Microphone, Screen Capture, Accessibility, Notification) |
| `providers/message_provider.dart` | ~12 | Camera/photo/file picker error messages |
| `providers/capture_provider.dart` | ~5 | Recording error messages |
| `providers/developer_mode_provider.dart` | ~5 | Webhook validation errors |
| `providers/voice_recorder_provider.dart` | 1 | Transcription error |

#### 6.2 UI Components (~15 strings)

| File | Est. Strings | Description |
|------|--------------|-------------|
| `pages/settings/widgets/plans_sheet.dart` | 3 | Remaining: "Cancel Subscription", "Omi Training" (x2) |
| `pages/conversations/sync_widgets/location_permission_dialog.dart` | 3 | Location permission dialog text |
| `pages/conversations/widgets/merge_action_bar.dart` | 1 | "OK" label |

#### 6.3 PDF Export Strings (~10 strings)

| File | Est. Strings | Description |
|------|--------------|-------------|
| `pages/conversation_detail/share.dart` | ~10 | PDF export labels (Transcript Export, Conversation Export, Title, Overview, Action Items, Summary) |
