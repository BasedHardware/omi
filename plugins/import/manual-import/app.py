...
client = openai.OpenAI(
    ...
    http_client=httpx.Client(
        limits=httpx.Limits(max_keepalive_connections=5, max_connections=10), timeout=httpx.Timeout(timeout=30.0)
    ),
)
            ...
            if bullet_points:
                ...
                for i, point in enumerate(bullet_points[1:], start=1):
                    ...
                    if len(current_memory + connector + point_text) <= MAX_MEMORY_LENGTH:
                        ...
                        print(
                            f"  📌 Extracted consolidated memory from section '{section_title}' (part {len(consolidated_memories)})"
                        )
...
def submit_memories():
    ...
    try:
        ...
        if not user_id:
            ...
            return (
                jsonify({"success": False, "error": "No user ID provided. Please include 'uid' in your request."}),
                400,
            )
        ...
        for memory in all_memories:
            ...
            # The integration API stores imported facts as memories.
            ...
            # Send the request to the documented memories endpoint with dynamic user_id.
        ...
        print("\n" + "=" * 50)
        print(
            f"📊 SUMMARY: Processed {len(all_memories)} consolidated memories for user {user_id}. {success_count} succeeded, {error_count} failed."
        )
