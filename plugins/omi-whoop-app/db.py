"""
Redis-based storage for Whoop tokens and user settings.
...
def store_whoop_tokens(uid: str, access_token: str, refresh_token: str, expires_at: str):
    """Store Whoop OAuth2 tokens for a user."""
    import sys
    r = _get_redis()
...
def get_whoop_tokens(uid: str) -> Optional[Dict[str, Any]]:
...
def update_whoop_tokens(uid: str, access_token: str, refresh_token: str, expires_at: str):
    """Update Whoop tokens."""
    import sys
    r = _get_redis()
...
def delete_whoop_tokens(uid: str):
    """Delete Whoop tokens for a user."""
    r = _get_redis()
...
def store_oauth_state(state: str, uid: str):
    """Store OAuth state -> uid mapping for CSRF verification.

    Whoop requires state to be exactly 8 characters, so we store
    the state as the key and uid as the value.
    """
    r = _get_redis()

    if r:
        key = f"whoop:oauth_state:{state}"
        r.set(key, uid)
...
def get_uid_from_oauth_state(state: str) -> Optional[str]:
    """Get uid from stored OAuth state."""
    r = _get_redis()
...
def store_user_setting(uid: str, key: str, value: Any):
    ...
    if r:
        redis_key = f"whoop:settings:{uid}"
