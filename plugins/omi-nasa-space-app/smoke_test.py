...
def run_smoke_tests() -> None:
    ...
    with TestClient(app) as client:
        ...
        print(f"   ✓ Health probe passed: status={data.get('status')}, upstream={data.get('upstream_nasa_api')}")
        ...
        expected_tools = ["get_astronomy_picture", "get_near_earth_asteroids", "search_nasa_media"]
        ...
        print("   ✓ APOD retrieved successfully with full title, explanation, and media links")
        ...
        assert "Miss Distance:" in neo_text or "No near-Earth asteroid encounters" in neo_text
        print("   ✓ Near-Earth asteroid trajectories successfully tracked and calculated")

        # 5. Search NASA Official Media Archive (James Webb)
        print("\n5. Testing POST /tools/search-nasa-media with query 'James Webb'...")
        resp = client.post("/tools/search-nasa-media", json={"query": "James Webb", "limit": 3})
        assert resp.status_code == 200
        media_text = resp.json().get("result", "")
        assert "NASA Official Media Archive" in media_text
        assert "NASA ID:" in media_text
        print("   ✓ NASA multimedia search returned valid mission assets and preview links")

        # 6. Search with Zero Results
        print("\n6. Testing POST /tools/search-nasa-media with non-existent query...")
        resp = client.post("/tools/search-nasa-media", json={"query": "zzzzzzzzzzzzzzzzzzzzzzzzz"})
        assert resp.status_code == 200
