"""
Redis-based storage for Twitter tokens and user settings.
...
def store_twitter_tokens(uid: str, access_token: str, refresh_token: str, expires_at: str, username: str = "", twitter_user_id: str = ""):
    """Store Twitter OAuth2 tokens for a user."""
    import sys
    r = _get_redis()
...
def get_twitter_tokens(uid: str) -> Optional[Dict[str, Any]]:
...
def update_twitter_tokens(uid: str, access_token: str, refresh_token: str, expires_at: str):
    ...
    if r:
        ...
        if data:
            ...
            print(f"DB: Updated tokens in Redis")
            sys.stdout.flush()
    else:
        tokens = _load_json(TOKENS_FILE)
        if uid in tokens:
            tokens[uid]["access_token"] = access_token
            tokens[uid]["refresh_token"] = refresh_token
            tokens[uid]["expires_at"] = expires_at
            tokens[uid]["updated_at"] = datetime.utcnow().isoformat()
            _save_json(TOKENS_FILE, tokens)
            print(f"DB: Updated tokens in file")
            sys.stdout.flush()


def delete_twitter_tokens(uid: str):
...
def store_user_setting(uid: str, key: str, value: Any):
    ...
    if r:
        redis_key = f"twitter:settings:{uid}"
