"""Hermetic regression tests for the Hive chat-tool shared-secret guard.

Covers the `hive_tools_auth` dependency (fail-closed 503 when unconfigured, 401
on missing/wrong token, accept on Bearer header or `hive_tools_token` query
param) and asserts every chat-tool route is wired to it.
"""
...
import hive_tools_auth as auth

TEST_SECRET = 'test-hive-tools-secret'

PROTECTED_PATHS = {
    '/tools/hive_get_projects',
    '/tools/hive_get_tasks',
    '/tools/hive_create_task',
    '/tools/hive_search',
    '/tools/hive_update_task_status',
}
...
class HiveToolsAuthUnitTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_401_when_no_token(self):
        os.environ['HIVE_TOOLS_SECRET'] = TEST_SECRET
        with self.assertRaises(HTTPException) as ctx:
            auth.require_hive_tools_auth(_FakeRequest())
    ...
    def test_401_on_wrong_query_token(self):
        ...
        with self.assertRaises(HTTPException) as ctx:
            auth.require_hive_tools_auth(
                _FakeRequest(query_params={'hive_tools_token': 'wrong'})
            )
        self.assertEqual(ctx.exception.status_code, 401)
    ...
    def test_accepts_query_token(self):
        ...
        self.assertIsNone(
            auth.require_hive_tools_auth(
                _FakeRequest(query_params={'hive_tools_token': TEST_SECRET})
            )
        )


class HiveRoutesAuthWiringTests(unittest.TestCase):
    ...
    def test_all_chat_tool_routes_require_shared_secret(self):
        ...
        for route in self.app.routes:
            for dep in getattr(route, 'dependencies', None) or []:
                ...
                if getattr(callable_, '__name__', '') == 'require_hive_tools_auth':
...
try:
    ...
    class HiveToolsAuthIntegrationTests(unittest.TestCase):
        @classmethod
        def setUpClass(cls):
        ...
        def test_unauthenticated_request_is_rejected(self):
            os.environ['HIVE_TOOLS_SECRET'] = TEST_SECRET
            response = self.client.post('/tools/hive_get_projects', json={'uid': 'victim'})
