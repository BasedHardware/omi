...
on that user's stored Hive token. They must only be reachable by the trusted
...
or a `hive_tools_token` query parameter, compared in constant time against the
`HIVE_TOOLS_SECRET` environment variable.
...
_HIVE_TOOLS_SECRET_ENV = 'HIVE_TOOLS_SECRET'
_QUERY_TOKEN_PARAM = 'hive_tools_token'


def _configured_secret() -> str | None:
    secret = os.getenv(_HIVE_TOOLS_SECRET_ENV)
    if secret is None:
        return None
    secret = secret.strip()
...
def require_hive_tools_auth(request: Request) -> None:
    """Reject unauthenticated callers of the Hive chat-tool routes."""
    secret = _configured_secret()
    if secret is None:
        raise HTTPException(status_code=503, detail='hive tools auth is not configured')
