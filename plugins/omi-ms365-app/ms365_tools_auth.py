...
The chat-tool dispatcher under `/tools/{tool_name}` reads `uid` from the
request body and acts on that user's stored Microsoft 365 token. It must only
be reachable by the trusted Omi backend, so the route is gated behind this
dependency: the caller presents a shared secret either as an
`Authorization: Bearer <secret>` header or a `ms365_tools_token` query
parameter, compared in constant time against the `MS365_TOOLS_SECRET`
...
def require_ms365_tools_auth(request: Request) -> None:
    """Reject unauthenticated callers of the MS365 chat-tool routes."""
