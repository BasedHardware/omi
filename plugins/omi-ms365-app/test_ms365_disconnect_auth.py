"""Hermetic regression tests for the Microsoft 365 disconnect HMAC guard.

Covers the `ms365_disconnect_auth` dependency: fail-closed 503 when the signing secret is
...
TEST_SECRET = "test-ms365-disconnect-secret"


class Ms365DisconnectAuthUnitTests(unittest.TestCase):
    def setUp(self):
        self._old = os.environ.get("MS365_DISCONNECT_SECRET")

    def tearDown(self):
        if self._old is None:
            os.environ.pop("MS365_DISCONNECT_SECRET", None)
        else:
            os.environ["MS365_DISCONNECT_SECRET"] = self._old
    ...
    def test_401_on_missing_or_wrong_signature(self):
        os.environ["MS365_DISCONNECT_SECRET"] = TEST_SECRET
        with self.assertRaises(HTTPException) as ctx:
            auth.require_disconnect_auth("test-uid", "")
