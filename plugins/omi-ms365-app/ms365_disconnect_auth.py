"""Shared-secret (HMAC) guard for Microsoft 365 disconnect links.
...
The signing secret is read from the ``MS365_DISCONNECT_SECRET`` environment
...
_MS365_DISCONNECT_SECRET_ENV = "MS365_DISCONNECT_SECRET"


def _configured_secret() -> str | None:
    """Return the configured secret, or None if not set."""
    secret = os.getenv(_MS365_DISCONNECT_SECRET_ENV)
    if secret is None:
        return None
    secret = secret.strip()
...
def require_disconnect_auth(uid: str, sig: str) -> None:
    ...
    if not secret:
        ...
        raise HTTPException(status_code=503, detail="ms365 disconnect auth is not configured")
