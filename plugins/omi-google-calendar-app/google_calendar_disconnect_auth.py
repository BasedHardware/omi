"""Shared-secret (HMAC) guard for Google Calendar disconnect links.
...
The signing secret is read from the ``GOOGLE_CALENDAR_DISCONNECT_SECRET``
environment variable.  Mirror the structure of
...
_GOOGLE_CALENDAR_DISCONNECT_SECRET_ENV = "GOOGLE_CALENDAR_DISCONNECT_SECRET"


def _configured_secret() -> str | None:
    """Return the configured secret, or None if not set."""
    secret = os.getenv(_GOOGLE_CALENDAR_DISCONNECT_SECRET_ENV)
    if secret is None:
        return None
    secret = secret.strip()
...
def require_disconnect_auth(uid: str, sig: str) -> None:
    ...
    if not secret:
        ...
        raise HTTPException(status_code=503, detail="google calendar disconnect auth is not configured")
