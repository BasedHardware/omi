"""Hermetic regression tests for the Google Calendar chat-tool shared-secret guard.

Covers the `google_calendar_tools_auth` dependency (fail-closed 503 when
unconfigured, 401 on missing/wrong token, accept on Bearer header or
`google_calendar_tools_token` query param) and asserts every chat-tool route is
wired to it.
"""
...
import google_calendar_tools_auth as auth

TEST_SECRET = 'test-google-calendar-tools-secret'
...
class GoogleCalendarToolsAuthUnitTests(unittest.TestCase):
    def setUp(self):
        self._old = os.environ.get('GOOGLE_CALENDAR_TOOLS_SECRET')

    def tearDown(self):
        if self._old is None:
            os.environ.pop('GOOGLE_CALENDAR_TOOLS_SECRET', None)
        else:
            os.environ['GOOGLE_CALENDAR_TOOLS_SECRET'] = self._old

    def test_fails_closed_when_unconfigured(self):
        os.environ.pop('GOOGLE_CALENDAR_TOOLS_SECRET', None)
        with self.assertRaises(HTTPException) as ctx:
            auth.require_google_calendar_tools_auth(_FakeRequest())
        self.assertEqual(ctx.exception.status_code, 503)

    def test_401_when_no_token(self):
        os.environ['GOOGLE_CALENDAR_TOOLS_SECRET'] = TEST_SECRET
        with self.assertRaises(HTTPException) as ctx:
            auth.require_google_calendar_tools_auth(_FakeRequest())
    ...
    def test_401_on_wrong_bearer_token(self):
        os.environ['GOOGLE_CALENDAR_TOOLS_SECRET'] = TEST_SECRET
        with self.assertRaises(HTTPException) as ctx:
            auth.require_google_calendar_tools_auth(
    ...
    def test_401_on_wrong_query_token(self):
        ...
        with self.assertRaises(HTTPException) as ctx:
            auth.require_google_calendar_tools_auth(
                _FakeRequest(query_params={'google_calendar_tools_token': 'wrong'})
            )
        self.assertEqual(ctx.exception.status_code, 401)
    ...
    def test_accepts_query_token(self):
        ...
        self.assertIsNone(
            auth.require_google_calendar_tools_auth(
                _FakeRequest(query_params={'google_calendar_tools_token': TEST_SECRET})
            )
        )


class GoogleCalendarRoutesAuthWiringTests(unittest.TestCase):
    ...
    def test_all_chat_tool_routes_require_shared_secret(self):
        ...
        for route in self.app.routes:
            for dep in getattr(route, 'dependencies', None) or []:
                ...
                if getattr(callable_, '__name__', '') == 'require_google_calendar_tools_auth':
...
try:
    ...
    class GoogleCalendarToolsAuthIntegrationTests(unittest.TestCase):
        @classmethod
        def setUpClass(cls):
        ...
        def test_unauthenticated_request_is_rejected(self):
            os.environ['GOOGLE_CALENDAR_TOOLS_SECRET'] = TEST_SECRET
            response = self.client.post('/tools/list_events', json={'uid': 'victim'})
