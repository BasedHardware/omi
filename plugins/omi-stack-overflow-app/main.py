...
def _clean_text(value: Optional[str]) -> str:
    ...
    # Strip provider markup before decoding entities so code generics such as &lt;vector&gt;
    # are preserved rather than turned into apparent HTML tags and stripped.
    text = re.sub(r"</?(p|pre|blockquote|ul|ol|li|h[1-6])[^>]*>", "\n", value, flags=re.IGNORECASE)
...
def _format_answer(item: dict[str, Any], index: int) -> str:
    ...
    owner = (owner_raw.get("display_name") if isinstance(owner_raw, dict) else None) or "unknown"
    score = item.get("score", 0)
