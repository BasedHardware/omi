...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(log_dir / "mentor.log"), logging.StreamHandler()],
)
...
class MessageBuffer:
    def __init__(self):
        ...
        logger.debug(
            f"MessageBuffer initialized with cleanup_interval={self.cleanup_interval}, silence_threshold={self.silence_threshold}"
        )
        ...
        with self.lock:
            if session_id not in self.buffers:
                ...
                if time_since_activity > self.silence_threshold:
                    logger.info(
                        f"Silence period detected for session {session_id}. Time since activity: {time_since_activity:.2f}s"
                    )
    ...
    def cleanup_old_sessions(self):
        ...
        with self.lock:
            ...
            logger.info(
                f"Cleanup complete. Removed {len(expired_sessions)} sessions. Active sessions: {len(self.buffers)}"
            )
        ...
        with self.lock:
            ...
            for session_id, message_dict in self.last_notification_times.items():
                for message_id, last_time in message_dict.items():
                    ...
                    logger.info(
                        f"  - Time since last notification: {time_since_notification:.1f}s (threshold: {REMINDER_INTERVAL}s)"
                    )
                    logger.info(
                        f"  - Time since last reminder: {time_since_reminder:.1f}s (threshold: {REMINDER_INTERVAL}s)"
                    )

                    if (
                        time_since_notification >= REMINDER_INTERVAL and last_reminder == 0
                    ):  # Only if no reminder sent yet
...
def send_reminder_notification(session_id, message_id):
    ...
    try:
        ...
        logger.info(
            f"Sending reminder notification to {notification_url} for session {session_id}, message {message_id} with aid {APP_ID}"
        )
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"Failed to send reminder notification. Status code: {response.status_code}, Response: {response.text}"
            )
...
Remember: First evaluate silently, then either respond with empty string OR give direct, opinionated advice.""".format(
        ...
        try:
            ...
            for segment in segments:
                ...
                if text:
                    ...
                    logger.info(
                        f"Processing segment - is_user: {is_user}, timestamp: {timestamp}, text: {text[:50]}..."
                    )
                    ...
                    if buffer_data['silence_detected']:
                        ...
                        logger.info(
                            f"Words after silence: {buffer_data['words_after_silence']}/{message_buffer.min_words_after_silence} needed"
                        )
                    ...
                    if can_append:
                        logger.info(
                            f"Appending to existing message. Current length: {len(buffer_data['messages'][-1]['text'])}"
                        )
            ...
            if (
                (time_since_last_analysis >= ANALYSIS_INTERVAL or buffer_data['last_analysis_time'] == 0)
                and buffer_data['messages']
                and not buffer_data['silence_detected']
                and message_id
            ):  # Only proceed if we have a message_id
...
def status():
    ...
    return jsonify({"active_sessions": active_sessions, "uptime": uptime})
