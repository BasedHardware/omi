"""Hermetic regression tests for the Shopify chat-tool shared-secret guard.

Covers the `shopify_tools_auth` dependency (fail-closed 503 when unconfigured,
401 on missing/wrong token, accept on Bearer header or `shopify_tools_token`
...
import shopify_tools_auth as auth

TEST_SECRET = 'test-shopify-tools-secret'
...
class ShopifyToolsAuthUnitTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_401_when_no_token(self):
        os.environ['SHOPIFY_TOOLS_SECRET'] = TEST_SECRET
        with self.assertRaises(HTTPException) as ctx:
            auth.require_shopify_tools_auth(_FakeRequest())
    ...
    def test_401_on_wrong_bearer_token(self):
        os.environ['SHOPIFY_TOOLS_SECRET'] = TEST_SECRET
        with self.assertRaises(HTTPException) as ctx:
            auth.require_shopify_tools_auth(
    ...
    def test_401_on_wrong_query_token(self):
        ...
        with self.assertRaises(HTTPException) as ctx:
            auth.require_shopify_tools_auth(
                _FakeRequest(query_params={'shopify_tools_token': 'wrong'})
            )
        self.assertEqual(ctx.exception.status_code, 401)
    ...
    def test_accepts_query_token(self):
        ...
        self.assertIsNone(
            auth.require_shopify_tools_auth(
                _FakeRequest(query_params={'shopify_tools_token': TEST_SECRET})
            )
        )


class ShopifyRoutesAuthWiringTests(unittest.TestCase):
    ...
    def test_all_chat_tool_routes_require_shared_secret(self):
        ...
        for route in self.app.routes:
            for dep in getattr(route, 'dependencies', None) or []:
                ...
                if getattr(callable_, '__name__', '') == 'require_shopify_tools_auth':
...
try:
    ...
    class ShopifyToolsAuthIntegrationTests(unittest.TestCase):
        @classmethod
        def setUpClass(cls):
        ...
        def test_unauthenticated_request_is_rejected(self):
            os.environ['SHOPIFY_TOOLS_SECRET'] = TEST_SECRET
            response = self.client.post('/tools/get_analytics', json={'uid': 'victim'})
