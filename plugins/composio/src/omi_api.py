...
async def create_memory(memory: MemoryCreate, uid: str):
    ...
    if not success:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to create memory in OMI")
...
async def create_memories_batch(data: MemoryBatch):
    ...
    for memory_text in data.memories:
        success = create_fact(user_id=data.uid, text=memory_text, source="other", source_spec="notion")
...
async def process_pending_memories(uid: str, limit: int = 10):
    ...
    for memory in pending_memories:
        success = create_fact(
            user_id=uid, text=memory["memory_text"], source=memory["source"] or "other", source_spec="notion"
        )
