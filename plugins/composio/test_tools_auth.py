"""Hermetic regression tests for the Composio backend-route auth guard.

Before this fix, `/api/omi/facts`, `/api/omi/facts/batch` and
`/api/notion/{blocks,page}` trusted a client-supplied uid and created
memories in Omi with the app's integration key or read the user's Notion
workspace with that user's stored OAuth token — with no caller
authentication. Covers `require_composio_tools_auth`: 503 when
