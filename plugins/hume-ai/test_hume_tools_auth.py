"""Hermetic regression tests for the hume-ai shared-secret auth guard.

Before this fix, `/audio`, `/save-emotion-memory` and
`/force-send-notification` trusted a client-supplied `uid` query parameter and
wrote emotion memories into any user's Omi account with the app's integration
key or pushed notifications to any user — with no caller authentication.

Covers `require_hume_tools_auth`: 503 when unconfigured, 401 on missing/wrong
...
def _load_auth_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "hume_tools_auth", APP_DIR / "hume_tools_auth.py"
    )
