"""Hermetic regression tests for the ShipBob disconnect HMAC guard.

Covers the `shipbob_disconnect_auth` dependency: fail-closed 503 when the signing secret is
...
import shipbob_disconnect_auth as auth

TEST_SECRET = "test-shipbob-disconnect-secret"


class ShipBobDisconnectAuthUnitTests(unittest.TestCase):
    def setUp(self):
        self._old = os.environ.get("SHIPBOB_DISCONNECT_SECRET")

    def tearDown(self):
        if self._old is None:
            os.environ.pop("SHIPBOB_DISCONNECT_SECRET", None)
        else:
            os.environ["SHIPBOB_DISCONNECT_SECRET"] = self._old
    ...
    def test_401_on_missing_or_wrong_signature(self):
        os.environ["SHIPBOB_DISCONNECT_SECRET"] = TEST_SECRET
        with self.assertRaises(HTTPException) as ctx:
            auth.require_disconnect_auth("test-uid", "")
