"""Shared-secret (HMAC) guard for ShipBob disconnect links.
...
The signing secret is read from the ``SHIPBOB_DISCONNECT_SECRET`` environment
variable.  This mirrors the structure of the existing
``plugins/omi-shipbob-app/shipbob_tools_auth.py`` (merged via PR #14684),
extending the same fail-closed philosophy to disconnect endpoints.
"""
...
_SHIPBOB_DISCONNECT_SECRET_ENV = "SHIPBOB_DISCONNECT_SECRET"


def _configured_secret() -> str | None:
    """Return the configured secret, or None if not set."""
    secret = os.getenv(_SHIPBOB_DISCONNECT_SECRET_ENV)
    if secret is None:
        return None
    secret = secret.strip()
...
def require_disconnect_auth(uid: str, sig: str) -> None:
    ...
    if not secret:
        ...
        raise HTTPException(status_code=503, detail="shipbob disconnect auth is not configured")
