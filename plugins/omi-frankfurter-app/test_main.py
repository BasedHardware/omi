...
class HelperFunctionTests(unittest.TestCase):
    ...
    def test_format_decimal(self):
        ...
        self.assertEqual(main._format_decimal(0.00001234), "0.00001234")
        self.assertEqual(main._format_decimal("0.00000005"), "0.00000005")

    def test_frankfurter_base_url(self):
...
class FrankfurterToolTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_convert_currency_small_rates_precision(self):
        mock_data = {
            "amount": 1.0,
            "base": "USD",
            "date": "2026-09-15",
            "rates": {"SMALL": 0.000042},
        }
        with patch.object(main, "_request_json", new_callable=AsyncMock) as mock_req:
            mock_req.return_value = mock_data
            req = main.ConvertCurrencyRequest(amount=1, from_currency="USD", to_currencies=["SMALL"])
            resp = await main.convert_currency(req)
            self.assertIsNone(resp.error)
            self.assertIn("- SMALL: 0.000042", resp.result)
