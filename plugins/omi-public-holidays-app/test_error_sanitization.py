import pytest
import httpx

from .main import (
    get_public_holidays,
    get_next_public_holidays,
    get_long_weekends,
    list_supported_countries,
)
from .chat_tool_response import ChatToolResponse


@pytest.fixture
def mock_http_error(monkeypatch):
    """Monkeypatch httpx.get to raise an HTTPError for all calls."""
    def raise_http_error(*args, **kwargs):
        raise httpx.HTTPError("Mocked network failure")

    monkeypatch.setattr(httpx, "get", raise_http_error)


def test_get_public_holidays_error_sanitization(mock_http_error):
    response = get_public_holidays("US", 2024)
    assert isinstance(response, ChatToolResponse)
    assert response.error == (
        "An error occurred while fetching public holidays. Please try again later."
    )
    assert response.result is None


def test_get_next_public_holidays_error_sanitization(mock_http_error):
    response = get_next_public_holidays("US", count=3)
    assert isinstance(response, ChatToolResponse)
    assert response.error == (
        "An error occurred while fetching the next public holidays. Please try again later."
    )
    assert response.result is None


def test_get_long_weekends_error_sanitization(mock_http_error):
    response = get_long_weekends("US", 2024)
    assert isinstance(response, ChatToolResponse)
    assert response.error == (
        "An error occurred while fetching long weekend data. Please try again later."
    )
    assert response.result is None


def test_list_supported_countries_error_sanitization(mock_http_error):
    response = list_supported_countries()
    assert isinstance(response, ChatToolResponse)
    assert response.error == (
        "An error occurred while retrieving supported countries. Please try again later."
    )
    assert response.result is None
