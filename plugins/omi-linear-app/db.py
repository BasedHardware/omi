"""
Redis-based storage for Linear tokens and user settings.
...
def store_linear_tokens(uid: str, access_token: str, refresh_token: str, expires_at: int):
    """Store Linear tokens for a user."""
    r = _get_redis()
    ...
    if r:
        # Use Redis
        key = f"linear:tokens:{uid}"
        r.set(key, json.dumps(token_data))
        # Set expiry to 365 days (Linear tokens are long-lived)
...
def get_linear_tokens(uid: str) -> Optional[Dict[str, Any]]:
    """Get Linear tokens for a user."""
    r = _get_redis()
...
def delete_linear_tokens(uid: str):
    """Delete Linear tokens for a user."""
    r = _get_redis()
...
def store_user_setting(uid: str, key: str, value: Any):
    ...
    if r:
        redis_key = f"linear:settings:{uid}"
...
def store_default_team(uid: str, team_id: str, team_name: str):
    """Store the default team for a user."""
    store_user_setting(uid, "default_team", {
        "id": team_id,
        "name": team_name
    })


def get_default_team(uid: str) -> Optional[Dict[str, str]]:
    """Get the default team for a user."""
    return get_user_setting(uid, "default_team")
