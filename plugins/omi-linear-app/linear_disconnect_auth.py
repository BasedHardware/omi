"""Shared-secret (HMAC) guard for Linear disconnect links.
...
The signing secret is read from the ``LINEAR_DISCONNECT_SECRET`` environment
...
_LINEAR_DISCONNECT_SECRET_ENV = "LINEAR_DISCONNECT_SECRET"


def _configured_secret() -> str | None:
    """Return the configured secret, or None if not set."""
    secret = os.getenv(_LINEAR_DISCONNECT_SECRET_ENV)
    if secret is None:
        return None
    secret = secret.strip()
...
def require_disconnect_auth(uid: str, sig: str) -> None:
    ...
    if not secret:
        ...
        raise HTTPException(status_code=503, detail="linear disconnect auth is not configured")
