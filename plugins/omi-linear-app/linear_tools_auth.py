...
on that user's stored Linear token. They must only be reachable by the trusted
...
or a `linear_tools_token` query parameter, compared in constant time against
the `LINEAR_TOOLS_SECRET` environment variable.
...
_LINEAR_TOOLS_SECRET_ENV = 'LINEAR_TOOLS_SECRET'
_QUERY_TOKEN_PARAM = 'linear_tools_token'


def _configured_secret() -> str | None:
    secret = os.getenv(_LINEAR_TOOLS_SECRET_ENV)
    if secret is None:
        return None
    secret = secret.strip()
...
def require_linear_tools_auth(request: Request) -> None:
    """Reject unauthenticated callers of the Linear chat-tool routes."""
    secret = _configured_secret()
    if secret is None:
        raise HTTPException(status_code=503, detail='linear tools auth is not configured')
