"""Tests for the notion chat-tools auth guard (#14455).

Before this fix, all /tools/* routes trusted the uid in the JSON body —
anyone could act with the victim's stored notion credentials. Covers
require_notion_tools_auth: 503 when unconfigured, 401 on missing/wrong
...
SECRET = "test-notion-tools-secret"


def _load_auth_module():
    spec = importlib.util.spec_from_file_location(
        "notion_tools_auth", APP_DIR / "notion_tools_auth.py"
    )
...
class TestRequireAuth(unittest.TestCase):
    def setUp(self):
        ...
        os.environ["NOTION_TOOLS_SECRET"] = SECRET
    ...
    def test_wrong_token_rejected(self):
        ...
        with self.assertRaises(fastapi.HTTPException) as ctx:
            auth.require_notion_tools_auth(req)
        self.assertEqual(ctx.exception.status_code, 401)

    def test_bearer_token_accepted(self):
        req = _FakeRequest(headers={"Authorization": f"Bearer {SECRET}"})
        self.assertIsNone(auth.require_notion_tools_auth(req))

    def test_query_token_accepted(self):
        req = _FakeRequest(query_params={"notion_tools_token": SECRET})
        self.assertIsNone(auth.require_notion_tools_auth(req))
