...
class TimelineParser extends BaseTagParser {
  static final _timelinePattern = RegExp(r'<timeline([^>]*)>([\s\S]*?)</timeline>', caseSensitive: false);

  static final _eventPattern = RegExp(r'<event\s+([^>]*)>([\s\S]*?)</event>', caseSensitive: false);
  ...
  TimelineSegment? _parseTimeline(String timelineAttributes, String innerContent) {
    ...
    for (final eventMatch in _eventPattern.allMatches(innerContent)) {
      ...
      events.add(TimelineEventData.fromParsed(attributes: eventAttrs, innerContent: eventContent));
    }

    if (events.isEmpty) return null;

    return TimelineSegment(TimelineDisplayData(title: attributes['title'], events: events));
  }
}
