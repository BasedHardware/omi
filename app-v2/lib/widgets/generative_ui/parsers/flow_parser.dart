...
class FlowParser extends BaseTagParser {
  static final _flowPattern = RegExp(r'<flow\s+([^>]*)>([\s\S]*?)</flow>', caseSensitive: false);

  static final _stepPattern = RegExp(r'<step(?:\s+([^>]*))?>([^<]*)</step>', caseSensitive: false);
  ...
  FlowData? _parseFlow(String flowAttrString, String flowContent) {
    ...
    for (final stepMatch in _stepPattern.allMatches(flowContent)) {
      ...
      if (stepContent.trim().isNotEmpty) {
        steps.add(FlowStepData.fromParsed(attributes: stepAttrs, innerContent: stepContent));
      }
    }

    return FlowData(title: title, steps: steps);
  }
}
