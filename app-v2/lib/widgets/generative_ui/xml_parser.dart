...
/// A segment containing chart data (pie / donut / bar).
...
/// A segment containing an aggregated story briefing
/// (timeline + quotes + follow-ups).
...
class XmlTagParser {
  /// Non-journalist component parsers. HighlightParser is included to handle
  /// block-level highlights (multi-line, list items). Simple inline highlights
  /// are handled by the markdown renderer's HighlightSyntax.
  ...
  List<ContentSegment> parse(String content) {
    ...
    for (final parser in _baseParsers) {
      for (final match in parser.findMatches(content)) {
        matches.add(_TagMatch(start: match.start, end: match.end, match: match, parser: parser));
      }
    }
    ...
    if (hasJournalistTags) {
      ...
      if (journalistMatches.isNotEmpty) {
        ...
        briefingData = StoryBriefingData(timeline: timeline, quoteBoard: quoteBoard, followups: followups);
      }
    }
...
class _TagMatch {
  ...
  const _TagMatch({required this.start, required this.end, required this.match, required this.parser});
}
