...
class FlowStepData {
  ...
  const FlowStepData({required this.content, this.type = FlowStepType.main});

  factory FlowStepData.fromParsed({required Map<String, String> attributes, required String innerContent}) {
    return FlowStepData(content: innerContent.trim(), type: FlowStepType.fromString(attributes['type']));
  }
}
...
class FlowData {
  ...
  const FlowData({required this.title, this.steps = const []});
