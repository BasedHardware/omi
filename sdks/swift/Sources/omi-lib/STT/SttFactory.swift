    ...
    public static func makeStreaming(
        ...
        case .parakeet:
            guard let url = parakeetAPIURL, !url.isEmpty else {
                throw NSError(domain: "omi.stt", code: 3, userInfo: [NSLocalizedDescriptionKey: "HOSTED_PARAKEET_API_URL required"])
            }
            return OmiParakeetTranscriber(apiURLString: url, sampleRate: sampleRate, onTranscript: onTranscript)
        case .whisper:
            throw NSError(domain: "omi.stt", code: 4, userInfo: [NSLocalizedDescriptionKey: "Use OmiWhisperTranscriber for offline frames; streaming Whisper uses getLiveTranscription"])
        }
    }
}
