...
تحديث جلسة OAuth قاربت على الانتهاء دون إعادة تسجيل الدخول — ينطبق فقط على تسجيل الدخول عبر المتصفح (OAuth). بالنسبة لمفاتيح `omi_dev_*` لا يجري هذا الأمر أي تحديث؛ بدّل المفتاح من تطبيق الويب في `Developer → API Keys`:
...
| :---: | :--- | :--- |
| `0` | نجاح | اكتمل الأمر |
| `1` | خطأ في الاستدعاء | تحقق omi-cli نفسه (مثلاً `--browser` و`--api-key` معاً، خيار غير صالح، إدخال فارغ) |
| `2` | خطأ في الوصول أو الوسائط | لم يتم تسجيل الدخول، المفتاح غير صحيح أو منتهي — وكذلك أخطاء المحلل (راية غير معروفة، وسيط ناقص) |
...
إذا لم تحدد ملفاً شخصياً، يستخدم CLI قيمة متغير البيئة `OMI_PROFILE` أولاً، ثم الملف الشخصي النشط من ملف الإعدادات، ثم `default`. ترتيب الأولوية: `--profile`، ثم `OMI_PROFILE`، ثم الملف النشط في `~/.omi/config.toml`، ثم `default`.
