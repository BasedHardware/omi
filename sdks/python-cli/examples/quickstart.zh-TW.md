...
| --- | --- | --- |
| `0` | 成功 | — |
| `1` | 用法錯誤（參數錯誤、缺少參數、驗證失敗） | 檢查錯誤訊息 |
| `2` | 認證失敗 | 執行 `omi auth login` |
| `3` | 伺服器錯誤（5xx、連線失敗） | 檢查網路連線，稍後重試 |
| `4` | 觸發速率限制（429） | 等待錯誤訊息中的 `Retry-After` 時長後重試 |
| `5` | 找不到（404） | 確認資源 ID 是否正確 |

完整契約見 [`sdks/python-cli/README.md`](../README.md#exit-codes-stable-contract)；
離開碼由 `omi_cli/errors.py` 定義，腳本可據此區分「可重試」與「終止」錯誤。
