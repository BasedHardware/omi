...
### Custom key prefix and index name

Specify a custom key prefix or index name if you run multiple datasets on the same Redis instance:

```sh
python memories_to_redis.py memories.json -o memories.redis --key-prefix user1:memory: --index-name idx:user1_memories
```
...
Before or after inserting documents, create the RediSearch index using `redis-cli`:
...
Pipe the generated commands into `redis-cli`:
...
| `--index-name` | RediSearch index name | `idx:memories` |
