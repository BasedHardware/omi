# Convert a memory-list export to SQLite
...
def format_tags(tags):
    """Join tag list with '; ' for easy text query; handles lists or arbitrary values."""
    if tags is None:
        return ""
...
if __name__ == "__main__":
    ...
    print(f"{loaded} row(s) loaded, {added} new, {loaded - added} updated, {total} memory row(s) in database")
```
...
Query the database. Breakdown of memories by category:

```sh
python -m sqlite3 memories.sqlite "SELECT category, COUNT(*) AS n FROM memories GROUP BY category ORDER BY n DESC"
```

Search memory content:

```sh
python -m sqlite3 memories.sqlite "SELECT category, content, created_at FROM memories WHERE content LIKE '%meeting%' ORDER BY created_at DESC"
```

Find manually added or unreviewed memories:

```sh
python -m sqlite3 memories.sqlite "SELECT id, category, content FROM memories WHERE manually_added = 1"
```

`created_at` and `updated_at` are stored as UTC `YYYY-MM-DD HH:MM:SS` text, so
they sort chronologically and work with SQLite's `date()` and `strftime()`.
`tags` are joined with `; ` for straightforward substring matching, and the
entire source item is kept in `raw_json` for `json_extract`. The loader
validates every input file before writing, applies each run as a single
transaction, and replaces rows that share an `id`.
