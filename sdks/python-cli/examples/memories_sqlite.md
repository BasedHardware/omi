...
def rows_from(source):
    ...
    for item in items:
        ...
        memory_content = item.get("content") or item.get("text") or item.get("description") or ""
        rows.append((
            str(item_id),
            text(memory_content),
...
Find the newest work memories:

```sh
python -m sqlite3 memories.sqlite "SELECT id, content, created_at FROM memories WHERE category = 'work' ORDER BY created_at DESC LIMIT 10"
```

Search memories by keyword:

```sh
python -m sqlite3 memories.sqlite "SELECT content, category, created_at FROM memories WHERE content LIKE '%architecture%' ORDER BY created_at DESC"
```

Count manually added vs automatically extracted memories:

```sh
python -m sqlite3 memories.sqlite "SELECT CASE manually_added WHEN 1 THEN 'manual' ELSE 'automatic' END AS source, COUNT(*) AS count FROM memories GROUP BY manually_added"
```

`created_at` and `updated_at` are stored as UTC `YYYY-MM-DD HH:MM:SS` text, so they sort
chronologically and work with SQLite's `date()`, `datetime()`, and `strftime()`. `category`
and `manually_added` are indexed for fast filtering and grouping. Every original object is
preserved verbatim in `raw_json` for `json_extract`. The loader validates every input file
before writing, applies each run as a single transaction, and replaces rows that share an `id`,
so loading the same page twice leaves exactly one row per memory item.
