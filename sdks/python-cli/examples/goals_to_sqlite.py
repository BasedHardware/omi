...
def to_float(value):
    """Safely convert number to float or None."""
    if value is None:
        return None
    try:
        return float(value)
...
def compute_progress_pct(current, target):
    """Calculate numeric progress percentage safely."""
    try:
        c = float(current)
        t = float(target)
        if t > 0:
...
def boolean_to_int(value):
    """Normalize active flag to integer 0 or 1 for SQLite."""
...
def rows_from(source):
    ...
    for item in items:
        ...
        curr = to_float(item.get("current_value"))
        targ = to_float(item.get("target_value"))
        rows.append((
            str(item_id),
            text(title),
            ...
            curr,
            targ,
            compute_progress_pct(curr, targ),
            text(item.get("unit")),
            to_float(item.get("min_value")),
            to_float(item.get("max_value")),
