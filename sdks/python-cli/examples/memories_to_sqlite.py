#!/usr/bin/env python3
"""
Convert Omi memories JSON exports into a relational SQLite database.

Enables fast querying, category grouping, deduplication, and full-text search
across captured personal memories and learnings without third-party dependencies.
"""
...
def text(value):
    """Store loosely typed API fields as text; anything non-null is coerced."""
    if value is None:
        return None
...
def utc_stamp(value):
    """Normalise an ISO-8601 timestamp to UTC 'YYYY-MM-DD HH:MM:SS' for SQLite date functions."""
...
def boolean_to_int(value):
    """Normalize deleted status to integer 0 or 1 for SQLite."""
...
def rows_from(source):
    """Parse memory rows from a file path or '-' for stdin."""
    if source == "-":
    ...
    for item in items:
        ...
        content_val = item.get("content") or item.get("text") or item.get("title") or ""
        ...
        rows.append((
            ...
            boolean_to_int(item.get("deleted")),
            text(item.get("conversation_id") or item.get("source_id")),
