...
| **אימות בדפדפן (OAuth דרך Google/Apple)** | מחשב פיתוח אישי ותחנות עבודה מקומיות | `omi auth login --browser` (Google) או `omi auth login --browser --provider apple` |
...
# 2) API key — הדבקת מפתח מאתר app.omi.me (מומלץ לסוכנים/CI)
```
...
```bash
# אזהרה: העברת המפתח בארגומנט עלולה להישמר בהיסטוריית ה-shell; מומלץ להשתמש ב-`omi auth login` אינטראקטיבי
omi auth login --api-key omi_dev_...

# מומלץ לאוטומציה ו-CI/CD: הזרקת משתנה סביבה ממנהל סודות
export OMI_API_KEY=omi_dev_...
```

### בדיקת מצב האימות
* `omi auth status`: מציג את פרופיל האימות המקומי, את תוקף טוקן ה-OAuth אם זמין, וערך מוסתר חלקית (עובד במצב לא מקוון).
...
`omi-cli` תומך בפלט JSON מובנה בפקודות הנתונים. פקודות מטא כגון `--version` ו-`--help` עדיין מחזירות טקסט רגיל. בעבודה עם כלי עיבוד כגון `jq` או סקריפטים ב-Python, יש להעביר את הדגל `--json` כדגל **גלובלי** לפני פקודת המשנה:
...
| `1` | שגיאת שימוש/אימות נתונים (Usage Error) | שגיאות ולידציה באפליקציה או אפשרויות סותרות |
| `2` | שגיאת אימות/תחביר Click | אימות שגוי או שגיאות תחביר ופרמטרים של Click |
