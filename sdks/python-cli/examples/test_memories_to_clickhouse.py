...
candidate_paths = [
    Path(__file__).resolve().parent / "memories_to_clickhouse.py",
    Path(__file__).resolve().parent.parent / "examples" / "memories_to_clickhouse.py",
    Path("sdks/python-cli/examples/memories_to_clickhouse.py").resolve(),
    Path("examples/memories_to_clickhouse.py").resolve(),
]
script_path = next((p for p in candidate_paths if p.exists()), None)
if script_path is None:
    raise RuntimeError(f"Could not locate memories_to_clickhouse.py from {__file__}")
...
convert_memories_to_clickhouse = m2c.convert_memories_to_clickhouse
escape_clickhouse_array = m2c.escape_clickhouse_array
escape_clickhouse_string = m2c.escape_clickhouse_string
format_memory_for_clickhouse = m2c.format_memory_for_clickhouse
format_sql_insert = m2c.format_sql_insert
generate_clickhouse_schema = m2c.generate_clickhouse_schema
main = m2c.main
parse_clickhouse_datetime = m2c.parse_clickhouse_datetime
...
class MemoriesToClickHouseTests(unittest.TestCase):
    ...
    def test_escape_clickhouse_array(self):
        ...
        self.assertEqual(escape_clickhouse_array(["coffee", "tea"]), "['coffee', 'tea']")
    ...
    def test_main_cli_file_io(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            in_file.write_text(json.dumps([{"id": "cli-1", "content": "Test"}]), encoding="utf-8")
