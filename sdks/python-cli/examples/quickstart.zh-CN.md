...
| --- | --- | --- |
| `0` | 成功 | — |
| `1` | 用法错误（参数错误、缺少参数、校验失败） | 检查错误信息 |
| `2` | 认证失败 | 运行 `omi auth login` |
| `3` | 服务端错误（5xx、连接失败） | 检查网络连接，稍后重试 |
| `4` | 触发限流（429） | 等待错误信息中的 `Retry-After` 时长后重试 |
| `5` | 未找到（404） | 确认资源 ID 是否正确 |

完整契约见 [`sdks/python-cli/README.md`](../README.md#exit-codes-stable-contract)；
退出码由 `omi_cli/errors.py` 定义，脚本可据此区分「可重试」与「终止」错误。
