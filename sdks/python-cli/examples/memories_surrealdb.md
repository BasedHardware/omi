...
SurrealDB is a modern multi-model database that combines the simplicity of document stores with relational tables, graph traversals, and full-text search. This recipe generates idempotent schema definitions and `UPSERT` statements that seamlessly handle nested metadata, arrays, and typed timestamps.
...
```sh
surreal import --conn http://localhost:8000 --user root --pass root --ns omi --db ai memories.surql
```
