# Build a memory knowledge digest (categories, visibility, and tags)

Use this recipe to see a high-level executive summary of your Omi knowledge base
and captured memories: it turns one or more `memory list` exports into a
structured Markdown digest with category breakdowns, visibility splits, top tags,
and a knowledge index. It reads saved JSON exports, makes no network requests,
and produces a single clean Markdown file.
...
Save the following as `memories_digest.py`:
...
```sh
python memories_digest.py memories.json memories_digest.md
```
