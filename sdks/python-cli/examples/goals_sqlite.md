...
Use this recipe to store, query, and analyze your tracked Omi goals, targets,
progress metrics, and active habit milestones in a local SQLite database. It
reads saved JSON exports, makes no network requests, and normalises timestamps
to UTC text so SQLite date and time functions work seamlessly. You need
Python 3.10+ and an authenticated `omi-cli` for the initial export. The
`python -m sqlite3` interactive shell examples below require Python 3.12+.

Export up to 100 goals (including completed and inactive milestones):

```sh
omi --json goal list --limit 100 --include-inactive > goals_0.json
```

Check that the command succeeded before converting the file. The `--limit`
option is capped at 100 per request. Passing `--include-inactive` ensures both
in-progress and achieved goals are included.
...
Load the exported files (repeat with new exports at any time; duplicate IDs are updated idempotently):

```sh
python goals_to_sqlite.py goals.sqlite goals_0.json
```

Query the database. Count goals grouped by type and active status:

```sh
python -m sqlite3 goals.sqlite "SELECT goal_type, is_active, COUNT(*) AS count FROM goals GROUP BY goal_type, is_active ORDER BY count DESC"
```

List active numeric or scale goals with current progress versus target:

```sh
python -m sqlite3 goals.sqlite "SELECT id, title, current_value, target_value, unit FROM goals WHERE is_active = 1 ORDER BY created_at DESC"
```

Calculate progress completion percentage for numeric goals:

```sh
python -m sqlite3 goals.sqlite "SELECT title, current_value, target_value, ROUND((current_value * 100.0) / target_value, 1) AS pct_done FROM goals WHERE target_value > 0 AND is_active = 1"
```

Search goals by keyword in title:

```sh
python -m sqlite3 goals.sqlite "SELECT id, title, goal_type, is_active FROM goals WHERE title LIKE '%workout%' OR title LIKE '%learn%'"
```

Find recently updated or completed goals:

```sh
python -m sqlite3 goals.sqlite "SELECT id, title, updated_at, is_active FROM goals ORDER BY updated_at DESC LIMIT 10"
```

`created_at` and `updated_at` are stored as UTC `YYYY-MM-DD HH:MM:SS` text, so
they sort chronologically and work with SQLite's `date()`, `datetime()`, and
`strftime()`. `goal_type`, `is_active`, and `created_at` have indexes for fast
filtering. Every original object is preserved verbatim in `raw_json` for
`json_extract`. The loader validates every input file before writing, applies
each run as a single transaction, and replaces rows that share an `id`, so
loading the same export twice leaves exactly one row per goal.
