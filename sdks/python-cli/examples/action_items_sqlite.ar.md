# تحويل تصدير عناصر العمل إلى SQLite

استخدم هذه الوصفة لتخزين، والاستعلام عن، والبحث في مهام Omi وعناصر العمل في قاعدة بيانات SQLite محلية. يقرأ السكربت ملفات تصدير JSON المحفوظة، ولا يقوم بأي طلبات عبر الشبكة، ويوحد الطوابع الزمنية كنص بتوقيت UTC لتشغيل دوال التاريخ والوقت في SQLite بسلاسة. تحتاج إلى بايثون 3.10+ (`Python 3.10+`) وواجهة سطر أوامر `omi-cli` مصادق عليها لإجراء التصدير الأولي.

تصدير ما يصل إلى 200 عنصر عمل:
...
احفظ ما يلي باسم `action_items_to_sqlite.py`:
...
تحميل الصفحات المُصدَّرة (يمكن تكرار الأمر مع تصديرات جديدة في أي وقت):

```sh
python action_items_to_sqlite.py tasks.sqlite action_items_0.json action_items_200.json
```

الاستعلام عن قاعدة البيانات. حساب المهام المفتوحة مقابل المكتملة:
...
العثور على عناصر العمل المعلقة، الأحدث أولاً:
...
البحث في المهام حسب الكلمة المفتاحية:
...
يتم تخزين `created_at` و `updated_at` و `due_at` كنص بتوقيت UTC بصيغة `YYYY-MM-DD HH:MM:SS`، بحيث يتم فرزها زمنياً وتعمل مع دوال SQLite مثل `date()` و `datetime()` و `strftime()`. يتم تخزين `completed` كـ `0` أو `1` مع فهرس مخصص لتصفية الحالة بسرعة. يتم الاحتفاظ بكل كائن أصلي حرفياً في `raw_json` لاستعلامات `json_extract`. تتحقق أداة التحميل من صحة كل ملف إدخال قبل الكتابة، وتنفذ كل عملية كمعاملة واحدة (single transaction)، وتستبدل الصفوف التي تشترك في نفس المعرف `id`، بحيث يؤدي تحميل نفس الصفحة مرتين إلى ترك صف واحد بالضبط لكل عنصر عمل.
