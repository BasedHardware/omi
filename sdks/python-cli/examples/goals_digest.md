# Build a goal tracking digest (KPIs, goal types, and progress report)

Use this recipe to see how your personal goals and metrics are progressing:
it turns one or more `goal list` exports into a structured Markdown digest with
overall KPIs, type breakdown, completion statistics, and individual progress.
It reads saved JSON exports, makes no network requests, and writes a single
...
Save the following as `goals_digest.py`:
...
```sh
python goals_digest.py goals.json goals_digest.md
```
