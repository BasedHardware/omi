...
Обновете ја OAuth сесијата што ѝ истекува рокот без повторна најава — важи само за најава преку прелистувач (OAuth). За клучеви `omi_dev_*` командата не обновува; заменете го клучот во веб-апликацијата под `Developer → API Keys`:
...
| `1` | Грешка при повик | Сопствена валидација на omi-cli (пр. `--browser` и `--api-key` истовремено, невалиден избор, празен влез) |
| `2` | Грешка при пристап или аргументи | Не сте најавени, клучот е невалиден или истечен — и грешки на парсерот (непознато знаме, аргумент што недостасува) |
...
Ако не наведете профил, CLI прво го користи профилот од променливата на околината `OMI_PROFILE`, па активниот профил од конфигурациската датотека, па `default`. Редослед на предност: `--profile` → `OMI_PROFILE` → активен профил во `~/.omi/config.toml` → `default`.
