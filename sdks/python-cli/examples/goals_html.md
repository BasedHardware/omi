# Build a self-contained HTML dashboard of your goals and OKRs

Use this recipe to browse, track, and print a structured visual dashboard of your
Omi goals, targets, and OKRs without the CLI or a spreadsheet. It turns one or more
`goal list` exports into an offline, interactive HTML dashboard featuring:

- **Executive summary metrics**: total goals count, active vs completed breakdown, overall completion rate percentage, and real-time visible counter.
- **Client-side live search**: instant filtering across goal titles, metric types, and targets as you type with zero server dependencies.
- **Visual progress meters**: dynamic progress bars with percentage completion and unit formatting for numeric, scale, and boolean goals.
- **Zero external assets**: no remote CSS, JavaScript CDNs, or telemetry trackers; 100% private, self-contained, and print-ready (`@media print`).
...
Generate a dashboard displaying only currently active goals:

```sh
python examples/goals_to_html.py goals.json active_goals.html --status active
```
...
The standalone exporter is located at `examples/goals_to_html.py`:
