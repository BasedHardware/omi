...
Open `report.html` in any browser. Each goal shows a progress bar (`current_value / target_value`, matching the percentage convention used by [`goals_csv.md`](goals_csv.md)), falling back to the `min_value`–`max_value` range when `target_value` isn't usable (e.g. `0`, for boolean/scale goals). Goals are split into an **Active** section and a **Completed / Inactive** section.
