# omi-cli mo e aoga kautolu

> Tau tuhi fakatonuga mo e aoga LLM (Claude Code, Cursor, do do kautolu).

## A ātaga aoga CLI ma e kautolu

* **JSON** `--json` fakafofou ha JSON monumo ki le stdout *monumo* — monumo fakatonuga, monumo spinner. Mea monumo ki le stderr `{"error": "...", "detail": "..."}`.
* **Exit code** `0` monumo / `1` amanogi / `2` auth / `3` server / `4` rate limited / `5` monumo. Kautolu ʻia ʻia ʻeaʻia ʻia JSON contract.
...
ʻA Omi Desktop ʻia fakafofou ha local API, ʻe kautolu ʻia gahua screen history, recap, SQL mo e task ʻeaʻia ha cloud dev API:
...
`screenshot_id` ki a stderr kia ʻe kautolu ʻia fakafofou ha ID
...
* Fakafofou `--profile <name>` ʻa e kautolu haʻunga haʻunga Omi.
...
* Fakafofou `--verbose` mo e ʻāfā — ʻia fakafofou `METHOD path → status (Ns)` ki a stderr
