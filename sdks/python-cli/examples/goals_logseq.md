...
Drop `goals.md` into your Logseq graph's `pages/` directory and open it. `progress::` uses the same `current_value / target_value` convention as [`goals_csv.md`](goals_csv.md), falling back to the `min_value`–`max_value` range when `target_value` isn't usable.
