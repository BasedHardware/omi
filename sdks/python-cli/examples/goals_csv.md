...
Use this recipe to review your Omi goals, habit milestones, and completion percentages
in a spreadsheet (Google Sheets, Microsoft Excel, Apple Numbers). It reads saved JSON
exports, makes no network requests, calculates progress percentages (`progress_pct`),
and outputs UTF-8 with BOM (`utf-8-sig`) so desktop spreadsheet apps parse symbols,
accents, and numbers seamlessly. You need Python 3.10+ and an authenticated `omi-cli`
...
Include inactive or archived goals:
...
Import the output in Excel, Google Sheets, or Apple Numbers. The exporter calculates
`progress_pct` automatically, escapes spreadsheet formula trigger characters (`=`, `+`,
`-`, `@`) to protect against CSV injection, and writes UTF-8 with BOM (`utf-8-sig`) for
accurate column parsing. Existing destinations are protected from unintended overwrites.
