# Convert a goal export to an Excel workbook (.xlsx)

Use this recipe when you want your Omi goals, targets, and habit progress
tracked in Excel with real cell types: `current_value` and `target_value` become
numerical cells you can compute with, `progress %` is dynamically calculated
via formula (`=D2/E2`), `created_at` and `updated_at` become sortable datetime
cells, the header row is frozen, and an AutoFilter is pre-configured. It reads
saved JSON exports, makes no network requests, and writes using safe atomic
I/O.
...
Save the following as `goals_to_xlsx.py`:
...
```sh
python goals_to_xlsx.py goals.xlsx goals_0.json
```

Open `goals.xlsx` in Excel, Google Sheets, or Apple Numbers. Numerical metrics
are ready for pivot tables and charts, timestamps format according to your
spreadsheet locale, and formula-injection defenses prevent malicious strings
from executing.
