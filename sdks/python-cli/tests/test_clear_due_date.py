...
def test_due_date_update_states(authed_profile, respx_mock, cli_runner, options, body):
...
def test_conflicting_due_date_flags(authed_profile, respx_mock, monkeypatch, capsys):
    monkeypatch.setattr(
        sys, "argv", ["omi", "--json", "action-item", "update", "a1", "--due-at", "2026-09-10", "--clear-due-at"]
    )
