...
> Looking for localized guides? See the [🇳🇬 How to take start use omi-cli (Nigerian Pidgin Quickstart)](examples/quickstart.pcm.md), the [omi-cli 快速上手（上海闲话）(Wu Chinese Quickstart)](examples/quickstart.wuu.md), the [omi-cli 快速入門（台語）(Min Nan Quickstart)](examples/quickstart.nan.md), the [omi-cli 快速上手（客家話）(Hakka Quickstart)](examples/quickstart.hak.md), the [Daj yungh omi-cli (Zhuang Quickstart)](examples/quickstart.za.md), or the [دەستپێکردنی خێرا بە omi-cli (Sorani Quickstart)](examples/quickstart.ckb.md).
