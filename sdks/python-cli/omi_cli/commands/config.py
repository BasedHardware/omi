...
def path(typer_ctx: typer.Context) -> None:
    ...
    if ctx.renderer.json_mode:
        ...
        ctx.renderer.emit(str(config.path))
...
def set_value(
    ...
    ctx.renderer.success(f"Set {key} = {display_value} on profile {profile.name}.")
...
def profile_use(
    ...
    cfg.save(config)
    ctx.renderer.success(f"Active profile: {name}.")
...
def profile_delete(
    ...
    ctx.renderer.confirm(f"Delete profile '{name}'?", yes=confirm)
    config.delete_profile(name)
    cfg.save(config)
    ctx.renderer.success(f"Deleted profile {name}.")
