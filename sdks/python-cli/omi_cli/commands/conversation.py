...
def create_conversation(
    ...
    ctx.renderer.success(
        f"Conversation queued: [bold]{escape(str(result.get('id') or ''))}[/bold] (status={result.get('status')})"
    )
...
def from_segments(
    ...
    ctx.renderer.success(f"Conversation queued: [bold]{escape(str(result.get('id') or ''))}[/bold]")
    ctx.renderer.emit(result)
