...
// Audio codec enum
export let BleAudioCodec = /*#__PURE__*/function (BleAudioCodec) {
...
export let DeviceConnectionState = /*#__PURE__*/function (DeviceConnectionState) {
...
const OmiModule = NativeModules.OmiModule ? NativeModules.OmiModule : new Proxy({}, {
  get() {
...
export function connect(deviceId, onConnectionStateChanged) {
  return OmiModule.connect(deviceId).then(() => {
    if (onConnectionStateChanged) {
      const subscription = eventEmitter.addListener('connectionStateChanged', ({
        id,
        state
      }) => {
        if (id === deviceId) {
          onConnectionStateChanged(id, state === 'connected' ? DeviceConnectionState.CONNECTED : DeviceConnectionState.DISCONNECTED);
        }
      });
...
export function getBleAudioBytesListener(deviceId, onAudioBytesReceived) {
  return OmiModule.startAudioBytesNotifications(deviceId).then(() => {
    const subscription = eventEmitter.addListener('audioBytesReceived', ({
      id,
      bytes
    }) => {
      if (id === deviceId) {
