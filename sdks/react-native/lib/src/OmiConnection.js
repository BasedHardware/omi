...
export class OmiConnection {
     ...
     */
    async connect(deviceId, onConnectionStateChanged) {
        ...
        try {
            ...
            device.onDisconnected((_, disconnectedDevice) => {
                ...
                if (onConnectionStateChanged) {
                    onConnectionStateChanged(disconnectedDevice.id, DeviceConnectionState.DISCONNECTED);
                }
            });
            if (onConnectionStateChanged) {
    ...
    async getAudioCodec() {
        ...
        try {
            ...
            const omiService = services.find((service) => service.uuid.toLowerCase() === OMI_SERVICE_UUID.toLowerCase());
            if (!omiService) {
            ...
            const codecCharacteristic = characteristics.find((char) => char.uuid.toLowerCase() === AUDIO_CODEC_CHARACTERISTIC_UUID.toLowerCase());
     ...
     */
    async startAudioBytesListener(onAudioBytesReceived) {
        if (!this.device) {
            throw new Error('Device not connected');
        }
        try {
            ...
            const characteristics = await omiService.characteristics();
            const audioDataStreamCharacteristic = characteristics.find((char) => char.uuid.toLowerCase() === AUDIO_DATA_STREAM_CHARACTERISTIC_UUID.toLowerCase());
            if (!audioDataStreamCharacteristic) {
                console.error('Audio data stream characteristic not found');
                return null;
            }
            try {
                console.log('Setting up audio bytes notification for characteristic:', audioDataStreamCharacteristic.uuid);
     ...
     */
    async stopAudioBytesListener(subscription) {
        if (subscription) {
            subscription.remove();
        }
    }
