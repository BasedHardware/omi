...
DEVICE_NAME = "Friend"
SERVICE_AUDIO_UUID = "19B10000-E8F2-537E-4F6C-D104768A1214"
CHARACTERISTIC_AUDIO_UUID = "19B10001-E8F2-537E-4F6C-D104768A1214"

SERVICE_STORAGE_UUID = "87654321-4321-8765-4321-1234567890ab"
STORAGE_NOTIFY_CHARACTERISTIC_UUID = "87654321-4321-8765-4321-1234567890ac"
STORAGE_WRITE_CHARACTERISTIC_UUID = "87654321-4321-8765-4321-1234567890ad"

MODE = 1 # 1 for normal mode. 2 for get date from sd card
...
async def main():
    ...
    async with BleakClient(audio_recorder.address, services=[SERVICE_AUDIO_UUID,SERVICE_STORAGE_UUID], disconnect_callback=handle_ble_disconnect) as client:
        print("Connected to AudioRecorder")
        services = await client.get_services()
        audio_service = services.get_service(SERVICE_AUDIO_UUID)
        audio_characteristic = audio_service.get_characteristic(CHARACTERISTIC_AUDIO_UUID)

        storage_service = services.get_service(SERVICE_STORAGE_UUID)
        storage_notify_characteristic = storage_service.get_characteristic(STORAGE_NOTIFY_CHARACTERISTIC_UUID)
        storage_write_characteristic = storage_service.get_characteristic(STORAGE_WRITE_CHARACTERISTIC_UUID)

        print("Notify:",storage_notify_characteristic,"Write:",storage_write_characteristic)
        ...
        async def write_to_new_characteristic(client, value):
            try:
                await client.write_gatt_char(storage_write_characteristic.uuid, struct.pack('i', value))#struct.pack('i', value))
                print(f"Wrote value {value} to new characteristic")
            except Exception as e:
                print(f"Failed to write to new characteristic: {e}")


        async def record_audio():
            await client.start_notify(audio_characteristic.uuid, handle_audio_data)
            #await client.start_notify(storage_notify_characteristic.uuid, handle_storage_data)
            storage_notify = await client.read_gatt_char(storage_notify_characteristic.uuid)
            storage_notify = struct.unpack('i', storage_notify)[0]
            print(f"files in storage {storage_notify}")
            await write_to_new_characteristic(client,MODE) # Data sended
            print("Recording audio...")
            t1 = time.time()
            await asyncio.sleep(CAPTURE_TIME)
            print("Recording stopped")
            print("time recording:",time.time()-t1)
            await client.stop_notify(audio_characteristic.uuid)
            #await client.stop_notify(storage_notify_characteristic.uuid)
