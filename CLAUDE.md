...
- **Named test bundles**: When testing a feature or bug fix, ALWAYS create a separate named bundle with `OMI_APP_NAME="omi-feature-name" ./run.sh`. This installs to `/Applications/omi-feature-name.app` with bundle ID `com.omi.omi-feature-name`, running side-by-side with "Omi Dev" and "Omi Beta". NEVER overwrite "Omi Dev" when testing a specific change — the user may have it running. Connect agent-swift with `--bundle-id com.omi.omi-feature-name`.
- **ALWAYS prefix named bundles with `omi-`** (e.g., `omi-fix-rewind`, `omi-6512-polling`, `omi-vision-test`) so they are visually grouped in `/Applications/`.
- Keep the bundle suffix and app name identical so auth callbacks reopen the correct app. Example: `omi-1233.app` should use `com.omi.omi-1233`, `omi-search.app` should use `com.omi.omi-search`, and mismatches like `omi-1233.app` with `com.omi.desktop-dev` are not allowed.
