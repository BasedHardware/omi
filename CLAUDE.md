...
- **Blocking calls in async** — these block the event loop: `database.*` functions (Firestore sync SDK), `open()`/`shutil.*` (file I/O), `upload_*`/`delete_*` from storage (GCS SDK), `creds.refresh()` (Google auth HTTP). In `async def`, always offload via `await run_blocking(executor, func, args)` from `utils.executors`. Use `critical_executor` for auth/rate-limits/Firestore, `storage_executor` for GCS/file I/O, `sync_executor` for STT/VAD. Never use bare `asyncio.to_thread()` (it uses the shared default pool — saturation stalls DNS and other asyncio internals).
