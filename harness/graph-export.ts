...
export const graphBrowserExport = (snapshot: GraphSnapshot, trajectory: readonly { cycle_id: string; trajectory: object }[]) => {
  ...
  return {
    ...
    entities: snapshot.entities.map((item) => ({ entity_id: item.entity.entity_id, handle: item.entity.handle })).sort((left, right) => left.entity_id.localeCompare(right.entity_id)),
    claims: safe.claims.map((item) => ({ revision_id: item.revision_id, lineage_id: item.claim.claim_lineage_id, predicate: item.claim.predicate })).sort((left, right) => left.revision_id.localeCompare(right.revision_id)),
    walks: snapshot.entities.map((item) => ({ anchor: `entity:${item.entity.entity_id}`, paths: walk(safe, { anchor: `entity:${item.entity.entity_id}`, max_hops: 3, result_cap: 50 }).paths })).sort((left, right) => left.anchor.localeCompare(right.anchor)),
    trajectory,
  };
};
