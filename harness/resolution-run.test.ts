...
test("T8 resolution run incrementally creates then reuses a cluster", async () => {
  ...
  expect(result.assignments).toEqual({ "m-1": "cluster:m-1", "m-2": "cluster:m-1" });
  expect(result.clusters).toEqual({ "cluster:m-1": { cluster_id: "cluster:m-1", label: "Alice Rivera" } });
});
