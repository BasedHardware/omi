...
static int write_characteristic(uint16_t handle, const uint8_t *value, uint16_t length)
{
...
static int verify_setting(const struct bt_uuid *uuid, uint8_t written, uint8_t expected)
{
    ...
    err = write_characteristic(handle, &written, 1);
    if (err) {
        return err;
    }
...
static int verify_contract(void)
{
    ...
    err = verify_setting(&gain_uuid.uuid, 9, 8);
    if (err) {
        return err;
    }
    err = verify_setting(&capture_uuid.uuid, 1, 1);
    if (err) {
        return err;
    }
    err = find_characteristic(&name_uuid.uuid);
    if (err) {
        return err;
    }
    static const uint8_t name[] = "Omi BSim";
    err = write_characteristic(discovered_handle, name, sizeof(name) - 1);
    if (err) {
        return err;
    }
    err = read_characteristic(discovered_handle);
    if (err || read_length != sizeof(name) - 1 || memcmp(read_value, name, read_length)) {
        return err ? err : -EINVAL;
    }
    err = find_characteristic(&sleep_uuid.uuid);
    if (err) {
        return err;
    }
    const uint8_t sleep = 1;
    return write_characteristic(discovered_handle, &sleep, 1);
}
