...
1. Bump both `CONFIG_BT_DIS_FW_REV_STR` and
   `CONFIG_MCUBOOT_IMGTOOL_SIGN_VERSION` in `omi/firmware/omi/omi.conf`, keeping
   their base versions equal. The build gate rejects drift because MCUboot may
   otherwise skip a changed app-core image during a dual-core OTA.
