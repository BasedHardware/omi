...
#include "lib/core/rtc_time_state.h"
#include "lib/core/sd_card.h"
#include "lib/core/settings.h"

LOG_MODULE_REGISTER(rtc, CONFIG_LOG_DEFAULT_LEVEL);

static rtc_time_state_t utc_state;
...
{
    k_mutex_lock(&rtc_lock, K_FOREVER);
    bool valid = rtc_time_state_is_valid(&utc_state);
    k_mutex_unlock(&rtc_lock);
    return valid;
}
...
{
    k_mutex_lock(&rtc_lock, K_FOREVER);
    uint64_t now_ms = rtc_time_state_now_ms(&utc_state, k_uptime_get());
    k_mutex_unlock(&rtc_lock);
    return now_ms;
}
...
{
    if (utc_epoch_s < RTC_TIME_MIN_VALID_EPOCH_S) {
        return -EINVAL;
    }

    if (utc_epoch_s > UINT64_MAX / 1000ULL) {
        return -ERANGE;
    }

    k_mutex_lock(&rtc_lock, K_FOREVER);
    int err = rtc_time_state_set_live_sync(&utc_state, utc_epoch_s * 1000ULL, k_uptime_get());
    if (!err) {
     ...
     */
    err = k_work_submit(&rtc_persist_work);
    if (err < 0) {
        LOG_ERR("Failed to schedule rtc_epoch persistence (err %d)", err);
        return err;
    }
...
void init_rtc(void)
{
    ...
    uint64_t saved_epoch_ms = saved_epoch_s <= UINT64_MAX / 1000ULL ? saved_epoch_s * 1000ULL : 0U;
    k_mutex_lock(&rtc_lock, K_FOREVER);
    rtc_time_state_init_from_persisted(&utc_state, saved_epoch_ms, k_uptime_get());
    k_mutex_unlock(&rtc_lock);

    if (saved_epoch_s == 0U) {
        LOG_WRN("RTC not synchronized yet (no persisted epoch)");
    } else if (saved_epoch_ms == 0U) {
        LOG_ERR("RTC persisted epoch overflow; waiting for live synchronization");
    } else {
        LOG_WRN("RTC persisted epoch is historical only; waiting for live synchronization");
    }
}
