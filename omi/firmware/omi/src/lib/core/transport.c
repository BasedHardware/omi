...
#define DEVICE_NAME_PAYLOAD_VERSION 1
                                                  ...
                                                  uint8_t flags)
{
    ...
    if (len < 1 || bytes[0] != DEVICE_NAME_PAYLOAD_VERSION || len - 1 > APP_SETTINGS_DEVICE_NAME_MAX_LEN) {
        LOG_WRN("Invalid device name write: %u bytes", len);
        return BT_GATT_ERR(BT_ATT_ERR_INVALID_ATTRIBUTE_LEN);
    }
    if (app_settings_save_device_name(bytes + 1, len - 1)) {
        return BT_GATT_ERR(BT_ATT_ERR_UNLIKELY);
    }
    return len;
}
...
static ssize_t settings_device_name_read_handler(struct bt_conn *conn,
                                                 ...
                                                 uint16_t offset)
{
    uint8_t payload[1 + APP_SETTINGS_DEVICE_NAME_MAX_LEN];
    payload[0] = DEVICE_NAME_PAYLOAD_VERSION;
    uint16_t name_len = app_settings_get_device_name(payload + 1, APP_SETTINGS_DEVICE_NAME_MAX_LEN);
    return bt_gatt_attr_read(conn, attr, buf, len, offset, payload, 1 + name_len);
}
...
features_read_handler(struct bt_conn *conn, const struct bt_gatt_attr *attr, void *buf, uint16_t len, uint16_t offset)
{
    ...
    features |= OMI_FEATURE_DEVICE_NAME_STORAGE;
