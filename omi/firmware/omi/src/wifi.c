...
{
    LOG_INF("WiFi state transition: %s -> %s", wifi_state_str(current_wifi_state), wifi_state_str(new_state));
...
static void tcp_update_server_addr_locked(void)
{
    ...
    tcp_server_addr_valid =
        (tcp_server_port > 0) && (zsock_inet_pton(AF_INET, tcp_server_addr, &tcp_sock_addr.sin_addr) == 1);
}
...
static int wifi_connect(void)
{
    ...
    LOG_INF("Security: %s, MFP: %d, Band: auto", wifi_security_txt(params.security), params.mfp);

    LOG_INF("Calling net_mgmt(NET_REQUEST_WIFI_CONNECT)...");
    if (net_mgmt(NET_REQUEST_WIFI_CONNECT, iface, &params, sizeof(struct wifi_connect_req_params))) {
...
K_THREAD_DEFINE(start_wifi_thread_id, 8192, start_wifi_thread, NULL, NULL, NULL, 6, 0, -1);
...
static void net_mgmt_callback_init(void)
{
    ...
    net_mgmt_init_event_callback(&net_shell_mgmt_cb,
                                 ...
                                 NET_EVENT_IPV4_DHCP_BOUND | NET_EVENT_IPV4_ADDR_ADD | NET_EVENT_IF_DOWN |
                                     NET_EVENT_IF_UP);

    net_mgmt_add_event_callback(&net_shell_mgmt_cb);

    net_mgmt_init_event_callback(&wifi_scan_mgmt_cb, wifi_scan_event_handler, NET_EVENT_WIFI_SCAN_DONE);
...
int setup_wifi_credentials(const char *ssid, const char *password)
{
    ...
    LOG_INF("Credentials updated: SSID=%s, password_len=%d", wifi_ssid, (int) strlen(wifi_password));
