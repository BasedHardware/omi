...
const struct device *audio_speaker;
...
struct gpio_dt_spec haptic_gpio_pin = {.port = DEVICE_DT_GET(DT_NODELABEL(gpio1)), .pin=11, .dt_flags = 0};


struct gpio_dt_spec speaker_gpio_pin = {.port = DEVICE_DT_GET(DT_NODELABEL(gpio0)), .pin=4, .dt_flags = 0};
...
void generate_gentle_chime(int16_t *buffer, int num_samples)
{
    ...
    const float frequencies[] = {523.25f, 659.25f, 783.99f, 1046.50f}; // C5, E5, G5, C6
    ...
    for (int i = 0; i < num_samples; i++)
    {
        ...
        {
           sample += sinf(2 * (float)PI * frequencies[j] * t) * (1.0f - t);
        }
        int16_t int_sample = (int16_t)(sample / num_freqs * 32767 * 0.5f);
