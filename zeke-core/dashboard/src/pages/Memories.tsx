    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 mt-2">Things Zeke remembers about you and your conversations.</p>
        </div>
        <button
      ...
      {showAddForm && (
        <div className="bg-slate-900 rounded-xl p-6 border border-slate-700">
          <h2 className="text-lg font-semibold text-white mb-4">Add New Memory</h2>
          <form onSubmit={handleCreate} className="space-y-4">
            <textarea
              value={newMemory}
              onChange={(e) => setNewMemory(e.target.value)}
              placeholder="Enter something you want Zeke to remember..."
              className="w-full bg-slate-800 text-white rounded-lg px-4 py-3 border border-slate-600 focus:outline-none focus:border-blue-500 min-h-[100px]"
            />
      ...
      <form onSubmit={handleSearch} className="flex gap-3">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full bg-slate-800 text-white rounded-lg pl-10 pr-4 py-3 border border-slate-600 focus:outline-none focus:border-blue-500"
          />
        </div>
        <button
          type="submit"
      ...
      {searchResults.length > 0 && (
        <div className="bg-blue-900/30 border border-blue-700 rounded-xl p-6">
          <h2 className="text-lg font-semibold text-blue-300 mb-4">Search Results</h2>
          <ul className="space-y-3">
            {searchResults.map((result, index) => (
              <li key={index} className="p-3 bg-blue-900/50 rounded-lg text-blue-100">
                {result}
              </li>
            ))}
          </ul>
      ...
      <div className="bg-slate-900 rounded-xl border border-slate-700 divide-y divide-slate-700">
        {memories.length === 0 ? (
          ...
          memories.map((memory) => (
            <div key={memory.id} className="p-4 flex items-start justify-between gap-4">
              <div className="flex-1">
                <p className="text-slate-200">{memory.content}</p>
