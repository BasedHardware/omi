...
test('Settings names Flutter UserProfile fromJson type-wrong GET migration_status instead of remapping to a Name chip', async () => {
  mockAuth.hasCloudSession.mockResolvedValue(true);
  mockBackend.request.mockImplementation(async request => {
    if (request.path === '/v1/users/profile') {
      return {
...
test('Settings names Flutter DailySummary.fromGenerated type-wrong GET action_items item instead of remapping to a headline chip', async () => {
  mockAuth.hasCloudSession.mockResolvedValue(true);
  mockBackend.request.mockImplementation(async request => {
    if (request.path === '/v1/users/daily-summaries?limit=3&offset=0') {
      return {
...
test('Settings names Flutter DailySummary.fromGenerated type-wrong GET conversation_ids item instead of remapping to a headline chip', async () => {
  mockAuth.hasCloudSession.mockResolvedValue(true);
  mockBackend.request.mockImplementation(async request => {
    if (request.path === '/v1/users/daily-summaries?limit=3&offset=0') {
      return {
...
test('Settings names Flutter DailySummary.fromGenerated type-wrong GET memories_created instead of remapping to a headline chip', async () => {
  mockAuth.hasCloudSession.mockResolvedValue(true);
  mockBackend.request.mockImplementation(async request => {
    if (request.path === '/v1/users/daily-summaries?limit=3&offset=0') {
      return {
...
test('Settings names Flutter ChangelogContent.fromJson type-wrong GET title as load-error instead of remapping What\'s New', async () => {
  mockAuth.hasCloudSession.mockResolvedValue(true);
  mockBackend.request.mockImplementation(async request => {
    if (request.path === '/v1/announcements/changelogs?limit=5') {
      return {
        ...
        body: JSON.stringify([
          {
            ...
            content: {
              title: 1,
              changes: [{title: 'Offline replay', description: ''}],
            },
          },
          {
            id: 'ann-good',
...
test('Settings names Flutter Person.fromJson type-wrong GET color_idx instead of omitting People', async () => {
  mockAuth.hasCloudSession.mockResolvedValue(true);
  mockBackend.request.mockImplementation(async request => {
    if (request.path === '/v1/users/people?include_speech_samples=false') {
      return {
...
test('Settings names Flutter Person.fromGenerated type-wrong GET speech_samples item instead of omitting People', async () => {
  mockAuth.hasCloudSession.mockResolvedValue(true);
  mockBackend.request.mockImplementation(async request => {
    if (request.path === '/v1/users/people?include_speech_samples=false') {
      return {
