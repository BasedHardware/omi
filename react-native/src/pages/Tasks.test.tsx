...
test('tasks page names GET goals without add or a write sheet', async () => {
  ...
  expect(tree).toContain('Whitespace id (2/2)');
  expect(tree).toContain('Next line id (2/2)');
  expect(tree).toContain('Padded id (1/1)');
...
test('tasks page names Flutter Goal.fromJson padded GET current_value instead of remapping to a progress chip', async () => {
  const request = jest.fn(async request => {
    if (request.path === '/v1/goals/all') {
      return {
        id: request.id,
...
test('tasks page names Flutter Goal.fromGenerated type-wrong GET advice instead of remapping to a progress chip', async () => {
  const request = jest.fn(async request => {
    if (request.path === '/v1/goals/all') {
      return {
        id: request.id,
