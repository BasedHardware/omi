...
test('compact Tasks tab names Flutter Goal.fromGenerated type-wrong GET advice instead of remapping to a progress chip', async () => {
  const request = jest.fn(async request => {
    if (request.path === '/v1/goals/all') {
      return {
        id: request.id,
...
test('compact Tasks tab names Flutter Goal.fromGenerated type-wrong GET metric instead of remapping to a progress chip', async () => {
  const request = jest.fn(async request => {
    if (request.path === '/v1/goals/all') {
      return {
        id: request.id,
  ...
  const request = jest.fn(async request => {
    if (request.path === '/v1/users/daily-summaries?limit=3&offset=0') {
      return {
        ...
        status: 200,
        body: JSON.stringify({
          summaries: [{id: 'sum-empty', date: '2020-01-14', headline: ' \t'}],
        }),
      };
    }
    return {id: request.id, status: 404, body: null};
