...
test('keeps GET task integrations when default_app exceeds 256', () => {
  expect(
    parseOmiTaskIntegrations(
      JSON.stringify({
        integrations: {
...
test('keeps GET task integrations when default_app or a map key exceeds 10000', () => {
  const key = 'k'.repeat(10001);
  expect(
    parseOmiTaskIntegrations(
      JSON.stringify({
        integrations: {
