...
test('connected device names Flutter FirmwareUpdate padded GET version as up to date instead of Latest Version', async () => {
  omiBackend.request.mockReset();
  omiBackend.request.mockResolvedValue({
    id: 'omi-firmware-latest',
    status: 200,
...
test('connected device names Flutter FirmwareUpdate padded BLE firmwareLatestQuery as up to date instead of Latest Version', async () => {
  omiBackend.request.mockReset();
  omiBackend.request.mockResolvedValue({
    id: 'omi-firmware-latest',
    status: 200,
...
test('connected device names Flutter FirmwareUpdate type-wrong GET ota_update_steps item instead of remapping to a Latest Version chip', async () => {
  omiBackend.request.mockReset();
  omiBackend.request.mockResolvedValue({
    id: 'omi-firmware-latest',
    status: 200,
  ...
  expect(output).toContain(`"${firmwareLatestVersionCopy()}"`);
  expect(output).toContain(
    desktopReadErrorCopy(new Error('Omi firmware is malformed')),
  );
  expect(output).not.toContain('"Latest",": "');
  expect(output).not.toContain('1.3.0');
  expect(output).not.toContain('Install');
...
test('connected device names Flutter FirmwareUpdate type-wrong GET zip_url instead of remapping to a Latest Version chip', async () => {
  omiBackend.request.mockReset();
  omiBackend.request.mockResolvedValue({
    id: 'omi-firmware-latest',
    status: 200,
