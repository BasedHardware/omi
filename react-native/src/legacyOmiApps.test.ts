...
test('old apps name Flutter App.fromGenerated type-wrong GET reviews item instead of remapping to a catalog name', () => {
  const review = {
    rated_at: '2026-09-07T00:00:00.000Z',
    review: 'Great',
    score: '5',
  ...
  expect(() =>
    parseOmiApp(
      JSON.stringify({id: 'notes', name: 'Notes', reviews: [null]}),
      'notes',
    ),
  ).toThrow('Omi app is malformed');
  expect(() =>
    parseOmiApp(
      JSON.stringify({id: 'notes', name: 'Notes', chat_tools: [null]}),
      'notes',
    ),
  ).toThrow('Omi app is malformed');
  expect(() =>
    parseOmiApp(
      JSON.stringify({
        id: 'notes',
        name: 'Notes',
        external_integration: {actions: [null]},
      }),
      'notes',
    ),
  ).toThrow('Omi app is malformed');
  expect(() =>
    parseOmiApp(
      JSON.stringify({
        id: 'notes',
        name: 'Notes',
        external_integration: {auth_steps: [null]},
      }),
      'notes',
    ),
  ).toThrow('Omi app is malformed');
...
test('old apps name Flutter App.fromGenerated type-wrong GET reviews item instead of attaching a catalog name', async () => {
  const request = jest.fn(async () => ({
    id: 'app',
    status: 200,
    body: JSON.stringify({
