...
test('parseRecordingTranscript uses production Listen segment join when stored text is also present', () => {
  expect(
    parseRecordingTranscript(
...
test('opens a recording row into the full transcript detail and retires it on back', async () => {
  ...
  try {
    ...
    expect(textOf(renderer)).not.toContain('Captured (device time)');
    expect(textOf(renderer)).not.toContain('Time unavailable');
    expect(textOf(renderer)).not.toContain('1970');
    expect(textOf(renderer)).not.toContain('Started ·');
...
test('invalid_audio transcript failures name a decode error instead of a generic transcription miss', async () => {
  const transcription = deviceTranscriptionProjection({
    sessionId: 'session-one',
    state: 'failed',
    providerResult: null,
