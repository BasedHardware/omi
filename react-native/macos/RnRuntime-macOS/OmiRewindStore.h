...
- (NSDictionary *)list:(NSDictionary *)input error:(NSError **)error {
  ...
  const char *sql = "SELECT id,timestamp,substr(trim(appName, char(9,10,11,12,13,32,133,160,5760,8192,8193,8194,8195,8196,8197,8198,8199,8200,8201,8202,8232,8233,8239,8287,12288,65279)),1,256),substr(trim(coalesce(windowTitle,''), char(9,10,11,12,13,32,133,160,5760,8192,8193,8194,8195,8196,8197,8198,8199,8200,8201,8202,8232,8233,8239,8287,12288,65279)),1,1024),CAST(round((julianday(timestamp)-2440587.5)*86400000) AS INTEGER) FROM screenshots WHERE (timestamp<?1 OR (timestamp=?1 AND id<?2)) AND (?3='' OR instr(lower(appName),lower(?3))>0 OR instr(lower(windowTitle),lower(?3))>0 OR instr(lower(ocrText),lower(?3))>0) ORDER BY timestamp DESC,id DESC LIMIT ?4";
