...
/**
 * THE SHARED QA SYNTHESIZER — canonical home.
 *
 * Both doors must run the SAME synthesizer or item-for-item identity between
 * them is impossible: `model_version` and the summary text are hashed into
 * `render_hash`, which is hashed into the candidate ref, which is keyed into the
 * public item id. Node-level agreement still passes while ids differ, which is
 * exactly how that kind of divergence hides.
 *
 * This implementation is BE-SURFACE's, promoted from
 * `apps/service/composition/qa-synthesizer.ts` into the shared `apps/qa/` layer
 * (which `apps/service` already depends on) so there is one copy rather than two
 * that agree today and drift next week. Theirs should become a re-export of
 * this; until it does, `synthesizer-parity.test.ts` fails the moment the two
 * disagree on any input.
 *
 * It is deliberately theirs and not mine: my earlier version emitted the
 * internal structural node id (`retrieval-node-v1:<hash>`) into user-visible
 * summary text, which the frontend contract forbids ("do not expose internal
 * IDs"). This one derives the sentence from projected claim structure and never
 * consults an evidence excerpt.
 *
