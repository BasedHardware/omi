...
describe("worker request contract", () => {
  ...
  test("Firebase verifier outage sends production retry-after on chat Settings Listen and generation-events GET", async () => {
    ...
    try {
      ...
      const session = await handler.fetch(
        onTheWireRequest(
          "/v1/device-sessions/aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",
          {
            authorization: "Bearer firebase-id-token",
            "x-omi-client-id": "desktop-client",
          }
      ...
      expect(session.headers.get("retry-after")).toBe("1");
      expect((await session.json()) as unknown).toEqual(unavailable);
  ...
  test("tasks GET does not omit a neighboring row when indentLevel is negative", async () => {
    await d1Mock
      .prepare(
        "INSERT INTO tasks (id, account_id, description, completed, completed_at, due_at, owner, source, provenance, sort_order, indent_level, created_at, updated_at, revision) VALUES (?, ?, ?, 0, NULL, NULL, NULL, 'assistant', '[]', 1, 0, 1, 1, NULL)"
      )
      .bind("task:readable-indent", "test-account", "readable indent")
      .run();
    await d1Mock
      .prepare(
        "INSERT INTO tasks (id, account_id, description, completed, completed_at, due_at, owner, source, provenance, sort_order, indent_level, created_at, updated_at, revision) VALUES (?, ?, ?, 0, NULL, NULL, NULL, 'assistant', '[]', 2, ?, 2, 2, NULL)"
      )
      .bind("task:negative-indent", "test-account", "negative indent", -1)
      .run();
  ...
  test("conversations GET does not omit a neighboring row when completed segment windows omit string text", async () => {
    await d1Mock
      .prepare(
        "INSERT INTO device_sessions (id, account_id, device_id, codec, state, r2_prefix, started_at, ended_at, created_at, updated_at) VALUES (?, ?, 'pendant', 21, 'complete', ?, 1, 1, 1, 1)"
      )
      .bind("session-readable-omitted", "test-account", "r2-readable-omitted")
      .run();
    await d1Mock
      .prepare(
        "INSERT INTO device_transcriptions (session_id, account_id, state, available_at, text, updated_at) VALUES (?, ?, 'completed', 1, 'Recorded words', 1)"
      )
      .bind("session-readable-omitted", "test-account")
      .run();
    await d1Mock
      .prepare(
        "INSERT INTO device_sessions (id, account_id, device_id, codec, state, r2_prefix, started_at, ended_at, created_at, updated_at) VALUES (?, ?, 'pendant', 21, 'complete', ?, 2, 2, 2, 2)"
    ...
    await d1Mock
      ...
      .bind(
        "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaa02",
        "test-account",
        JSON.stringify([{ start: 0, end: 0.1 }])
      )
      .run();
    ...
    const transcript = await fetchWorker(
      "/v1/device-sessions/aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaa02/transcript",
      { headers: authenticatedHeaders }
    );
    expect(transcript.status).toBe(503);
    expect(transcript.headers.get("retry-after")).toBeNull();
  ...
  test("conversations GET does not omit a neighboring row when discarded leading packets are negative", async () => {
    await d1Mock
      .prepare(
        "INSERT INTO device_sessions (id, account_id, device_id, codec, state, r2_prefix, started_at, ended_at, created_at, updated_at) VALUES (?, ?, 'pendant', 21, 'complete', ?, 1, 1, 1, 1)"
      )
      .bind("session-readable-packets", "test-account", "r2-readable-packets")
      .run();
    await d1Mock
      .prepare(
      ...
      .bind("session-readable-packets", "test-account")
      .run();
    await d1Mock
      .prepare(
        "INSERT INTO device_sessions (id, account_id, device_id, codec, state, r2_prefix, started_at, ended_at, created_at, updated_at) VALUES (?, ?, 'pendant', 21, 'complete', ?, 2, 2, 2, 2)"
      )
      .bind("session-negative-packets", "test-account", "r2-negative-packets")
      .run();
    await d1Mock
      .prepare(
        "INSERT INTO device_transcriptions (session_id, account_id, state, available_at, text, discarded_leading_packets, updated_at) VALUES (?, ?, 'completed', 1, 'Stored speech', ?, 2)"
      )
      .bind("session-negative-packets", "test-account", -1)
      .run();
  ...
  test("conversations GET does not omit a neighboring row when ended_at precedes started_at", async () => {
    await d1Mock
      .prepare(
        "INSERT INTO device_sessions (id, account_id, device_id, codec, state, r2_prefix, started_at, ended_at, created_at, updated_at) VALUES (?, ?, 'pendant', 21, 'complete', ?, 1, 1, 1, 1)"
      )
      .bind("session-readable-clocks", "test-account", "r2-readable-clocks")
      .run();
    await d1Mock
      .prepare(
        "INSERT INTO device_transcriptions (session_id, account_id, state, available_at, text, discarded_leading_packets, updated_at) VALUES (?, ?, 'completed', 1, 'Recorded words', 0, 1)"
      )
      .bind("session-readable-clocks", "test-account")
      .run();
    await d1Mock
      .prepare(
        "INSERT INTO device_sessions (id, account_id, device_id, codec, state, r2_prefix, started_at, ended_at, created_at, updated_at) VALUES (?, ?, 'pendant', 21, 'complete', ?, 10, 1, 10, 10)"
      )
      .bind(
    ...
    await d1Mock
      .prepare(
        "INSERT INTO device_transcriptions (session_id, account_id, state, available_at, text, discarded_leading_packets, updated_at) VALUES (?, ?, 'completed', 1, 'Stored speech', 0, 10)"
      )
      .bind("cccccccc-cccc-4ccc-8ccc-cccccccccccc", "test-account")
      .run();
    ...
    const metadata = await fetchWorker(
      "/v1/device-sessions/cccccccc-cccc-4ccc-8ccc-cccccccccccc",
      { headers: authenticatedHeaders }
    );
  ...
  test("conversations GET does not omit a neighboring row when a recording session id exceeds 256 characters", async () => {
    await d1Mock
      .prepare(
        "INSERT INTO device_sessions (id, account_id, device_id, codec, state, r2_prefix, started_at, ended_at, created_at, updated_at) VALUES (?, ?, 'pendant', 21, 'complete', ?, 1, 1, 1, 1)"
      )
      .bind("session-readable-printable-id", "test-account", "r2-readable-printable-id")
      .run();
    await d1Mock
      .prepare(
        "INSERT INTO device_transcriptions (session_id, account_id, state, available_at, text, discarded_leading_packets, updated_at) VALUES (?, ?, 'completed', 1, 'Recorded words', 0, 1)"
      )
      .bind("session-readable-printable-id", "test-account")
      .run();
    ...
    await d1Mock
      ...
      .bind(oversized, "test-account", "r2-oversized-session-id")
      .run();
    await d1Mock
      .prepare(
        "INSERT INTO device_transcriptions (session_id, account_id, state, available_at, text, discarded_leading_packets, updated_at) VALUES (?, ?, 'completed', 1, 'Stored speech', 0, 2)"
      )
      .bind(oversized, "test-account")
      .run();
  ...
  test("conversations GET lists a 256-character recording session id", async () => {
    const bounded = "x".repeat(256);
    await d1Mock
      .prepare(
        "INSERT INTO device_sessions (id, account_id, device_id, codec, state, r2_prefix, started_at, ended_at, created_at, updated_at) VALUES (?, ?, 'pendant', 21, 'complete', ?, 1, 1, 1, 1)"
      )
      .bind(bounded, "test-account", "r2-bounded-session-id")
      .run();
    await d1Mock
      .prepare(
        "INSERT INTO device_transcriptions (session_id, account_id, state, available_at, text, discarded_leading_packets, updated_at) VALUES (?, ?, 'completed', 1, 'Recorded words', 0, 1)"
      )
      .bind(bounded, "test-account")
      .run();
    ...
    expect(envelope.status).toBe(200);
    expect(
      ((await envelope.json()) as { items: Array<{ id: string }> }).items.map(
        (item) => item.id
      )
    ).toEqual([`recording:${bounded}`]);
  ...
  test("conversations GET does not omit a neighboring row when a recording session id carries whitespace", async () => {
    await d1Mock
      .prepare(
        "INSERT INTO device_sessions (id, account_id, device_id, codec, state, r2_prefix, started_at, ended_at, created_at, updated_at) VALUES (?, ?, 'pendant', 21, 'complete', ?, 1, 1, 1, 1)"
      )
      .bind("session-readable-opaque-id", "test-account", "r2-readable-opaque-id")
      .run();
    await d1Mock
      .prepare(
        "INSERT INTO device_transcriptions (session_id, account_id, state, available_at, text, discarded_leading_packets, updated_at) VALUES (?, ?, 'completed', 1, 'Recorded words', 0, 1)"
      )
      .bind("session-readable-opaque-id", "test-account")
      .run();
    await d1Mock
      .prepare(
        "INSERT INTO device_sessions (id, account_id, device_id, codec, state, r2_prefix, started_at, ended_at, created_at, updated_at) VALUES (?, ?, 'pendant', 21, 'complete', ?, 2, 2, 2, 2)"
      )
      .bind("session space", "test-account", "r2-space-session-id")
      .run();
    await d1Mock
      .prepare(
        "INSERT INTO device_transcriptions (session_id, account_id, state, available_at, text, discarded_leading_packets, updated_at) VALUES (?, ?, 'completed', 1, 'Stored speech', 0, 2)"
      )
      .bind("session space", "test-account")
      .run();
  ...
  test("history GET of an assistant whose terminal type disagrees is production chat unavailable", async () => {
    const id = "type-mismatch-assistant";
    const createdAt = 1;
    const message = {
      id,
      text: "invented answer",
    ...
    const mismatched = await fetchWorker("/v1/chat-messages?limit=50", {
      headers: authenticatedHeaders,
    });
    expect(mismatched.status).toBe(503);
    expect(mismatched.headers.get("retry-after")).toBe("60");
    expect((await mismatched.json()) as unknown).toEqual({
      error: {
        code: "service_unavailable",
        retryable: true,
        action: "retry",
  ...
  test("history GET of an assistant whose terminal createdAt disagrees is production chat unavailable", async () => {
    const id = "created-at-mismatch-assistant";
    const createdAt = 1;
    const message = {
      id,
      text: "invented answer",
    ...
    await d1Mock
      ...
      .bind(
        ...
        JSON.stringify({
          id: "2",
          kind: "done",
          message: { ...message, createdAt: -1, updatedAt: -1 },
        })
      )
      .run();
  ...
  test("history GET of an assistant whose terminal payloadHash disagrees is production chat unavailable", async () => {
    const id = "payload-hash-mismatch-assistant";
    const createdAt = 1;
    const message = {
      id,
      text: "invented answer",
    ...
    await d1Mock
      ...
      .bind(
        ...
        JSON.stringify({
          id: "2",
          kind: "done",
          message: { ...message, payloadHash: "sha256:other" },
        })
      )
      .run();
  ...
  test("history GET of an assistant whose terminal revision disagrees is production chat unavailable", async () => {
    const id = "revision-mismatch-assistant";
    const createdAt = 1;
    const message = {
      id,
      text: "invented answer",
  ...
  test("history GET of an assistant whose terminal attachments disagree is production chat unavailable", async () => {
    const id = "attachments-mismatch-assistant";
    const createdAt = 1;
    const message = {
      id,
      text: "invented answer",
