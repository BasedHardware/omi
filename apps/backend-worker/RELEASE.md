...
- `STAGING_WORKER_URL`: the public URL used by `verify:release` after the deploy. Native capture (`/v1/device-sessions`) reads this origin from `OMI_V5_BACKEND_URL` (https only; loopback, `api.omi.me`, `*.workers.dev`, or the reviewed `omi-platform-dev-*.run.app` DEV class). The repository does not record a `workers.dev` default. Capture-path `/v1/settings` follows the stamped New origin. Native account/profile/subscription and `/v1/apps` stay on `https://api.omi.me`.
