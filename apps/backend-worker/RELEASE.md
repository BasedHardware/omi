...
`GET /v1/device-sessions/:id` returns the stored session projection without R2 or a capture-ownership receipt. Invalid ids stay grammar `not_found`. Missing or foreign sessions are `device_session_not_found`. `GET /v1/device-sessions` lists that same D1 projection, including `captured_at_ms`, without R2. Missing D1 and unexpected store throws use Listen retry-after 1. `POST /v1/device-sessions` and `POST /v1/device-sessions/:id/complete` persist that same session row without R2. Audio appends and transcription still require the bound store. `GET /v1/device-sessions/:id/transcript` returns account-scoped processing state, text, segments and any discarded-leading-packet count. Conversations project these recordings as private records and the app reads full text through native authenticated transport.
