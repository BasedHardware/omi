          ...
          child: Column(
            children: [
              ListTile(
                ...
                onTap: () => _exportAudio(context, memory),
              ),
              ListTile(
                title: const Text('Export Transcript'),
                leading: const Icon(Icons.description),
                onTap: () => _exportTranscript(memory),
              ),
              ListTile(
                title: const Text('Export Summary'),
                leading: const Icon(Icons.summarize),
                onTap: () => _exportSummary(memory),
              ),
              ListTile(
                title: const Text('Export to Markdown'),
                leading: const Icon(Icons.description),
                onTap: () => _exportMarkdown(memory),
              ),
              ListTile(
                title: const Text('Export to PDF'),
                leading: const Icon(Icons.picture_as_pdf),
                onTap: () => _exportPDF(memory),
              ),
              ListTile(
                title: const Text('Full Export'),
                leading: const Icon(Icons.download),
                onTap: () => _fullExport(memory, context),
              ),
            ],
          ),
        ),
...
void _exportAudio(BuildContext context, Memory memory) async {
  ...
  await Share.shareXFiles([XFile(audioFile.path)], text: "GENERATED BY FRIEND, basedhardware.com");
}

void _exportPDF(Memory memory) async {
  ...
  await Share.shareXFiles([XFile(file.path)], text: "GENERATED BY FRIEND, basedhardware.com");
}

void _exportTranscript(Memory memory) async {
  final directory = await getApplicationDocumentsDirectory();
  final transcriptFile = File('${directory.path}/memory_transcript.txt');
  final transcript = "GENERATED BY FRIEND, basedhardware.com\n\n${memory.getTranscript()}";
  await transcriptFile.writeAsString(transcript);

  await Share.shareXFiles([XFile(transcriptFile.path)], text: "GENERATED BY FRIEND, basedhardware.com");
}

void _exportSummary(Memory memory) async {
  final directory = await getApplicationDocumentsDirectory();
  final summaryFile = File('${directory.path}/memory_summary.txt');
  final summary = "GENERATED BY FRIEND, basedhardware.com\n\n${memory.structured.target!.overview}";
  await summaryFile.writeAsString(summary);

  await Share.shareXFiles([XFile(summaryFile.path)], text: "GENERATED BY FRIEND, basedhardware.com");
}

void _fullExport(Memory memory, BuildContext context) async {
  try {
    final directory = await getApplicationDocumentsDirectory();
    ...
    await Share.shareXFiles([XFile(zipFile.path)], text: "GENERATED BY FRIEND, basedhardware.com");
  } catch (e) {
    if (!context.mounted) {
      return;
    }
...
void _exportMarkdown(Memory memory) async {
  ...
  await Share.shareXFiles([XFile(markdownFile.path)], text: "GENERATED BY FRIEND, basedhardware.com");
}
