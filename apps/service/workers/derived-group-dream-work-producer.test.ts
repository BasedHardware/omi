...
describe("derived group dream work adapter", () => {
  ...
  test("fails closed on missing input, strategy mismatch, and withheld materialization", async () => {
    const adapter = defineDerivedGroupDreamWorkAdapter({
    ...
    await expect(adapter.produce(context, leasedJob(), mismatchStrategy)).resolves.toMatchObject({
      kind: "failed",
      error_code: "dependency_unavailable",
    });
