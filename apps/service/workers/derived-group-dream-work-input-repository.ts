...
import {
  ...
  derivedGroupDreamInputSnapshotDigest,
  parseDerivedGroupDreamInputSnapshot,
  type DerivedGroupDreamInputSnapshot,
} from "./derived-group-dream-work-adapter";

const INPUT_REPOSITORY_PORT: unique symbol = Symbol("derived-group-dream-work-input-repository");
const STAGED_INPUT_VERSION = "derived-group-dream-work-staged-input-v1" as const;
const DIGEST = /^[a-f0-9]{64}$/;
const TOKEN = /^[\x21-\x7e]{1,256}$/;

export interface DerivedGroupDreamWorkInputStageBody {
  readonly pending_job: Readonly<DurableMemoryWorkJob>;
  readonly snapshot: Readonly<DerivedGroupDreamInputSnapshot>;
}

export interface DerivedGroupDreamWorkInputStageRequest extends DerivedGroupDreamWorkInputStageBody {
  readonly request_digest: string;
}

export interface StagedDerivedGroupDreamWorkInput {
...
export type DerivedGroupDreamWorkInputStageOutcome =
  | Readonly<{ kind: "staged" | "replayed"; input: StagedDerivedGroupDreamWorkInput }>
  | Readonly<{ kind: "idempotency_conflict" }>
  | CommonOutcome;

export type DerivedGroupDreamWorkInputLoadOutcome =
  | Readonly<{ kind: "found"; snapshot: DerivedGroupDreamInputSnapshot }>
  | Readonly<{ kind: "not_found" | "stale_lease" | "ineligible_state" }>
  | CommonOutcome;

export interface DerivedGroupDreamWorkInputRepository {
  readonly [INPUT_REPOSITORY_PORT]: true;
  stage(
    context: AuthorizedLedgerWriteContext,
    request: DerivedGroupDreamWorkInputStageRequest,
  ): Promise<DerivedGroupDreamWorkInputStageOutcome>;
  load(
    context: AuthorizedLedgerWriteContext,
...
export interface DerivedGroupDreamWorkInputRepositoryImplementation {
  stage(
    context: AuthorizedLedgerWriteContext,
    request: DerivedGroupDreamWorkInputStageRequest,
  ): Promise<unknown>;
...
const fail = (code: string): never => { throw new TypeError(`derived group dream work input ${code}`); };
...
const pendingForContext = (
  ...
  if (job.state !== "pending" || job.work_kind !== "derived_group_dream"
...
const leasedForContext = (
  ...
  if (job.state !== "leased" || job.work_kind !== "derived_group_dream" || job.lease === null
...
export const derivedGroupDreamWorkInputStageRequestDigest = (
  body: DerivedGroupDreamWorkInputStageBody,
): string => {
  const job = parseDurableMemoryWorkJob(body.pending_job);
  const snapshot = parseDerivedGroupDreamInputSnapshot(body.snapshot);
  assertDerivedGroupDreamInputSnapshotMatchesJob(snapshot, job);
  return sha256CanonicalContent({
    contract_version: "derived-group-dream-work-input-stage-request-v1",
    pending_state_digest: durableMemoryWorkStateDigest(job),
    snapshot_digest: derivedGroupDreamInputSnapshotDigest(snapshot),
  });
};

export const stagedDerivedGroupDreamWorkInputId = (jobValue: DurableMemoryWorkJob): string => {
  const job = parseDurableMemoryWorkJob(jobValue);
  return `dgwi1_${sha256CanonicalContent({
    contract_version: "derived-group-dream-work-staged-input-id-v1",
...
export const materializeStagedDerivedGroupDreamWorkInput = (
  request: DerivedGroupDreamWorkInputStageRequest,
): StagedDerivedGroupDreamWorkInput => {
  const job = parseDurableMemoryWorkJob(request.pending_job);
  const snapshot = parseDerivedGroupDreamInputSnapshot(request.snapshot);
  assertDerivedGroupDreamInputSnapshotMatchesJob(snapshot, job);
  return Object.freeze({
    version: STAGED_INPUT_VERSION,
    staged_input_id: stagedDerivedGroupDreamWorkInputId(job),
    owner_account_id: job.owner_account_id,
    job_id: job.job_id,
...
export const parseStagedDerivedGroupDreamWorkInput = (
  value: unknown,
  ...
  const snapshot = parseDerivedGroupDreamInputSnapshot(input["snapshot"]);
  ...
  }) satisfies StagedDerivedGroupDreamWorkInput;
  if (!Number.isSafeInteger(staged.account_epoch) || staged.account_epoch < 0
    || !/^dgwi1_[a-f0-9]{64}$/.test(staged.staged_input_id)
    ...
    || staged.snapshot_digest !== derivedGroupDreamInputSnapshotDigest(snapshot)) fail("staged_input_mismatch");
  if (expectedJob !== undefined) {
    ...
    assertDerivedGroupDreamInputSnapshotMatchesJob(snapshot, job);
    if (staged.staged_input_id !== stagedDerivedGroupDreamWorkInputId(job)
      ...
      || staged.stage_request_digest !== derivedGroupDreamWorkInputStageRequestDigest({
        pending_job: pending,
        snapshot,
...
export const defineDerivedGroupDreamWorkInputRepository = (
  implementation: DerivedGroupDreamWorkInputRepositoryImplementation,
): DerivedGroupDreamWorkInputRepository => Object.freeze({
  ...
  async stage(contextValue, requestValue) {
    ...
    const snapshot = parseDerivedGroupDreamInputSnapshot(root["snapshot"]);
    assertDerivedGroupDreamInputSnapshotMatchesJob(snapshot, job);
    const body = Object.freeze({ pending_job: job, snapshot });
    const requestDigest = digest(root["request_digest"]);
    if (requestDigest !== derivedGroupDreamWorkInputStageRequestDigest(body)) fail("request_digest_mismatch");
    ...
    const staged = parseStagedDerivedGroupDreamWorkInput(outcome["input"], job);
  ...
  async load(contextValue, jobValue) {
    ...
    const staged = parseStagedDerivedGroupDreamWorkInput(root["input"], job);
