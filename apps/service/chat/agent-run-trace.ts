...
export interface AgentRunTraceBundle {
  ...
  /**
   * Canonical integrity checksum over every field above. This is not a
   * signature and does not authenticate an artifact editor; replay only
   * accepts bytes whose checksum and derived projections agree.
   */
...
// These values are deliberately generated by the exporter.  Checking source
// IDs against them would reject valid IDs such as `run:1`, `event:1`, or
// `attempt`, because the remapped labels intentionally have those prefixes.
const REMAPPED_TRACE_KEYS = new Set([
  "schema", "schemaVersion", "buildId", "runId", "eventLabel", "runLabel", "attemptLabel",
  "contextLabel", "sourceLabel", "ownerLabel", "callLabel", "idemLabel", "approvalLabel",
  "usageLabel", "recoveryLabel", "fromAttemptLabel", "toAttemptLabel", "stateLabel",
...
const validateNoRawSecrets = (bundle: AgentRunTraceBundle, sourceEvents: readonly AgentRunEvent[], bytes: string): void => {
  ...
  jsonContentStringValues(parsed, (text) => strings.push(text));
