#!/usr/bin/env bash
set -euo pipefail
# One entrypoint for isolated mobile sessions (SCA-487): doctor/acquire/start/
# seed/reset/status/evidence/stop/recover/release. See MOBILE_SESSIONS.md.
...
PYTHONPATH="$HARNESS_PYTHONPATH" "$PYTHON_BIN" -m dev_harness.mobile_session "$@"
