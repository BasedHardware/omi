...
def test_operation_elapsed_time_measures_work_not_receipt_serialization(rig, monkeypatch):
    ...
    def create(spec):
        ...
        child.send = delayed
        return child
    monkeypatch.setattr(se, 'utc_now', lambda: (datetime(2026, 9, 17, tzinfo=timezone.utc) +
                                               timedelta(seconds=now[0])).isoformat().replace('+00:00', 'Z'))
    engine = live.LiveSession(rig.directory.parent, rig.directory, load_lease=lambda: rig.lease,
        source=lambda: rig.stamp, factory=create, screenshot=rig.screenshot, monotonic=lambda: now[0])
    engine.start()
...
def test_vm_auth_urls_are_sanitized_by_shape_not_one_fixture_secret(rig):
    ...
    def create(spec):
        ...
        def read(timeout_s):
            ...
            if armed:
                armed = False
                return json.dumps([{'event': 'app.log', 'params': {'appId': 'app-fixture',
                    'log': 'VM service ws://127.0.0.1:43123/' + secret + '/ws', 'error': False}}])
...
def test_read_control_cannot_steal_an_inflight_restart_response(rig):
    ...
    with ThreadPoolExecutor(max_workers=1) as pool:
        ...
        try:
            ...
            try:
                assert engine.request('controls', generation=1, params={'method': 'state'}, timeout_s=0.3)['outcome'] == 'ok'
            except live.LiveError as error:
