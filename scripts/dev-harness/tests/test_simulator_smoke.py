...
def test_live_held_lease_fails_fast_without_polling_controls(tmp_path: Path) -> None:
    ...
    engine = _engine(tmp_path, acquire=acquire, factory=factory, env=env, platform="android")
    ...
    try:
        ...
        assert f"session oms-v3android held by live pid {holder.pid}" in text
...
def test_session_id_live_foreign_owner_fails_fast_without_flutter(tmp_path: Path) -> None:
    env = {"OMI_LOCAL_STATE_ROOT": str(tmp_path / "state")}
    lease = ms.acquire(REPO_ROOT, env, name="v3android", platform_name="android", listeners=lambda port: ())
    holder = subprocess.Popen(["sleep", "30"])
    time.sleep(0.05)
    ...
    engine = _engine(tmp_path, factory=factory, env=env, platform="android")
    started = time.monotonic()
    try:
        with pytest.raises(smoke.SmokeBlocked) as err:
