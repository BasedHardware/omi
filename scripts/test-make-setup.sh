...
# Compare physical paths on BOTH sides. Git Bash exposes /tmp as a logical mount
# while BASH_SOURCE resolves the same directory through its Windows path; macOS
# does the mirror image, handing Make a logical /var/... while `pwd -P` yields
# /private/var/.... Resolving only the expectation made this pass on one platform
# and fail on the other for the same correct behaviour.
