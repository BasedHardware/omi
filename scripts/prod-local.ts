 ...
 *   - Mint a Firebase identity. There is no Auth emulator in deployed mode,
 *     no custom-token issuer, and no stub verifier. The process will say so
 *     at startup. A client must present a real Firebase ID token; this
 *     script cannot issue one.
...
const LOCAL_FIREBASE_PROJECT_ID = "omi-local-pg";
const LOCAL_APPLICATION_ID = "app:omi-local-pg";
