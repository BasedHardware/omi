...
RAW = re.compile(r'\b(?:track|trackEvent|capture)\s*\(')
IDENT = re.compile(r'^[a-z][A-Za-z0-9]*$')
SENSITIVE = re.compile(r'text|transcript|memory|email|address|token|user.?id|device.?id|name|content', re.I)
...
def validate(doc, root=ROOT, prior=None):
    ...
    if prior:
        ...
        for key, value in old.items():
            if key not in now or any(now[key][field] != value[field] for field in ('wire_name', 'properties', 'phase', 'intent')):
    ...
    for event in events:
        ...
        if event['phase'] != 'point' or event['intent'] is not None:
            errors.append(f'{label}: attempt/outcome requires the F1 contract before emission')
        for key, spec in event['properties'].items():
            if not IDENT.fullmatch(key) or SENSITIVE.search(key) or spec != {'type': 'bool'}:
...
def render(doc):
    ...
    for e in doc['events']:
        ...
        dart += ['  @override', f'  String get wireName => {json.dumps(e["wire_name"])};', '  @override', '  Map<String, Object> get properties => {' + ', '.join(json.dumps(k)+': '+k for k in fields) + '};', '}']
        plan.append(f"| {e['id']} | {e['wire_name']} | {', '.join(fields) or 'none'} | {e['status']} | {', '.join(e['consumers'])} |")
    return '\n'.join(dart)+'\n', '\n'.join(plan)+'\n'


def raw_violations(path, source, prior, adopted):
    if not path.startswith('app/lib/') or not path.endswith('.dart') or (prior is not None and path not in adopted):
        return []
    # Deliberately lexical; strip comments, retain calls even through receiver aliases.
    source = re.sub(r'/\*.*?\*/|//[^\n]*', '', source, flags=re.S)
    return [f'{path}: raw analytics call; use registry/typed_events.dart and events.json (or migrate this file before recording adoption)'] if RAW.search(source) else []
