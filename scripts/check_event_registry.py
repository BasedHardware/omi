...
def property_expression(key, spec):
    return f'{key}.wireName' if spec['type'] == 'enum' else key
...
def render(doc):
    ...
    for e in doc['events']:
        ...
        dart += ['  @override', f'  String get wireName => {dart_string(e["wire_name"])};', '  @override', '  Map<String, Object> get properties => {' + ', '.join(dart_string(fields[k]['wire_name'])+': '+property_expression(k, fields[k]) for k in fields) + '};', '}']
