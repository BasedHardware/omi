...
Audio requests contain `{ "chunks": [{ "chunkIndex": 0, "bytesBase64": "..." }] }`. Each batch has 1–128 complete packets with consecutive indexes, at most 1 MiB of decoded audio and at most 2 MiB of encoded request body. Preserve packet boundaries. Indexes start at zero; advance the client acknowledgment only after the whole batch succeeds. Retrying an existing index with identical bytes succeeds without incrementing counters, including after completion; different bytes at the same index return 409. A conflicting batch does not accept its other new packets. A new packet cannot be added to a completed session. Storage failures leave reserved packets pending so an identical batch retry can recover them. The successful-upload counter changes only once per packet and prevents completion while storage is pending. The former single-packet request shape is rejected; all in-tree clients send batches.
