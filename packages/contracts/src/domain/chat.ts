 ...
 * Attachments, chart_data, memories citations, and LangSmith provenance are a
 * foundation gap, not a decision made here — surfaced for the adapter, not
 * silently modeled.
 *
 ...
 * the user). It is not something a client may ever WRITE: the legacy enum is
 * `{human, ai}` (`backend/models/chat.py`), so authoring `"unknown"` produces
 * a message the server must reject, and the outbox would dead-letter a write
 * the user believes they sent. Enforced by construction — `ChatMessageOp`'s
 * create arm takes this type, so the narrowing is a compile error, not a
 * convention.
 */
export type ChatMessageAuthoredSender = Exclude<ChatMessageSender, "unknown">;
...
export interface ChatMessage {
   ...
   * the payload — see `chatMessagePayloadHash` in `@omi-core/domain`.
   *
...
/**
 * Terminal stream frame body (ADR-005 §3 / INV-CHAT-002): the complete
 * canonical message, never a delta. Deltas are advisory rendering hints;
 * reconnect and journal replay heal from this shape alone.
 */
export type ChatTerminalFrame = { kind: "done"; message: ChatMessage };
