...
- **notifications-job** (`modal/job.py`) — Hourly cron job, reads Firestore/Redis, sends push notifications, and emits the weekly read-only Chat-first materialization-health verdict consumed by its routed Cloud Monitoring decision review. X connector sync no longer runs here; it moved to x-connector-sync-job below. It has no canonical maintenance flags or Typesense secrets; its deploy workflow removes only those retired bindings and preserves unrelated notification env.
