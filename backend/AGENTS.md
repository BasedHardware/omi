...
- **notifications-job** (`modal/job.py`) — Hourly notifications cron; also emits the weekly read-only Chat-first materialization-health verdict. Never owns X sync or canonical-memory maintenance. Selection mode + run lock: `docs/runbooks/daily-summary-cutover.md`.
- **x-connector-sync-job** (`modal/x_connector_sync_job.py`) — Six-hour X sync Cloud Run Job; sync-contract failures exit nonzero. Runbook: `docs/runbooks/x-connector-sync-job.md`.
