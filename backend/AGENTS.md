...
- **agent-proxy** (`agent-proxy/main.py`, `routers/agent_provider_proxy.py`) — GKE. HTTPS/WebSocket proxy at `https://agent.omi.me/v1/agent` or `https://agent.omiapi.com/v1/agent`, with matching WebSocket endpoints. Validates Firebase ID tokens, looks up `agentVm` in Firestore, and reaches the VM only through its private VPC IP with the per-VM auth token. The VM's Anthropic-compatible route authenticates that per-VM token and forwards the fixed chat-agent lane through the internal LLM gateway; provider keys never enter the VM.
