...
frame-request-retention-job (modal/frame_request_retention_job.py)  [independent cron]
```
...
- **frame-request-retention-job** (`modal/frame_request_retention_job.py`) — Independent bounded Cloud Run Job for temporary JIT frame metadata, ambiguous-upload objects, promotion cleanup receipts, and conversation-deletion outboxes. It uses a six-day lifecycle/soft-delete-disabled temporary bucket separate from the no-expiry conversation-photo bucket. Its workflow admits exact `main` and validates both live buckets plus the dedicated hourly Scheduler before publishing an image. Keep the legacy cleanup call in memory maintenance until `FRAME_REQUEST_RETENTION_INDEPENDENT_HEALTHY=true` is an operator-backed health decision.
