...
- **memory-maintenance-job** (`modal/memory_maintenance_job.py`) — Cloud Run Job for canonical maintenance (normalization → TTL audit → terminal consolidation/promotion). Deploy manually via `.github/workflows/gcp_memory_maintenance_job.yml`; auto-dev on `main` via `gcp_memory_maintenance_job_auto_dev.yml`. Flag ownership and readiness order: `docs/runbooks/canonical-memory-rollout-flags.md`. Scheduler owns cadence; prod declares `MEMORY_MODE=off` until Gate 3; job users come from `CANONICAL_MEMORY_USERS` in `config/canonical_memory_cohort.py`.
