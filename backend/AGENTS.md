...
Hosted VAD failover is bounded by `HOSTED_VAD_CONNECT_TIMEOUT_SECONDS` (default `3`) and `HOSTED_VAD_READ_TIMEOUT_SECONDS` (default `30`). Invalid or non-positive values fall back to those safe defaults so an unreachable hosted VAD cannot consume the sync-job stale budget before local ONNX VAD runs.
