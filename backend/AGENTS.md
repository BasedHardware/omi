...
Chat SSE deadlines: `AGENT_STREAM_FIRST_EVENT_TIMEOUT_SECONDS` (default `25`), `AGENT_STREAM_PROGRESS_HEARTBEAT_SECONDS` (default `20`), `AGENT_STREAM_MAX_DURATION_SECONDS` (default `150`), and `AGENT_STREAM_CANCEL_GRACE_SECONDS` (default `2`) bound silent setup/producer work and keep valid long tool calls observable. Values must be positive. The agent's provider call is bounded per attempt by `AGENT_STREAM_PROVIDER_IDLE_TIMEOUT_SECONDS` (default `45`, an idle gap not a generation cap) and re-issued on transport-class failures up to `AGENT_STREAM_PROVIDER_MAX_ATTEMPTS` (default `3`) spaced by `AGENT_STREAM_PROVIDER_RETRY_BACKOFF_SECONDS` (default `1`); keep the per-attempt bound well under the max duration or a stalled attempt leaves no room to retry. Public shared-conversation chat is separately fail-closed behind `PUBLIC_SHARED_CONVERSATION_CHAT_MODE=off|gateway` (default `off`) and requires route-scoped frontend OIDC identity plus an opaque per-IP subject.
