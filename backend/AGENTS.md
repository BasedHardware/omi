...
- **memory-maintenance-job** (`modal/memory_maintenance_job.py`) — Cloud Run Job for universal Short-term maintenance (normalization → TTL audit → terminal consolidation/promotion). Deploy manually via `.github/workflows/gcp_memory_maintenance_job.yml`; auto-dev on `main` via `gcp_memory_maintenance_job_auto_dev.yml`. Global incident/readiness controls and rollback order live in `docs/runbooks/universal-memory-operations.md`; there is no UID inventory or enrollment step. Env contract is validated by `backend/scripts/validate-backend-runtime-env.py`; Scheduler owns cadence. When enabled, L2/required-processing uses the gateway-only `omi:auto:memory-l2` Luna lane; workflows derive the endpoint after the serving gate and Cloud Run VPC probe.
...
| **Universal dev-cloud proof** (`v3_dev_cloud_proof.py` + deployed branch revision) | Multi-user synthetic matrix, indexes, IAM, auth, and global rollback on dev-cloud | Local hermetic fakes or production observation |
| **Production observation** (`docs/runbooks/universal-memory-operations.md`) | Revision identity, Scheduler/job health, content-free origin/maintenance metrics, and global-stop rehearsal | Substitute for dev-cloud or hermetic behavior proof |
