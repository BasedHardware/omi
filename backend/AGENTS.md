...
Free-tier AFM Stage 1 deployment: `FREE_TIER_LOCAL_PROCESSING=true` only in dev; prod is explicitly `false` with an empty cohort. Set the GitHub Actions variable **`DEV_FREE_TIER_LOCAL_PROCESSING_COHORT`** (the `development` environment is the right scope; the `DEV_` name is deliberate because `vars` falls back to repository scope, and production workflows never read it) to explicit `uid:<uid>` tokens for accounts with AFM-capable Macs. Unset/empty admits nobody; percentages and malformed values fail deployment rendering. The shared ConfigMap feeds listener/pusher; Cloud Run receives escaped values from the renderer. Deploy the backend stack before independently rolling pusher so the ConfigMap key exists. `FREE_TIER_EMERGENCY_STOP=false` is explicit on every host and can be changed to `true` to revoke admission; `FREE_TIER_MEMORY_SUPPRESSION` stays unplumbed/unset because it has no local replacement. The roster also includes sync-backfill and integration (both run `main:app`), plus desktop-backend's shared stop reader. Local checks: `backend/scripts/pre-deploy-check.sh`, `backend/scripts/check_runtime_env_compose.sh`, and the free-tier/config-renderer unit tests. No deployed activation or AFM acceptance is implied by these repository checks.
