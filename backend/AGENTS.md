...
- **pusher** (`pusher/main.py`) — Receives audio via binary WebSocket protocol and owns inline conversation finalization. Calls diarizer and deepgram for speaker sample extraction (`utils/speaker_identification.py` → `utils/speaker_sample.py`). In dev, its canonical `MEMORY_*` rollout flags must stay aligned with `backend-listen`; an enrolled finalizer must never default to `MEMORY_MODE=off`.
