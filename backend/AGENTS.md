...
- **llm-gateway** (`llm_gateway/main.py`) — Internal FastAPI service for Omi-managed LLM auto lanes. Called by backend with service auth for `omi:auto:*` chat-completions routes; not exposed to clients. Public shared-conversation chat uses only the dedicated `omi:auto:public-shared-conversation-chat` lane and returns unavailable on every gateway fault. The gateway also owns the OpenAI-shaped `/v1/embeddings` surface (`omi:auto:openai-embeddings`, `omi:auto:gemini-embeddings`) and, for company-paid desktop Gemini traffic, the `omi:auto:desktop-vertex-*` lanes: `routers/desktop_proxy.py` stays the desktop BFF (auth/metering/limits) and translates Gemini JSON ↔ the OpenAI surface via `utils/llm/desktop_gemini_gateway.py`, while the PT pin/overflow/host policy runs inside `VertexGeminiProvider` via `utils/llm/vertex_pt_routing.py` (single policy module; Gemini BYOK stays a thin direct AI Studio path in the proxy).
