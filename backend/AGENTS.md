...
- Calendar overlap outranks discard (SCA-381): when `should_discard_conversation` says discard, `_get_structured` keeps the conversation if `[started_at, finished_at]` overlaps a non-declined meeting — stored `users/{uid}/meetings` first, then a read-only Google lookup with `require_accepted=True` (cancelled or user-declined events never keep). The override only fires on a positive hit; disconnected calendar, missing token, or lookup errors leave the discard verdict standing. `GET /v1/calendar/capture-gaps?start=&end=` (`routers/google_calendar.py`) returns the inverse surface — confirmed, timed (≤8h) events in the window with no overlapping non-discarded conversation — without ever creating conversations.
