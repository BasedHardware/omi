...
- Meeting identity reaches the summarization prompt through `utils/conversations/meeting_context.resolve_meeting_context`, called before `_get_structured`. Sources, best first: stored meeting (exact `redis_db` conversation→meeting mapping, else a time-overlap query over `users/{uid}/meetings`) → `calendar_meeting_context` on the create request → overlapping Google Calendar event (read-only) → conferencing-window OCR. Every layer degrades to no context; none may fail the conversation. The stored-meeting layer is on by default with `CONVERSATION_STORED_MEETING_CONTEXT_ENABLED` as its kill switch, because it is the only source needing neither an OAuth grant nor the Redis mapping. Calendar **write-back** stays behind `GOOGLE_CALENDAR_AUTO_LINK_ENABLED` and still runs only after summarization.
- `users/{uid}/meetings` is populated **only** by clients calling `POST /v1/calendar/meetings`. No shipping client does: `desktop/macos` has no EventKit/`EKEventStore` usage and never calls the generated `storeCalendarMeetingV1CalendarMeetingsPost`, so the collection is empty for desktop users and the stored-meeting layer finds nothing. Closing this needs a client that reads the system calendar and POSTs `calendar_event_id`, `calendar_source`, `title`, `start_time`, `end_time`, and `participants[{name,email}]`.
