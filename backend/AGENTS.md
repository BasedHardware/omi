...
- **memory-maintenance-job** (`modal/memory_maintenance_job.py`) — Cloud Run Job for canonical maintenance (normalization → TTL audit → terminal consolidation/promotion). Its env contract is validated by `backend/scripts/validate-backend-runtime-env.py`; Scheduler owns cadence, prod stays `MEMORY_MODE=off` until Gate 3, and cohorts use `CANONICAL_MEMORY_USERS`. When enabled, L2/required-processing uses the gateway-only `omi:auto:memory-l2` Luna lane; workflows derive the endpoint after the serving gate and Cloud Run VPC probe.
