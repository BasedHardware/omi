...
- **notifications-job** (`modal/job.py`) — Hourly cron job, reads Firestore/Redis, sends push notifications, runs X connector sync, and emits the weekly read-only Chat-first materialization-health verdict consumed by its routed Cloud Monitoring decision review. It has no canonical maintenance flags or Typesense secrets; its deploy workflow removes only those retired bindings and preserves unrelated notification/X-sync env. Executions take a Redis run lock (`notifications_job:run_lock`, 55 min) so an overlapping Scheduler fire skips the notification section. Daily-summary recipient selection is staged by `DAILY_SUMMARY_SELECTION_MODE=legacy|shadow|indexed` (code default `legacy`): `indexed` needs the user docs to carry explicit `daily_summary_enabled` / `daily_summary_hour_local` (write-time defaults in `database/notifications.py` plus the one-time `scripts/backfill_daily_summary_schedule_fields.py`), and `shadow` logs `daily_summary_selection_shadow` set diffs to prove that before flipping.
