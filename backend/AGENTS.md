...
- **notifications-job** (`modal/job.py`) — Hourly cron job, reads Firestore/Redis, sends push notifications, runs X connector sync, drains the retired screen-activity cloud copies (Firestore rows + Pinecone vectors; `SCREEN_ACTIVITY_CLOUD_PURGE_USERS_PER_RUN`, default 200 users/pass, `0` disables), and emits the weekly read-only Chat-first materialization-health verdict consumed by its routed Cloud Monitoring decision review. No canonical maintenance flags or Typesense secrets; its deploy removes only those bindings and keeps other notification/X-sync env. Selection mode + run lock: `docs/runbooks/daily-summary-cutover.md`.
