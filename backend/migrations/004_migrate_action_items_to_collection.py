...
def process_conversation_batch(conversations_batch: List[Dict[str, Any]], batch_num: int) -> Dict[str, int]:
            ...
            for action_item in action_items:
                ...
                if batch_operations >= FIRESTORE_BATCH_SIZE:
                    safe_firestore_operation(
                        lambda: batch_write.commit(),
                        f"Batch commit for batch {batch_num}"
                    )
                    batch_write = db.batch()
                    batch_operations = 0
                    time.sleep(0.2)
    ...
    if batch_operations > 0:
        try:
            safe_firestore_operation(
                lambda: batch_write.commit(),
                f"Final batch commit for batch {batch_num}"
            )
        except Exception as e:
    ...
    log_progress(f"Batch {batch_num}: Processed {batch_conversations} conversations, "
                f"migrated {batch_action_items} action items, {batch_errors} errors")
    
    return {
        'conversations': batch_conversations,
        'action_items': batch_action_items,
        'errors': batch_errors
    }
...
def get_conversations_for_user_batch(uid, offset=0, limit=None):
    ...
    try:
        ...
        query = conversations_ref.where(
            filter=firestore.FieldFilter('structured.action_items', '!=', [])
...
def get_conversations_with_action_items():
    ...
    while True:
        ...
        for user_doc in users_batch:
            ...
            while True:
                user_conversations = get_conversations_for_user_batch(
                    uid, offset=conv_offset
                )
...
def verify_migration():
    ...
    for user_doc in sample_users_docs:
        ...
        conversations_with_items_docs = safe_firestore_operation(
            lambda: conversations_ref.where(
                filter=firestore.FieldFilter('structured.action_items', '!=', [])
            ).limit(10).get(),
        ...
        dedicated_action_items_docs = safe_firestore_operation(
            lambda: action_items_ref.limit(100).get(),
            f"Get dedicated action items for user {uid}"
        )
        dedicated_count = len(dedicated_action_items_docs)
        
        log_progress(f"User {uid}: {conv_action_items_count} items in conversations, "
