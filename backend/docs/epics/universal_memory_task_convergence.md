...
| Product and developer documentation | `PRODUCT.md`, `product/invariants/**`, `backend/docs/epics/{memory_normative_architecture.md,universal_memory_task_convergence.md}`, `backend/docs/memory/domain_model.md`, `backend/utils/memory/ARCHITECTURE.md`, `backend/utils/task_intelligence/ARCHITECTURE.md`, `backend/docs/task_intelligence_baseline.md`, `docs/doc/developer/backend/{canonical_memory_architecture.md,canonical_memory_architecture.html,task_candidate_lifecycle.mdx,goal_workstream_lifecycle.mdx}` |
| Runbooks and proof documents | `backend/docs/runbooks/{universal-memory-operations.md,historical-memory-repair.md,account-cohort-cutover.md}`, memory IAM/readiness epics, and operational evidence-marker docs. The per-UID enrollment/activation/proof-order runbooks were deleted. |
...
- [x] Update `backend/docs/memory/domain_model.md`, memory architecture maps, Mintlify developer architecture, and task-intelligence architecture/baseline.
...
The code convergence does not authorize deployment or destructive cleanup. Before enabling the release in production, operators must record the offline/dev proofs, mixed-account latency and read-cost budget, outbox/provider lag, deletion reconciliation, and rollback rehearsal described below and in `backend/docs/runbooks/universal-memory-operations.md`. Those runtime receipts are intentionally not fabricated in this PR.
