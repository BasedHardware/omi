...
for the accepted-send denominator and `omi_audio_timeline_rejects_total{mode="legacy|v2",reason="..."}`
for bounded reject reasons. These counts contain no user or session labels. A
mapped legacy segment retains its provider-native persisted timestamps; the
mapping only attaches a private capture window for live speaker identification.
