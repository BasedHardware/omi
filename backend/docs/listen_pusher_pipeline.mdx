...
Absent/missing target IDs do not partition temporal membership. Missing explicit targets and hint-only empty live
stubs use ordinary temporal assignment; stubs remain untouched. Existing explicit
targets retain live attachment and identity even when empty.
Source, device and lock state isolate components; unknown device is its own partition.

`utils/conversation_continuity.py` owns the shared predicate. Realtime measures its persisted finished-at clock (currently callback wall time);
sync measures VAD/word interval ends. Realtime can also configure its timeout per session, while WALs
...
live-flap stub per WAL. Repeated durable client-origin reconnects now reuse the same active continuation
inside the silence window, even if the immutable original is terminal. Expired
empty continuations are cleaned before another STT attempt through the existing
content-fenced lifecycle deletion. The last abandoned stub still uses stale recovery.
Different existing explicit WAL targets are not retroactively merged. See `utils/sync/ARCHITECTURE.md` for recovery and indexing limits.


## Opt-in provider routing

`STT_ROUTING_MODE=legacy` (default/unset) preserves the existing provider path.
`ordered` enables `config/stt_routing.py` and one `LiveSTTRoute` attempt list for
initial connect and mid-session replacement; `health` also uses shared Redis
admission. The adapter is `routers/listen/provider_routing.py`. Multichannel,
custom STT and BYOK remain on their existing paths.

Order comes from `STT_SERVICE_MODELS`, read per new session. Optional
`STT_ROUTING_POLICY_JSON` is an array of `{token, priority, weight}` (at most 16):
lower priority first, weighted seeded order within a priority. Invalid policy
falls open to environment order with telemetry. Existing serving permissions,
credentials, language and capabilities still constrain every candidate. Soniox
can be a connect-time leg in this path. A session never retries a consumed provider.

Health mode requires `STT_HEALTH_NAMESPACE`, a non-secret deployment/account/surface
identity that isolates dev and production. Missing namespace or unavailable Redis
falls open to order, visibly. Redis clients have 150ms socket deadlines and no
backoff; health entries expire in 900 seconds. Three transient failures open for
30–37.5 seconds; typed budget/payment/quota/auth failures open immediately for
300–375 seconds. One half-open probe gets a 30-second lease. Only nonblank transcript
content closes that probe or emits recovery; socket acceptance is not success.
Connector attempts have an eight-second async budget; native SDK cancellation
latency still needs isolated validation. No credentials or raw provider text
enter health keys or labels. The receiver monitor flushes transcript health receipts.

Rollback is `STT_ROUTING_MODE=legacy` for new sessions. No remote policy hot reload,
hedged billable connections, client retry hint, or historical conversation merge
ships here. Environment changes need the existing deployment/restart process.
