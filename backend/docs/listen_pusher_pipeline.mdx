...
> Last updated: 2026-09-13 (cross-device duplicate capture folded at finalization; provider-scoped speaker identity and bounded clustering; durable meeting receipt; bounded client-kind propagation; end-of-conversation wake-word command adjudication; private conversation start-location provenance; explicit finalization capability admission; semantic release probes; identity-bound Pusher promotion receipts)
>
...
The duplicate is resolved at finalization instead (#3244). Before the LLM
discard gate, `process_conversation` asks `utils.conversations.duplicate_capture`
whether another capture client's conversation already carries this one:
`completed` or `processing` conversations of the same user whose activity
clock runs past this recording's `started_at` (`get_conversations_finished_after`,
served by the existing `(status, finished_at)` composite), from a different
`client_device_id` (or platform/source pair on legacy rows), covering at least
80% of this recording's wall window for at least 30 seconds, and holding at
least 50% of its word bigrams. Containment is measured against *this*
conversation's words, so a capture with speech the other one lacks keeps a low
score and survives. When it matches, the conversation takes the same discard
exit as a scrap — transcript and audio stay on the row, no summary, memories,
or integrations are produced — and `external_data.duplicate_capture_of` names
the primary.

Exactly one side yields: a `completed` counterpart is always primary, and a
`processing` counterpart is primary only when it was created earlier, so two
sessions timing out on the same silence and finalizing concurrently cannot both
discard themselves or both survive. Reprocessing (`force_process`) never
consults the check — the existing "reprocess never discards" contract holds —
and a failed candidate read fails open to the ordinary gate through
`record_fallback(component='conversation_finalization')`, which is the pre-fix
outcome of two visible conversations rather than a lost one.
