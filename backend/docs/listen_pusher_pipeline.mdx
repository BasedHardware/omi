...
components, with deterministic chunk anchors and redirect tombstones for late
bridges. Explicit capture IDs survive a missing server conversation. Source,
device, lock state, and capture identity isolate components; unknown device IDs
are their own partition. No client or wire-schema change is required.
