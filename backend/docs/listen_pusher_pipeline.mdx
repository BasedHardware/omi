...
neighboring epoch or replayed into a newer conversation. Ownership of recent
...
owner. A late batch whose owning conversation is still `in_progress` is
...
against a bounded digest ledger of recently flushed runs (`(start, end,
sha256)` per chunk); a range that can be neither byte-compared nor
digest-matched is a conflict — the frame is dropped and counted in
`omi_audio_timeline_replay_conflicts_total`, never partially stored. Chunk
...
conflicting content fails closed. Listing rehydrates spans; grouping splits
