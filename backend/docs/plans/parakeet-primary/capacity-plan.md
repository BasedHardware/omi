...
- Change rolling update to `maxUnavailable: 0`, `maxSurge: 1`, with surge node capacity already schedulable. Add a disruption budget allowing at most one unavailable replica, but retain a minimum two healthy service replicas where the approved traffic budget requires it. HPA scale-down and rollout strategy still need their own safety checks; a PDB does not govern every termination path.
- Require hostname separation for the two warm replicas after scheduling proof. Use preferred zone spreading initially, record its actual placement, and do not claim zone-failure tolerance. To require zone resilience, qualify cross-zone placement and replace the single-pod-loss sizing term with loss of the largest zone. Hard zone constraints without available GPUs can prevent scheduling.
