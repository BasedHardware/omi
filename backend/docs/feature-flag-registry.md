...
| `FREE_TIER_LOCAL_PROCESSING` + `FREE_TIER_LOCAL_PROCESSING_COHORT` | Free-tier AFM local processing (Stage 1) | runtime_env, explicit UID cohort | closed: unset/empty cohort admits nobody; malformed/percentage cohort fails deploy | dev only, accounts with AFM-capable Macs named by the GitHub Actions variable `DEV_FREE_TIER_LOCAL_PROCESSING_COHORT` (development environment scope; never read by production workflows); prod nobody | no | `true` dev / `false` prod; cohort empty by default | `utils/free_tier_processing_policy.free_tier_local_processing_enabled(uid)` → `utils/free_tier_cohort`; all main-backend hosts including integration/backfill, listener/pusher, and desktop-backend deploy wiring |
| `FREE_TIER_MEMORY_SUPPRESSION` + `FREE_TIER_MEMORY_SUPPRESSION_COHORT` | Free-tier server memory suppression (S5) | runtime_env, cohort-scoped | closed: same rule; un-formed memories never backfill | nobody | no | both unset | `utils/free_tier_memory_policy.free_tier_memory_suppression_enabled(uid)`; sweep, connectors and `process_conversation.py` all pass `uid` |
| `FREE_TIER_EMERGENCY_STOP` | Free-tier environment stop | runtime_env | n/a (stop only) | nobody | no | explicit `false` dev/prod | `utils/free_tier_cohort.emergency_stop_engaged` — revokes every free-tier cohort incl. listed uids; also forces the three `BASIC_PLAN_GATE_*` switches off; needs a redeploy |
| `BASIC_PLAN_GATE_PROACTIVITY_ENABLED` | Basic-plan gate: desktop proactivity completions | runtime_env | closed (off) | nobody in prod (explicit `false`); dev/Beta `true` on desktop-backend | no | `false` prod / `true` dev | `utils.free_tier_basic_gates.basic_plan_gate_proactivity_enabled` — 402 `plan_gated` on `/v1/desktop/proactivity/completions`; unset identical to main before #14165 |
| `BASIC_PLAN_GATE_PROXY_EMBED_ENABLED` | Basic-plan gate: Gemini embed proxy | runtime_env | closed (off) | nobody in prod (explicit `false`); dev/Beta `true` on desktop-backend and Cloud Run backend API | no | `false` prod / `true` dev | `utils.free_tier_basic_gates.basic_plan_gate_proxy_embed_enabled` — 402 on `embedContent` / `batchEmbedContents` only; generate/stream stay gated unconditionally |
| `BASIC_PLAN_GATE_EAGER_EXTRACTION_ENABLED` | Basic-plan gate: identified-basic first-open / reprocess | runtime_env | closed (off) | nobody in prod (explicit `false`); dev/Beta `true` on every `process_conversation` host | no | `false` prod / `true` dev | `utils.free_tier_basic_gates.basic_plan_gate_eager_extraction_enabled` — `_flag_off_identified_basic_deny` in `process_conversation.py` |
| `free-tier-kill-switch-v1` | Free-tier remote stop | posthog | never blocks unless **definitively enabled** (JIT kill precedent); read only for an already-admitted uid | nobody (row missing) | no | — | `utils/free_tier_cohort._kill_switch_state` — read with the same client configuration and tri-state mapping as the JIT rollout but on this module's own dedicated executor (no `PostHogJITFlagProvider` sharing, so a stall cannot cross the JIT bulkhead); `free-tier-cohort-v1` exposure key is read but never admits |
...
| `CONVERSATION_NOTES_V2_ENABLED` | Conversation notes v2 | env | closed (off) unless set | prod + dev (true since 2026-09-01) on every `process_conversation` host: `gke/backend-listen`, `gke/pusher`, `cloud_run/backend` (reprocess), `cloud_run/backend-sync` (Cloud Tasks `conversation-finalization` for pendant/phone). `_base` default is false | no | `CONVERSATION_NOTES_V2_ENABLED` | `_flag_enabled` / `summary_pipeline_mode` in `backend/utils/conversations/process_conversation.py` |
| `CONVERSATION_CALENDAR_CONTEXT_READ_ENABLED` | Calendar context read | env | closed (off) | false prod, true dev (Beta Mac talks to dev). Same four-host roster as notes v2; omit on any host and that path silently skips calendar context | no | `CONVERSATION_CALENDAR_CONTEXT_READ_ENABLED` | `_calendar_context_read_enabled` in `process_conversation.py` |
| `CONVERSATION_OCR_CONTEXT_ENABLED` | OCR meeting context | env | closed (off) | false prod, true dev. Same four-host roster as notes v2 | no | `CONVERSATION_OCR_CONTEXT_ENABLED` | `_ocr_meeting_context_enabled` in `process_conversation.py` |
| `MEETING_NOTES_RICH_CONTEXT_ENABLED` | Meeting notes rich context | env | closed (off) | false prod, true dev. Same four-host roster as notes v2; omit on any host and that path silently keeps legacy notes | no | `MEETING_NOTES_RICH_CONTEXT_ENABLED` | `_flag_enabled` in `process_conversation.py` (`_get_structured` rich branch) |
| `MEETING_NOTES_SCREEN_TEXT_CONTEXT_ENABLED` | Screen/OCR text context for notes | env | closed (off) | false prod, true dev. Same four-host roster as notes v2; only read when the rich-context flag is on | no | `MEETING_NOTES_SCREEN_TEXT_CONTEXT_ENABLED` | `_flag_enabled` in `meeting_context_pack.py` (`gather_meeting_context_pack`) |
