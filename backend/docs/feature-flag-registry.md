...
| `FREE_TIER_LOCAL_PROCESSING` + `FREE_TIER_LOCAL_PROCESSING_COHORT` | Free-tier local processing (S6) | runtime_env, cohort-scoped | closed: flag on with cohort unset/malformed/no uid admits nobody | nobody (flag unset on every identity, 2026-09-09) | no | both unset | `utils/free_tier_processing_policy.free_tier_local_processing_enabled(uid)` → `utils/free_tier_cohort` (`uid:<uid>,pct:<n>`; `process_conversation.py` passes no uid yet, so that site lights nobody) |
| `FREE_TIER_MEMORY_SUPPRESSION` + `FREE_TIER_MEMORY_SUPPRESSION_COHORT` | Free-tier server memory suppression (S5) | runtime_env, cohort-scoped | closed: same rule; un-formed memories never backfill | nobody | no | both unset | `utils/free_tier_memory_policy.free_tier_memory_suppression_enabled(uid)`; sweep and connectors pass `uid`, `process_conversation.py` does not yet |
| `FREE_TIER_EMERGENCY_STOP` | Free-tier environment stop | runtime_env | n/a (stop only) | nobody | no | unset | `utils/free_tier_cohort.emergency_stop_engaged` — revokes every free-tier cohort incl. listed uids; also forces the three `BASIC_PLAN_GATE_*` switches off; needs a redeploy |
