...
def validate_bucket_contract(
    ...
    for index, rule in enumerate(rules):
        if isinstance(rule, Mapping) and isinstance(rule.get("condition"), Mapping):
            ...
            if any(key in condition for key in ("age", "createdBefore", "customTimeBefore", "daysSinceCustomTime")):
                errors.append(
...
def main() -> int:
    ...
    # A source/predeploy check may prove the runtime binding without knowing a
    # live bucket name. The live mode still requires --bucket and can pair it
    # with a gcloud describe JSON document.
    errors = validate_bucket_contract(args.bucket, lifecycle) if args.bucket or not args.runtime_env else []
    if args.runtime_env:
        try:
            import yaml
