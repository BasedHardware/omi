...
def check_dirty(files: dict[str, str], root: Path) -> list[str]:
    ...
    for path in root.rglob('*'):
        ...
        try:
            ...
            continue
        if 'AUTO-GENERATED by backend/scripts/generate_integration_sdks.py' in content:
            dirty.append(f'orphan {rel}')
    return dirty
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ...
        with tempfile.TemporaryDirectory() as expected_dir:
            ...
            write_tree(expected_root, files)
            _format_dart_tree(expected_root)
            files = {rel: (expected_root / rel).read_text(encoding='utf-8') for rel in files}
