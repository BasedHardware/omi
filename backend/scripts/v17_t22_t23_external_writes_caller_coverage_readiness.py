...
LOCAL_ROUTE_ADAPTER_AND_TEST_REFERENCES = {
    ...
    "legacy_write_guard": "backend/utils/memory/v17_default_read_rollout.py assert_legacy_memory_write_allowed_for_default_read_decision",
    "external_default_read_authorization": "backend/utils/memory/v17_product_authorization.py authorize_v17_external_default_memory_read",
    ...
    "cutover_checklist": "backend/scripts/v17_cutover_evidence_readiness.py",
    "ticket_source_of_truth": "docs/epics/v17_memory_implementation_tickets.md T22/T23",
    "oracle_review": "docs/epics/v17_t20_oracle_milestone_review.md Oracle P0-8/T22/T23",
    "existing_guard_and_caller_tests": "backend/tests/unit/test_v17_developer_memory_adapter.py; backend/tests/unit/test_v17_mcp_memory_adapter.py; backend/tests/unit/test_v17_chat_memory_adapter.py; backend/tests/unit/test_v17_product_authorization.py; backend/tests/unit/test_v17_app_key_memory_grant_assignment_readiness.py",
}

PROOF_MATRIX: Dict[str, Dict[str, Any]] = {
    "external_create_write_read_convergence": {
        "status": "NOT_RUN",
        "scope": "Inventory external create/edit/delete/list/search write/read convergence proof beginning with create paths before authoritative V17 reads.",
        "required_artifacts": [
            "Create-path fixture proving /v3, Developer API, MCP REST, MCP SSE tool, platform tool, chat tool, and agent caller writes converge into the authoritative V17-visible projection or are blocked before V17 authoritative reads.",
            "Read-after-create evidence from product, Developer API, MCP REST/SSE, chat, tool, and agent callers showing one canonical item identity, tier, category, visibility, provenance, ledger/outbox state, and no duplicate legacy/V17 presentation.",
        ...
        "pass_fail_criteria": "PASS only when every external create path either writes through the approved V17 convergence mechanism or is blocked before authoritative V17 reads can diverge from legacy writes.",
        "evidence": [],
    },
    "external_edit_write_read_convergence": {
        ...
        "required_artifacts": [
            ...
            "Read-after-edit proof that product default reads, Developer list/vector, MCP REST/SSE list/search, chat search, and platform tools return the edited V17-authoritative value and never stale legacy/vector content after write convergence is marked ready.",
            "Conflict proof for concurrent edits, stale revisions, malformed category/tier changes, review state changes, and rollback from V17 edit handling to legacy-safe behavior before cutover.",
        ],
        "pass_fail_criteria": "PASS only when edits converge across item store, projections, vector metadata, and all callers without stale legacy unsafe fallback after V17 writes.",
        "evidence": [],
    },
    "external_delete_write_read_convergence": {
        ...
        "required_artifacts": [
            ...
            "Bulk delete/account purge/delete account compatibility evidence tying legacy memory deletes, V17 memory_items, review records, import lineage, vectors, and repair outbox handling to one deletion outcome.",
        ],
        "pass_fail_criteria": "PASS only when deletes are durable, converged, visible to every caller as absence/tombstone per contract, and never bypass V17 deletion/review/import compatibility requirements.",
        "evidence": [],
    },
    "external_list_search_read_convergence": {
        ...
        "required_artifacts": [
            ...
            "No-split-brain report proving legacy users remain legacy-only while V17-enabled users do not see inconsistent write/read results across external callers.",
        ],
    ...
    "developer_api_write_read_paths": {
        "status": "NOT_RUN",
        "scope": "Inventory Developer API write/read paths for create, batch create, edit, delete, list, category filters, and vector search before authoritative V17 reads.",
        "required_artifacts": [
            ...
            "Read-after-write regression proving Developer API writes cannot remain legacy-only after V17 default reads are authoritative and category-filter compatibility no longer hides or resurrects records.",
    ...
    "mcp_rest_write_read_list_search_paths": {
        ...
        "required_artifacts": [
            ...
            "MCP REST error/empty/rollback response-shape compatibility evidence for disabled rollout, malformed grant state, missing scopes, revoked grants, and V17 enabled-but-empty states.",
        ],
    ...
    "mcp_sse_tool_write_read_list_search_paths": {
        ...
        "required_artifacts": [
            ...
            "Compatibility evidence that MCP tool advertisements/security schemes never grant V17 access by themselves and that tool calls fail closed when app/key/scope state is absent or malformed.",
        ],
    ...
    "chat_tool_agent_caller_coverage": {
        ...
        "required_artifacts": [
            "Chat search_memories_tool regression proving V17 default reads after external writes, disabled/no-grant denial or safe legacy mode, Archive default-unavailable, and compatible tool output text/JSON shape.",
            ...
            "Agent execution coverage for /v1/agent/execute-tool and agent remember/forget/update/search flows proving no direct legacy unsafe fallback after V17 writes and no unscoped Archive exposure.",
        ],
    ...
    "dual_write_outbox_or_v17_write_convergence_plan": {
        "status": "NOT_RUN",
        "scope": "Inventory dual-write/outbox or V17-write convergence plan required before authoritative external reads consume new write outcomes.",
        "required_artifacts": [
            "Design and tested implementation evidence choosing one convergence mechanism: durable V17 write service, safe dual-write, or outbox-driven convergence from legacy writes into V17 memory_items/projections/vectors.",
            ...
            "Cutover readiness proof that external writes cannot bypass the convergence mechanism once V17 reads are authoritative and that convergence failures block rollout instead of falling back unsafely.",
        ],
        "pass_fail_criteria": "PASS only when a durable, tested write convergence plan prevents legacy/V17 split-brain for all external write paths before authoritative V17 reads.",
        "evidence": [],
    },
    "delete_review_import_compatibility": {
        ...
        "required_artifacts": [
            ...
            "Rollback evidence proving delete/review/import state is not lost when switching between legacy-safe and V17-authoritative readers after external writes.",
        ],
    ...
    "no_legacy_unsafe_fallback_after_v17_writes": {
        "status": "NOT_RUN",
        "scope": "Inventory no legacy unsafe fallback after V17 writes for disabled, malformed, no-grant, partial-outage, and rollback states.",
        "required_artifacts": [
            "Truth table identifying when USE_LEGACY_SAFE is still legal after V17 writes, what fallback_projection_ready/reconciliation/generation proof is required, and when DENY_MEMORY must be returned instead.",
        ...
        "pass_fail_criteria": "PASS only when legacy fallback after V17 writes is either proven reconciled and safe or denied consistently across product, Developer API, MCP, chat, tools, and agents.",
        "evidence": [],
    },
    ...
    "archive_default_unavailable": {
        ...
        "required_artifacts": [
            ...
            "Rollback/disable evidence proving Archive remains default-unavailable in legacy-safe mode, V17-authoritative mode, and denied states.",
        ],
    ...
    "response_shape_compatibility": {
        "status": "NOT_RUN",
        "scope": "Inventory response-shape compatibility for external create/edit/delete/list/search and chat/tool/agent outputs before authoritative V17 reads.",
        "required_artifacts": [
            ...
            "Error/empty/denied/malformed/rollback fixtures proving new V17 policy, convergence, cursor, tier, provenance, and grant fields are additive or intentionally mapped without breaking existing clients.",
    ...
    "rollback_disable_behavior": {
        ...
        "required_artifacts": [
            "Rollback drill proving global/user/consumer disable states stop new V17 external writes or reads as required, preserve already-converged data, and avoid unsafe legacy fallback after V17 writes.",
            "Disable behavior matrix for product, /v3, Developer API, MCP REST/SSE, chat, tools, and agent callers across legacy-only users, shadow-only users, V17-authoritative users, and write-convergence blocked users.",
...
def build_readiness_artifact(config: T22T23ExternalWritesCallerCoverageReadinessConfig) -> Dict[str, Any]:
    return {
        ...
        "required_scope_summary": "external create/edit/delete/list/search write/read convergence; Developer API write/read paths; MCP REST/SSE write/read/list/search paths; chat/tool/agent caller coverage; dual-write/outbox or V17-write convergence plan; delete/review/import compatibility; no legacy unsafe fallback after V17 writes; app/key/scope grant enforcement; Archive default-unavailable; response-shape compatibility; rollback/disable behavior",
        ...
        "planned_safe_commands": [
            ...
            "python3 backend/scripts/v17_t22_t23_external_writes_caller_coverage_readiness.py --execute",
            "pytest tests/unit/test_v17_t22_t23_external_writes_caller_coverage_readiness.py -q",
            "pytest tests/unit/test_v17_cutover_evidence_readiness.py -q",
