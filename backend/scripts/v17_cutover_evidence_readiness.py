...
CUTOVER_EVIDENCE_GATES: Dict[str, Dict[str, Any]] = {
    ...
    "real_pinecone_validation": {
        ...
        "required_proof_commands_or_artifacts": [
            "python3 backend/scripts/v17_vector_search_provider_readiness.py --execute with approved Pinecone read-only credentials",
            "python3 backend/scripts/v17_pinecone_repair_validation_readiness.py --execute with approved throwaway namespace controls",
    ...
    "real_firestore_cloud_iam_rules_validation": {
        ...
        "required_proof_commands_or_artifacts": [
            "python3 backend/scripts/v17_firestore_rules_iam_proof.py --execute --project <approved-project>",
    ...
    "t21_v3_compatibility_cursor_pagination": {
        ...
        "required_proof_commands_or_artifacts": [
            "T21 `/v3` compatibility and cursor pagination test matrix for legacy/V17 readers",
            "Compatibility evidence for category filters, pagination cursors, stable ordering, deleted/non-active records, and default Archive-unavailable behavior",
            "Regression output for affected `/v3` endpoints and product/developer/MCP callers",
        ],
        "blockers": ["no T21 /v3 compatibility proof", "cursor pagination behavior not cutover-proven"],
        "evidence": [],
        "approval_claimed": False,
    },
    "t22_t23_external_writes_and_caller_coverage": {
        ...
        "required_proof_commands_or_artifacts": [
            "T22/T23 external writes and caller coverage matrix for create/edit/delete/list/search/tool/agent paths",
            "Durable V17 write convergence or tested dual-write/outbox evidence before authoritative external reads",
            "Caller coverage regression proving app/key/scope grants and no legacy unsafe fallback across developer, MCP REST/SSE, chat, product, and agent tools",
        ],
        "blockers": ["external write/read split-brain remains", "caller coverage incomplete"],
        "evidence": [],
        "approval_claimed": False,
    },
...
def build_readiness_artifact(config: CutoverEvidenceReadinessConfig) -> Dict[str, Any]:
    return {
        ...
        "planned_safe_commands": [
            "python3 backend/scripts/v17_cutover_evidence_readiness.py",
            "python3 backend/scripts/v17_cutover_evidence_readiness.py --execute",
        ],
