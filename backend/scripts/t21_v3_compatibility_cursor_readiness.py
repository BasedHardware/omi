...
LOCAL_ROUTE_ADAPTER_AND_TEST_REFERENCES = {
    ...
    "ticket_source_of_truth": "docs/epics/memory_implementation_tickets.md T21",
    "oracle_review": "docs/epics/memory_t20_oracle_milestone_review.md Oracle P0-8/T21",
    "existing_memory_read_tests": "backend/tests/unit/test_product_memory_read_service.py; backend/tests/unit/test_product_memory_router.py; backend/tests/unit/test_developer_memory_adapter.py; backend/tests/unit/test_memory_mcp_memory_adapter.py; backend/tests/unit/test_chat_memory_adapter.py",
...
PROOF_MATRIX: Dict[str, Dict[str, Any]] = {
    "v3_endpoint_legacy_shape_compatibility": {
        "status": "NOT_RUN",
        "scope": "Inventory /v3 endpoint compatibility proof for legacy and memory-backed readers while preserving legacy response model and additive-only fields.",
        "required_artifacts": [
            "Static and runtime response-shape diff for backend/routers/memories.py GET /v3/memories versus any memory-backed replacement path, including existing List[MemoryDB] parse compatibility.",
        ...
        "pass_fail_criteria": "PASS only when /v3 endpoint compatibility is demonstrated for legacy clients and any memory-read rollout remains additive, server-authorized, and rollback-safe.",
        "evidence": [],
    },
    "stable_cursor_pagination_and_ordering": {
        "status": "NOT_RUN",
        "scope": "Prove stable cursor pagination and stable ordering for legacy/memory readers before high-volume imports become visible.",
        "required_artifacts": [
            ...
            "A/B proof that legacy offset pagination from backend/database/memories.py get_memories and memory cursor pagination do not duplicate, skip, or reorder records under inserts/deletes between pages.",
        ...
        "pass_fail_criteria": "PASS only when stable cursor pagination provides deterministic pages with no duplicate/skip behavior and stable ordering is documented and tested across legacy/memory rollback.",
        "evidence": [],
    },
    "category_filters_and_developer_category_filtering": {
        "status": "NOT_RUN",
        "scope": "Prove category filters and developer category filtering behave compatibly across legacy /v3, memory product reads, developer API, MCP, and chat callers.",
        "required_artifacts": [
            ...
            "Developer category filtering proof that GET /v1/dev/user/memories category requests do not bypass required memory authorization or expose legacy-only split-brain results during rollout.",
    ...
    "disabled_malformed_no_grant_behavior": {
        "status": "NOT_RUN",
        "scope": "Prove disabled/malformed/no-grant behavior is fail-closed and compatible across legacy/memory readers and external callers.",
        "required_artifacts": [
            "Truth table for disabled/malformed/no-grant states covering product, developer, MCP REST, MCP SSE, chat, and /v3 readers, including explicit USE_MEMORY/USE_LEGACY_SAFE/DENY_MEMORY decisions.",
            "Regression output proving revoked grants and malformed rollout state do not silently downgrade to unsafe legacy reads after memory writes are enabled.",
    ...
    "enabled_but_empty_behavior": {
        ...
        "required_artifacts": [
            "Enabled-but-empty fixture matrix for empty authoritative memory store, empty-after-filtering, empty-after-hydration, and genuinely empty user account.",
    ...
    "deleted_non_active_and_archive_default_unavailable": {
        "status": "NOT_RUN",
        "scope": "Prove deleted/non-active records remain excluded and Archive default-unavailable remains preserved across /v3 and memory read compatibility paths.",
        "required_artifacts": [
            ...
            "Rollback proof that non-active and Archive exclusions remain identical when switching between legacy and memory-compatible read paths.",
        ],
    ...
    "external_response_shape_compatibility": {
        ...
        "required_artifacts": [
            "Schema diff artifact for /v3, /memory product, /v1/dev/user/memories, MCP REST, MCP SSE, and chat search memory tool responses.",
    ...
    "product_developer_mcp_chat_caller_regression": {
        ...
        "required_artifacts": [
            "Product route regression for /v3 compatibility and /memory product reads, including stable cursor pagination, category filters, stable ordering, disabled/no-grant, enabled-but-empty, deleted/non-active, and Archive default-unavailable cases.",
            "Developer API regression for list/vector/category paths proving developer category filtering and response-shape compatibility across legacy/memory rollout decisions.",
