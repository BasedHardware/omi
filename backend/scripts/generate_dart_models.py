...
DEFAULT_OUTPUT_PATH = ROOT_DIR / 'app' / 'lib' / 'backend' / 'schema' / 'gen' / 'conversation_wire.g.dart'
...
def dart_type_for(schema: dict[str, Any], required: bool) -> DartType:
    ...
    if schema_type == 'array':
        item_type = dart_type_for(unwrapped.get('items', {'type': 'object'}), True)
...
def fields_for_schema(schema_name: str, schema: dict[str, Any]) -> list[Field]:
    ...
    for wire_name, prop_schema in schema.get('properties', {}).items():
        fields.append(
            Field(
                ...
                dart_type=dart_type_for(prop_schema, wire_name in required),
...
def build_output(spec: dict[str, Any]) -> str:
    ...
    chunks = [
        ...
        '// Generated by backend/scripts/generate_dart_models.py from docs/api-reference/app-client-openapi.json.',
        '',
    ]
    for schema_name in TARGET_SCHEMAS:
        chunks.append(emit_class(schema_name, fields_for_schema(schema_name, schemas[schema_name])))
        chunks.append('')
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--output', default=str(DEFAULT_OUTPUT_PATH), help='Dart output path')
...
def main() -> int:
    ...
    generated = build_output(json.loads(Path(args.spec).read_text()))
    if args.check:
