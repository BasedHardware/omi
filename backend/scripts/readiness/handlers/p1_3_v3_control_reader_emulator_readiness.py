...
import sys as _sys
from pathlib import Path as _CtrlPath

_CTRL_BACKEND = _CtrlPath(__file__).resolve().parents[3]
if str(_CTRL_BACKEND) not in _sys.path:
    _sys.path.insert(0, str(_CTRL_BACKEND))

from scripts.readiness.loader import build_report as _external_gate_report


def _linked_readiness_proofs() -> dict[str, Any]:
    external = _external_gate_report("p1_3_v3_external_compatibility_readiness", execute=False)
    return {
        "control_reader_contract_proof": external["control_reader_contract_proof"],
        "control_reader_readiness_proof": external["control_reader_readiness_proof"],
        "get_runtime_wiring_readiness_proof": external["get_runtime_wiring_readiness_proof"],
        "get_dependency_auth_readiness_proof": external["get_dependency_auth_readiness_proof"],
    }
...
def build_report(*, execute: bool = False, env: Mapping[str, str] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "linked_readiness_proofs": _linked_readiness_proofs(),
        ...
        "summary": {
            ...
            "linked_readiness_proof_count": len(_linked_readiness_proofs()),
