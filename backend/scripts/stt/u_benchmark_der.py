...
def run_der_benchmark(manifest: list):
    ...
    print(
        tabulate(
            table, headers=['Conv', 'Spk', 'Dur', 'Turns', 'DG DER', 'DG Spk', 'Mod DER', 'Mod Spk'], tablefmt='grid'
        )
    )

    valid_dg = [r for r in results if f'deepgram_der' in r]
    valid_mod = [r for r in results if f'modulate_der' in r]
    print('\nSUMMARY:')
    if valid_dg:
        print(
