#!/usr/bin/env bash
# Verify composed runtime_env.yaml with the repository's locked PyYAML dependency.
set -euo pipefail
...
exec "$BACKEND_PYTHON" "$ROOT_DIR/backend/deploy/compose_runtime_env.py" --check "$@"
