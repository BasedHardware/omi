...
def test_purge_derived_user_data_isolates_backends_and_reloads_conversation_ids(monkeypatch):
    ...
    monkeypatch.setattr(
        account_deletion, 'delete_all_projection_images', lambda uid: calls.append(('projection_images', uid)) or 2
    )
    monkeypatch.setattr(
        account_deletion,
...
def test_purge_derived_user_data_continues_after_each_failure(monkeypatch):
    ...
    monkeypatch.setattr(
        account_deletion, 'delete_all_projection_images', MagicMock(side_effect=Exception('projection gcs down'))
    )
    monkeypatch.setattr(
    ...
    account_deletion.delete_all_projection_images.assert_called_once_with('uid1')
...
def test_purge_derived_user_data_fails_required_vectors_when_index_missing(monkeypatch):
    ...
    monkeypatch.setattr(account_deletion, 'delete_all_projection_images', MagicMock())
