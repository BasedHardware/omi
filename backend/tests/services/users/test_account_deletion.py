...
def test_background_wipe_user_data_preserves_order(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda uid, **_kwargs: calls.append(('purge', uid)) or {'required_failures': [], 'best_effort_failures': []},
    )
    monkeypatch.setattr(
        account_deletion.users_db,
        'delete_user_data',
...
def test_purge_derived_user_data_requires_empty_follow_up_descriptor_projection(monkeypatch):
    ...
    monkeypatch.setattr(
        account_deletion, 'purge_canonical_derived_user_data', lambda uid: calls.append(('canonical', uid))
    )
    monkeypatch.setattr(
        account_deletion,
...
def test_purge_claims_conversations_to_a_fixed_point_before_vector_cleanup(monkeypatch):
    ...
    monkeypatch.setattr(account_deletion, 'get_memory_ids', MagicMock(return_value=[]))
