...
class TestPhotoServerSideCacheContract:
    """Photo ack must mean server-side cache is durable, not just socket-received."""

    def test_photo_processing_ack_happens_after_durable_photo_store(self):
        router_path = os.path.join(os.path.dirname(__file__), '..', '..', 'routers', 'transcribe.py')
        with open(router_path, encoding='utf-8') as f:
            source = f.read()

        start = source.index('async def process_photo')
        end = source.index('async def handle_image_chunk', start)
        process_photo = source[start:end]

        store_index = process_photo.index('store_conversation_photos')
        processing_index = process_photo.index('PhotoProcessingEvent')
        described_index = process_photo.index('PhotoDescribedEvent')

        assert store_index < processing_index < described_index

    def test_meta_wearables_photo_cache_route_is_rest_not_listen_socket(self):
        router_path = os.path.join(os.path.dirname(__file__), '..', '..', 'routers', 'conversations.py')
        with open(router_path, encoding='utf-8') as f:
            source = f.read()

        assert '/v1/meta-wearables/photos/cache' in source
        assert 'MetaWearablesPhotoCacheRequest' in source
        assert 'store_conversation_photos' in source
        assert 'ConversationSource.openglass' in source
        assert 'ConversationStatus.processing' in source
...
def _build_meta_wearables_photo_cache_harness():
    source = Path('routers/conversations.py').read_text()
    start = source.index('class MetaWearablesPhotoCacheRequest')
    end = source.index('@router.post("/v1/conversations"', start)
    ...
    class FakeDatetime:
        ...
        def now(cls, tz):
            cls.calls += 1
            return datetime(2026, 7, 5, 12, 0, cls.calls, tzinfo=tz)
    ...
    class FakeConversationPhoto:
        def __init__(self, id, base64, created_at, discarded):
            self.id = id
            self.base64 = base64
    ...
    class FakeDocument:
        ...
        def get(self):
            if len(self.path) >= 6 and self.path[-2] == 'photos':
                uid = self.path[1]
                conversation_id = self.path[3]
                photo_id = self.path[5]
                photo = self.db.photos.get((uid, conversation_id, photo_id))
                return FakeSnapshot(photo is not None, photo)
            if len(self.path) >= 4 and self.path[-2] == self.db.conversations_collection:
                conversation = self.db.conversations.get(self.path[-1])
                return FakeSnapshot(conversation is not None, conversation)
    ...
    class FakeFirestore:
        ...
        def collection(self, name):
            return FakeCollection(self.db, (name,))

    class FakeConversationsDb:
        conversations_collection = 'conversations'

        def __init__(self):
            self.conversations = {'conversation-1': {'id': 'conversation-1'}}
            self.photos = {}
            self.store_calls = 0

        def get_conversation(self, uid, conversation_id):
            self.get_conversation_photos(uid, conversation_id)
            return {'id': conversation_id}

        def update_conversation(self, uid, conversation_id, data):
            return None

        def upsert_conversation(self, uid, conversation):
            self.conversations[conversation['id']] = conversation
            return None

        def get_conversation_photos(self, uid, conversation_id):
            raise AssertionError('photo cache must use direct photo lookup')

        def store_conversation_photos(self, uid, conversation_id, photos):
            self.store_calls += 1
            for photo in photos:
                self.photos[(uid, conversation_id, photo.id)] = photo.dict()

    fake_conversations_db = FakeConversationsDb()
    env = {
        'Optional': Optional,
        ...
        'auth': SimpleNamespace(
            get_current_user_uid=lambda: 'uid-1',
            with_rate_limit=lambda dependency, key: dependency,
        ),
        'datetime': FakeDatetime,
        'timezone': timezone,
        'uuid': __import__('uuid'),
        'conversations_db': fake_conversations_db,
        'db_client_module': SimpleNamespace(db=FakeFirestore(fake_conversations_db)),
        'Conversation': FakeConversation,
        'Structured': lambda: {},
        'ConversationStatus': SimpleNamespace(processing='processing'),
        'ConversationSource': SimpleNamespace(openglass='openglass'),
        'ConversationPhoto': FakeConversationPhoto,
    }
    exec(endpoint_source, env)
    return fake_conversations_db, env['MetaWearablesPhotoCacheRequest'], env['cache_meta_wearables_photo']


def test_meta_wearables_photo_cache_retry_does_not_store_duplicate():
    fake_conversations_db, request_cls, cache_meta_wearables_photo = _build_meta_wearables_photo_cache_harness()
    ...
    first = cache_meta_wearables_photo(request, uid='uid-1')
    second = cache_meta_wearables_photo(request, uid='uid-1')

    assert second.photo_id == first.photo_id
    assert fake_conversations_db.store_calls == 1
    assert len(fake_conversations_db.photos) == 1


def test_meta_wearables_photo_cache_same_hash_different_capture_time_stores_new_photo():
    fake_conversations_db, request_cls, cache_meta_wearables_photo = _build_meta_wearables_photo_cache_harness()

    first_request = request_cls(
        base64='frame-1',
        conversation_id='conversation-1',
    ...
    second_request = request_cls(
        base64='frame-2',
        conversation_id='conversation-1',
        captured_at=datetime(2026, 7, 5, 12, 0, 1, tzinfo=timezone.utc),
        device_uuid='device-1',
        frame_sha256='same-frame',
    )

    first = cache_meta_wearables_photo(first_request, uid='uid-1')
    second = cache_meta_wearables_photo(second_request, uid='uid-1')

    assert second.photo_id != first.photo_id
    assert fake_conversations_db.store_calls == 2
    assert len(fake_conversations_db.photos) == 2


def test_meta_wearables_photo_cache_retry_without_conversation_id_reuses_cache_conversation():
    fake_conversations_db, request_cls, cache_meta_wearables_photo = _build_meta_wearables_photo_cache_harness()
...
def test_meta_wearables_photo_cache_without_frame_hash_keeps_random_fallback():
    fake_conversations_db, request_cls, cache_meta_wearables_photo = _build_meta_wearables_photo_cache_harness()
