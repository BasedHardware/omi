...
1. ``GET /v1/goals/advice`` (get_current_goal_advice) previously called the
   advice route handler with positional arguments that -- depending on how the
   wrapper is defined -- could bind uid/goal_id in the wrong order. The fix
   uses explicit keywords so the wrapper always forwards (uid, goal_id) to
   ``get_goal_advice_llm``.
...
def _fake_client_with_goal(doc_data):
