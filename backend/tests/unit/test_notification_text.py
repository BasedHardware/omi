...
def test_strips_bold_and_bullets() -> None:
    assert (
        to_plain_text("- **US President**: Donald Trump")
...
def test_flattens_parenthesized_link_and_image_destinations() -> None:
    assert (
        to_plain_text("[Function](https://example.test/wiki/Function_(mathematics))")
        == "Function"
    )
    assert (
        to_plain_text("[Function](https://example.test/wiki/Function_(mathematics)).")
    ...
    assert (
        to_plain_text("See [docs](https://omi.me) (version 2.0).")
        == "See docs (version 2.0)."
    )
    assert (
        to_plain_text("[One](https://a.com(1)) and [Two](https://b.com(2))")
        == "One and Two"
    )
    assert to_plain_text('[Link](https://example.com "my title")') == "Link"
    assert (
        to_plain_text(
            "Visit [this site](https://example.com) (or [mirror](https://mirror.com))."
        )
        == "Visit this site (or mirror)."
    )
