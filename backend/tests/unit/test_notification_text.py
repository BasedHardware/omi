...
def test_flattens_parenthesized_link_and_image_destinations() -> None:
    ...
    assert (
        to_plain_text('Visit [this site](https://example.com) (or [mirror](https://mirror.com)).')
        == 'Visit this site (or mirror).'
    )
    assert to_plain_text('[docs](https://en.wikipedia.org/wiki/Function_(mathematics) (disambiguation))') == 'docs'
