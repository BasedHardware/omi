...
def _record() -> dict[str, object]:
    ...
    return release_rings.build_record(
        ...
        topology={
            "beta": {
                "namespace": "beta-omi-backend",
                "cloud_run_services": {
                    "backend": "backend-beta",
                    "backend-sync": "backend-sync-beta",
                    "backend-sync-backfill": "backend-sync-backfill-beta",
                    "backend-integration": "backend-integration-beta",
                },
            },
            "prod": {
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("rendered_config", "beta/backend", "gs://release-records/config.json", "immutable GCS object reference"),
    ],
)
...
def test_active_pointer_promote_and_hold_are_monotonic() -> None:
    ...
    promoted = release_rings.build_active_pointer(
        ring="beta", release_id="2026-07-20.1", existing=existing, updated_at="2026-07-20T00:00:00+00:00"
    )
    assert promoted["previous_verified_release_id"] == "2026-07-19.1"

    held = release_rings.build_active_pointer(
        ring="beta", release_id="2026-07-20.2", existing=promoted, hold=True, updated_at="2026-07-20T00:01:00+00:00"
    )
...
def test_materialize_runtime_config_captures_public_values_and_remote_secret_versions(tmp_path: Path) -> None:
    ...
    materialized = release_rings.materialize_runtime_config(
        ...
        public_values={"CLOUD_RUN_VPC_SUBNET": "subnet-a", "API_URL": "https://api-beta.example"},
    )

    assert materialized["network"]["subnet"] == {"value": "subnet-a"}
    assert materialized["services"]["backend"]["env"]["URL"] == {"value": "https://api-beta.example"}
    assert materialized["services"]["backend"]["secrets"]["API_KEY"]["version"] == "12"
    assert materialized["services"]["eso"]["version"] == "13"


def test_release_record_rejects_beta_that_reuses_prod_cloud_run_service_names() -> None:
    record = _record()
    record["topology"]["beta"]["cloud_run_services"] = record["topology"]["prod"]["cloud_run_services"]

    assert any("must not reuse prod service identities" in error for error in release_rings.validate_record(record))
