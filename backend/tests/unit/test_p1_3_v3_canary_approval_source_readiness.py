...
def test_canary_approval_source_readiness_runner_exists_and_is_safe_by_default():
    root = Path(__file__).resolve().parents[2]
    script_path = root / "scripts" / "p1_3_v3_canary_approval_source_readiness.py"
...
def test_canary_approval_source_contract_requires_iam_rules_privacy_readiness_before_wiring():
    ...
    assert static_rules["rules_emulator_test"] == "backend/scripts/firestore_rules_emulator_test.mjs"
...
def test_canary_approval_source_readiness_json_summary_and_parent_links_are_stable():
    ...
    assert "test_p1_3_v3_canary_approval_source_readiness.py" in test_sh
    assert "p1_3_v3_canary_approval_source_readiness.py" in ticket_doc
    assert "canary/approval artifact source-selection and ownership/IAM readiness" in ticket_doc
    assert "p1_3_v3_canary_approval_source_readiness.py" in oracle_doc
