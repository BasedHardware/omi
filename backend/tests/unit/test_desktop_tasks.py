...
class TestExtractTasks:
    ...
    def test_extract_tasks_returns_result(self, mock_llm, mock_ctx):
        ...
        result = asyncio.get_event_loop().run_until_complete(extract_tasks("uid1", "base64img", "Chrome", "GitHub PR"))
        assert result["has_new_tasks"] is True
        assert len(result["tasks"]) == 1
    ...
    def test_extract_tasks_no_tasks(self, mock_llm, mock_ctx):
        ...
        result = asyncio.get_event_loop().run_until_complete(extract_tasks("uid1", "base64img"))
        assert result["has_new_tasks"] is False
        assert result["tasks"] == []
    ...
    def test_source_app_fallback(self, mock_llm, mock_ctx):
        ...
        result = asyncio.get_event_loop().run_until_complete(extract_tasks("uid1", "base64img", "Slack", "General"))
    ...
    def test_includes_context_in_prompt(self, mock_llm, mock_ctx):
        ...
        mock_parser = MagicMock()
        mock_llm.with_structured_output.return_value = mock_parser
        mock_parser.ainvoke = AsyncMock(return_value=TaskExtractionResult(has_new_tasks=False))
        asyncio.get_event_loop().run_until_complete(extract_tasks("uid1", "base64img", "VS Code", "main.py"))
