...
def _report(execute=False):
    ...
    module = _load_module(root / 'scripts' / 'v17_p1_3_v3_get_dependency_seam_readiness.py')
...
def test_get_dependency_seam_readiness_is_registered_and_does_not_import_routes():
    root = Path(__file__).resolve().parents[2]
    script_path = root / 'scripts' / 'v17_p1_3_v3_get_dependency_seam_readiness.py'
    source = script_path.read_text(encoding='utf-8')
    test_sh = (root / 'test.sh').read_text(encoding='utf-8')
    external_script = (root / 'scripts' / 'v17_p1_3_v3_external_compatibility_readiness.py').read_text(encoding='utf-8')
    runtime_script = (root / 'scripts' / 'v17_p1_3_v3_get_runtime_wiring_readiness.py').read_text(encoding='utf-8')
    ...
    assert 'test_v17_v3_get_dependency_seam.py' in test_sh
    assert 'test_v17_p1_3_v3_get_dependency_seam_readiness.py' in test_sh
    ...
    assert 'v17_p1_3_v3_get_dependency_seam_readiness.py' in ticket_doc
    assert 'pre-wiring GET dependency seam/adapter readiness' in ticket_doc
    assert 'v17_p1_3_v3_get_dependency_seam_readiness.py' in oracle_doc
