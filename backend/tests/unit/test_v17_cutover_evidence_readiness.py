...
def _default_artifact():
    ...
    module = _load_module(root / "scripts" / "v17_cutover_evidence_readiness.py")
...
def test_cutover_evidence_runner_exists_and_defaults_blocked_without_approval():
    root = Path(__file__).resolve().parents[2]
    script_path = root / "scripts" / "v17_cutover_evidence_readiness.py"
...
def test_cutover_evidence_docs_and_ticket_reference_runner_and_non_claims():
    ...
    for text in (oracle, tickets):
        assert "v17_cutover_evidence_readiness.py" in text
