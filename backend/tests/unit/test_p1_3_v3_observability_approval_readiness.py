...
def _report(execute=False):
    return build_readiness_report("p1_3_v3_observability_approval_readiness.py", execute=execute)


def test_observability_approval_readiness_runner_exists_and_is_safe_by_default():
    root = Path(__file__).resolve().parents[2]
    script_path = root / "scripts" / "p1_3_v3_observability_approval_readiness.py"
...
def test_observability_approval_readiness_registered_in_test_runner_docs_and_parent_readiness():
    ...
    assert "p1_3_v3_observability_approval_readiness.py" in external_readiness
