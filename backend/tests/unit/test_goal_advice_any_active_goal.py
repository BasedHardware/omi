"""Goal advice must work for every active goal, not only the first three Firestore returns.

`get_goal_advice` found the goal by scanning `get_user_goals(uid)`, which defaults to `limit=3`
and applies it to the Firestore query before any ordering. A user can have more active goals
than that (the app allows four, the focus cap is five, background and paused goals are active
too), so advice for any goal outside that arbitrary first page raised "Goal not found" and the
broad except returned the canned fallback sentence instead of real advice.
"""
...
class _Goals:
    """Active goals ordered by document id, the order Firestore uses when no order_by is given."""
...
def _stub_advice(monkeypatch, goals):
    prompts = []
    ...
    monkeypatch.setattr(
        ...
        lambda _name: SimpleNamespace(
            invoke=lambda prompt: prompts.append(prompt) or SimpleNamespace(content='Ship the landing page this week.')
        ),
    )
    return prompts
...
def test_advice_for_an_achieved_goal_still_falls_back(monkeypatch):
    prompts = _stub_advice(monkeypatch, {'goal_a': {**_goal('Run a 10k'), 'status': 'achieved', 'is_active': False}})
