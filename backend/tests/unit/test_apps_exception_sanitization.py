...
def test_generate_app_endpoint_masks_internal_error_detail(monkeypatch):
    ...
    with pytest.raises(HTTPException) as caught:
        asyncio.run(apps_routes.generate_app_endpoint(req, uid="user-1"))
...
def test_generate_app_icon_endpoint_masks_internal_error_detail(monkeypatch):
    monkeypatch.setattr(apps_routes, "run_blocking", AsyncMock(return_value=None))
    monkeypatch.setattr(apps_routes, "track_usage", MagicMock())
    ...
    with pytest.raises(HTTPException) as caught:
        asyncio.run(apps_routes.generate_app_icon_endpoint(req, uid="user-1"))
...
def test_mcp_oauth_callback_masks_token_exchange_error_detail(monkeypatch):
    ...
    resp = asyncio.run(apps_routes.mcp_oauth_callback(code="auth_code_123", state="valid_state"))
...
def test_mcp_oauth_callback_masks_tool_discovery_error_detail(monkeypatch):
    monkeypatch.setattr(apps_routes, "parse_state_token", lambda state: ("app-1", "user-1"))
    monkeypatch.setattr(
        apps_routes,
        "run_blocking",
...
def test_refresh_mcp_tools_masks_discovery_error_detail(monkeypatch):
    ...
    with pytest.raises(HTTPException) as caught:
        asyncio.run(apps_routes.refresh_mcp_tools(app_id="app-1", uid="user-1"))
...
def test_add_mcp_server_masks_oauth_client_registration_error(monkeypatch):
    monkeypatch.setattr(apps_routes, "run_blocking", AsyncMock(return_value={"id": "app-1", "uid": "user-1"}))
    monkeypatch.setattr(apps_routes, "discover_oauth_metadata", AsyncMock(return_value={"registration_endpoint": "https://oauth.example.com/reg"}))
    ...
    monkeypatch.setattr(apps_routes, "generate_state_token", lambda *a: "st")
    monkeypatch.setattr(apps_routes, "generate_pkce_pair", lambda: ("v", "c"))
    monkeypatch.setattr(apps_routes, "build_auth_url", lambda *a, **k: "https://auth.url")
    ...
    with pytest.raises(HTTPException) as caught:
        asyncio.run(apps_routes.add_mcp_server(req, uid="user-1"))
...
def test_add_mcp_server_masks_mcp_tool_discovery_error(monkeypatch):
    monkeypatch.setattr(apps_routes, "run_blocking", AsyncMock(return_value={"id": "app-1", "uid": "user-1"}))
