...
def test_update_wrapped_status_done(monkeypatch):
    ...
    success = wrapped_db.update_wrapped_status(
        'user-123', 2025, WrappedStatus.DONE, result=result_payload
    )
...
def test_update_wrapped_status_error(monkeypatch):
    ...
    success = wrapped_db.update_wrapped_status(
        'user-123', 2025, WrappedStatus.ERROR, error="Quota exceeded"
    )
...
def test_update_wrapped_status_concurrent_exception(monkeypatch):
    ...
    fake_wrapped_doc.get.return_value = fake_snapshot
    fake_snapshot.exists = True
    fake_wrapped_doc.update.side_effect = Exception("NotFound: document deleted concurrently")
    monkeypatch.setattr(wrapped_db, 'db', fake_db)
...
def test_update_wrapped_progress_concurrent_exception(monkeypatch):
    ...
    fake_wrapped_doc.get.return_value = fake_snapshot
    fake_snapshot.exists = True
    fake_wrapped_doc.update.side_effect = Exception("Firestore write error")
    monkeypatch.setattr(wrapped_db, 'db', fake_db)
...
def test_is_wrapped_stuck_scenarios():
    ...
    assert wrapped_db.is_wrapped_stuck({'status': WrappedStatus.PROCESSING, 'updated_at': old_time}, stale_minutes=15) is True

    # Processing and recent timestamp -> not stuck
    recent_time = datetime.now(timezone.utc) - timedelta(minutes=5)
    assert wrapped_db.is_wrapped_stuck({'status': WrappedStatus.PROCESSING, 'updated_at': recent_time}, stale_minutes=15) is False
