...
must reach the notification body (bounded to 500 chars), and every rejection must carry
the ``{'app_id', 'type': 'app_rejected', 'navigate_to': '/apps/<app_id>'}`` data payload
so tapping the notification opens the app page (HomeDeepLink maps the ``apps`` alias).
"""
...
from routers.apps import RejectAppRequest, reject_app

ADMIN_KEY = 'test-admin-key'
EXPECTED_DATA = {'app_id': 'app-1', 'type': 'app_rejected', 'navigate_to': '/apps/app-1'}


def _reject(payload) -> MagicMock:
    ...
    with patch.dict('os.environ', {'ADMIN_KEY': ADMIN_KEY}), patch.object(
        ...
        reject_app(app_id='app-1', uid='dev-1', payload=payload, secret_key=ADMIN_KEY)
    return send_notification


def test_reject_app_forwards_reason_and_deep_link_payload():
    send_notification = _reject(RejectAppRequest(reason='Spam keywords'))
...
def test_reject_app_reason_is_length_bounded():
    with pytest.raises(ValueError):
        RejectAppRequest(reason='x' * 501)
