"""Hermetic unit tests for error sanitization in the MCP router.

Verifies that:
1. GET /v1/mcp/memories catches ValueError during category parsing, logs error
   type safely, and returns generic HTTP 400 without leaking Enum exception text.
2. GET /v1/mcp/conversations catches ValueError during category parsing, logs error
   type safely, and returns generic HTTP 400 without leaking Enum exception text.
"""
...
MCP_SOURCE_FILE = Path(__file__).resolve().parents[2] / "routers" / "mcp.py"


def _get_endpoint_source(endpoint_name: str) -> str:
    source = MCP_SOURCE_FILE.read_text(encoding="utf-8")
...
class McpErrorSanitizationTests(unittest.TestCase):
