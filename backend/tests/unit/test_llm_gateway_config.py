...
def test_loads_default_gateway_config():
    ...
    assert config.route_artifacts[ACTIVE_ROUTE].primary.model == 'gpt-6-luna'
...
def test_gateway_route_overrides_do_not_change_the_legacy_model_profile():
    ...
    assert get_model('memories') == 'gpt-6-luna'
    assert get_model('fair_use') == 'gpt-6-luna'
    assert get_model('chat_agent') == 'gpt-6-luna'

    assert config.route_artifacts['route.conv_discard.model_config.001'].primary.model == 'gpt-5-nano'
    assert config.route_artifacts['route.memories.model_config.001'].primary.model == 'gpt-6-luna'
    assert config.route_artifacts['route.fair_use.model_config.001'].primary.model == 'gpt-6-luna'
    assert config.route_artifacts['route.chat_agent.model_config.001'].primary.provider == 'openai'
    assert config.route_artifacts['route.chat_agent.model_config.001'].primary.model == 'gpt-6-luna'
...
def test_memory_l2_gateway_lane_resolves_to_luna():
    ...
    assert get_model('memory_l2') == 'gpt-6-luna'
...
def test_desktop_proactive_lanes_are_pinned_and_structured():
    ...
    assert reasoning_route.primary.model == 'gpt-6-luna'
