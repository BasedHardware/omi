"""Regression: the setup check must send `uid` as its own query param.

`POST /v1/apps/enable` and `POST /v1/oauth/token` both ask the app's
`setup_completed_url` whether setup is done, and both built the request as
`setup_completed_url + '?uid=...'`. For a setup URL that already has a query
(`https://provider/status?app=omi`) that sent `?app=omi?uid=user-1`, so the
provider saw `app=omi?uid=user-1` and no `uid`, and the app could never be
enabled. Webhook delivery already picks `&` in that case. No live services.
"""
