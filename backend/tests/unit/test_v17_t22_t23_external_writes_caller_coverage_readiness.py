...
def _module():
    ...
    return _load_module(root / "scripts" / "v17_t22_t23_external_writes_caller_coverage_readiness.py")


def test_t22_t23_external_writes_caller_coverage_runner_exists_and_is_safe_by_default():
    root = Path(__file__).resolve().parents[2]
    script_path = root / "scripts" / "v17_t22_t23_external_writes_caller_coverage_readiness.py"
...
def test_t22_t23_readiness_is_linked_from_cutover_oracle_docs_and_ticket():
    ...
    for text in (oracle, tickets, cutover):
        assert "v17_t22_t23_external_writes_caller_coverage_readiness.py" in text
