...
def _adapters(events, **overrides):
    def auth(context):
        events.append('auth')
        return V17V3GetDependencyDecision.allowed('auth_ok', subject_uid='server-uid')
    ...
    return V17V3GetDependencyAdapters(**values)
...
def test_dependency_seam_runs_auth_first_and_rate_limit_before_projection_read():
    events = []
    result = plan_v17_v3_get_dependency_chain(_context(), _adapters(events))
    ...
    assert result.should_fetch_v17_projection is True
...
def test_dependency_seam_rejects_client_uid_override_before_control_or_reads():
    events = []
    result = plan_v17_v3_get_dependency_chain(_context(client_uid_override_present=True), _adapters(events))
...
def test_dependency_seam_preserves_non_enrolled_legacy_path_without_merge():
    events = []
    result = plan_v17_v3_get_dependency_chain(_context(enrolled=False), _adapters(events))
    ...
    assert result.v17_legacy_merge_allowed is False
...
def test_dependency_seam_fail_closed_cases_stop_before_reads():
    ...
    for field, value, code, expected_events in cases:
        events = []
        result = plan_v17_v3_get_dependency_chain(_context(**{field: value}), _adapters(events))
        assert result.status == 'BLOCKED'
...
def test_dependency_seam_uses_bounded_low_cardinality_decision_codes_only():
    events = []
    result = plan_v17_v3_get_dependency_chain(_context(backpressure_ready=False), _adapters(events))
