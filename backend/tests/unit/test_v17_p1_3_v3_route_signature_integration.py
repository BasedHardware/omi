...
def test_route_signature_integration_is_static_read_only_and_no_fastapi_imports():
    ...
    script_path = root / "scripts" / "v17_p1_3_v3_route_signature_integration.py"
...
def test_route_signature_integration_pins_current_v3_route_signatures_and_body_models():
    root = Path(__file__).resolve().parents[2]
    module = _load_module(root / "scripts" / "v17_p1_3_v3_route_signature_integration.py")
    report = module.build_report(execute=True)
...
def test_route_signature_integration_is_linked_from_readiness_test_runner_and_docs():
    ...
    assert proof["service"] == "backend/scripts/v17_p1_3_v3_route_signature_integration.py"
    assert proof["test"] == "backend/tests/unit/test_v17_p1_3_v3_route_signature_integration.py"
