...
class TestDeleteConversationScreenFramesClosesTheLoop:
    def test_deletes_gcs_blobs_for_every_frame_then_the_firestore_docs(self, monkeypatch):
        ...
        fake_conversations_db.delete_conversation_screen_frame_docs.return_value = 2
        monkeypatch.setattr(store_mod, "conversations_db", fake_conversations_db)
        ...
        fake_conversations_db.delete_conversation_screen_frame_docs.assert_called_once_with(UID, CONVERSATION_ID)

    def test_no_frames_is_a_clean_no_op(self, monkeypatch):
        fake_conversations_db = MagicMock()
        fake_conversations_db.get_conversation_screen_frames.return_value = []
        fake_conversations_db.delete_conversation_screen_frame_docs.return_value = 0
        monkeypatch.setattr(store_mod, "conversations_db", fake_conversations_db)
...
class TestDeleteSingleScreenFrame:
    def test_deletes_firestore_doc_and_gcs_blobs(self, monkeypatch):
        fake_conversations_db = MagicMock()
        fake_conversations_db.delete_conversation_screen_frame_doc.return_value = True
        monkeypatch.setattr(store_mod, "conversations_db", fake_conversations_db)
        ...
        fake_conversations_db.delete_conversation_screen_frame_doc.assert_called_once_with(
    ...
    def test_gcs_delete_still_runs_even_if_firestore_doc_was_already_gone(self, monkeypatch):
        fake_conversations_db = MagicMock()
        fake_conversations_db.delete_conversation_screen_frame_doc.return_value = False
        monkeypatch.setattr(store_mod, "conversations_db", fake_conversations_db)
