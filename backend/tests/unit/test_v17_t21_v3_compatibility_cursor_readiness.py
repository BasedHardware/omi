...
REQUIRED_REFERENCE_TERMS = [
    ...
    "v17_cutover_evidence_readiness.py",
]
...
def _module():
    ...
    return _load_module(root / "scripts" / "v17_t21_v3_compatibility_cursor_readiness.py")


def test_t21_v3_compatibility_cursor_runner_exists_and_is_safe_by_default():
    root = Path(__file__).resolve().parents[2]
    script_path = root / "scripts" / "v17_t21_v3_compatibility_cursor_readiness.py"
...
def test_t21_readiness_is_linked_from_cutover_oracle_docs_and_ticket():
    ...
    cutover = (repo / "backend" / "scripts" / "v17_cutover_evidence_readiness.py").read_text()

    for text in (oracle, tickets, cutover):
        assert "v17_t21_v3_compatibility_cursor_readiness.py" in text
