...
def _load_script():
    path = ROOT / "scripts" / "v17_v3_f5_real_service_evidence_readiness.py"
...
def test_docs_test_runner_and_parent_readiness_link_f5_preparation():
    root = ROOT.parent
    assert "test_v17_v3_f5_real_service_evidence_readiness.py" in (ROOT / "test.sh").read_text(encoding="utf-8")
    assert "f5_real_service_evidence_preparation_proof" in (
        ROOT / "scripts" / "v17_p1_3_v3_external_compatibility_readiness.py"
    ).read_text(encoding="utf-8")
    assert "f5_real_service_evidence_preparation_proof" in (
        ROOT / "scripts" / "v17_p1_3_v3_get_runtime_wiring_readiness.py"
    ).read_text(encoding="utf-8")
