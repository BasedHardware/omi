...
def _loaded_dependencies() -> Iterator[tuple[ModuleType, ModuleType, ModuleType, ModuleType]]:
    firebase_auth = _module('firebase_admin.auth', verify_id_token=lambda _token, **_kwargs: {'uid': 'user-1'})
...
def test_firebase_verification_uses_the_critical_executor() -> None:
    with _loaded_dependencies() as (dependencies, firebase_auth, _mcp_db, _dev_db):
        ...
        def verify_id_token(token: str, **_kwargs: Any) -> dict[str, str]:
        ...
        # The same clock-skew allowance verify_token() uses. Wired into only one
        # of the two call sites, a client whose clock runs slightly ahead of
        # Google's is rejected here and accepted there.
        assert calls[0][3] == {'clock_skew_seconds': dependencies.TOKEN_CLOCK_SKEW_SECONDS}
...
def test_firebase_verification_keeps_the_event_loop_responsive() -> None:
    with _loaded_dependencies() as (dependencies, firebase_auth, _mcp_db, _dev_db):
        ...
        async def exercise() -> None:
            ...
            def blocking_verify(_token: str, **_kwargs: Any) -> dict[str, str]:
...
def test_authentication_and_scope_failures_preserve_public_http_semantics() -> None:
    with _loaded_dependencies() as (dependencies, firebase_auth, _mcp_db, _dev_db):
        firebase_auth.verify_id_token = lambda _token, **_kwargs: (_ for _ in ()).throw(ValueError('invalid token'))
