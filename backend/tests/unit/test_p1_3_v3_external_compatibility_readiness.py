...
REQUIRED_GAPS = {
    ...
    "unsafe_legacy_fallback_after_memory_writes",
...
def test_v3_external_compatibility_runner_exists_and_is_safe_by_default():
    ...
    script_path = root / "scripts" / "p1_3_v3_external_compatibility_readiness.py"
...
def test_v3_external_compatibility_inventory_pins_exact_route_gaps_and_non_claims():
    root = Path(__file__).resolve().parents[2]
    module = _load_module(root / "scripts" / "p1_3_v3_external_compatibility_readiness.py")
...
def test_v3_readiness_pins_code_route_evidence_for_runtime_decision_blockers():
    ...
    assert create_surface["memory_write_convergence"] == "absent"
    ...
    assert delete_surface["memory_tombstone_convergence"] == "absent"
...
def test_v3_readiness_pins_decision_matrix_and_product_dependencies():
    ...
    assert decisions["enabled_empty"]["required_behavior"] == "return_empty_memory_result_without_legacy_fallback"
...
def test_v3_readiness_pins_prescriptive_oracle_defaults_and_escalations():
    ...
    assert oracle["session_slug"] == "v3-memory-compat-prescripti"
    assert oracle["production_approach"] == "additive_v3_compatibility_adapter_over_memory_authoritative_writes"
    ...
    assert oracle["write_convergence"] == "memory_writes_and_compatibility_projection_before_memory_reads"
    ...
    assert defaults["no_legacy_memory_merge_or_exception_fallback"]["unsafe_fallback_allowed"] is False
    ...
    assert escalations["legacy_no_cursor_compatibility_window"]["recommended_default"] == (
        "preserve_offset_only_for_non_memory_clients_require_cursor_for_memory_cohort"
    )
...
def test_v3_readiness_pins_oracle_implementation_shape_and_unsafe_approaches():
    ...
    assert shape["decision_service"] == "backend/utils/memory/v3_compatibility.py"
    assert shape["request_adapter"] == "backend/utils/memory/v3_request_adapter.py"
    assert shape["read_service"] == "backend/utils/memory/v3_memory_read_service.py"
    assert shape["response_adapter"] == "backend/utils/memory/v3_response_adapter.py"
    assert shape["projection_store"] == "users/{uid}/memories as memory-derived compatibility projection"
    assert shape["cursor_service"] == "backend/utils/memory/v3_cursor.py"
    assert shape["external_write_service"] == "backend/utils/memory/v3_write_convergence.py"
    ...
    assert "legacy_memory_result_merge" in unsafe
    assert "fallback_because_memory_returned_zero_results" in unsafe
    ...
    assert "apply_5000_first_page_override_to_memory_cursor" in unsafe
...
def test_v3_readiness_links_pure_decision_and_cursor_service_proofs_without_rollout_claims():
    ...
    assert proof["service"] == "backend/utils/memory/v3_compatibility.py"
    assert proof["test"] == "backend/tests/unit/test_v3_compatibility.py"
    ...
    assert cursor_proof["service"] == "backend/utils/memory/v3_cursor.py"
    assert cursor_proof["test"] == "backend/tests/unit/test_v3_cursor.py"
    ...
    assert projection_proof["service"] == "backend/utils/memory/v3_projection_readiness.py"
    assert projection_proof["test"] == "backend/tests/unit/test_v3_projection_readiness.py"
    ...
    assert read_service_proof["service"] == "backend/utils/memory/v3_memory_read_service.py"
    assert read_service_proof["test"] == "backend/tests/unit/test_v3_memory_read_service.py"
    ...
    assert write_convergence_proof["service"] == "backend/utils/memory/v3_write_convergence.py"
    assert write_convergence_proof["test"] == "backend/tests/unit/test_v3_write_convergence.py"
    ...
    assert response_adapter_proof["service"] == "backend/utils/memory/v3_response_adapter.py"
    assert response_adapter_proof["test"] == "backend/tests/unit/test_v3_response_adapter.py"
    ...
    assert request_adapter_proof["service"] == "backend/utils/memory/v3_request_adapter.py"
    assert request_adapter_proof["test"] == "backend/tests/unit/test_v3_request_adapter.py"
    ...
    assert route_planner_proof["service"] == "backend/utils/memory/v3_route_planner.py"
    assert route_planner_proof["test"] == "backend/tests/unit/test_v3_route_planner.py"
...
def test_v3_readiness_is_registered_in_test_runner_and_oracle_docs():
    ...
    assert "test_p1_3_v3_external_compatibility_readiness.py" in test_sh
    assert "test_v3_compatibility.py" in test_sh
    assert "test_v3_cursor.py" in test_sh
    assert "test_v3_projection_readiness.py" in test_sh
    assert "test_v3_memory_read_service.py" in test_sh
    assert "test_v3_write_convergence.py" in test_sh
    assert "test_v3_response_adapter.py" in test_sh
    assert "test_v3_request_adapter.py" in test_sh
    assert "test_v3_route_planner.py" in test_sh
    ...
    assert "p1_3_v3_external_compatibility_readiness.py" in ticket_doc
    assert "backend/utils/memory/v3_compatibility.py" in ticket_doc
    assert "backend/utils/memory/v3_cursor.py" in ticket_doc
    assert "backend/utils/memory/v3_projection_readiness.py" in ticket_doc
    assert "backend/utils/memory/v3_memory_read_service.py" in ticket_doc
    assert "backend/utils/memory/v3_write_convergence.py" in ticket_doc
    assert "backend/utils/memory/v3_response_adapter.py" in ticket_doc
    assert "backend/utils/memory/v3_request_adapter.py" in ticket_doc
    assert "backend/utils/memory/v3_route_planner.py" in ticket_doc
    ...
    assert "p1_3_v3_external_compatibility_readiness.py" in oracle_doc
    assert "backend/utils/memory/v3_compatibility.py" in oracle_doc
    assert "backend/utils/memory/v3_cursor.py" in oracle_doc
    assert "backend/utils/memory/v3_projection_readiness.py" in oracle_doc
    assert "backend/utils/memory/v3_memory_read_service.py" in oracle_doc
    assert "backend/utils/memory/v3_write_convergence.py" in oracle_doc
    assert "backend/utils/memory/v3_response_adapter.py" in oracle_doc
    assert "backend/utils/memory/v3_request_adapter.py" in oracle_doc
    assert "backend/utils/memory/v3_route_planner.py" in oracle_doc
