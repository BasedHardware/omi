"""Merging conversations that have photos must store the photos, not fail the merge.

`_collect_all_photos` returns the photo dicts read from Firestore, but the merge handed that list
straight to `store_conversation_photos`, which reads `photo.id` and `photo.model_dump()`. Any merge
with a photo raised AttributeError, went to `_handle_merge_failure` and deleted the merged
conversation, so the user never got the merge.
"""
