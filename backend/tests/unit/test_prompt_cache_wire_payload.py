...
def explicit_cache_llm():
    ...
    return ChatOpenAI(model='gpt-6-luna', api_key='test').bind(
...
def test_langchain_request_payload_preserves_flex_service_tier() -> None:
    flex_llm = ChatOpenAI(model='gpt-6-luna', api_key='test').bind(service_tier='flex')
...
def test_langchain_request_payload_keeps_explicit_options_without_breakpoint_for_unique_prompts() -> None:
    ...
    options = {'mode': 'explicit', 'ttl': '30m'}
    llm = ChatOpenAI(model='gpt-6-luna', api_key='test').bind(
        extra_body={'prompt_cache_options': options},
    )
...
def test_bind_explicit_cache_after_structured_output_keeps_extra_body() -> None:
    ...
    llm = ChatOpenAI(model='gpt-6-luna', api_key='test')
