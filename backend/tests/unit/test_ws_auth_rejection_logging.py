...
# Upstream defines these as bare Firebase exception classes and drives `endpoints` with them. Here the
# auth port owns the taxonomy (ADR-0034): `endpoints` catches `auth_errors.AuthError`, and the Firebase
# adapter is what TRANSLATES the classes below into it — so in this fork a raw Firebase exception never
# reaches that handler, and a test raising one would exercise the generic fallback instead of the
# severity classification it means to pin.
#
# Keeping the names (the @patch decorators and the firebase_admin stub reference them) but rooting each
# in its neutral counterpart, so `isinstance` holds on both sides: the stub still looks like
# firebase_admin.auth, and the handler still sees the port's own errors. What the file asserts —
# a client-caused rejection logs WARNING, a server-side certificate fetch failure logs ERROR — is
# unchanged, which is the whole point of keeping it.
...
# The port's name for "we could not fetch the signing keys, so we never evaluated the token".
