...
def test_history_date_str_uses_the_users_local_day_not_utcs(monkeypatch):
    # 2026-09-21 02:30 UTC is still 2026-09-20 evening in Los Angeles (UTC-7).
    monkeypatch.setattr(notifications, 'resolve_user_timezone', lambda uid: 'America/Los_Angeles')
    now = datetime(2026, 9, 21, 2, 30, tzinfo=timezone.utc)
    assert goals._history_date_str('u1', now) == '2026-09-20'


def test_history_date_str_uses_the_users_local_day_east_of_utc(monkeypatch):
    # 2026-09-20 23:30 UTC is already 2026-09-21 morning in Tokyo (UTC+9).
    monkeypatch.setattr(notifications, 'resolve_user_timezone', lambda uid: 'Asia/Tokyo')
    now = datetime(2026, 9, 20, 23, 30, tzinfo=timezone.utc)
    assert goals._history_date_str('u1', now) == '2026-09-21'


def test_history_date_str_falls_back_to_utc_for_an_unresolvable_zone(monkeypatch):
    # resolve_user_timezone already normalizes bad zones to "UTC", but the helper's own
    # fallback must also hold if that ever regresses or ZoneInfo lookup itself fails.
    monkeypatch.setattr(notifications, 'resolve_user_timezone', lambda uid: 'not-a-real-zone')
    now = datetime(2026, 9, 21, 2, 30, tzinfo=timezone.utc)
    assert goals._history_date_str('u1', now) == '2026-09-21'
