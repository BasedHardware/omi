                ...
                with patch.object(
                    ...
                    response = client.post(
                        ...
                        files=[
                            (
                                "files",
                                ("test.wav", io.BytesIO(b"\x00" * 100), "audio/wav"),
                            )
                        ],
                        headers={"X-App-Platform": "ios"},
                    )
    ...
    try:
        ...
        with patch.object(module, "retrieve_file_paths", return_value=["/tmp/test-uid_input.opus"]):
            with patch.object(
                module,
                "decode_files_to_wav",
                side_effect=module.HTTPException(400, "invalid opus"),
            ):
                with patch.object(module, "_cleanup_temp_voice_wavs", cleanup):
                    response = client.post(
                        ...
                        files=[
                            (
                                "files",
                                ("corrupt.opus", io.BytesIO(b"not-opus"), "audio/ogg"),
                            )
                        ],
                        headers={"X-App-Platform": "ios"},
                    )
