...
def _valid_input(**overrides):
    data = {
        'read_source': 'v17_compatibility_projection',
        'route_decision': 'use_v17',
        'failure_reason': 'none',
        ...
        'projection_source': 'v17_derived_compatibility_projection',
        'request_limit': 25,
        'request_cursor_present': False,
        'request_offset_disallowed_in_v17': True,
    ...
    return V17V3LocalTelemetryInput(**data)


def test_builds_exact_low_cardinality_v3_get_event_without_uid_or_payload():
    event = build_v17_v3_get_telemetry_event(_valid_input(request_limit=501))

    assert event['event_name'] == 'v17_v3_get_memory_read_decision'
...
def test_rejects_unbounded_failure_reason_and_secret_cursor_or_raw_content_extras():
    with pytest.raises(ValueError, match='failure_reason'):
        build_v17_v3_get_telemetry_event(_valid_input(failure_reason='traceback user abc123'))
    ...
    for key in forbidden_extra_keys:
        with pytest.raises(ValueError, match=key):
            build_v17_v3_get_telemetry_event(_valid_input(extra_labels={key: 'sensitive-value'}))
...
def test_rejects_user_ids_as_labels_and_unexpected_high_cardinality_labels():
    with pytest.raises(ValueError, match='extra_labels'):
        build_v17_v3_get_telemetry_event(_valid_input(extra_labels={'session_id': 'abc'}))

    with pytest.raises(ValueError, match='approval_owner'):
        build_v17_v3_get_telemetry_event(_valid_input(approval_owner='uid_123456'))


def test_fake_sink_records_events_and_null_sink_proves_no_production_call_by_default():
    telemetry_input = _valid_input()
    null_result = emit_v17_v3_get_telemetry(telemetry_input)
    assert null_result.emitted is False
    assert null_result.production_sink_call is False

    fake_sink = FakeV17V3TelemetrySink()
    fake_result = emit_v17_v3_get_telemetry(telemetry_input, sink=fake_sink)
    ...
    explicit_null = NullV17V3TelemetrySink()
    assert explicit_null.events == []


def test_read_disable_config_enrolled_users_fail_closed_for_missing_malformed_or_disabled():
    missing = decide_v17_v3_read_disable(config=None, enrolled_v17_user=True)
    assert missing.v17_reads_enabled is False
    ...
    malformed = decide_v17_v3_read_disable(config={'schema_version': '1'}, enrolled_v17_user=True)
    assert malformed.v17_reads_enabled is False
    assert malformed.failure_reason == 'control_malformed'
    assert malformed.fail_closed is True

    disabled = decide_v17_v3_read_disable(
        config=V17V3ReadDisableConfig(schema_version=1, v17_reads_enabled=True, emergency_read_disable=True),
        enrolled_v17_user=True,
    )
    assert disabled.v17_reads_enabled is False
...
def test_read_disable_config_enabled_only_for_valid_server_owned_config():
    decision = decide_v17_v3_read_disable(
        config=V17V3ReadDisableConfig(schema_version=1, v17_reads_enabled=True, emergency_read_disable=False),
        enrolled_v17_user=True,
    )

    assert decision.v17_reads_enabled is True
    ...
    non_enrolled = decide_v17_v3_read_disable(config=None, enrolled_v17_user=False)
    assert non_enrolled.v17_reads_enabled is False
