...
def test_mcp_oauth_authorize_rejects_non_qa_uid_before_grant_write(monkeypatch):
    ...
    with (
        ...
        # The auth PORT, not firebase_admin: mcp_sse verifies through `get_auth_provider()` so the
        # same route serves Firebase and OIDC (ADR-0034). `verify_token` returns a principal, and
        # the route reads `.uid` off it.
        patch.object(
            sse, 'get_auth_provider', lambda: SimpleNamespace(verify_token=lambda _t: SimpleNamespace(uid='other-user'))
        ),
