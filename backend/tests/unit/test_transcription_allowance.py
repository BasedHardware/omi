...
def _real_paywall(monkeypatch, sub, *, byok_enrolled: bool, fingerprints: dict, creation_ms, header: bool) -> None:
    ...
    monkeypatch.setattr(sub, 'desktop_trial_paywall_eligible', lambda plan, subscription=None: True)
    # `created_at`, not Firebase's `user_metadata.creation_timestamp`: the auth port exposes account age
    # as a neutral epoch-ms field so the paywall reads the same on Firebase and OIDC (ADR-0034), and
    # `subscription.py` reads it that way. Upstream's stub is the Firebase shape, which our code never
    # looks at — leaving it means the account reads as ageless and the paywall silently never applies.
    monkeypatch.setattr(sub, '_get_user', lambda uid: SimpleNamespace(created_at=creation_ms))
