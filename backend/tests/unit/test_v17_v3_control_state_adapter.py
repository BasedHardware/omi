...
def _config(mode=V17Mode.read, enabled_users=('uid-a',)):
    return V17RolloutConfig(enabled_users=set(enabled_users), mode=mode)
...
def _read_gate(enabled=True):
    return {'v17_reads_enabled': enabled, 'kill_switch_active': False}
...
def _ready_docs(uid='uid-a', control_doc=None):
    ...
    return {
        ...
        'memory_control/v17_global_read_gate': _read_gate(True),
        'memory_control/v17_write_convergence_gate': _write_gate(True),
    }


def test_non_enrolled_returns_legacy_eligibility_without_firestore_read():
    db = FakeDb()
    result = read_v17_v3_control(uid='uid-a', db_client=db, rollout_config=_config(enabled_users=()))
    ...
    decision = decide_v17_v3_control_route(
        V17V3ControlReaderRequest('uid-a', 50, False, False),
        result,
    )
...
def test_enrolled_reads_exact_control_doc_once_with_existing_timeout_and_no_memory_items_touch():
    ...
    result = read_v17_v3_control(uid='uid-a', db_client=db, rollout_config=_config())

    assert result.cohort_enrolled is True
    assert result.source_path == 'users/uid-a/memory_control/state'
    assert result.state is not None
    assert db.get_calls[0] == ('users/uid-a/memory_control/state', V17_DEFAULT_READ_ROLLOUT_TIMEOUT_SECONDS)
    assert db.get_calls.count(('users/uid-a/memory_control/state', V17_DEFAULT_READ_ROLLOUT_TIMEOUT_SECONDS)) == 1
...
def test_off_shadow_write_map_to_legacy_authoritative_and_do_not_read_global_gates():
    for mode in (V17Mode.off, V17Mode.shadow, V17Mode.write):
        db = FakeDb({V17Collections(uid='uid-a').memory_control_state: _doc(mode=mode.value)})
        result = read_v17_v3_control(uid='uid-a', db_client=db, rollout_config=_config(mode=V17Mode.read))

        assert result.state.effective_mode == mode
        assert db.document_calls == ['users/uid-a/memory_control/state']
        decision = decide_v17_v3_control_route(V17V3ControlReaderRequest('uid-a', 50, False, False), result)
        assert decision.route_family == V17V3ControlRouteFamily.LEGACY_PRIMARY
...
def test_global_ceiling_never_elevates_lower_persisted_mode_and_caps_higher_persisted_mode():
    assert resolve_v17_v3_effective_mode(V17Mode.read, V17Mode.write) == V17Mode.write
    assert resolve_v17_v3_effective_mode(V17Mode.write, V17Mode.read) == V17Mode.write
    assert resolve_v17_v3_effective_mode('shadow', 'read') == V17Mode.shadow
    assert resolve_v17_v3_effective_mode('off', 'read') == V17Mode.off
...
def test_omi_chat_grants_only_enable_default_memory_and_archive_is_strict_boolean():
    ...
    decision = decide_v17_v3_control_route(V17V3ControlReaderRequest('uid-a', 50, False, False), result)
    assert decision.reason == V17V3ControlDecisionReason.NO_DEFAULT_MEMORY_GRANT
...
def test_global_read_and_convergence_docs_are_read_only_for_effective_read_mode():
    ...
    read_v17_v3_control(uid='uid-a', db_client=read_db, rollout_config=_config())
    assert 'memory_control/v17_global_read_gate' in read_db.document_calls
    assert 'memory_control/v17_write_convergence_gate' in read_db.document_calls

    write_db = FakeDb({V17Collections(uid='uid-a').memory_control_state: _doc(mode='write')})
    read_v17_v3_control(uid='uid-a', db_client=write_db, rollout_config=_config())
...
def test_missing_malformed_uid_mismatch_unsupported_schema_and_transport_failures_are_typed():
    cases = [
        (FakeDb({}), V17V3ControlDecisionReason.MISSING_CONTROL_DOC),
        (
            FakeDb({V17Collections(uid='uid-a').memory_control_state: {'uid': 'uid-a'}}),
            V17V3ControlDecisionReason.UNSUPPORTED_CONTROL_SCHEMA,
        ),
        (
            FakeDb({V17Collections(uid='uid-a').memory_control_state: _doc(uid='other')}),
            V17V3ControlDecisionReason.UID_MISMATCH,
        ),
        (
            FakeDb({V17Collections(uid='uid-a').memory_control_state: _doc(schema_version=99)}),
            V17V3ControlDecisionReason.UNSUPPORTED_CONTROL_SCHEMA,
        ),
        (
            FakeDb(fail_paths={V17Collections(uid='uid-a').memory_control_state}),
            V17V3ControlDecisionReason.CONTROL_READ_FAILED,
        ),
        (
            FakeDb({V17Collections(uid='uid-a').memory_control_state: _doc(mode_epoch='bad')}),
...
def test_archive_defaults_false_strict_boolean_and_archive_denial_is_403():
    ...
    assert result.state.archive_allowed is False
    decision = decide_v17_v3_control_route(V17V3ControlReaderRequest('uid-a', 50, False, False, True), result)
