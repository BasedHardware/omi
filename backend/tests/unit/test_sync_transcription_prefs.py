...
def _disable_sync_background_cleanup(monkeypatch):
    """Keep process_segment cleanup submissions from leaving sleeping executor threads in unit tests."""
    import routers.sync as sync_mod

    future = MagicMock()
    future.result.return_value = None
    monkeypatch.setattr(sync_mod, 'submit_with_context', MagicMock(return_value=future), raising=False)
...
class TestBuildPersonEmbeddingsCache:
    ...
    def test_loads_people_embeddings(self, mock_users_db):
        ...
        mock_users_db.get_people.return_value = [
            {'id': 'p1', 'name': 'Alice', 'speaker_embedding': [0.2] * 512, 'speech_samples': [{'id': 's1'}]},
            {'id': 'p2', 'name': 'Bob'},  # no embedding
            {'id': 'p3', 'name': 'Carol', 'speaker_embedding': [0.3] * 512, 'speech_samples': [{'id': 's2'}]},
        ]
