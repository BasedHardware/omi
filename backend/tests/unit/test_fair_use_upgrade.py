...
class TestIsHardRestrictedPaidGuard:
    ...
    def test_paid_free_exhausted_restrict_returns_false(self, mock_speech, mock_users):
        """Paid user with free_exhausted restrict should NOT be hard-restricted."""
        _fair_use_db.get_fair_use_state.return_value = {
            'stage': 'restrict',
            'last_classifier_type': 'free_exhausted',
            'restrict_until': datetime.utcnow() + timedelta(days=7),
    ...
    @patch.object(fair_use_mod, 'FAIR_USE_EXEMPT_UIDS', set())
    @patch.object(fair_use_mod, 'get_rolling_speech_ms')
    def test_paid_abuse_restrict_returns_true(self, mock_speech, mock_users):
        """Paid user with abuse-derived restrict should still be hard-restricted."""
        _fair_use_db.get_fair_use_state.return_value = {
            'stage': 'restrict',
            'last_classifier_type': 'audiobook',
            'restrict_until': datetime.utcnow() + timedelta(days=7),
    ...
    @patch.object(fair_use_mod, 'FAIR_USE_EXEMPT_UIDS', set())
    @patch.object(fair_use_mod, 'get_rolling_speech_ms')
    def test_free_free_exhausted_restrict_returns_true(self, mock_speech, mock_users):
        """Free user with free_exhausted restrict should still be hard-restricted."""
        _fair_use_db.get_fair_use_state.return_value = {
            'stage': 'restrict',
            'last_classifier_type': 'free_exhausted',
            'restrict_until': datetime.utcnow() + timedelta(days=7),
    ...
    @patch.object(fair_use_mod, 'FAIR_USE_EXEMPT_UIDS', set())
    @patch.object(fair_use_mod, 'get_rolling_speech_ms')
    def test_expired_paid_free_exhausted_restrict_returns_true(self, mock_speech, mock_users):
        """Expired paid sub with free_exhausted restrict should still be hard-restricted."""
        _fair_use_db.get_fair_use_state.return_value = {
            'stage': 'restrict',
            'last_classifier_type': 'free_exhausted',
            'restrict_until': datetime.utcnow() + timedelta(days=7),
        }
        mock_users.get_user_valid_subscription.return_value = None  # Expired
        mock_speech.return_value = {'daily_ms': 8000000, 'three_day_ms': 0, 'weekly_ms': 0}
