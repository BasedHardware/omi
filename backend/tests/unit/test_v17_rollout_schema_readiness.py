...
def test_rollout_schema_readiness_runner_exists_and_is_safe_by_default():
    ...
    script_path = root / "scripts" / "v17_rollout_schema_readiness.py"
...
def test_rollout_schema_readiness_examples_parse_with_shared_rollout_normalizer():
    root = Path(__file__).resolve().parents[2]
    module = _load_module(root / "scripts" / "v17_rollout_schema_readiness.py")
    artifact = module.build_readiness_artifact(module.RolloutSchemaReadinessConfig(execute=False))
...
def test_rollout_schema_docs_use_canonical_v1_shape_and_label_legacy_aliases_as_rejected():
    ...
    for doc_text in (oracle, tickets):
        assert "v17_rollout_schema_readiness.py" in doc_text
        assert "schema_version=1" in doc_text
        assert "canonical nested `grants.<consumer>.default_memory`" in doc_text
        assert "production_rollout_approved=false" in doc_text
