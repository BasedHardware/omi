...
def test_list_returns_service_payload(monkeypatch):
    monkeypatch.setattr(
        ...
        "get_prompts",
        lambda uid: SpeakerTagPromptsResponse(status="cooldown"),
    )
...
def test_answer_success(monkeypatch):
    ...
    def apply(uid, data, schedule=None):
        ...
        return {
            "status": "ok",
            "quality_outcome": SpeakerTagPromptQualityOutcome.person_missed_known,
        }
...
def test_clip_rejects_long_windows_and_missing_audio(monkeypatch):
    ...
    assert (
        client.get(
            ...
            params={"conversation_id": "c1", "start": 0, "end": 30},
        ).status_code
        == 400
    )
    ...
    assert (
        client.get(
            "/v1/speaker-tag-prompts/clip",
            params={"conversation_id": "c1", "start": 0, "end": 5},
        ).status_code
        == 404
    )
...
def test_clip_rejects_deleted_and_locked_before_storage(monkeypatch):
    ...
    def get_conversation(uid, cid):
        ...
        return {
            "id": cid,
            "deleted": cid == "deleted-c",
            "is_locked": cid == "locked-c",
        }
    ...
    assert (
        client.get(
            "/v1/speaker-tag-prompts/clip",
            params={"conversation_id": "deleted-c", "start": 0, "end": 5},
        ).status_code
        == 404
    )
    assert (
        client.get(
            "/v1/speaker-tag-prompts/clip",
            params={"conversation_id": "locked-c", "start": 0, "end": 5},
        ).status_code
        == 402
    )
...
def test_settings_patch_passes_source_and_drops_it_from_updates(monkeypatch):
    ...
    response = _client(monkeypatch).patch(
        ...
        json={"save_other_voice_profiles": False, "source": "first_prompt"},
    )
    assert response.status_code == 200 and response.json()["save_other_voice_profiles"] is False
    assert seen == {
        "updates": {"save_other_voice_profiles": False},
        "source": "first_prompt",
    }
