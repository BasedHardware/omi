...
def index(monkeypatch) -> MagicMock:
    """The vector store the delete goes through.

    Upstream patches the module-level Pinecone ``index``; the port removed it (ADR-0033) and the delete
    runs through ``_vector_store().delete_by_ids(namespace, ids)``. The double keeps the name and the
    ``.delete(ids=..., namespace=...)`` call shape this file asserts on, so the chunking contract — the
    provider's 1,000-id per-delete limit — is pinned exactly as upstream pins it, one seam further in.
    """
    fake = MagicMock()

    class _PortOverIndex:
        @staticmethod
        def delete_by_ids(namespace, ids):
            fake.delete(ids=list(ids), namespace=namespace)

    monkeypatch.setattr(vector_db, '_vector_store', lambda: _PortOverIndex())
