"""Voice questions from the Omi button must not leave decoded audio on the API pod.

`retrieve_file_paths` stages the upload as `syncing/{uid}/*.bin` and `decode_files_to_wav` writes
`syncing/{uid}/*.wav` next to it. The route cleans up through `_cleanup_temp_voice_wavs`, which only
deleted paths under `/tmp/{uid}_`, so every decoded voice question stayed on local disk.
"""
