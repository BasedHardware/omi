...
def test_projection_sync_converges_compatibility_row_across_create_update_and_delete(monkeypatch):
    ...
    # The worker tick still reads events/control/items through the injected ``db_client`` fake, but
    # the real projection side effects now write through the neutral storage port (WP2/ADR-0028 —
    # the injected client was removed from ``_canonical_outbox_side_effects``). Back the port with a
    # ``FakeDocumentStore`` sharing this test's ``db.docs`` so both seams see the same store.
    monkeypatch.setattr(_store_factory, "_store", FakeDocumentStore(backing=db.docs))
