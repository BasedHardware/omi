...
SOURCE_PATHS = (
    Path(__file__).resolve().parents[2] / "routers" / "task_integrations.py",
    Path(__file__).resolve().parents[2] / "utils" / "task_integrations_ops.py",
)
BLOCKING = {"get_task_integration", "set_task_integration", "delete_task_integration"}


def _trees():
    return [ast.parse(path.read_text(encoding="utf-8")) for path in SOURCE_PATHS]
