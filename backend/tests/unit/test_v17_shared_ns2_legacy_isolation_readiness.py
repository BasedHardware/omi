...
def test_shared_ns2_readiness_runner_exists_and_defaults_not_run_read_only():
    ...
    script_path = root / "scripts" / "v17_shared_ns2_legacy_isolation_readiness.py"
...
def test_shared_ns2_readiness_execute_is_read_only_and_requires_provider_config():
    root = Path(__file__).resolve().parents[2]
    module = _load_module(root / "scripts" / "v17_shared_ns2_legacy_isolation_readiness.py")
...
def test_shared_ns2_docs_reference_non_claims_and_remaining_provider_proof():
    ...
    for text in (oracle, tickets):
        assert "v17_shared_ns2_legacy_isolation_readiness.py" in text
