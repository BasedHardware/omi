...
MEMORY_PRODUCT_SOURCE_FILE = BACKEND_DIR / "routers" / "memory_product.py"
...
class MemoryProductErrorSanitizationTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls._sanitize_search_error = staticmethod(_sanitize_search_error)
    ...
    def test_sanitize_search_error_behavioral(self):
        _sanitize = self._sanitize_search_error
        ...
        http_exc = _sanitize(mismatch_err, "Product memory search")
        ...
        http_exc_doc = _sanitize(doc_err, "Vector memory search")
        ...
        limit_err = _sanitize(ValueError("invalid limit value"), "test")
        self.assertEqual(limit_err.detail, "Invalid limit parameter")

        offset_err = _sanitize(ValueError("invalid offset value"), "test")
        self.assertEqual(offset_err.detail, "Invalid offset parameter")

        as_of_err = _sanitize(ValueError("as_of must be timezone-aware"), "test")
