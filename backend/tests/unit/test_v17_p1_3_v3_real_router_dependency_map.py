...
def test_real_router_dependency_map_runner_exists_and_preserves_safety_flags():
    ...
    script_path = root / "scripts" / "v17_p1_3_v3_real_router_dependency_map.py"
...
def test_real_router_dependency_map_pins_unsafe_imports_and_required_stubs():
    root = Path(__file__).resolve().parents[2]
    module = _load_module(root / "scripts" / "v17_p1_3_v3_real_router_dependency_map.py")
    report = module.build_report(execute=True)
...
def test_real_router_dependency_map_is_linked_from_readiness_test_runner_and_docs():
    ...
    assert proof["service"] == "backend/scripts/v17_p1_3_v3_real_router_dependency_map.py"
    assert proof["test"] == "backend/tests/unit/test_v17_p1_3_v3_real_router_dependency_map.py"
