...
def test_real_router_get_testclient_runner_exists_and_preserves_safety_flags():
    ...
    script_path = root / "scripts" / "v17_p1_3_v3_real_router_get_testclient.py"
...
def test_real_router_get_testclient_executes_get_only_and_keeps_mutations_blocked():
    root = Path(__file__).resolve().parents[2]
    module = _load_module(root / "scripts" / "v17_p1_3_v3_real_router_get_testclient.py")
    report = module.build_report(execute=True)
    probe = report["probe"]
...
def test_real_router_get_testclient_confirms_v17_adapters_not_invoked_and_links_readiness():
    ...
    readiness = _load_module(root / "scripts" / "v17_p1_3_v3_external_compatibility_readiness.py").build_report(
        execute=True
    )
    proof = readiness["real_router_get_testclient_proof"]
    assert proof["service"] == "backend/scripts/v17_p1_3_v3_real_router_get_testclient.py"
    assert proof["test"] == "backend/tests/unit/test_v17_p1_3_v3_real_router_get_testclient.py"
