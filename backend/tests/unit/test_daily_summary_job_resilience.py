...
def _loaded_job() -> Iterator[Tuple[ModuleType, ModuleType, FakeRedis, RecordedFallbacks]]:
    ...
    def _fcm_tokens_for(users: Any) -> dict:
        # This fork resolves the push transport in the service layer (ADR-0011/0041), so the fan-out
        # asks the DB for recipients keyed by uid instead of reading them off the user tuple. The tests
        # here seed tokens in that tuple; hand the same ones back, or every assertion about a token
        # reaching the sender would fail for a reason that has nothing to do with resilience.
        return {uid: tokens for uid, tokens, *_rest in users}

    notification_db = _module(
        'database.notifications',
        get_users_for_daily_summary=no_db_work,
        get_users_token_in_timezones=no_db_work,
        get_fcm_tokens_for_users=_fcm_tokens_for,
        get_unifiedpush_endpoints_by_uid=lambda *_a, **_k: {},
