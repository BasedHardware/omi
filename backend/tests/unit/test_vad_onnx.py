...
def _mock_run_vad_window_speech(window, state):
    """Mock that always detects speech (prob=0.9)."""
    new_state = state.copy()
    new_state += 0.01
    return 0.9, new_state


def _mock_run_vad_window_silence(window, state):
    """Mock that never detects speech (prob=0.1)."""
    new_state = state.copy()
    return 0.1, new_state
...
def _mock_run_vad_window_pattern(pattern):
    ...
    def _fn(window, state):
        idx = call_count[0]
        call_count[0] += 1
        is_speech = pattern[idx] if idx < len(pattern) else pattern[-1]
        new_state = state.copy()
        new_state += 0.01
        return (0.9 if is_speech else 0.1), new_state
...
class TestOnnxSessionWiring:
    ...
    def test_make_fresh_state_shape(self):
        """make_fresh_state returns correct shape and dtype."""
...
class TestRunVadWindow:
    ...
    def test_input_shapes_and_output(self, mock_get_sess):
        ...
        prob, new_state = run_vad_window(window, state)
        ...
        assert feed_dict['input'].shape == (1, VAD_WINDOW_SAMPLES)
