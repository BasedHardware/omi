...
def _install_common(monkeypatch, conversation, completed: list[str]) -> None:
    ...
    monkeypatch.setattr(
        processing.first_open_obligations_db,
        "complete_first_open_effect",
        lambda _uid, _cid, _token, effect, **_kwargs: completed.append(effect) or True,
    )
    monkeypatch.setattr(processing.first_open_obligations_db, "first_open_effect_is_authorized", lambda *_args: True)
    monkeypatch.setattr(processing.first_open_obligations_db, "commit_first_open_folder_count", lambda *_args: True)
    monkeypatch.setattr(processing.first_open_obligations_db, "commit_first_open_app_result", lambda *_args: True)
    monkeypatch.setattr(processing.first_open_obligations_db, "commit_first_open_app_usage", lambda *_args: True)
    monkeypatch.setattr(
        processing,
...
def test_first_open_initializes_empty_folders_then_assigns(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        processing.first_open_obligations_db, "commit_first_open_conversation_patch", lambda *_args, **_kwargs: True
    )
