...
REQUIRED_REFERENCE_TERMS = [
    "v17_vector_search_provider_readiness.py",
    "v17_shared_ns2_legacy_isolation_readiness.py",
    "v17_pinecone_repair_validation_readiness.py",
...
def _module():
    ...
    return _load_module(root / "scripts" / "v17_t20_repair_projection_consistency_readiness.py")


def test_t20_repair_projection_consistency_runner_exists_and_is_safe_by_default():
    root = Path(__file__).resolve().parents[2]
    script_path = root / "scripts" / "v17_t20_repair_projection_consistency_readiness.py"
...
def test_t20_readiness_is_linked_from_cutover_oracle_docs_and_ticket():
    ...
    for text in (oracle, tickets, cutover):
        assert "v17_t20_repair_projection_consistency_readiness.py" in text
