...
def _ready_context(**overrides):
    ...
    return V17V3ProjectionReadinessContext(**values)


def test_ready_v17_derived_projection_allows_v3_read_cutover_with_no_legacy_fallback():
    decision = decide_v17_v3_projection_readiness(_ready_context())

    assert decision.state == V17V3ProjectionReadinessState.READY
    ...
    assert decision.reason == 'v17_derived_projection_ready'
    assert decision.source == 'v17_derived_compatibility_projection'
    ...
    assert decision.headers == {
        ...
        'X-Omi-Memory-Projection-Source': 'v17_derived_compatibility_projection',
    }


def test_external_write_convergence_for_create_update_delete_is_required_before_v17_reads():
    cases = [
    ...
    for field, reason in cases:
        decision = decide_v17_v3_projection_readiness(_ready_context(**{field: False}))

        assert decision.state == V17V3ProjectionReadinessState.BLOCKED
...
def test_projection_generation_and_account_generation_must_be_present_current_and_consistent():
    ...
    for overrides, reason in cases:
        decision = decide_v17_v3_projection_readiness(_ready_context(**overrides))
...
def test_projection_source_must_be_v17_derived_not_ad_hoc_mapping_or_legacy_direct_read():
    for source in [None, 'memory_items_ad_hoc_mapping', 'legacy_direct_read', 'users_memories_legacy_projection']:
        decision = decide_v17_v3_projection_readiness(_ready_context(projection_source=source))

        assert decision.state == V17V3ProjectionReadinessState.BLOCKED
        assert decision.read_cutover_allowed is False
        assert decision.reason == 'projection_source_not_v17_derived'
...
def test_enabled_empty_projection_can_return_empty_list_only_when_readiness_is_ok():
    ready_empty = decide_v17_v3_projection_readiness(_ready_context(projection_empty=True))

    assert ready_empty.state == V17V3ProjectionReadinessState.READY_EMPTY
    ...
    assert ready_empty.reason == 'v17_derived_projection_ready_empty'
    assert ready_empty.legacy_fallback_allowed is False

    blocked_empty = decide_v17_v3_projection_readiness(
        _ready_context(projection_empty=True, freshness_fence_present=False)
    )
    assert blocked_empty.state == V17V3ProjectionReadinessState.BLOCKED
...
def test_projection_readiness_api_does_not_expose_legacy_fallback_knob_or_archive_short_term_default_visibility():
    decision_fields = set(V17V3ProjectionReadinessDecision.__dataclass_fields__)
    ...
    source = inspect.getsource(decide_v17_v3_projection_readiness)
