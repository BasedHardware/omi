...
def test_ledger_drain_scales_uid_page_without_raising_per_user_mutation_budget():
    """The uid page and the per-user mutation budget are two different bounds, and only one of them
    protects a single account from a runaway run.

    Upstream renamed the page bound to ``MAX_MAINTENANCE_UIDS_PER_RUN`` and raised it (200 -> 400) in
    the +1002 merge. The literal is theirs to move; what must not move with it is the RELATION — the
    page can grow to drain the backlog faster, and that must never widen how much a single user's
    ledger may be mutated in one run.
    """
    from utils.memory.knowledge_ledger_migration import MAX_LEDGER_MIGRATION_MUTATIONS_PER_RUN

    assert cron.MAX_MAINTENANCE_UIDS_PER_RUN == 400
    assert MAX_LEDGER_MIGRATION_MUTATIONS_PER_RUN == 100
    assert cron.MAX_MAINTENANCE_UIDS_PER_RUN > MAX_LEDGER_MIGRATION_MUTATIONS_PER_RUN
