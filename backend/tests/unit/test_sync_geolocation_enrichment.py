...
``_build_fakes`` is shared with ``test_sync_transcription_prefs`` so the two
files cannot drift on the stub set required to import the sync pipeline.
"""
...
from tests.unit.test_sync_transcription_prefs import _build_fakes
...
@pytest.mark.asyncio
async def test_coordinator_without_geolocation_never_calls_the_geocoder(pipeline):
    captured = _prepare(pipeline, ['/tmp/a.wav'])
    resolver_calls = []
    ...
    assert resolver_calls == [None]  # the resolver's own None-passthrough contract
