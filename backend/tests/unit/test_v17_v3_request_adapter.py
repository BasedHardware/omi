...
def test_legacy_offset_request_preserves_limit_offset_as_legacy_primary_only():
    adapted = adapt_v17_v3_request_parameters({'limit': '25', 'offset': '50'}, enrolled=False)
    ...
    assert adapted.v17_cursor_mode is False
    ...
    first_page = adapt_v17_v3_request_parameters({'limit': '100', 'offset': '0'}, enrolled=False)
    assert first_page.limit == 5000
    assert first_page.applies_first_page_5000_override is True


def test_v17_cursor_mode_requires_cursor_or_first_page_without_offset_and_never_expands_to_5000():
    first_page = adapt_v17_v3_request_parameters({'limit': '100'}, enrolled=True)

    assert first_page.valid is True
    assert first_page.read_mode == 'default_memory'
    assert first_page.source == 'v17_compatibility_projection'
    assert first_page.limit == 100
    assert first_page.offset is None
    assert first_page.cursor is None
    assert first_page.v17_cursor_mode is True
    assert first_page.legacy_primary is False
    assert first_page.applies_first_page_5000_override is False

    next_page = adapt_v17_v3_request_parameters({'limit': '75', 'cursor': 'opaque.cursor'}, enrolled=True)
    ...
    for params, reason in [
        ...
        ({'limit': '5000'}, 'legacy_first_page_5000_not_allowed_in_v17_cursor_mode'),
        ({'limit': '0'}, 'limit_out_of_range'),
        ({'limit': '501'}, 'limit_out_of_range'),
    ]:
        adapted = adapt_v17_v3_request_parameters(params, enrolled=True)
...
def test_category_and_supported_filters_are_normalized_into_stable_filter_hash_and_cursor_binding():
    adapted = adapt_v17_v3_request_parameters(
        {
            'limit': '50',
            'cursor': 'opaque',
            'category': ' Work ',
    ...
    same = adapt_v17_v3_request_parameters(
        {
            'reviewed': 'false',
            'visibility': 'visible',
            'category': 'work',
    ...
    different = adapt_v17_v3_request_parameters(
    ...
    assert adapted.cursor_binding['source'] == 'v17_compatibility_projection'
...
def test_include_archive_defaults_false_unavailable_and_explicit_archive_is_blocked_for_v3_default():
    default = adapt_v17_v3_request_parameters({'limit': '25'}, enrolled=True)
    ...
    explicit = adapt_v17_v3_request_parameters({'limit': '25', 'include_archive': 'true'}, enrolled=True)
...
def test_invalid_parameter_shapes_fail_closed_without_fastapi_dependency():
    ...
    with pytest.raises(V17V3RequestAdapterError) as exc:
        adapt_v17_v3_request_parameters({'limit': object()}, enrolled=True, raise_on_invalid=True)
