...
def _module():
    ...
    return _load_module(root / "scripts" / "v17_p1_3_caller_api_compatibility_readiness.py")


def test_p1_3_runner_exists_and_is_safe_by_default():
    root = Path(__file__).resolve().parents[2]
    script_path = root / "scripts" / "v17_p1_3_caller_api_compatibility_readiness.py"
...
def test_p1_3_readiness_is_linked_from_oracle_ticket_and_test_runner():
    ...
    for text in (oracle, tickets):
        assert "v17_p1_3_caller_api_compatibility_readiness.py" in text
    ...
    assert "test_v17_p1_3_caller_api_compatibility_readiness.py" in test_sh
