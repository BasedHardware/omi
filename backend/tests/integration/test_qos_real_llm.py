...
# ---------------------------------------------------------------------------
# Max profile — OpenAI features (gpt-5.4 flagship)
# ---------------------------------------------------------------------------
class TestMaxOpenAIFlagship:
    """Test gpt-5.4 features in max profile respond to real prompts."""
    ...
    def test_flagship_feature_responds(self, feature):
        ...
        assert model == 'gpt-5.4', f"{feature} should be gpt-5.4 in max, got {model}"
        llm = get_llm(feature)
...
# ---------------------------------------------------------------------------
# Max profile — OpenAI features (gpt-4.1-mini mid-tier)
# ---------------------------------------------------------------------------
class TestMaxOpenAIMini:
    """Test gpt-4.1-mini features in max profile respond to real prompts."""
    ...
    def test_mini_feature_responds(self, feature):
        model = get_model(feature)
        assert model == 'gpt-4.1-mini', f"{feature} should be gpt-4.1-mini in max, got {model}"
        llm = get_llm(feature)
        response = llm.invoke(SIMPLE_PROMPT)
        assert response.content.strip(), f"{feature} ({model}) returned empty response"
        print(f"  {feature} ({model}): {response.content.strip()[:60]}")
...
# ---------------------------------------------------------------------------
# Max profile — OpenAI special models
# ---------------------------------------------------------------------------
class TestMaxOpenAISpecial:
    """Test o4-mini and gpt-4.1 features in max profile."""

    def test_learnings_o4_mini(self):
        model = get_model('learnings')
        assert model == 'o4-mini', f"learnings should be o4-mini in max, got {model}"
        llm = get_llm('learnings')
        response = llm.invoke(SIMPLE_PROMPT)
        assert response.content.strip(), f"learnings ({model}) returned empty response"
        print(f"  learnings ({model}): {response.content.strip()[:60]}")

    def test_chat_graph_gpt41(self):
        model = get_model('chat_graph')
        assert model == 'gpt-4.1', f"chat_graph should be gpt-4.1 in max, got {model}"
        llm = get_llm('chat_graph')
        response = llm.invoke(SIMPLE_PROMPT)
        assert response.content.strip(), f"chat_graph ({model}) returned empty response"
        print(f"  chat_graph ({model}): {response.content.strip()[:60]}")


# ---------------------------------------------------------------------------
# Max profile — OpenRouter features
# ---------------------------------------------------------------------------
class TestMaxOpenRouter:
    """Test OpenRouter features in max profile respond to real prompts."""

    def test_persona_chat(self):
        model = get_model('persona_chat')
        assert model == 'google/gemini-flash-1.5-8b'
        llm = get_llm('persona_chat')
        response = llm.invoke(SIMPLE_PROMPT)
        assert response.content.strip(), f"persona_chat ({model}) returned empty response"
        print(f"  persona_chat ({model}): {response.content.strip()[:60]}")

    def test_persona_chat_premium(self):
        model = get_model('persona_chat_premium')
        assert model == 'anthropic/claude-3.5-sonnet'
        llm = get_llm('persona_chat_premium')
        response = llm.invoke(SIMPLE_PROMPT)
        assert response.content.strip(), f"persona_chat_premium ({model}) returned empty response"
        print(f"  persona_chat_premium ({model}): {response.content.strip()[:60]}")
...
# ---------------------------------------------------------------------------
# Max profile — Anthropic (via get_model + anthropic_client)
# ---------------------------------------------------------------------------
class TestMaxAnthropic:
...
# ---------------------------------------------------------------------------
# Max profile — Perplexity (via get_model + HTTP client)
# ---------------------------------------------------------------------------
class TestMaxPerplexity:
...
class TestProfileRouting:
    ...
    def test_active_profile_is_max(self):
...
class TestStreamingClients:
    ...
    def test_streaming_openrouter(self):
        ...
        response = llm.invoke(SIMPLE_PROMPT)
        assert response.content.strip(), "Streaming persona_chat returned empty"
        print(f"  streaming persona_chat: {response.content.strip()[:60]}")
...
class TestCacheKeyReal:
    ...
    def test_cache_key_with_gpt54(self):
