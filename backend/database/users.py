...
def save_llm_oauth_credential(uid: str, provider: str, credential: dict[str, Any]) -> None:
...
def get_llm_oauth_credential(uid: str, provider: str | None = None) -> dict[str, Any] | None:
...
def get_llm_oauth_status(uid: str) -> dict[str, Any]:
    data = db.collection('users').document(uid).get().to_dict() or {}
...
def delete_llm_oauth_credential(uid: str, provider: str) -> None:
    if provider not in LLM_OAUTH_PROVIDERS:
        raise ValueError(f'Unsupported LLM OAuth provider: {provider}')
    user_ref = db.collection('users').document(uid)
    data = user_ref.get().to_dict() or {}
