"""WhatsApp DB layer: durable per-message processed-ledger + deterministic ids.

Structurally identical to ``database/imessage.py`` but keyed under the ``whatsapp``
integration. The ledger stores one claim document per ingested message at
``users/{uid}/integrations/whatsapp/processed_messages/{key}`` where
...
ingest race so the same WhatsApp message can never become two conversations.

Database-layer only (Firestore access). Orchestration lives in
``utils/whatsapp_connector.py`` — database modules must not import ``utils/``.
"""
...
def release_message(uid: str, key: str, *, firestore_client=None) -> None:
    ...
    try:
        ...
        logger.warning(f'whatsapp: failed to release claim key={key} uid={uid}: {e}')
