...
class StoredDocument:
    ...
    (e.g. ``users/{uid}/people/{pid}``); ``id`` is its last segment.
    """
    ...
    @classmethod
    def present(cls, path: str, data: Dict[str, Any]) -> "StoredDocument":
        return cls(id=path.rsplit("/", 1)[-1], path=path, exists=True, data=data)
