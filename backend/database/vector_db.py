...
if PINECONE_API_KEY and PINECONE_INDEX_NAME:
    try:
        ...
        try:
            ...
            logger.info("Pinecone initialized with index: %s", PINECONE_INDEX_NAME)
        except Exception as e:
            logger.warning("Could not initialize Pinecone index '%s': %s", PINECONE_INDEX_NAME, e)
            index = None
    except Exception as e:
    ...
    logger.info("Pinecone disabled (no API key or index name in environment).")
...
def _require_index_or_noop(func):
    """Decorator to skip Pinecone operations when index is not configured."""
    def wrapper(*args, **kwargs):
        if index is None:
            logger.debug("Pinecone index not configured — skipping %s", func.__name__)
            return None
        return func(*args, **kwargs)
    return wrapper


@_require_index_or_noop
def upsert_vector(uid: str, conversation: Conversation, vector: List[float]):
    ...
    logger.info("upsert_vector result: %s", res)
    return res
...
def upsert_vector2(uid: str, conversation: Conversation, vector: List[float], metadata: dict):
    ...
    logger.info("upsert_vector2 result: %s", res)
    return res
...
def upsert_vectors(uid: str, vectors: List[List[float]], conversations: List[Conversation]):
    ...
    res = index.upsert(vectors=data, namespace="ns1")
    logger.info("upsert_vectors result: %s", res)
    return res
...
def query_vectors(query: str, uid: str, starts_at: int = None, ends_at: int = None, k: int = 5) -> List[str]:
    ...
    return [item["id"].replace(f"{uid}-", "") for item in xc.get("matches", [])]
...
def query_vectors_by_metadata(
    ...
    if not xc.get("matches"):
        if len(filter_data["$and"]) == 3:
            filter_data["$and"].pop(1)
            logger.debug("query_vectors_by_metadata retrying without structured filters: %s", json.dumps(filter_data))
            xc = index.query(
                vector=vector,
                filter=filter_data,
                namespace="ns1",
...
def delete_vector(uid: str, conversation_id: str):
    ...
    logger.info("delete_vector %s result: %s", vector_id, result)
    return result
