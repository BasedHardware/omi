...
def get_hourly_history_for_today(
    uid: str,
    date: datetime,
    start: Optional[datetime] = None,
    end: Optional[datetime] = None,
) -> List[Dict[str, Any]]:
    """Get hourly usage for a displayed day, respecting its UTC boundaries.

    A user's local day commonly spans two UTC calendar dates.  The optional
    range keeps the chart aligned with the total while retaining the old
    UTC-day behavior for callers that do not provide it.
    """
    start = start or date.replace(hour=0, minute=0, second=0, microsecond=0)
    ...
    while cursor < end:
        ...
        for doc in query.stream():
            ...
            if not start <= bucket < end:
                continue
            stats = hourly_totals.setdefault(
                bucket,
                {
                    'transcription_seconds': 0,
                    'words_transcribed': 0,
            ...
            for key in stats:
                stats[key] += cast(int, data.get(key, 0) or 0)
        cursor += timedelta(days=1)

    history: List[Dict[str, Any]] = [
        {'date': bucket.isoformat().replace('+00:00', 'Z'), **stats}
        for bucket, stats in hourly_totals.items()
    ]
...
def get_current_user_usage(
    ...
    if period == 'today':
        ...
        response['history'] = get_hourly_history_for_today(uid, now, start=start, end=end)
    elif period == 'monthly':
