...
def persist_processing_result_with_lifecycle(
    ...
    *,
    ...
    snapshot against a concurrent delete or terminal transition before merging
    generated content back into the conversation document.

    A discard is not fenced.  It is the system's own verdict that a conversation
    held nothing, never a user's decision — users delete, which is a separate
    field with its own guard — and every processor re-derives that verdict from
    the content in front of it.  So a write cannot resurrect something that is
    still empty, while fencing one stranded conversations that a later sync had
    filled with speech: transcribed, untitled, and invisible to their owner.
    """
    conversation_data.pop('updated_at', None)
    if 'audio_base64_url' in conversation_data:
        del conversation_data['audio_base64_url']
