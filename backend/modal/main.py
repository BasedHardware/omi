...
def speaker_identification(uid: str, audio_file: UploadFile = File, segments: str = Form(...)) -> Any:  # type: ignore[reportUnknownArgumentType]  # FastAPI File default
    logger.info('speaker_identification')
    return speaker_identification_endpoint(uid, audio_file, segments)


@app.post('/v1/vad')
def vad(file: UploadFile = File) -> Any:  # type: ignore[reportUnknownArgumentType]  # FastAPI File default
    logger.info('vad')
