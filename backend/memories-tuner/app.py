...
# App configuration
st.set_page_config(
    page_title="OMI Memory Quality Tuner",
    page_icon="🧠",
    layout="wide"
)
...
page = st.sidebar.radio(
    "Navigation",
    ["Home", "Manual Data Entry", "Label Memories", "Tune Prompt", "Test Memories"],
    key="navigation"
)
...
if st.session_state["page"] == "home":
    ...
    with col2:
        ...
        if stats["high_quality"] >= 5:
            ...
            st.warning(f"⚠️ You need at least 5 high-quality examples to start tuning. Currently have: {stats['high_quality']}")
    ...
    with col2:
        ...
        if LANGFUSE_ENABLED:
            ...
            st.warning("⚠️ Langfuse is not configured properly. Memory generation will still work, but logging will be disabled.")
...
# Manual data entry page
    ...
    with st.form("memory_data_entry"):
        ...
        user_name = st.text_input(
            "User's name:",
            help="The name of the user in the conversation"
        )
        ...
        if i < len(entries):
            ...
            with st.form("memory_rating_form"):
                ...
                feedback = st.text_area(
                    "Optional Feedback",
                    placeholder="What could be improved? What's missing or incorrect?"
                )
...
# Tuning page
    ...
    if stats["high_quality"] < 5:
        st.warning(f"⚠️ It's recommended to have at least 5 high-quality examples for tuning. Currently have: {stats['high_quality']}")
...
# Testing page
    ...
    with st.form("test_memory_generation"):
        ...
        use_optimized = st.checkbox(
            ...
            value=optimized_exists,
            disabled=not optimized_exists
        )
        ...
        for i, test in enumerate(reversed(st.session_state["test_history"])):
            with st.expander(f"Test {len(st.session_state['test_history']) - i} - {test['timestamp']}"):
                ...
                if st.button("Delete This Test", key=f"delete_{test['id']}"):
                    st.session_state["test_history"] = [t for t in st.session_state["test_history"] if t['id'] != test['id']]
                    st.rerun()
