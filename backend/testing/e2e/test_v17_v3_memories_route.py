"""
Hermetic route/runtime coverage for V17 GET /v3/memories wiring.
...
legacy/off while the route can be driven through fake V17 runtime decisions.
"""
...
@contextmanager
def _override_v17_runtime(client, runtime):
    import routers.memories as memories_router

    client.app.dependency_overrides[memories_router.get_v17_v3_get_runtime] = lambda: runtime
    try:
        yield
    finally:
        client.app.dependency_overrides.pop(memories_router.get_v17_v3_get_runtime, None)


def _runtime(*, enabled: bool, source_decision: str, service=None):
    import routers.memories as memories_router

    return memories_router.V17V3GetRuntime(
        enabled=enabled,
...
def test_default_off_uses_legacy_memories_and_emits_no_v17_headers(client, auth_headers):
    """Default production dependency is hard off: legacy data is returned, V17 headers are absent."""
...
def test_enabled_legacy_primary_decision_preserves_legacy_path(client, auth_headers):
    """A non-enrolled/fake legacy-primary runtime must still read legacy and never call V17 service."""
    ...
    def should_not_be_called(_params, _adapters):  # pragma: no cover - assertion path
        raise AssertionError("legacy_primary must not invoke the V17 read service")

    with _override_v17_runtime(
        client,
...
def test_enrolled_v17_fake_success_maps_body_cursor_and_allowlisted_headers(client, auth_headers):
    """An enrolled fake V17 runtime can return MemoryDB JSON and only allowlisted route headers."""
    create = client.post(
        "/v3/memories",
        json={"content": "v17 projected memory", "category": "manual", "visibility": "public"},
    ...
    with _override_v17_runtime(client, _runtime(enabled=True, source_decision="v17_read", service=fake_v17_service)):
    ...
    assert resp.headers["x-omi-memory-read-source"] == "v17_compatibility_projection"
...
def test_enrolled_v17_fake_error_fails_closed_with_public_error_and_headers(client, auth_headers):
    """V17 service errors fail closed instead of falling back to legacy data."""
    ...
    with _override_v17_runtime(client, _runtime(enabled=True, source_decision="v17_read", service=failing_v17_service)):
...
def test_malformed_v17_runtime_fails_closed_without_production_activation(client, auth_headers):
    """A v17_read decision without a fake service is a closed 503, not a production dependency lookup."""
    seed_memory("123", _memory_doc("legacy-malformed-runtime", "must not be returned"))

    with _override_v17_runtime(client, _runtime(enabled=True, source_decision="v17_read", service=None)):
        resp = client.get("/v3/memories", headers=auth_headers)
