...
# Debug metrics — exposed via /debug/buffers in the chaos harness
debug_metrics = {
    'audiobuffer_len': 0,
    'trigger_audiobuffer_len': 0,
    'audio_chunks': 0,
    'last_update_ts': 0.0,
}


def _record_buffer_metrics(audiobuffer: bytearray, trigger_audiobuffer: bytearray) -> None:
    debug_metrics['audiobuffer_len'] = len(audiobuffer)
    debug_metrics['trigger_audiobuffer_len'] = len(trigger_audiobuffer)
    debug_metrics['last_update_ts'] = time.time()
            ...
            while websocket_active:
                ...
                if header_type == 101:
                    ...
                    if (
                        ...
                        audio_bytes_event.set()  # Wake consumer immediately
                        audiobuffer = bytearray()
                    _record_buffer_metrics(audiobuffer, trigger_audiobuffer)
                    continue
