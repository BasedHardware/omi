...
def delete_documents_in_batch(query, batch_size=450):
    ...
    while True:
        ...
        logger.info(f"    Deleted {num_docs_in_batch} documents from a batch.")  # Verbose
...
def process_user_subcollections(user_id: str) -> int:
    ...
    for subcollection_name in USER_SUBCOLLECTIONS:
        ...
        if deleted_in_subcollection > 0:
            logger.info(f"    Deleted {deleted_in_subcollection} docs from {subcollection_name} for user {user_id}.")
...
def migrate_remove_soft_deleted_data(ignore_users_file=None):
    ...
    """
    logger.info("Starting migration to remove soft-deleted documents...")
    ...
    if ignore_users_file:
        try:
            ...
            logger.info(f"Ignoring {len(ignored_user_ids)} users from file: {ignore_users_file}")
        except FileNotFoundError:
            logger.warning(f"Warning: Ignore users file not found: {ignore_users_file}. Processing all users.")
        except Exception as e:
            logger.error(f"Warning: Error reading ignore users file {ignore_users_file}: {e}. Processing all users.")

    user_ids = [uid for uid in all_user_ids if uid not in ignored_user_ids]

    total_users = len(user_ids)
    if total_users == 0:
        logger.info("No users found to process after applying ignore list (if any).")
    else:
        logger.info(
            ...
            for future in concurrent.futures.as_completed(future_to_user):
                ...
                try:
                    ...
                    if deleted_for_user > 0:
                        logger.info(
                            f"  User {user_id}: Total {deleted_for_user} documents deleted from subcollections."
                        )
                except Exception as exc:
                    logger.error(f"  User {user_id} generated an exception: {exc}")
                finally:
    ...
    for collection_name in ROOT_COLLECTIONS:
        logger.info(f"  Checking root collection: {collection_name}")
        collection_ref = db.collection(collection_name)
        query = collection_ref.where(filter=FieldFilter('deleted', '==', True))
        deleted_in_root_collection = delete_documents_in_batch(query)
        if deleted_in_root_collection > 0:
            logger.info(f"  Deleted {deleted_in_root_collection} documents from {collection_name}.")
...
if __name__ == '__main__':
    ...
    logger.info("Running migration script: remove_soft_deleted_documents.py")
    logger.info("Ensure GOOGLE_APPLICATION_CREDENTIALS environment variable is set.")
