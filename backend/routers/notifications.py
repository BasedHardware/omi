...
from models.other import FcmTokenResponse, SaveFcmTokenRequest, SaveUnifiedPushEndpointRequest
...
@router.post('/v1/users/unifiedpush-endpoint', response_model=FcmTokenResponse)
def save_unifiedpush_endpoint(
    ...
    # Same device-key composition as fcm-token (ios_abc123 / android_xyz456) so a device's UnifiedPush
    # endpoint and FCM token share a key namespace and replace-per-device semantics.
    platform = x_app_platform or 'unknown'
    device_hash = x_device_id_hash or 'default'
    device_key = f"{platform}_{device_hash}"
