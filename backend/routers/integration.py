...
from utils.apps import verify_api_key, app_can_read_tasks # Added app_can_read_tasks
import database.redis_db as redis_db
import database.memories as memory_db
from database.redis_db import get_enabled_apps, r as redis_client
import database.notifications as notification_db
import database.action_items as action_items_db # Added
...
async def get_tasks_via_integration(
    ...
    api_key = authorization.replace('Bearer ', '')
    if not apps_utils.verify_api_key(app_id, api_key):
        raise HTTPException(status_code=403, detail="Invalid API key")
    ...
    if isinstance(start_date, str) and start_date:
        try:
            if len(start_date) == 10:  # YYYY-MM-DD
                dt = datetime.strptime(start_date, '%Y-%m-%d')
                parsed_start_date = dt.replace(hour=0, minute=0, second=0, microsecond=0, tzinfo=timezone.utc)
            else:
    ...
    if isinstance(end_date, str) and end_date:
        try:
            if len(end_date) == 10:  # YYYY-MM-DD
                dt = datetime.strptime(end_date, '%Y-%m-%d')
                parsed_end_date = dt.replace(hour=23, minute=59, second=59, microsecond=999999, tzinfo=timezone.utc)
            else:
    ...
    if isinstance(due_start_date, str) and due_start_date:
        try:
            if len(due_start_date) == 10:  # YYYY-MM-DD
                ...
                parsed_due_start_date = dt.replace(hour=0, minute=0, second=0, microsecond=0, tzinfo=timezone.utc)
            else:
                parsed_due_start_date = datetime.fromisoformat(due_start_date.replace('Z', '+00:00'))
        except ValueError:
            raise HTTPException(
                status_code=400,
    ...
    parsed_due_end_date = None
    if isinstance(due_end_date, str) and due_end_date:
        try:
            if len(due_end_date) == 10:  # YYYY-MM-DD
                dt = datetime.strptime(due_end_date, '%Y-%m-%d')
                parsed_due_end_date = dt.replace(hour=23, minute=59, second=59, microsecond=999999, tzinfo=timezone.utc)
            else:
                parsed_due_end_date = datetime.fromisoformat(due_end_date.replace('Z', '+00:00'))
        except ValueError:
            raise HTTPException(
                status_code=400,
    ...
    tasks = action_items_db.get_action_items(
        ...
        due_start_date=parsed_due_start_date,
        due_end_date=parsed_due_end_date,
        limit=limit,
        offset=offset,
    )
    ...
    for task in tasks:
        try:
            if task.get('is_locked', False):
                # Optionally, filter out locked tasks completely or redact sensitive info
                # For now, we'll just redact the description as an example.
                task['description'] = "Locked Task (Content Hidden)"
