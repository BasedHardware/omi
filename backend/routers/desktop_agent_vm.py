...
def _claim_vm_if_allowed_txn(tx, deletion_path: str, user_path: str, candidate: dict[str, Any]) -> tuple[dict[str, Any], bool]:
...
def _set_vm_if_current(
    ...
    return _store().run_transaction(
        lambda tx: _set_vm_if_current_txn(
            tx, f"account_deletions/{uid}", f"users/{uid}", expected_vm_name, expected_auth_token, status, ip, zone
        )
    )


def _delete_vm_if_current_txn(tx, user_path: str, expected_vm_name: str, expected_auth_token: str) -> bool:
    snapshot = tx.get(user_path)
    current = (snapshot.to_dict() or {}).get("agentVm") if snapshot.exists else None
    if not isinstance(current, dict):
        return False
...
def _delete_vm_if_current(uid: str, expected_vm_name: str, expected_auth_token: str) -> bool:
    return _store().run_transaction(
        lambda tx: _delete_vm_if_current_txn(tx, f"users/{uid}", expected_vm_name, expected_auth_token)
    )
