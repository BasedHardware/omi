...
def get_apps(
    uid: str = Depends(auth.get_current_user_uid), include_reviews: bool = True
):
...
def create_app(
    ...
    if not data.get("is_paid"):
        ...
        if data["is_paid"] is True:
            ...
            if data.get("price") < 0.0:
                raise HTTPException(
                    status_code=422, detail="Price cannot be a negative value"
                )
    ...
    if external_integration := data.get("external_integration"):
        if (
            external_integration.get("triggers_on") is None
            and len(external_integration.get("actions", [])) == 0
        ):
            raise HTTPException(
                status_code=422, detail="Triggers on or actions is required"
            )
        # Trigger on
        if external_integration.get("triggers_on"):
            external_integration["webhook_url"] = external_integration[
                "webhook_url"
            ].strip()
            if external_integration.get("setup_instructions_file_path"):
                external_integration["setup_instructions_file_path"] = (
                    external_integration["setup_instructions_file_path"].strip()
                )
                if external_integration["setup_instructions_file_path"].startswith(
                    "http"
                ):
        ...
        if actions := external_integration.get("actions"):
            for action in actions:
                if not action.get("action"):
                    raise HTTPException(
                        status_code=422,
                        detail="Action field is required for each action",
                    )
    ...
    os.makedirs("_temp/plugins", exist_ok=True)
    ...
    if "external_integration" in data:
        ...
        if (
            not ext_int.get("app_home_url")
            and ext_int.get("auth_steps")
            and len(ext_int["auth_steps"]) == 1
        ):
...
async def update_persona(
    ...
    if "omi" in data.get("connected_accounts", []) and "omi" not in persona.get(
        "connected_accounts", []
    ):
    ...
    if persona["approved"] and (
        persona["private"] is None or persona["private"] is False
    ):
...
def get_persona_details(uid: str = Depends(auth.get_current_user_uid)):
    ...
    if app.private is not None:
        if app.private and app.uid != uid:
            raise HTTPException(
                status_code=403, detail="You are not authorized to view this Persona"
            )
...
async def get_or_create_user_persona(uid: str = Depends(auth.get_current_user_uid)):
    ...
    persona_data = {
        ...
        "username": increment_username(
            (user.get("display_name") or "MyPersona").replace(" ", "").lower()
        ),
...
def update_app(
    ...
    if file:
        if (
            "image" in plugin
            and len(plugin["image"]) > 0
            and plugin["image"].startswith("https://storage.googleapis.com/")
        ):
...
def get_app_details(app_id: str, uid: str = Depends(auth.get_current_user_uid)):
    ...
    if app.private is not None:
        if app.private and app.uid != uid:
            raise HTTPException(
                status_code=403, detail="You are not authorized to view this app"
            )
    ...
    if app.thumbnails:
        app.thumbnail_urls = [
            get_app_thumbnail_url(thumbnail_id) for thumbnail_id in app.thumbnails
        ]
...
def review_app(app_id: str, data: dict, uid: str = Depends(auth.get_current_user_uid)):
    ...
    if app.uid == uid:
        raise HTTPException(
            status_code=403, detail="You are not authorized to review your own app"
        )

    if app.private and app.uid != uid:
        raise HTTPException(
            status_code=403, detail="You are not authorized to review this app"
        )
...
def update_app_review(
    app_id: str, data: dict, uid: str = Depends(auth.get_current_user_uid)
):
    if "score" not in data:
        raise HTTPException(status_code=422, detail="Score is required")
...
def reply_to_review(
    ...
    if app.uid != uid:
        raise HTTPException(
            status_code=403, detail="You are not authorized to reply to this app review"
        )
...
def change_app_visibility(
    app_id: str, private: bool, uid: str = Depends(auth.get_current_user_uid)
):
    app = get_available_app_by_id(app_id, uid)
    app = App(**app) if app else None
    if not app:
...
def generate_description_endpoint(
    data: dict, uid: str = Depends(auth.get_current_user_uid)
):
    if data["name"] == "":
...
async def get_twitter_profile_data(
    handle: str, uid: str = Depends(auth.get_current_user_uid)
):
...
async def get_twitter_initial_message(
    username: str, uid: str = Depends(auth.get_current_user_uid)
):
    persona = get_persona_by_username_db(username)
    if persona:
        message = generate_persona_intro_message(
            persona["persona_prompt"], persona["name"]
        )
    ...
    try:
        ...
        for persona in personas:
            ...
            if "omi" not in connected_accounts:
                ...
                update_data["persona_prompt"] = await generate_persona_prompt(
                    uid, update_data
                )
                update_data["description"] = generate_persona_desc(
                    uid, update_data["name"]
                )
...
async def upload_app_thumbnail_endpoint(
    file: UploadFile = File(...), uid: str = Depends(auth.get_current_user_uid)
):
...
def create_api_key_for_app(app_id: str, uid: str = Depends(auth.get_current_user_uid)):
    ...
    if app.get("uid") != uid:
        raise HTTPException(
            status_code=403,
            detail="You are not authorized to create API keys for this app",
        )
...
def list_api_keys(app_id: str, uid: str = Depends(auth.get_current_user_uid)):
    ...
    if app.get("uid") != uid:
        raise HTTPException(
            status_code=403,
            detail="You are not authorized to view API keys for this app",
        )
...
def delete_api_key(
    app_id: str, key_id: str, uid: str = Depends(auth.get_current_user_uid)
):
    app = get_available_app_by_id(app_id, uid)
    if not app:
        raise HTTPException(status_code=404, detail="App not found")
    ...
    if app.get("uid") != uid:
        raise HTTPException(
            status_code=403,
            detail="You are not authorized to delete API keys for this app",
        )
