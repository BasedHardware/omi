                                    ...
                                    async def _handle_focus(fid, img, app, wtitle):
                                        try:
                                            result = await analyze_focus(
                                                uid=uid, image_b64=img, app_name=app, window_title=wtitle
                                            )
                                            _send_message_event(
                                                FocusResultEvent(
                                                    frame_id=fid,
                                ...
                                if 'tasks' in analyze_types:
                                    ...
                                    async def _handle_tasks(fid, img, app, wtitle):
                                        try:
                                            ...
                                            _send_message_event(
                                                TasksExtractedEvent(frame_id=fid, tasks=result.get('tasks', []))
                                            )
                                        except Exception as e:
                                ...
                                if 'memories' in analyze_types:
                                    ...
                                    async def _handle_memories(fid, img, app, wtitle):
                                        try:
                                            ...
                                            _send_message_event(
                                                MemoriesExtractedEvent(
                                                    frame_id=fid, memories=result.get('memories', [])
                                                )
                                            )
                                        except Exception as e:
