...
if ADMIN_KEY is None:
    raise Exception("ADMIN_KEY is not set")


@router.post("/v1/plugins/enable")
def enable_plugin_endpoint(
    plugin_id: str, uid: str = Depends(auth.get_current_user_uid)
):
    ...
    if plugin.works_externally() and plugin.external_integration.setup_completed_url:
        res = requests.get(
            plugin.external_integration.setup_completed_url + f"?uid={uid}"
        )
...
@router.post("/v1/plugins/report-comp", tags=["v1"])
def report_plugin_comp(
    plugin_id: str, comp_count: float, uid: str = Depends(auth.get_current_user_uid)
):
    print("uid", uid)
    ...
    update_plugin_comp_count(plugin_id, comp_count)
    return {"status": "ok"}
...
def review_plugin(
    plugin_id: str, data: dict, uid: str = Depends(auth.get_current_user_uid)
):
