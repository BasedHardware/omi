...
def approve_plugin(plugin_id: str, uid: str,secret_key: str = Header(...)):
    ...
    if token:
        send_notification(token, 'App Approved 🎉', f'Your app {plugin["name"]} has been approved and is now available for everyone to use 🥳')
    return {'status': 'ok'}


@router.post('/v1/plugins/{plugin_id}/reject', tags=['v1'])
def reject_plugin(plugin_id: str, uid: str, secret_key: str = Header(...)):
    if secret_key != os.getenv('ADMIN_KEY'):
        raise HTTPException(status_code=403, detail='You are not authorized to perform this action')
    ...
    if token:
        ...
        send_notification(token, 'App Rejected 😔', f'Your app {plugin["name"]} has been rejected. Please make the necessary changes and resubmit for approval.')
    return {'status': 'ok'}
