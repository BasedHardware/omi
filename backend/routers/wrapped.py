...
@router.get('/v1/wrapped/{year}/test', response_model=WrappedStatusResponse, tags=['wrapped-test'])
def get_wrapped_status_test(year: int, uid: str):
    """
    [PUBLIC TEST] Get wrapped status for a given UID.
    """
    if year != 2025:
        raise HTTPException(status_code=400, detail="Only year 2025 is currently supported")
...
@router.post('/v1/wrapped/{year}/test/generate', response_model=GenerateWrappedResponse, tags=['wrapped-test'])
def generate_wrapped_test(year: int, uid: str):
    """
    [PUBLIC TEST] Start wrapped generation for a given UID.
    """
    if year != 2025:
        raise HTTPException(status_code=400, detail="Only year 2025 is currently supported")
