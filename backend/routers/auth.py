...
async def auth_authorize(
    ...
    await asyncio.to_thread(set_auth_session, session_id, session_data, 300)
...
async def auth_callback_google(
    ...
    # Retrieve session
    session_data = await asyncio.to_thread(get_auth_session, state)
    if not session_data:
    ...
    await asyncio.to_thread(set_auth_code, auth_code, code_data, 300)
...
async def auth_token(
    ...
    raw_code_data = await asyncio.to_thread(get_auth_code, code)
    if not raw_code_data:
        raise HTTPException(status_code=400, detail="Invalid or expired code")

    # Clean up used code
    await asyncio.to_thread(delete_auth_code, code)
