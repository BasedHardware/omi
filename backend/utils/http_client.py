...
def get_ntfy_client() -> httpx.AsyncClient:
    ...
    The push server is an operator-configured internal service (e.g. ``http://ntfy`` on the on-prem
    network), so — unlike partner webhooks — these POSTs are intentionally NOT routed through the SSRF
    guard (``assert_public_http_url``), which would reject a private address.
    """
    return _get_client(
        'ntfy',
        lambda: httpx.AsyncClient(
