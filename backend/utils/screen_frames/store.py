...
The Firestore-only CRUD lives in database/conversations.py, mirroring the
...
conversations_db.delete_conversation (Firestore-only) from the router,
...
def persist_screen_frame_docs(uid: str, conversation_id: str, frame_docs: List[Dict[str, Any]]) -> bool:
    ...
    if not frame_docs:
        return True
    return conversations_db.store_conversation_screen_frames(uid, conversation_id, frame_docs)
...
def delete_screen_frame(uid: str, conversation_id: str, frame_id: str) -> bool:
    ...
    """
    existed = conversations_db.delete_conversation_screen_frame_doc(uid, conversation_id, frame_id)
...
def delete_conversation_screen_frames(uid: str, conversation_id: str) -> int:
    ...
    for frame in frames:
        ...
        if frame_id:
            storage.delete_screen_frame_blobs(uid, conversation_id, frame_id)
    return conversations_db.delete_conversation_screen_frame_docs(uid, conversation_id)
