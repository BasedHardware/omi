...
Your task is to:
1. Identify the CURRENT SPEAKER'S own name when the transcript explicitly says it.
...
- Names that are addressed, mentioned, requested, or reported are NOT the speaker:
  - "Hey Alice, can you help?" -> null (Alice is being addressed)
  - "Bob, come here." -> null (Bob is being addressed)
  - "Alice and Bob, listen up." -> null (Alice and Bob are being addressed)
  - "I told Alice about it." -> null (Alice is mentioned)
  - "I saw Bob yesterday." -> null (Bob is mentioned)
  - "I was talking to Mike about the project." -> null (Mike is mentioned)
  - "Alice said she's coming." -> null (reporting what Alice said)
  - "We need to ask Mike about this." -> null (Mike is not the current speaker)
...
def _has_self_identification_cue(text: str) -> bool:
    if not text:
        return False
    return any(pattern.search(text) for pattern in _compiled_self_identification_cues)


def _normalize_speakers(value: Any) -> Optional[List[str]]:
    if value is None:
        return None
...
async def identify_speaker_and_clean_transcript(
    ...
    1. Identify the current speaker's own name.
    ...
    try:
        ...
        return {
            "speakers": _normalize_speakers(data.get("speakers")),
...
async def identify_speaker_from_transcript(
    ...
    if not _has_self_identification_cue(transcript):
        return None
