...
``ChatStorage.sqlite``), normalizes new threads, and POSTs them here. This module
turns those threads into real Omi conversations (with per-message speaker + person
...
There is no OAuth — the WhatsApp session lives on the user's Mac (gated behind Full
Disk Access to read the database), so connecting IS the consent signal. All ingested
content is gated behind stored consent (`enabled`) and per-sender opt-out
(`opted_out_handles`).

Correctness mirrors the iMessage connector (see database/whatsapp.py):

- **Durable per-message ledger.** Every ingested message is claimed with an atomic
  Firestore create at `users/{uid}/integrations/whatsapp/processed_messages/{key}`
  BEFORE its conversation is persisted, so a failed conversation stays retryable and
  concurrent ingests can't duplicate a message.
- **Deterministic day-windowing.** New messages are grouped by (chat_id, calendar
  ...
  chat+day across multiple syncs converge into ONE conversation. The first sync of a
  window CREATES + fully post-processes it; later syncs APPEND raw segments and
  extend finished_at (no second memory/summary run — see _enrich_conversations).

The platform-agnostic segment/window helpers are imported from imessage_connector to
avoid duplicating that logic; only the ledger, integration key, ConversationSource,
...
from database import whatsapp as whatsapp_db
...
def whatsapp_conversation_id(uid: str, chat_id: str, day: str) -> str:
...
# ---------------------------------------------------------------------------
# State (stored in users/{uid}/integrations/whatsapp)
# ---------------------------------------------------------------------------
...
def get_settings(uid: str) -> WhatsAppSettings:
    doc = _get_doc(uid)
...
def update_settings(uid: str, settings: WhatsAppSettings) -> WhatsAppSettings:
    _save_doc(
        uid,
        {
...
def get_status(uid: str) -> WhatsAppStatus:
    doc = _get_doc(uid)
...
def _build_new_conversation(
    ...
    """A Conversation carrying the deterministic id + whatsapp source. Passing a
...
def _persist_window(
    ...
    for m in messages:
        ...
        if whatsapp_db.claim_message(uid, key, chat_id, m.message_id):
            won.append(m)
            won_keys.append(key)
        else:
            race_skipped += 1
    ...
    try:
        ...
        for key in won_keys:
            whatsapp_db.release_message(uid, key)
        raise
...
async def ingest_threads(uid: str, req: WhatsAppIngestRequest) -> WhatsAppIngestResponse:
    """Accept normalized WhatsApp threads, populate People, and kick off conversation
    ...
    candidates: List[Tuple[WhatsAppThread, object, str]] = []
    ...
    claimed = await run_blocking(db_executor, whatsapp_db.filter_claimed_keys, uid, all_keys)
    ...
    for (chat_id, day), g in groups.items():
        ...
        conv_id = whatsapp_conversation_id(uid, chat_id, day)
