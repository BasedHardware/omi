...
def _get_speech_profiles_bucket(required: bool = False) -> Optional[Any]:
    ...
    if speech_profiles_bucket:
        return get_storage_client().bucket(speech_profiles_bucket)
...
def upload_postprocessing_audio(file_path: str) -> str:
    bucket = get_storage_client().bucket(postprocessing_audio_bucket)
...
def upload_conversation_recording(file_path: str, uid: str, conversation_id: str) -> str:
    bucket = get_storage_client().bucket(memories_recordings_bucket)
...
def get_syncing_file_temporal_url(file_path: str):
    bucket = get_storage_client().bucket(syncing_local_bucket)
    blob = bucket.blob(file_path)
    blob.upload_from_filename(file_path)
...
def upload_app_logo(file_path: str, app_id: str):
    bucket = get_storage_client().bucket(omi_apps_bucket)
    path = f'{app_id}.png'
    blob = bucket.blob(path)
...
def upload_app_thumbnail(file_path: str, thumbnail_id: str) -> str:
    bucket = get_storage_client().bucket(app_thumbnails_bucket)
...
def upload_multi_chat_files(files_name: List[str], uid: str) -> Dict[str, str]:
    ...
    """
    bucket = get_storage_client().bucket(chat_files_bucket)
...
def get_desktop_update_signed_url(blob_path: str, expiration_hours: int = 1) -> str:
    ...
    """
    bucket = get_storage_client().bucket(desktop_updates_bucket)
