...
def upload_chat_image(file_path: str, unique_filename: str) -> str:
    """
    Upload a single image file to Google Cloud Storage for vision API usage.
    
    Falls back to local storage if BUCKET_CHAT_FILES is not configured (for local dev).
    
    TODO: Add encryption support before uploading
    ...
    """
    # LOCAL DEVELOPMENT FALLBACK: If no bucket configured, store locally
    if not chat_files_bucket:
        import shutil
        from pathlib import Path
        
        print("⚠️  BUCKET_CHAT_FILES not set - using local storage (dev mode)")
        
        # Create local storage directory
        local_storage_dir = Path("_chat_files/chat_images")
        ...
        # Return absolute path (will be used to read the file later)
    ...
    blob = bucket.blob(path)
    blob.cache_control = 'private, max-age=0'  # Don't cache as these are temporary
    blob.upload_from_filename(file_path)
