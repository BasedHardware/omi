...
def delete_audio_chunks(uid: str, conversation_id: str, timestamps: List[float]) -> None:
    ...
    for timestamp in timestamps:
        ...
        formatted_timestamp = f'{timestamp:.3f}'
        # Try per-chunk and batch extensions
        for extension in ['.enc', '.bin', '.batch.enc', '.batch.bin']:
...
def list_audio_chunks(uid: str, conversation_id: str) -> List[dict]:
    ...
    for blob in blobs:
        ...
        # Per-chunk: '1234567890.123.bin' or '1234567890.123.enc'
        # Batch: '1234567890.123-1234567895.123.batch.bin' or '.batch.enc'
        filename = blob.name.split('/')[-1]
        if not (filename.endswith('.bin') or filename.endswith('.enc')):
            continue
        try:
            # Strip extension: .batch.enc/.batch.bin first, then .enc/.bin
