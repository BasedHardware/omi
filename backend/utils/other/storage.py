...
def _get_signed_url(blob, minutes):
    ...
    try:
        ...
        print(f"Failed to generate signed URL for {blob.name}: {e}")
        # For development/testing, return a placeholder URL that indicates the issue
        # In production, you'd want to configure proper service account credentials
        placeholder_url = f'https://storage.googleapis.com/{blob.bucket.name}/{blob.name}?auth_error=no_private_key'
        print(f"Returning placeholder URL: {placeholder_url}")
        # Still cache the placeholder to avoid repeated failed attempts
        cache_signed_url(blob.name, placeholder_url, minutes * 60)
...
def upload_multi_chat_files(files_name: List[str], uid: str) -> dict:
    ...
    Returns:
        dict: A dictionary mapping original filenames to their signed URLs
    """
    ...
    dictFiles = {}
    for name, upload_result in zip(files_name, result):
        if isinstance(upload_result, Exception):
            print("Failed to upload {} due to exception: {}".format(name, upload_result))
        else:
            # Generate signed URL for the uploaded file
            blob_name = f'{uid}/{os.path.basename(name)}'
            blob = bucket.blob(blob_name)
            signed_url = _get_signed_url(blob, 24 * 60)  # 24 hour expiry
            dictFiles[name] = signed_url
    return dictFiles
...
def upload_chat_file_thumbnail(file_path: str, uid: str) -> str:
    ...
    Returns:
        str: The URL of the uploaded thumbnail (signed if possible, public otherwise).
    """
    ...
    try:
        blob.upload_from_filename(file_path)
        print(f"Successfully uploaded thumbnail to GCS: {blob_name}")
        
        # First, try to make the blob publicly readable
        try:
            blob.make_public()
            public_url = blob.public_url
            print(f"Made thumbnail public, URL: {public_url}")
            return public_url
        except Exception as e:
            print(f"Could not make thumbnail public: {e}")
            
            # Fallback to signed URL
            try:
                url = _get_signed_url(blob, 24 * 60)  # 24 hour expiry
                print(f"Generated signed thumbnail URL: {url[:100]}...")
                return url
            except Exception as e2:
                print(f"Could not generate signed URL: {e2}")
                # Last resort: return the basic public URL (may not work but better than empty)
                fallback_url = f'https://storage.googleapis.com/{chat_files_bucket}/{blob_name}'
