...
async def _get_users_in_timezone(target_time: str) -> Any:
    ...
    # Recipients are push-backend-shaped: FCM tokens for fcm, endpoint URLs for unifiedpush.
    # send_bulk_notification interprets them accordingly.
    gather_fn = (
    ...
    chunk_results = await asyncio.gather(
        *[run_blocking(db_executor, gather_fn, chunk) for chunk in timezone_chunks]
    )
