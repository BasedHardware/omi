...
async def send_daily_summary_notification() -> DailySummaryCronOutcome:
    ...
    * **The run reports itself**: a structured line per failed user and one
    ...
    async def process_hour(group: Tuple[int, List[str]]) -> ProcessOutcome:
        ...
        # Ours (ADR-0011/0041, cubic PR 10887 #427). `users` arrives BACKEND-NEUTRAL from the database
        # layer as (uid, user_document, time_zone): the query must not know whether this deployment
        # delivers over FCM or UnifiedPush. Resolve the transport ONCE per hour group — once, so a
        # PUSH_NOTIFICATION_BACKEND typo records its fallback a single time rather than per user — read
        # the recipients for it, and reshape to (uid, recipients, time_zone), which is exactly the tuple
        # every function below already reads (`user_data[1]`). Upstream's budget, checkpoint and
        # backfill machinery is untouched by this and keeps working on the same shape.
        #
        # A user with NO recipient stays in the list on purpose: this path both CREATES the daily summary
        # and delivers it, so dropping the undeliverable would leave /v1/users/daily-summaries empty for
        # them. Delivery is already a safe no-op — `_deliver_current_day_summary` skips an empty
        # recipient list — and on-prem `PUSH_NOTIFICATION_BACKEND=disabled` is first-class.
        backend = resolve_push_backend()
        recipients_by_uid = await _recipients_by_uid(users, timezones, backend)
        users = [(uid, recipients_by_uid.get(uid, []), tz) for uid, _user_document, tz in users]
...
async def _get_users_for_daily_summary(
...
) -> Tuple[List[Tuple[str, Dict[str, Any], Any]], Optional[BaseException], bool]:
    ...
    *partial* read, and a caller that cannot tell the difference finishes the
    ...
    # (uid, user_document, time_zone): neutral. The caller resolves the transport and swaps the middle
    # element for that transport's recipients — see the fan-out above.
...
async def _recipients_by_uid(
    ...
    This layer owns the choice, not `database/notifications.py`: that helper reads persistence and
    returns WHO is eligible plus their document, so the same query serves either transport.
    """
    if backend == DISABLED:
...
async def _send_notification_for_time(target_time: str, title: str, body: str) -> Any:
    ...
    # dispatch, so a PUSH_NOTIFICATION_BACKEND typo records its fallback once, not twice
    # (review 4939247683, ADR-0011/0041).
    backend = resolve_push_backend()
