...
* ``resolve_chat_context`` runs in the nightly report job. It reads only the
  ...
  chat_session_id — and never touches ``text``. Its output is a pointer list
...
def _find_rated_message(uid: str, message_id: str) -> Optional[Tuple[str, Dict[str, Any]]]:
    docs = _messages_ref(uid).where(filter=FieldFilter('id', '==', message_id)).limit(1).stream()
    doc = next(docs, None)
    if doc is None:
        return None
...
def resolve_chat_context(
    ...
    *regardless of session*, because a user who gives up on a bad answer and
    ...
    found = _find_rated_message(uid, message_id)
    if found is None:
    ...
    try:
        before_query = _messages_ref(uid)
        if session_id:
            before_query = before_query.where(filter=FieldFilter('chat_session_id', '==', session_id))
        before_docs = list(
            before_query.where(filter=FieldFilter('created_at', '<', rated_at))
    ...
    # --- After: any session, within the window.
    try:
...
def hydrate_context(pointer: FeedbackContextPointer) -> FeedbackContextHydrated:
    ...
    for turn in pointer.turns:
        found = _find_rated_message(pointer.uid, turn.message_id)
        if found is None:
        ...
        _, raw = found
        try:
            decrypted = decrypt_message_payload(raw, pointer.uid)
        except Exception as e:
            logger.error(f'Failed to decrypt message {turn.message_id}: {e}')
            hydrated.unavailable.append(turn.message_id)
            continue
        text = str(decrypted.get('text') or '')
