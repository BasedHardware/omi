...
async def build_signed_beta_manifest(tag: str, *, reader: Any | None = None, now: Any | None = None) -> dict[str, Any]:
    ...
    names = ("Omi.zip", "omi.dmg", "Omi.Beta.zip", "omi-beta.dmg", "desktop-smoke-result-beta.json")
    ...
    smoke_bytes = await candidate_read_github(source, "download", urls["desktop-smoke-result-beta.json"])
    if "sha256:" + hashlib.sha256(smoke_bytes).hexdigest() != digests["desktop-smoke-result-beta.json"]:
