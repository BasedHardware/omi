...
RATE_POLICIES: dict[str, tuple[int, int]] = {
    ...
    # Dev API. Read limits are separate from write limits so a polling client
    # cannot consume the processing/write budget. Developer and MCP API-key
    # contexts are keyed by app/key identity when available.
