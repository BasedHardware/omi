...
RATE_POLICIES: dict[str, tuple[int, int]] = {
    ...
    # From-segments: text-segment upload (e.g. Context after cloud /v4/listen), so
    # cheaper than :create — no live STT, just persistence + product-aware enrichment.
    "conversations:from-segments": (30, 3600),
    # Screen activity sync — OCR text rows from Context / Desktop. Bounds Firestore
    # if capture runs 24/7; each request may write up to 100 rows.
    "screen_activity:sync": (120, 3600),
