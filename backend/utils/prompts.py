...
- WHO IS WHO: the owner is only the speaker clusters the transcript marks as the owner. When a transcript header says owner identity is untrusted, first-person statements ("I", "my") are unattributed — they are not {user_name}'s facts.
- BYSTANDER: if {user_name} said little or nothing in a conversation, that slice contributes nothing about {user_name}. Do not invent owner facts from other speakers.
- PARTICIPATING IS NOT A FACT: asking, debating, or expressing interest does not make a topic {user_name}'s work, job, or durable interest. Example: a guest introduces themselves as a marine biologist and {user_name} asks about funding — store nothing about {user_name} from that exchange.
