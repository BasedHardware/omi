    ...
    try:
        ...
        if existing_descriptions:
            existing_context = f"\n\nExisting tasks (DO NOT duplicate):\n" + "\n".join(
                [f"- {desc}" for desc in existing_descriptions[:10]]
            )
        ...
        if match:
            ...
            if tasks and isinstance(tasks, list):
                ...
                for task in tasks:
                    ...
                    if any(
                        existing_desc in description.lower() or description.lower() in existing_desc
                        for existing_desc in existing_descriptions
                        if len(existing_desc) > 10
                    ):
...
def extract_tasks_from_conversation(uid: str, user_message: str, ai_response: str) -> None:
    ...
    try:
        ...
        existing_descriptions = [
            t.get('description', '').lower().strip() for t in existing_tasks if t.get('description')
        ]

        existing_context = ""
        if existing_descriptions:
            existing_context = f"\n\nExisting tasks (DO NOT duplicate these):\n" + "\n".join(
                [f"- {desc}" for desc in existing_descriptions[:15]]
            )
...
{ai_response[:2500]}"""
        ...
        prompt = f"""Extract actionable tasks from this conversation.
            ...
            for task in tasks:
                ...
                for existing_desc in existing_descriptions:
                    ...
                    if description_lower == existing_desc or (
                        len(existing_desc) > 15
                        and (description_lower in existing_desc or existing_desc in description_lower)
                    ):
                        is_duplicate = True
