...
from utils.llm.model_config import get_model_config, model_uses_gpt56_request_contract
...
def shared_conversation_cache_supported() -> bool:
    ...
    if should_route_features_through_gateway():
        # generated_route_overrides.yaml pins both lanes to OpenAI gpt-6-luna.
        return True
    note_route = get_model_config('conv_structure')
    memory_route = get_model_config('memory_l1')
    return note_route == memory_route and note_route[1] == 'openai' and model_uses_gpt56_request_contract(note_route[0])
