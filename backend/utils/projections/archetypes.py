...
WHAT THIS MODULE MAY NOT DO, which is the correction that produced its current shape. It may
not fill Subject or Environment. Written as a prescription rather than a schema, the symbol
layer became the subject of the frame: a request to depict relocating to Lisbon returned a
faceless presence inside a closed ring, with no Lisbon anywhere in it. The archetype supplies
the *form* an image of that situation takes; the person's own week supplies what is in it.
...
AUTHORSHIP. The surviving strings are transcribed from the prototype run that cleared its
declared 7-of-10 gate at 9-of-10. An agent may re-slot them, and may cut what belongs to
another owner, but may not reword them for tone or concision — that would discard the
measurement while keeping the appearance of it.
"""
