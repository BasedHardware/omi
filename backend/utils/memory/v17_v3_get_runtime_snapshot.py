"""Framework-independent request-scoped snapshot contract for future V17 `/v3` GET reads.

The builder is intentionally pure: it performs no FastAPI wiring, I/O, Firestore
reads/writes, provider/vector/network calls, telemetry sink calls, or mutations.
It validates that independently supplied, server-owned request components are
internally coherent before a future composed service may read a projection.
"""
