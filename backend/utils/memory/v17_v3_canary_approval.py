"""Pure local V17 `/v3` canary enrollment and approval artifact seam.

This module defines the future server-owned canary/approval artifact shape for
`GET /v3/memories` V17 runtime wiring. It is intentionally local-only: no router
imports, no Firestore/cloud/provider reads, no telemetry sink calls, and no
production approval claim. Callers inject an artifact dictionary in tests or a
future approved server-owned reader; missing or malformed input fails closed.
"""
