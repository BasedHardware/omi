"""Pure local V17 `/v3` GET telemetry and rollback config seams.

This module is deliberately route-adjacent only: it does not import FastAPI,
Firestore, metrics clients, cloud SDKs, or production telemetry sinks. Future
`GET /v3/memories` wiring can inject a real sink/config reader after approval;
today the default sink is a no-op and tests use the fake sink.
"""
