"""Local-only V17-V3-F6B evidence approval/run-record validation."""
...
from utils.memory.v17_v3_gcp_evidence_config import EvidenceTargetRegistry, ValidationError
...
def _require_exact_fields(raw: dict[str, Any], expected: frozenset[str], label: str) -> None:
    ...
    if missing:
        raise RunRecordValidationError(f"{label} missing fields: {sorted(missing)}")
    if unknown:
        raise RunRecordValidationError(f"{label} unknown fields: {sorted(unknown)}")
