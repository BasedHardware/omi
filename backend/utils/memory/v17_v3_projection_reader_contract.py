"""Typed contract for the fenced server-owned V17 `/v3` projection reader.

Pure contract only: no routers, no legacy fallback, no Firestore clients, no writes,
and no provider/network calls live here. The reader requires an independently
caller-supplied expected account generation; callers must not copy it from the
projection state being verified.
"""
