...
def normalize_global_read_gate(data) -> GlobalReadGateDecision:
    ...
    memory_reads_enabled = data.get('memory_reads_enabled')
...
def read_global_read_gate(*, db_client) -> GlobalReadGateDecision:
...
def normalize_write_convergence_gate(data) -> WriteConvergencePolicy:
...
def read_write_convergence_gate(*, db_client) -> WriteConvergencePolicy:
...
def normalize_default_read_rollout_decision(
    *, uid: str, source_path: str, consumer: str, data
    ...
    try:
        ...
        if data.get('schema_version') != DEFAULT_READ_ROLLOUT_SCHEMA_VERSION:
            return disabled_default_read_rollout_decision(
...
def read_default_read_rollout(*, uid: str, db_client, consumer: str) -> DefaultReadRolloutDecision:
...
def read_archive_read_rollout(*, uid: str, db_client, consumer: str) -> DefaultReadRolloutDecision:
...
def read_rollout_state_doc(*, uid: str, db_client):
    source_path = MemoryCollections(uid=uid).memory_control_state
    try:
        snapshot = _get_firestore_document_snapshot(db_client.document(source_path))
        data = snapshot.to_dict() if getattr(snapshot, 'exists', True) else None
...
def build_default_read_rollout_observability(decision: DefaultReadRolloutDecision) -> dict:
...
def build_default_read_rollout_audit_event(decision: DefaultReadRolloutDecision) -> dict:
    enabled = decision.memory_default_enabled
    return {
        'uid': decision.uid,
        'source_path': decision.source_path,
...
def build_default_read_rollout_decision_counters(events: list[dict]) -> dict:
...
def build_default_read_rollout_audit_events(decisions: dict[str, DefaultReadRolloutDecision]) -> dict:
...
def render_default_read_rollout_metrics(counters: dict) -> str:
...
def build_default_read_rollout_observability_report(
    ...
    return {
        ...
        'decision_metrics_prometheus': render_default_read_rollout_metrics(audit['counters']),
        'consumers': {
...
DefaultReadRolloutDecision = DefaultReadRolloutDecision
GlobalReadGateDecision = GlobalReadGateDecision
LegacyMemoryWriteGuardDecision = LegacyMemoryWriteGuardDecision
WriteConvergencePolicy = WriteConvergencePolicy
GLOBAL_READ_GATE_PATH = GLOBAL_READ_GATE_PATH
WRITE_CONVERGENCE_GATE_PATH = WRITE_CONVERGENCE_GATE_PATH
