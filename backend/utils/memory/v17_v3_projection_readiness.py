"""Pure/local `/v3` V17 compatibility projection readiness seam.

This module intentionally accepts only caller-supplied dataclass inputs. It does
not import routers, database clients, Firestore/Pinecone/provider SDKs, or perform
I/O. It models the fail-closed readiness checks that must pass before an enrolled
`/v3` caller can read the V17-derived compatibility projection.
"""
