"""Safe-by-default V17 `/v3` dev-cloud proof preparation helpers.

This module prepares local evidence artifacts for the mandatory non-production
Gate 2 described in `docs/rollout/v17-v3-proof-order.md`. It never writes to
Firestore, never accepts production targets, and never claims dev-cloud GO. The
actual Gate 2 proof still requires a deployed branch backend in a dedicated dev
cloud project.
"""
...
from utils.memory.v17_v3_limited_rollout_config import GLOBAL_READ_GATE_PATH, WRITE_CONVERGENCE_GATE_PATH
