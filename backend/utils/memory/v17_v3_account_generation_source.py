"""Trusted server-owned account-generation source for future V17 `/v3` GET reads.

This module is intentionally small and fake-injectable. It reads the independent
server-owned V17 state-head document at ``users/{uid}/memory_state/head`` and
normalizes only the account generation needed by future route wiring. It does not
read control/projection documents, import routers, trust client input, mutate
state, or wire runtime behavior.
"""
