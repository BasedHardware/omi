...
def _load_assertions_for_canonical_graph_items(
    ...
    *,
    ...
    assertions_collection = f'{kg_db.users_collection}/{uid}/{kg_db.memory_graph_assertions_collection}'
    assertion_ids = sorted(item_by_id)
    assertions_by_id: Dict[str, MemoryGraphAssertion] = {}
    for start in range(0, len(assertion_ids), CANONICAL_GRAPH_ASSERTION_BATCH_SIZE):
        batch_ids = assertion_ids[start : start + CANONICAL_GRAPH_ASSERTION_BATCH_SIZE]
        for snapshot in document_store.get_many_documents(assertions_collection, batch_ids):
...
def _read_canonical_graph_page_once(
    ...
    *,
    ...
    memory_items_collection = MemoryCollections(uid=uid).memory_items
    start_after = (
        {'value': cursor_boundary[0], 'id': cursor_boundary[1]} if cursor_boundary is not None else None
    )
    snapshots = document_store.query_collection(
