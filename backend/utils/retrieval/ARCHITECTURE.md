...
- Desktop Gemini `embedContent` / `batchEmbedContents` are gated by default when the request advertises `X-Omi-Local-Embeddings` (bare capability marker from a client that can keep free users off Gemini). `_enforce_managed_plan_gate` then 402s basic-plan embed calls with `plan_gated` (entitlement feature `screen_frame_judge`; product name `screen_text_embedding`). Requests without the header fail open to Gemini and emit `desktop_embed_legacy_client` (uid-free, `plan_class` only) so the stale-client share is visible. `DESKTOP_EMBED_PLAN_GATE_DISABLED=1` is the kill switch. `RELEASE_PROBE_UID` and Gemini BYOK stay allowed. Rollout is merge then release; no backend config step.
