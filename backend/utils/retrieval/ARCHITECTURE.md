...
- Desktop Gemini `embedContent` / `batchEmbedContents` stay ungated until `DESKTOP_EMBED_PLAN_GATE_ENABLED` is on (default off). When on, `desktop_proxy._enforce_managed_plan_gate` 402s basic-plan embed calls with `plan_gated` (entitlement feature `screen_frame_judge`; product name `screen_text_embedding`). `RELEASE_PROBE_UID` and Gemini BYOK stay allowed. Turn the flag on only after the client that stops calling Gemini for free plans is released.
