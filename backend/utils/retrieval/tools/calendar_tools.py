...
async def get_calendar_events_tool(
    ...
    try:
        ...
        try:
            ...
            if e.is_auth_error:
                ...
                if new_token:
                    try:
                        ...
                        logger.error(
                            "❌ Error after token refresh: %s",
                            type(retry_error).__name__,
                            exc_info=True,
                        )
                        emit_sync_failed(telemetry_context, retry_error)
                        return "Error fetching calendar events. Please try again later."
                else:
            ...
            logger.error(
                "❌ Unexpected error fetching calendar events: %s",
                type(e).__name__,
                exc_info=True,
            )
            emit_sync_failed(telemetry_context, e)
        ...
        logger.error(
            "❌ Unexpected error in get_calendar_events_tool: %s",
            type(e).__name__,
            exc_info=True,
        )
        if telemetry_context:
            emit_sync_failed(telemetry_context, e)
        return "Unexpected error fetching calendar events. Please try again later."
...
async def create_calendar_event_tool(
    ...
    try:
            ...
            if e.is_auth_error:
                ...
                if new_token:
                    try:
                        ...
                        return "Error creating calendar event. Please try again later."
                else:
                    logger.error(f"❌ Token refresh failed")
                    return (
                        "Google Calendar authentication expired. Please reconnect your Google Calendar from settings."
        ...
        logger.error(
            "❌ Unexpected error in create_calendar_event_tool: %s",
            type(e).__name__,
            exc_info=True,
        )
        return "Unexpected error creating calendar event. Please try again later."
...
async def delete_calendar_event_tool(
    ...
    try:
        ...
        try:
            ...
            if e.is_auth_error:
                ...
                if new_token:
                    try:
                        ...
                        logger.error(
                            "❌ Error deleting calendar events on retry: %s",
                            type(retry_error).__name__,
                            exc_info=True,
                        )
                        return "Error deleting calendar events. Please try again later."
                else:
                    return (
                        "Google Calendar authentication expired. Please reconnect your Google Calendar from settings."
                    )
            ...
            logger.error(
                "❌ Unexpected error searching events to delete: %s",
                type(e).__name__,
                exc_info=True,
            )
            return "Error searching for calendar events. Please try again later."

    except Exception as e:
        logger.error(
            "❌ Unexpected error in delete_calendar_event_tool: %s",
            type(e).__name__,
            exc_info=True,
        )
        return "Unexpected error deleting calendar events. Please try again later."
    ...
    try:
        ...
        if not target_event_id:
            ...
            try:
                ...
                logger.error(
                    "❌ Error searching for event: %s",
                    type(e).__name__,
                    exc_info=True,
                )
                return "Error searching for calendar event. Please try again later."
        ...
        try:
            ...
            if e.is_auth_error:
                ...
                if new_token:
                    try:
                        ...
                        logger.error(
                            "❌ Error getting calendar event on retry: %s",
                            type(retry_error).__name__,
                            exc_info=True,
                        )
                        return "Error getting calendar event. Please try again later."
                else:
                    return (
                        "Google Calendar authentication expired. Please reconnect your Google Calendar from settings."
                    )
            ...
            logger.error(
                "❌ Unexpected error getting event: %s",
                type(e).__name__,
                exc_info=True,
            )
            return "Error getting calendar event. Please try again later."
            ...
            if e.is_auth_error:
                ...
                if new_token:
                    try:
                        ...
                        logger.error(
                            "❌ Error updating calendar event on retry: %s",
                            type(retry_error).__name__,
                            exc_info=True,
                        )
                        return "Error updating calendar event. Please try again later."
                else:
                    return (
                        "Google Calendar authentication expired. Please reconnect your Google Calendar from settings."
                    )
        ...
        logger.error(
            "❌ Unexpected error in update_calendar_event_tool: %s",
            type(e).__name__,
            exc_info=True,
        )
        return "Unexpected error updating calendar event. Please try again later."
