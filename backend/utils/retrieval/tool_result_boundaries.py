...
def preserve_chat_memory_tool_result_boundary(tool_name: str, result: str) -> str:
    """Fail closed if V17 memory evidence markers appear without the full quoted boundary.

    The Anthropic tool caller passes string tool output directly back into the
    model as `tool_result.content`. V17 chat memory adapters intentionally emit
    ...
    text = str(result)
    if not _looks_like_v17_memory_evidence(text):
        return text

    if _has_required_v17_memory_boundary(text):
        return text

    return V17_CHAT_MEMORY_SAFE_NO_RESULT


def _looks_like_v17_memory_evidence(text: str) -> bool:
    markers = ('content_quoted=', 'source_marker=v17_default_memory', 'source_marker=v17_vector_memory')
    return any(marker in text for marker in markers)


def _has_required_v17_memory_boundary(text: str) -> bool:
    if V17_CHAT_MEMORY_BOUNDARY_NOTICE not in text:
        return False
    if V17_CHAT_MEMORY_POLICY_MARKER not in text:
        return False
    if 'archive_default_visible=False' not in text:
        return False
    if 'content_quoted=' not in text:
        return False
    return 'source_marker=v17_default_memory' in text or 'source_marker=v17_vector_memory' in text
