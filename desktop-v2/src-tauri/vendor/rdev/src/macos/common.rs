...
pub unsafe fn convert(
    ...
    if let Some(event_type) = option_type {
        // PATCHED: never call `create_string_for_key`. Upstream rdev invokes
        // it from inside the CGEventTap callback (a non-main RunLoop thread).
        // On macOS 14+ the underlying `TSMGetInputSourceProperty` enforces
        // `dispatch_assert_queue(main)` and SIGTRAPs the process — see
        // crash report `nooto-desktop-v2-2026-04-22-104619.ips`. We don't
        // consume `event.name` anywhere in the app, so dropping it is safe.
