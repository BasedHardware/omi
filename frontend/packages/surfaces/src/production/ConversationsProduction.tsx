...
export function ConversationsProduction({ store, foldersStore, fixture, detailId, initialFolderId, locale = "en", onReady }: {
  ...
  return (
    <main className="production-shell" aria-label={t(locale, "conversations.title")} data-production-shell="true" data-route="conversations" data-surface-state={status.refresh.phase} data-qa-fixture={fixture ?? "none"} data-consumer-semantic={`conversations:visible:${visibleRows.length}:total:${rows.length}:folders:${folders.length}:detail:${selected ? "shown" : detailId ? "missing" : "none"}`}>
