/**
 * Platform conversations store honesty: coverage is unknown until an honest
 * page is read, and is never restored from cache.
 */

import assert from "node:assert/strict";
import { test } from "node:test";
import type { HttpClient, HttpResponse } from "@omi-core/contracts";
import { PlatformConversationsStore } from "@omi-core/domain";
...
const pageFor = (wireCase: string): unknown => {
  const rows = readRatifiedCorpus("conversations-read-conformance") as readonly ConversationsCorpusRow[];
...
test("coverage is unknown until an honest page is read, and is never restored from cache", async () => {
  const store = disk();
  const first = await PlatformConversationsStore.open(
    store.openBridge("u1"),
    env,
  ...
  const reopened = await PlatformConversationsStore.open(store.openBridge("u1"), env, new Scripted([]));
...
test("a failed read drops the coverage claim but keeps the cached items", async () => {
  const store = await PlatformConversationsStore.open(disk().openBridge("u1"), env, new Scripted([
