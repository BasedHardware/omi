...
import { buildCreateTask, buildDeleteTask, buildPatchTask, tasksCodec, toPendingOp } from "./tasks-codec.js";
...
export class TasksStore {
  ...
  static async open(bridge: StorageBridge, env: Env, http: HttpClient): Promise<TasksStore> {
    ...
    const outbox = await Outbox.open(bridge, env, transport);
  ...
  async create(description: string, dueAt?: number): Promise<void> {
    await this.outbox.enqueue(toPendingOp(buildCreateTask(this.env, description, dueAt)));
    this.notify();
  }

  async patch(id: RecordId, patch: TaskPatch): Promise<void> {
    await this.outbox.enqueue(toPendingOp(buildPatchTask(this.env, id, patch)));
    this.notify();
  }

  async delete(id: RecordId): Promise<void> {
    await this.outbox.enqueue(toPendingOp(buildDeleteTask(this.env, id)));
    this.notify();
  }
