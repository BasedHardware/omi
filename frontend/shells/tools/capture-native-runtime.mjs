...
function probeScript(manifest) {
  ...
  return `(() => { const q = new URL(location.href).searchParams; const expectedDomain = ${domain}; const expectedAccess = ${access}; const expectedState = ${state}; const expectedTheme = ${theme}; const domain = q.get('qa') || ''; const access = q.get('accessibility') || ''; const wanted = q.get('state') || ''; const theme = q.get('theme') || ''; const renderedTheme = document.documentElement.dataset.themeSelection || ''; if (domain !== expectedDomain || access !== expectedAccess || wanted !== expectedState || theme !== expectedTheme || renderedTheme !== expectedTheme) return JSON.stringify({schema:'omi.native-runtime-marker/v1',domain,theme,accessibility:access,events:[]}); const root = document.querySelector('main[data-production-shell]'); if (!root || root.dataset.route !== expectedDomain) return JSON.stringify({schema:'omi.native-runtime-marker/v1',domain,theme,accessibility:access,events:[]}); const target = root.querySelector('*') || root; const style = getComputedStyle(target); const events = []; if (access === 'none') events.push({type:'lifecycle',name:'state',value:root.dataset.surfaceState || '',passed:true}); else if (access === 'reduced_motion') { events.push({type:'computed_style',name:'transition_duration',value:style.transitionDuration || '',passed:true}); events.push({type:'computed_style',name:'animation_name',value:style.animationName || '',passed:true}); events.push({type:'computed_style',name:'animation_duration',value:style.animationDuration || '',passed:true}); } else if (access === 'reduced_transparency') events.push({type:'computed_style',name:'backdrop_filter',value:style.backdropFilter || '',passed:true}); return JSON.stringify({schema:'omi.native-runtime-marker/v1',domain,theme,accessibility:access,events}); })()`;
}
...
function runIos(manifest, outputDir) {
  ...
  const started = new Date();
  const result = spawnSync("xcodebuild", args, { cwd: path.join(iosRoot, "app"), env, encoding: "utf8", timeout: 300_000, maxBuffer: 64 * 1024 * 1024 });
  const finished = new Date();
  if (result.status !== 0) throw new Error(`iOS native runtime UI test failed (${result.status ?? result.error?.message ?? "signal"}); typed host marker is required`);
