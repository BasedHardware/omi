...
class _DelayedDocumentRunner {
  ...
  final List<({String source, Completer<void> release})> pending =
      <({String source, Completer<void> release})>[];

  Future<void> call(String source) async {
    final assignment = RegExp(
      r'__omiNativeChatDocumentGeneration=(\d+);',
    ).firstMatch(source);
    if (assignment != null) {
...
void main() {
  test(
    'pending stream frame cannot execute after reset and reused s1',
    () async {
  ...
  test(
    'pending staging reply cannot execute after close and reused a1',
    () async {
      final runner = _DelayedDocumentRunner();
      final sink = ChatBridgeJavaScriptSink(
        runner.call,
