...
String p7Response({int size = 3, Map<String, Object>? additions}) => jsonEncode(<String, Object>{
...
Future<Map<String, dynamic>> stageAndReply(ChatAttachmentStagingHost host, List<String> scripts) async {
...
void main() {
  test('picker cancellation returns cancelled without opening an upload', () async {
    final client = FakeChatNativeHttpClient();
    final picker = FakeChatAttachmentPicker(null);
    final scripts = <String>[];
    final host = makeHost(client: client, picker: picker, scripts: scripts, custody: ShellCredentialCustody('token'));
    final reply = await stageAndReply(host, scripts);
    expect(reply, <String, Object>{'ok': false, 'id': 'a1', 'reason': 'cancelled'});
    expect(client.requests, isEmpty);
    await host.close();
  });

  test('50 MiB file streams as exactly one multipart file and exact no-name P7 response succeeds', () async {
    ...
    final picked = FakePickedChatAttachment(
      sizeBytes: fileSize,
      stream: Stream<List<int>>.fromIterable(List<List<int>>.filled(chunkCount, chunk)),
    );
    ...
    expect(request.headers['content-type'], 'multipart/form-data; boundary=omi-chat-test-boundary');
    expect(request.declaredContentLength, request.bodyByteCount);
    expect('name="file"'.allMatches('${request.prefixText}${request.suffixText}'), hasLength(1));
  ...
  test('unsafe P7 descriptors redirects and non-201 responses never expose server metadata', () async {
    ...
    final cases = <({int status, bool redirect, Object body, String marker})>[
      ...
      (status: 201, redirect: false, body: {'attachment': valid, 'extra': true}, marker: 'attachment-opaque-01'),
      (status: 201, redirect: true, body: {'attachment': valid}, marker: 'attachment-opaque-01'),
      (status: 200, redirect: false, body: {'attachment': valid}, marker: 'attachment-opaque-01'),
      (status: 201, redirect: false, body: 'not-json', marker: 'not-json'),
    ];

    for (final item in cases) {
      final picked = FakePickedChatAttachment(sizeBytes: 3, stream: Stream<List<int>>.value(<int>[1, 2, 3]));
      final raw = item.body is String ? item.body as String : jsonEncode(item.body);
      final client = FakeChatNativeHttpClient()
        ..responses.add(
          FakeChatNativeHttpResponse(
      ...
      final reply = await stageAndReply(host, scripts);
      expect(reply, <String, Object>{'ok': false, 'id': 'a1', 'reason': 'shell-error'});
      expect(scripts.join(), isNot(contains(item.marker)));
      await host.close();
    }
  });

  test('caller metadata and signed-out custody are refused before upload dispatch', () async {
    final picked = FakePickedChatAttachment(sizeBytes: 3, stream: Stream<List<int>>.value(<int>[1, 2, 3]));
    final client = FakeChatNativeHttpClient();
    final scripts = <String>[];
    final custody = ShellCredentialCustody('token');
    custody.observeResponse(method: 'DELETE', path: '/v1/session/current', status: 204);
    final host = makeHost(client: client, picker: FakeChatAttachmentPicker(picked), scripts: scripts, custody: custody);
  ...
  test('sign-out near misses retain authority for a new upload', () async {
    final custody = ShellCredentialCustody('token');
    custody.observeResponse(method: 'DELETE', path: '/v1/session/other', status: 204);
    final client = FakeChatNativeHttpClient()
      ..responses.add(
        FakeChatNativeHttpResponse(
          statusCode: 201,
  ...
  test('an already opened upload lease may finish after exact sign-out', () async {
    final custody = ShellCredentialCustody('token');
    final client = FakeChatNativeHttpClient()
  ...
  test('navigation cancels the request and file stream and drops every late callback', () async {
    final fileBytes = StreamController<List<int>>();
    final picked = FakePickedChatAttachment(sizeBytes: 3, stream: fileBytes.stream);
    final client = FakeChatNativeHttpClient()
      ..responses.add(
        FakeChatNativeHttpResponse(
          statusCode: 201,
    ...
    await waitFor(() => client.requests.length == 1, reason: 'upload request');
  ...
  test('multipart omits per-part MIME for PDF PNG and text byte signatures', () async {
      ...
      final client = FakeChatNativeHttpClient()
        ..responses.add(
          FakeChatNativeHttpResponse(
            statusCode: 201,
            contentType: 'Application/JSON; Charset=UTF-8',
            bytes: Stream<List<int>>.value(utf8.encode(p7Response(size: body.length))),
          ),
        );
      final scripts = <String>[];
      final host = makeHost(
        client: client,
        picker: FakeChatAttachmentPicker(
          FakePickedChatAttachment(sizeBytes: body.length, stream: Stream<List<int>>.value(body)),
        ),
        scripts: scripts,
        custody: ShellCredentialCustody('token'),
      );
  ...
  test('addStream demand gates file production and cancellation interrupts it', () async {
    ...
    final host = makeHost(
      client: client,
      picker: FakeChatAttachmentPicker(FakePickedChatAttachment(sizeBytes: 3, stream: source())),
      scripts: scripts,
      custody: ShellCredentialCustody('token'),
    );
    await host.handleMessage('{"t":"pick-and-stage","id":"a-demand"}');
    await waitFor(() => client.requests.singleOrNull?.streamedChunks == 1, reason: 'first demanded file chunk');
    expect(produced, 1);
    client.requests.single.releaseDemand();
    await waitFor(() => client.requests.single.streamedChunks == 2, reason: 'second demanded file chunk');
    expect(produced, 2);
    await host.resetForNavigation();
    expect(client.requests.single.aborted, isTrue);
    client.requests.single.releaseDemand();
  ...
  test('file shrink and growth are rejected by the streamed byte counter', () async {
    for (final item in <({int declared, List<int> body})>[
      (declared: 3, body: <int>[1, 2]),
      (declared: 3, body: <int>[1, 2, 3, 4]),
    ]) {
      final client = FakeChatNativeHttpClient()
        ..responses.add(FakeChatNativeHttpResponse(statusCode: 201, contentType: 'application/json'));
      final scripts = <String>[];
      final host = makeHost(
        client: client,
        picker: FakeChatAttachmentPicker(
          FakePickedChatAttachment(sizeBytes: item.declared, stream: Stream<List<int>>.value(item.body)),
        ),
        scripts: scripts,
        custody: ShellCredentialCustody('token'),
      );
