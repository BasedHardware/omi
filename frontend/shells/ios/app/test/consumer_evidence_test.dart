...
RenderedConsumerObservation observation(ConsumerEvidenceRoute route) =>
    RenderedConsumerObservation(
      route: route,
      semantic: '${route.wireName}:rendered',
      transcript: route == ConsumerEvidenceRoute.listen
...
void main() {
  test(
    'real iOS writer replaces no prior run and atomically writes the exact seven-row document',
    () async {
      final scratch = await Directory.systemTemp.createTemp(
        'omi-ios-consumer-evidence-',
      );
      ...
      expect(
        await result.exists(),
        isFalse,
        reason: 'prior-run success must be removed before driving',
      );
    ...
    () async {
      expect(
        () => ConsumerEvidenceCollector(
          ...
          hashes: const ConsumerEvidenceTreeHashes(
            shell: hashA,
            surface: hashB,
          ),
        ),
      ...
      expect(
        () => ConsumerEvidenceTreeHashes.fromAssetJson(
          shellStamp: jsonEncode({
            'artifact': 'ios-bundle',
            'treeHash': 'stale',
          }),
          surfaceStamp: jsonEncode({
      ...
      expect(
        ...
        throwsFormatException,
        reason:
            'fixture-backed or launch-supplied fields are not rendered observation keys',
      );
      expect(
        () => RenderedConsumerObservation.decodeRenderedJson(
          '{"route":"listen","state":"ready","semantic":"listen"}',
        ),
        throwsFormatException,
      );
      expect(
      ...
      expect(
        () => collector.accept(
          observation(ConsumerEvidenceRoute.chat),
          ConsumerEvidenceRoute.memories,
        ),
        throwsStateError,
      );
      collector.accept(
        observation(ConsumerEvidenceRoute.memories),
        ConsumerEvidenceRoute.memories,
      );
      expect(
        () => collector.accept(
      ...
      expect(
        collector.finish,
        throwsStateError,
        reason: 'missing routes cannot write success',
      );
  ...
  test(
    'iOS host lifecycle drives each rendered route once and waits for a Listen transcript',
    () async {
      final scratch = await Directory.systemTemp.createTemp(
        'omi-ios-consumer-driver-',
      );
      addTearDown(() => scratch.delete(recursive: true));
      final result = File('${scratch.path}/result.json');
      ...
      for (
        var index = 0;
        index < ConsumerEvidenceRoute.values.length;
        index++
      ) {
      ...
      expect(
        listenStartAttempts,
        2,
        reason: 'the host must retry until the rendered control exists',
      );
