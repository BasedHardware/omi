...
export type ListenStartArgs = {
  ...
  language: string
  /** Use Deepgram instead of Omi backend for transcription. */
  useDeepgram?: boolean
}
...
export type OmiBridgeApi = {
  ...
  onDeepgramListenMessage: (cb: (msg: ListenMessage) => void) => () => void
  deepgramTtsSynthesize: (options: DeepgramTtsOptions) => Promise<DeepgramTtsResult>
  deepgramTtsVoices: () => Promise<DeepgramVoice[]>
  // Deepgram Voice Agent (STT + LLM + TTS)
  deepgramAgentStart: (args: { sessionId: string; config?: AgentConfig }) => Promise<void>
  deepgramAgentStop: (sessionId: string) => Promise<void>
...
export type DeepgramTtsResult = {
  ok: boolean
  audio?: number[] // Uint8Array as number[] for IPC transfer
  contentType?: string
  error?: string
}
...
export type AgentConfig = {
  ...
  // Personality
  agentName?: string // e.g. "friend", "aria", "max"
