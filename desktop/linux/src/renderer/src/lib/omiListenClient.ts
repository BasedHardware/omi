...
// Check if Deepgram is configured (main process has the API key)
function isDeepgramConfigured(): boolean {
  return typeof window.omi?.deepgramListenStart === 'function'
}
 ...
 * with AudioContext, then forwards each 4096-sample buffer to the main process
 * as Int16. The main process owns the WebSocket (needed to set the
 * Authorization header).
 *
 * When useDeepgram=true and Deepgram is configured, routes to Deepgram instead.
 * Optional tokenOverride allows bypassing Firebase auth (for Deepgram which uses its own API key).
 */
export async function startOmiListen(
  ...
  const token = tokenOverride || (user ? await user.getIdToken() : '')
  ...
  console.log(`[mic] stream acquired: tracks=${stream.getTracks().length}, active=${stream.active}`)
  stream.getTracks().forEach((t) => console.log(`[mic] track: kind=${t.kind}, enabled=${t.enabled}, readyState=${t.readyState}`))
  ...
  // Choose between Deepgram and Omi backend
  const useDeepgramBackend = useDeepgram && isDeepgramConfigured()
  const startFn = useDeepgramBackend ? window.omi.deepgramListenStart : window.omi.listenStart
  const feedFn = useDeepgramBackend ? window.omi.deepgramListenFeed : window.omi.listenFeed
  const stopFn = useDeepgramBackend ? window.omi.deepgramListenStop : window.omi.listenStop
  ...
  const unsub = window.omi.onListenMessage((msg) => {
    ...
    if (msg.kind === 'connected') {
      ...
      if (connected) {
        ...
        cb.onError(
          new Error(
            `${useDeepgramBackend ? 'Deepgram' : 'Omi'} /v4/listen closed (${msg.code}) ${msg.reason}`.trim()
          ),
          true
        )
      }
