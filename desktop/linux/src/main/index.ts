...
// Deepgram API key — used for custom transcription and TTS
// MAIN_VITE_ prefix required for electron-vite to expose it to main process
const DEEPGRAM_KEY = import.meta.env.MAIN_VITE_DEEPGRAM_API_KEY || ''
if (DEEPGRAM_KEY) {
  setDeepgramApiKey(DEEPGRAM_KEY)
  setDeepgramTtsApiKey(DEEPGRAM_KEY)
  setAgentApiKey(DEEPGRAM_KEY)
  console.log('[main] Deepgram API key loaded')
}
...
app.whenReady().then(async () => {
  ...
  // Deepgram: custom transcription (alternative to Omi v4/listen) and TTS
