...
const omi: OmiBridgeApi = {
  ...
  deepgramListenTestKey: (): Promise<{ ok: boolean; error?: string }> =>
    ipcRenderer.invoke('deepgram-listen:testKey'),
  deepgramListenStart: (args: ListenStartArgs) => ipcRenderer.invoke('deepgram-listen:start', args),
  deepgramListenStop: (sessionId: string) => ipcRenderer.invoke('deepgram-listen:stop', sessionId),
  deepgramListenFeed: (sessionId: string, pcm: ArrayBuffer) => {
    ipcRenderer.send('deepgram-listen:feed', sessionId, pcm)
  },
  onDeepgramListenMessage: (cb: (msg: ListenMessage) => void) => {
    const listener = (_e: Electron.IpcRendererEvent, msg: ListenMessage): void => cb(msg)
    ipcRenderer.on('omi-listen:message', listener) // Reuses same message channel
    return () => ipcRenderer.removeListener('omi-listen:message', listener)
  },
  deepgramTtsSynthesize: (options: DeepgramTtsOptions): Promise<DeepgramTtsResult> =>
    ipcRenderer.invoke('deepgram-tts:synthesize', options),
  deepgramTtsVoices: (): Promise<DeepgramVoice[]> => ipcRenderer.invoke('deepgram-tts:voices'),
  // Deepgram Voice Agent (STT + LLM + TTS)
  deepgramAgentStart: (args: { sessionId: string; config?: AgentConfig }) =>
    ipcRenderer.invoke('deepgram-agent:start', args),
  deepgramAgentStop: (sessionId: string) => ipcRenderer.invoke('deepgram-agent:stop', sessionId),
  deepgramAgentFeed: (sessionId: string, pcm: ArrayBuffer) => {
    ipcRenderer.send('deepgram-agent:feed', sessionId, pcm)
  },
  onDeepgramAgentMessage: (cb: (msg: AgentMessage) => void) => {
    const listener = (_e: Electron.IpcRendererEvent, msg: AgentMessage): void => cb(msg)
    ipcRenderer.on('deepgram-agent:message', listener)
    return () => ipcRenderer.removeListener('deepgram-agent:message', listener)
  },
  onDeepgramAgentAudio: (cb: (msg: AgentAudioMessage) => void) => {
