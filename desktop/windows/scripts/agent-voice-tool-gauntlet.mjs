...
const ONLY = (process.env.OMI_GAUNTLET_ONLY || '')
  .split(',')
  .map((s) => s.trim())
  .filter(Boolean)
...
const MATRIX = [
  // ── READS ──
  {
    id: 'get_action_items',
    category: 'read',
    request: 'What are my action items for today?',
    expect: 'get_action_items',
    alt: ['search_tasks', 'get_tasks']
  },
  {
    id: 'search_memories',
    category: 'read',
    request: 'Search my memories about my dog.',
    expect: 'search_memories',
    alt: ['get_memories', 'semantic_search']
  },
  ...
  },
  {
    ...
    category: 'read',
    request: 'Find my past conversations about groceries.',
    expect: 'search_conversations',
    alt: ['get_conversations', 'semantic_search']
  },
  {
    id: 'get_daily_recap',
    category: 'read',
    request: 'Give me a recap of what I did today.',
    expect: 'get_daily_recap',
    alt: ['get_conversations', 'search_conversations']
  },
  {
    id: 'semantic_search',
    category: 'read',
    request: 'What was I reading about on my screen earlier today?',
    expect: 'semantic_search',
    alt: ['get_work_context', 'search_conversations'],
    soft: true
  },
  {
    id: 'get_work_context',
    category: 'read',
    request: 'What am I working on right now on my computer?',
    expect: 'get_work_context',
    alt: ['semantic_search', 'capture_screen'],
    soft: true
  },
  ...
  //    row PASSES only if the backend TEXT actually changes.
  {
    ...
    category: 'mutation',
    request: 'Add buy zebra milk to my action items.',
    expect: 'create_action_item',
    re: /zebra/i,
    rest: (b, a) =>
      a.some((i) => /zebra/i.test(i.description)) && !b.some((i) => /zebra/i.test(i.description))
  },
  {
    id: 'update_action_item',
    category: 'mutation',
    request: 'Change my zebra milk task to say buy zebra oat milk instead.',
  ...
  },
  {
    id: 'delete_task',
    category: 'mutation',
    request: 'Delete my zebra milk task completely.',
    expect: 'delete_task',
    alt: ['search_tasks'],
    re: /zebra/i,
  ...
  {
    id: 'create_action_item_b',
    category: 'mutation',
    request: 'Add feed the penguin to my action items.',
    expect: 'create_action_item',
    re: /penguin/i,
    rest: (b, a) =>
      a.some((i) => /penguin/i.test(i.description)) &&
      !b.some((i) => /penguin/i.test(i.description))
  },
  {
    id: 'complete_task',
    category: 'mutation',
    request: 'Mark my penguin task as done.',
    expect: 'complete_task',
    alt: ['update_action_item', 'search_tasks'],
    re: /penguin/i,
    rest: (b, a, byId) => {
      const it = a.find((i) => /penguin/i.test(i.description))
      return !!it && byId.get(it.id)?.completed === true
    }
  },
  ...
  // ── DELEGATION ──
  {
    id: 'spawn_agent',
    category: 'delegation',
    request: 'Build me a simple tic tac toe web page.',
    expect: 'spawn_agent',
    alt: ['write', 'bash', 'edit']
  },
  ...
  // ── NEGATIVES ──
  {
    id: 'chitchat',
    category: 'negative',
    request: 'Hey Omi, how are you doing today?',
    expectNoTool: true
  },
  {
    id: 'calendar_temptation',
    category: 'negative',
    request: 'Put a lunch meeting on my calendar for tomorrow at noon.',
    expectNoTool: false,
    ...
    noPromise: ['create_calendar_event']
  }
]
...
function selected() {
  const rows = ONLY.length
    ? MATRIX.filter((t) => ONLY.includes(t.id) || ONLY.includes(t.category))
    : MATRIX
  return rows
}
...
function snapshotDefaults() {
  const out = ps(
    `Import-Module AudioDeviceCmdlets; $p=Get-AudioDevice -Playback; $r=Get-AudioDevice -Recording; "$($p.Index),$($r.Index)"`
  )
...
function routeCable() {
  ...
  if ((probe.stdout || '').trim() !== 'yes') {
    log('SKIP precondition: AudioDeviceCmdlets module not installed')
    return false
  }
  ...
  if (!(r.stdout || '').includes('ok')) {
    log('SKIP precondition: VB-Cable CABLE Input/Output not found')
    savedAudio = null
    return false
  }
...
function restoreAudio() {
  if (savedAudio) {
    ps(
      `Import-Module AudioDeviceCmdlets; Set-AudioDevice -Index ${savedAudio.play}|Out-Null; Set-AudioDevice -Index ${savedAudio.rec}|Out-Null`
    )
    log(`restored audio defaults (#${savedAudio.play}/#${savedAudio.rec})`)
    savedAudio = null
  }
}
...
function sapiSpeakToWav(text, wav) {
  const p1 = path.join(
    os.tmpdir(),
    `omi-g-${Date.now()}-${Math.random().toString(16).slice(2)}.ps1`
  )
  fs.writeFileSync(
    p1,
    [
  ...
  try {
    execFileSync('powershell', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', p1], {
      stdio: 'pipe',
      timeout: 120000
    })
  } finally {
...
function playWav(wav) {
  ps(`(New-Object System.Media.SoundPlayer '${wav.replace(/'/g, "''")}').PlaySync()`)
}
...
function findDshow() {
  ...
  for (const l of t.split(/\r?\n/)) {
    const m = l.match(/"([^"]*CABLE Output[^"]*)"/i)
    if (m) return m[1]
  }
  return null
}
function startCapture(dev, out) {
  return spawn(
    'ffmpeg',
    [
      '-hide_banner',
...
async function stopCapture(cap) {
  if (!cap || cap.exitCode !== null) return
...
function voicedMs(buf) {
  if (!buf || buf.byteLength < FRAME * 2) return 0
  const pcm = new Int16Array(buf.buffer, buf.byteOffset, Math.floor(buf.byteLength / 2))
  const fr = Math.floor(pcm.length / FRAME)
  let v = 0
  for (let f = 0; f < fr; f++) {
    let s = 0
    for (let i = 0; i < FRAME; i++) {
      const x = pcm[f * FRAME + i]
      s += x * x
    }
    if (Math.sqrt(s / FRAME) >= RMS_VOICED) v++
  }
  return v * 20
}

// ── Backend REST (ground truth for mutations) ────────────────────────────────────
function restBases(env) {
  return [env.VITE_OMI_DESKTOP_API_BASE, env.VITE_OMI_API_BASE, 'https://api.omi.me'].filter(
    Boolean
  )
}
async function restActionItems(base, tok) {
  const r = await fetch(`${base.replace(/\/$/, '')}/v1/action-items?limit=200`, {
  ...
  return items.map((x) => ({
    id: x.id || x.action_item_id || x.backendId,
    description: String(x.description || x.content || ''),
    completed: x.completed === true || x.completed_at != null || x.is_completed === true
  }))
}
async function restDelete(base, tok, id) {
  const r = await fetch(`${base.replace(/\/$/, '')}/v1/action-items/${id}`, {
    method: 'DELETE',
    headers: { Authorization: `Bearer ${tok}` }
  })
  return r.ok
...
async function mainWindow(app) {
  for (let i = 0; i < 50; i++) {
    const p =
      app.windows().find((w) => /#\/home/.test(w.url())) ||
      app
        .windows()
        .find(
          (w) =>
            !/#\/(capture|overlay|bar|insight-toast|meeting-toast|glow)/.test(w.url()) &&
            w.url() !== 'about:blank'
        )
    if (p) return p
...
async function barWindow(app) {
  for (let i = 0; i < 80; i++) {
    const p = app.windows().find((w) => /#\/bar/.test(w.url()))
    if (p) return p
    await new Promise((r) => setTimeout(r, 400))
  }
  return null
...
async function waitFor(page, fn, ms, label) {
  ...
  for (;;) {
    ...
    if (v) return v
    if (Date.now() > d) throw new Error('timeout ' + label)
    await new Promise((r) => setTimeout(r, 400))
  }
}
async function injectAuth(page, key, idToken, rt) {
  const c = decodeJwt(idToken)
  if (!c?.user_id) throw new Error('bad ID token')
  const user = {
    uid: c.user_id,
    email: c.email ?? null,
    emailVerified: true,
    isAnonymous: false,
    providerData: [],
    stsTokenManager: { refreshToken: rt, accessToken: idToken, expirationTime: c.exp * 1000 },
  ...
  await page.evaluate(
    ({ k, u }) => localStorage.setItem(`firebase:authUser:${k}:[DEFAULT]`, JSON.stringify(u)),
    { k: key, u: user }
  )
}
...
async function callVoiceTool(page, name, argsObj) {
  return page.evaluate(
    async ({ name, argsJSON }) => {
      try {
        return await window.omi.voiceToolExecute({ name, argumentsJSON: argsJSON })
      } catch (e) {
...
function agentSessionIds(raw) {
  ...
  for (const m of s.matchAll(
    /"?(?:id|session_?id|run_?id|agentId)"?\s*[:=]\s*"?([A-Za-z0-9_-]{6,})"?/g
  ))
    ids.add(m[1])
  return ids
}
...
async function main() {
  ...
  if (DO_BUILD) {
    ...
    execFileSync('node', [path.join(root, 'scripts', 'bundle-pimono-extension.mjs')], {
      stdio: 'inherit',
      cwd: root
    })
  }
  ...
  if (!fs.existsSync(mainEntry)) {
    log(`SKIP: build missing (${mainEntry}) — run with --build or 'pnpm build'`)
    process.exit(2)
  }
  if (!routeCable()) {
    restoreAudio()
  ...
  if (!dshow) {
    log('SKIP: ffmpeg dshow cannot see CABLE Output')
    restoreAudio()
    process.exit(2)
  }
  ...
  let idToken
  try {
    idToken = await exchangeRefreshToken(rt, key)
  } catch (e) {
    log(`SKIP: token exchange failed (${e.message})`)
    restoreAudio()
    process.exit(2)
  }
  ...
  for (const b of bases) {
    try {
      await restActionItems(b, idToken)
      restBase = b
      break
    } catch {
      /* next */
  ...
  try {
    app = await electron.launch({
      args: [mainEntry, `--user-data-dir=${ud}`],
    ...
    await page.evaluate(() => {
      const K = 'omi-windows-prefs-v1'
      const p = JSON.parse(localStorage.getItem(K) ?? '{}')
      p.onboardingCompletedAt = p.onboardingCompletedAt ?? Date.now()
      p.pttHubEnabled = true
    ...
    for (let i = 0; i < 40; i++) {
      catalog = await page.evaluate(async () => {
        try {
          return await window.omi.voiceHubToolCatalog()
        } catch {
          return []
        }
      })
    ...
    for (const t of rows) {
      ...
      for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        ...
        const priorRunIds = await page.evaluate(() => [
          ...new Set(window.__ev.map((e) => e.runId).filter(Boolean))
        ])
        log(`[${t.id}] (${t.category}) attempt ${attempt}: "${t.request}"`)
        await bar.evaluate(() => window.__omiPtt.beginHold())
        await new Promise((r) => setTimeout(r, 1400)) // capture-window spin-up margin
        const replyPcm = path.join(tmp, `${t.id}-${attempt}.pcm`)
        const cap = startCapture(dshow, replyPcm)
                ...
                (e) =>
                  ...
                  (e.type === 'run_finished' ||
                    ...
                    (cat === 'delegation' &&
                      ...
                      ['spawn_agent', 'write', 'bash', 'edit'].includes(e.name)))
              )
            },
        ...
        let rbuf = null
        try {
          rbuf = fs.readFileSync(replyPcm)
        } catch {
          /* */
        }
        att = {
          tools,
          toolInputs: toolEvents
            .filter((e) => e.status === 'started')
            .map((e) => ({ name: e.name, input: e.input })),
          replyText,
          spokeMs: voicedMs(rbuf),
          sawTerminal,
          attempts: attempt
        }
      ...
      const { tools, replyText, spokeMs } = att
      ...
      if (t.category === 'read') {
        ...
        row.detail = fired
          ...
          : alt
            ? `fired alt [${tools}]`
            : contextAnswered
              ? `answered from seeded context (no ${t.expect} call needed)`
              : tools.length
                ? `expected ${t.expect}, got [${tools}]`
                : `no tool fired (spoke=${spokeMs}ms)`
        ...
        row.itemBefore = beforeItem
          ? `"${beforeItem.description}" completed=${beforeItem.completed}`
          : '(none)'
        row.itemAfter = afterItem
          ? `"${afterItem.description}" completed=${afterItem.completed}`
          : '(none)'
        row.verdict =
          restOk === true
            ? 'PASS'
        ...
        if (spawned || newAgents.length) {
          row.verdict = 'PASS'
          row.detail = `spawn_agent=${spawned} newAgentSessions=${newAgents.length} coding=[${tools}]`
        } else if (didWork) {
          row.verdict = 'PASS(delegated-via-write)'
          row.detail = `coding-agent tools=[${tools}] but no spawn_agent/session signal observed`
        } else {
          row.verdict = 'FAIL'
          row.detail = `no delegation signal; tools=[${tools}]`
          exit = 1
        }
        ...
        row.detail = t.expectNoTool
          ? tools.length
            ? `expected NO tool, fired [${tools}]`
            : 'no tool (correct)'
          : forbidHit.length
            ? `fired forbidden [${forbidHit}]`
            : `no forbidden tool (tools=[${tools}])`
        if (row.verdict === 'FAIL') exit = 1
      }
      ...
      const promised = [...GLOBAL_FORBIDDEN_PROMISES, ...(t.noPromise || [])].filter((f) =>
        new RegExp('\\b' + f + '\\b', 'i').test(row.reply)
      )
      if (promised.length) {
        row.detail += ` · PROMISE-VIOLATION: reply mentions [${promised}]`
        row.promiseViolation = promised
        exit = 1
      }

      results.push(row)
      log(
        `[${t.id}] ${row.verdict} — ${row.detail} · spoke=${spokeMs}ms · reply="${(replyText || '').slice(0, 60)}"`
      )
      await new Promise((r) => setTimeout(r, 1500))
    }
    ...
    for (const d of [root, path.join(root, '..', '..')]) {
      for (const f of ['tic-tac-toe.html', 'tictactoe.html', 'tic_tac_toe.html', 'index.html']) {
        ...
        try {
          if (fs.existsSync(p) && /tic.?tac.?toe/i.test(fs.readFileSync(p, 'utf8'))) {
            fs.rmSync(p, { force: true })
            log(`cleanup: removed build artifact ${p}`)
          }
        } catch {
          /* */
        }
    ...
    if (spawnedAgents.size) {
      ...
      log(
        `spawned agent session(s) observed (local, torn down on app close): ${[...spawnedAgents].join(', ')}`
      )
      results.push({
        id: '__agents__',
        category: 'cleanup',
        verdict: 'INFO',
        detail: `spawned local agent sessions: ${[...spawnedAgents].join(', ')}`
      })
    }
  } catch (e) {
    log(`ERROR: ${e?.stack || e}`)
...
function printReport(results, uid) {
  ...
  for (const c of cats) {
    ...
    for (const r of rows) {
      ...
      log(
        `      req="${r.request}" · spoke=${r.spokeMs}ms · reply="${(r.reply || '').slice(0, 70)}"`
      )
    }
  }
  for (const r of results.filter((r) => r.category === 'cleanup'))
    log(`── CLEANUP ── ${r.verdict}: ${r.detail}`)
  log('──────────────────────────────────────────────────────────────')
  log(
    `  TALLY: ${pass}/${total} pass · ${unverified} unverified · promise-violations=${results.filter((r) => r.promiseViolation).length}`
  )
  log('══════════════════════════════════════════════════════════════')
  try {
    fs.writeFileSync(
      path.join(root, 'voice-tool-gauntlet-report.json'),
      JSON.stringify({ runId: RUN_ID, uid, results }, null, 2)
    )
  } catch {
    /* */
  }
