// The TaskAssistant's prompts. Pure strings — no network, no Electron.
//
// TASK_SYSTEM_PROMPT is Mac's `TaskAssistantSettings.defaultAnalysisPrompt`,
// ported line-for-line (a `static let` with no version constant, so — like
// Memory, unlike Focus — there is no prompt-version migration to port).
// buildUserPrompt reproduces Mac's per-call header (`extractTaskSingleStage`):
// the app name + today's date, then — for a messaging app — the messaging
// reminder block. The context sections Mac appends after (user profile, active
// / completed tasks) are assembled by the caller (context.ts), not here.

/** Verbatim Mac `defaultAnalysisPrompt` (TaskAssistantSettings.swift:117-289). */
export const TASK_SYSTEM_PROMPT = `You are a task commitment detector. Your ONLY job: find tasks the user has committed to in conversations, or unaddressed requests directed at the user.
...
const TASK_MESSAGING_REMINDER = `REMINDER — THIS IS A MESSAGING APP:
...
/** The per-call user message header (Mac `extractTaskSingleStage`,
 *  TaskAssistant.swift:891-908): the app name + today's date, then — for a
 *  messaging app — the messaging reminder block. `todayStr` is preformatted by
 *  the caller (Mac uses "yyyy-MM-dd (EEEE)"). `isMessagingApp` is decided by the
 *  caller from PROMPT_MESSAGING_APPS (appLists.ts). The context sections Mac
 *  appends after (user profile, active/completed tasks) are added by the caller. */
export function buildUserPrompt(
  appName: string,
  todayStr: string,
  isMessagingApp: boolean
): string {
  let prompt = `Screenshot from ${appName}. Today is ${todayStr}. Analyze this screenshot for any unaddressed request directed at the user.\n\n`
  if (isMessagingApp) {
    prompt += TASK_MESSAGING_REMINDER
  }
  return prompt
}
