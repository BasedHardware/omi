...
describe('appSettings', () => {
  ...
  it('sanitizes bad input back to safe defaults', () => {
    ...
    // Screen analysis is opt-in: absent or junk stays off.
    expect(sanitizeAppSettings(null).screenAnalysisEnabled).toBe(false)
    ...
    // Focus master flags are opt-in; cooldown and excluded-apps sanitize.
    expect(sanitizeAppSettings(null).focusEnabled).toBe(false)
    expect(sanitizeAppSettings(null).focusNotificationsEnabled).toBe(false)
    ...
    // Task: master flag is opt-in; interval reuses the cooldown sanitizer
