...
describe('appSettings', () => {
  ...
  it('sanitizes bad input back to safe defaults', () => {
    expect(sanitizeAppSettings({} as never)).toEqual({
      ...
      // No wearable paired; a paired one reconnects on its own, but streaming
      // its audio into a transcription session stays opt-in.
      device: { pairedDevice: null, autoReconnect: true, deviceListenEnabled: false },
      // Recovering audio the socket could not take happens on its own; keeping
      // every window (a full local archive) does not.
      offlineCapture: {
        autoSync: true,
    ...
    // A paired wearable is only restored when every field survives: a record
    // missing its id, or carrying a device type this build does not know,
    // would make reconnect target nothing, so it drops to "not paired" rather
    // than half-restoring.
    const paired = { id: 'dev-1', name: 'Omi CV1', type: 'omi' as const }
    expect(sanitizeAppSettings({ device: { pairedDevice: paired } } as never).device).toEqual({
      pairedDevice: paired,
      autoReconnect: true,
      deviceListenEnabled: false
    })
    expect(
      sanitizeAppSettings({ device: { pairedDevice: { name: 'x', type: 'omi' } } } as never).device
        .pairedDevice
    ).toBeNull()
    expect(
      sanitizeAppSettings({ device: { pairedDevice: { id: 'a', name: 'x', type: 'ring' } } } as never)
        .device.pairedDevice
    ).toBeNull()
    // A device with no advertised name falls back to its type rather than
    // showing an empty row.
    expect(
      sanitizeAppSettings({ device: { pairedDevice: { id: 'a', type: 'bee' } } } as never).device
        .pairedDevice
    ).toEqual({ id: 'a', name: 'bee', type: 'bee' })
    // Auto-reconnect is opt-out, listening is opt-in.
    expect(sanitizeAppSettings({ device: { autoReconnect: false } } as never).device.autoReconnect).toBe(
      false
    )
    expect(
      sanitizeAppSettings({ device: { deviceListenEnabled: 'yes' } } as never).device
        .deviceListenEnabled
    ).toBe(false)
    // Offline capture: auto sync is opt-out, retaining everything is opt-in.
    expect(
      sanitizeAppSettings({ offlineCapture: { autoSync: false } } as never).offlineCapture.autoSync
    ).toBe(false)
    expect(
      sanitizeAppSettings({ offlineCapture: { retainEverything: 'yes' } } as never).offlineCapture
        .retainEverything
    ).toBe(false)
    // Bounds keep a corrupt or hostile file from disabling retention entirely
    // or letting the log grow without limit.
    expect(
      sanitizeAppSettings({ offlineCapture: { retentionDays: 0 } } as never).offlineCapture
        .retentionDays
    ).toBe(1)
    expect(
      sanitizeAppSettings({ offlineCapture: { retentionDays: 100_000 } } as never).offlineCapture
        .retentionDays
    ).toBe(365)
    expect(
      sanitizeAppSettings({ offlineCapture: { maxBytes: 1 } } as never).offlineCapture.maxBytes
    ).toBe(64 * 1024 * 1024)
    expect(
      sanitizeAppSettings({ offlineCapture: { maxBytes: 'lots' } } as never).offlineCapture.maxBytes
