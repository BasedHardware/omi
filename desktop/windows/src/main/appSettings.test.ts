...
describe('appSettings', () => {
  ...
  it('sanitizes bad input back to safe defaults', () => {
    ...
    // Agent launch commands are handed to spawn(), so the sanitizer is a security
    // boundary, not tidying: only the three known ids survive, values are trimmed
    // and must be non-empty, and an over-long command line is dropped rather than
    // truncated (a truncated shell command is its own hazard).
    expect(sanitizeAppSettings({ agentCommands: { codex: '  codex acp  ' } } as never)).toMatchObject(
      { agentCommands: { codex: 'codex acp' } }
    )
    expect(
      sanitizeAppSettings({ agentCommands: { evil: 'calc.exe' } } as never).agentCommands
    ).toEqual({})
    expect(
      sanitizeAppSettings({ agentCommands: { codex: '   ' } } as never).agentCommands
    ).toEqual({})
    expect(
      sanitizeAppSettings({ agentCommands: { codex: 'x'.repeat(1025) } } as never).agentCommands
    ).toEqual({})
    expect(sanitizeAppSettings({ agentCommands: 'nope' } as never).agentCommands).toEqual({})
