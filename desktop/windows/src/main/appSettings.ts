...
export type AppSettings = {
  ...
  /** Launch commands for the external coding agents (OpenClaw/Hermes/Codex), set
   *  in Settings → Agents. Absent id = not connected (the matching
   *  OMI_*_ADAPTER_COMMAND env var still works as a power-user override); Claude
   *  Code is built in and needs no command.
   *
   *  These live in main, not renderer preferences, because they are executed:
   *  acp.ts spawns them. A renderer-held copy (the old localStorage home) made a
   *  single write to that origin's storage into persistent code execution,
   *  re-run on every agent turn. Main is now the sole authority — the renderer
   *  sends an agent id and never a command string. */
  agentCommands: AgentCommands
}

/** Launch commands for the external coding agents, keyed by adapter id. */
export type AgentCommands = { openclaw?: string; hermes?: string; codex?: string }
...
/** The agent ids that can carry a launch command (Claude Code is built in). */
export const AGENT_COMMAND_IDS = ['openclaw', 'hermes', 'codex'] as const

// Agent launch commands: a FIXED key set of bounded, non-empty strings. Unknown
// keys are dropped rather than carried, and each value is length-capped — these
// strings are handed to spawn(), so a corrupt or hostile settings file must not
// be able to smuggle in an extra adapter id or an unbounded command line.
function sanitizeAgentCommands(raw: unknown): AgentCommands {
  const out: AgentCommands = {}
  if (!raw || typeof raw !== 'object') return out
  const r = raw as Record<string, unknown>
  for (const id of AGENT_COMMAND_IDS) {
    const v = r[id]
    if (typeof v === 'string' && v.trim() && v.length <= 1024) out[id] = v.trim()
  }
  return out
}
...
export function sanitizeAppSettings(raw: Partial<AppSettings> | null | undefined): AppSettings {
  ...
  return {
    ...
    agentCommands: sanitizeAgentCommands(r.agentCommands)
  }
}
