...
export type AppSettings = {
  ...
  /** Paired wearable + its behavior. Main owns this because reconnect has to
   *  work with no UI window open. */
  device: DeviceSettings
  /** Offline audio capture: what is kept when the transcription socket is
   *  unavailable, and how it is uploaded afterwards. */
  offlineCapture: OfflineCaptureSettings
}
...
const WEARABLE_DEVICE_TYPES: readonly WearableDeviceType[] = [
  'omi',
  'openglass',
  'frame',
  'appleWatch',
...
// A paired device is only usable if all three fields survive: an entry missing
// its id or carrying an unknown type would make reconnect target nothing, so it
// is dropped back to "no device paired" rather than half-restored.
function sanitizePairedDevice(raw: unknown): WearableDeviceInfo | null {
  if (!raw || typeof raw !== 'object') return null
  const r = raw as Partial<WearableDeviceInfo>
  if (typeof r.id !== 'string' || r.id.length === 0 || r.id.length > 256) return null
  if (!WEARABLE_DEVICE_TYPES.includes(r.type as WearableDeviceType)) return null
  const name = typeof r.name === 'string' && r.name.length > 0 ? r.name.slice(0, 128) : r.type
  return { id: r.id, name: name as string, type: r.type as WearableDeviceType }
}

function sanitizeDevice(raw: Partial<DeviceSettings> | null | undefined): DeviceSettings {
  const r = raw ?? {}
  return {
    pairedDevice: sanitizePairedDevice(r.pairedDevice),
    // Opt-OUT: a paired device should come back on its own after a drop.
    autoReconnect: r.autoReconnect !== false,
    // Opt-IN: streaming device audio starts a transcription session, so it
    // stays off until the user asks for it.
    deviceListenEnabled: r.deviceListenEnabled === true
  }
}

// Offline capture keeps audio the transcription socket could not take. Auto
// sync is opt-OUT because the whole point is that a recovered recording becomes
// a conversation without the user chasing it; retaining EVERY window is opt-IN
// because it turns a recovery buffer into a full local archive.
function sanitizeOfflineCapture(
  raw: Partial<OfflineCaptureSettings> | null | undefined
): OfflineCaptureSettings {
  const r = raw ?? {}
  const days =
    typeof r.retentionDays === 'number' && Number.isFinite(r.retentionDays)
      ? Math.min(365, Math.max(1, Math.round(r.retentionDays)))
      : 14
  const maxBytes =
    typeof r.maxBytes === 'number' && Number.isFinite(r.maxBytes) && r.maxBytes > 0
      ? Math.min(64 * 1024 * 1024 * 1024, Math.max(64 * 1024 * 1024, Math.round(r.maxBytes)))
      : 2 * 1024 * 1024 * 1024
  return {
    autoSync: r.autoSync !== false,
    retainEverything: r.retainEverything === true,
    retentionDays: days,
    maxBytes
  }
}
...
export function sanitizeAppSettings(raw: Partial<AppSettings> | null | undefined): AppSettings {
  ...
  return {
    ...
    device: sanitizeDevice(r.device),
    offlineCapture: sanitizeOfflineCapture(r.offlineCapture)
  }
}
