// Kernel support utilities — Windows port of the macOS agent runtime's
// kernel-support.ts (desktop/macos/agent/src/runtime/kernel-support.ts).
//
// Everything the kernel layers share: status constants, delegation bounds,
// deterministic hashing / JSON canonicalization, the generic column-patch UPDATE
// helper, value coercers, the row->entity mappers, the *ColumnMap constants, the
// run-path helpers (bindingMetadata, mcpServersForBinding,
// refreshMcpAttemptContext, canonicalAdapterEventType, isStaleBindingError), and
// the action-queue projections.
//
// PR #3a landed the first group; PR #3b (this one) completes the file now that
// the kernel consumes the rest against live rows.

import { createHash } from 'node:crypto'
import { writeFileSync } from 'node:fs'
import { tmpdir } from 'node:os'
import type { AdapterStreamEvent, RuntimeAdapter } from '../codingAgent/interface'
import type {
  AdapterBinding,
  AgentArtifact,
  AgentDelegation,
...
import type { ExecuteAgentRunInput } from './kernelTypes'
import { StaleAdapterBindingError } from './kernelTypes'
import type { DesktopContextSnippetInput } from './desktopContextPacket'
import type { DesktopIntentSessionCandidate } from './desktopIntentRouter'
import type {
  QueueArtifactDeliveryInput,
  QueueCandidateInput,
  QueueDispatchInput,
  QueueOverrideInput
...
/**
 * Protocol version stamped into the MCP tool-context file the Omi tools bridge
 * reads. Windows runs the kernel in-process (no JSONL transport), so there is no
 * wire protocol to negotiate — but the context-file contract is shared with the
 * macOS tools bridge, which pins v2. Keep these in lockstep.
 */
export const KERNEL_MCP_PROTOCOL_VERSION = 2
...
export function isStaleBindingError(error: unknown): boolean {
  return (
    error instanceof StaleAdapterBindingError ||
    (error instanceof Error && error.name === 'StaleAdapterBindingError')
  )
}
...
/**
 * Map an adapter stream event onto the kernel's canonical event type. Windows
 * adapters emit a narrower event union than macOS (no `tool_use` / `error`
 * drafts — the ACP client projects those into `tool_activity`), so those cases
 * are absent rather than unreachable.
 */
export function canonicalAdapterEventType(event: AdapterStreamEvent): string | undefined {
  switch (event.type) {
    case 'text_delta':
...
export function mcpServersForBinding(
  ...
  return mcpServers.map((server) => {
    ...
    normalized.env = upsertEnv(
      env,
      'OMI_CONTEXT_FILE',
      contextFileForBinding(sessionId, adapterId, runtimeNodeId)
    )
    return normalized
  })
}
...
function contextFileForBinding(
  sessionId: string,
  adapterId: string,
  runtimeNodeId: string
): string {
...
export function desktopArtifactDeliveryFromRow(
...
export function desktopMemoryCandidateFromRow(
  row: Record<string, unknown>
): DesktopMemoryCandidate {
  return {
...
export function desktopAttentionOverrideFromRow(
  row: Record<string, unknown>
): DesktopAttentionOverride {
  return {
    ownerId: text(row.owner_id),
...
export function deliveryToQueueInput(
