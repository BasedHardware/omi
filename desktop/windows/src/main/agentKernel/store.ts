// SqliteAgentStore — the agent kernel's dedicated SQLite persistence layer
// (Windows port of the macOS agent runtime's `sqlite-store.ts`, frozen tag
// v0.12.72). It owns its own database file (`omi-agentd.sqlite3` under userData),
// isolated from the app's existing chat store (`omi.db`), which satisfies
// INV-CHAT-1 (kernel-owned transcript store) and makes the kernel flip a clean
// kill-switch. The single-active-attempt / single-active-binding constraints and
// startup crash recovery it enforces are the INV-AGENT-* control-plane
// invariants.
//
// Driver: production uses `better-sqlite3` (matching `src/main/ipc/db.ts`), but
// the class is written against a minimal structural `KernelDatabase` interface so
// tests can inject `node:sqlite`'s `DatabaseSync` — the same split used by
// `dbMigrations.ts`. This is required because better-sqlite3 is rebuilt for
// Electron's ABI and cannot load under plain-node Vitest. The better-sqlite3
// require is therefore lazy (see `defaultDatabaseFactory`), so importing this
// module in a test never touches the native binary.
//
// This store is INERT this PR — nothing constructs or drives it. PR #3
// (kernel-core) wires it into the app.
...
/** A row read back from SQLite (column name → value). */
export type Row = Record<string, unknown>

/** Result of a mutating statement, as returned by both supported drivers. */
export interface KernelRunResult {
  changes: number | bigint
  lastInsertRowid: number | bigint
}

/** Minimal prepared-statement surface satisfied by better-sqlite3 and node:sqlite. */
export interface KernelStatement {
  run(...params: unknown[]): KernelRunResult
  get(...params: unknown[]): unknown
  all(...params: unknown[]): unknown[]
  iterate(...params: unknown[]): IterableIterator<unknown>
}

/** Minimal database surface satisfied by better-sqlite3's `Database` and node:sqlite's `DatabaseSync`. */
export interface KernelDatabase {
  exec(sql: string): void
  prepare(sql: string): KernelStatement
  close(): void
}

/** Constructor shape for a {@link KernelDatabase} implementation. */
export type DatabaseFactory = new (path: string) => KernelDatabase

/** Options for constructing a {@link SqliteAgentStore}. */
...
/** Options for {@link probeSqliteRuntime}. */
...
/** Generate a prefixed kernel id: `<prefix>_<uuidWithoutDashes>`. */
...
/** Resolve the on-disk database path for a given kernel state directory. */
...
// this module (which tests do) must never touch the binary. Tests inject
// node:sqlite's DatabaseSync via `databaseFactory` and never hit this path.
function defaultDatabaseFactory(): DatabaseFactory {
  const requireFn = createRequire(import.meta.url)
  return requireFn('better-sqlite3') as unknown as DatabaseFactory
}

/**
 * Boot a throwaway in-memory database through every migration and a probe write
 * to verify the bundled SQLite runtime supports the AgentStore feature set
 * (STRICT tables, json_valid CHECKs, partial-unique indexes). Throws loudly if
 * not. Used at daemon startup as a fail-fast guard.
 */
export function probeSqliteRuntime(options: SqliteProbeOptions = {}): void {
  const Factory = options.databaseFactory ?? defaultDatabaseFactory()
  let db: KernelDatabase | undefined
  try {
    db = new Factory(':memory:')
    ...
    runTransaction(db, () => {
      ...
      ).run('ses_probe', 'owner_probe', 'open', 'probe', 'acp', 1, 1, 1)
      db?.prepare(
        'INSERT INTO events (event_id, session_id, type, payload_json, created_at_ms) VALUES (?, ?, ?, ?, ?)'
      ).run('evt_probe', 'ses_probe', 'probe.ok', '{}', 1)
    })
  } catch (error) {
    throw new Error(
      `Bundled SQLite runtime does not support required AgentStore features: ${messageFrom(error)}`
    )
  } finally {
    db?.close()
  }
...
/** SQLite-backed implementation of {@link AgentStore}. See file header for the driver split. */
export class SqliteAgentStore implements AgentStore {
  ...
  constructor(options: SqliteAgentStoreOptions = {}) {
    const databasePath = options.databasePath ?? defaultDatabasePath(options.stateDir)
    mkdirSync(dirname(databasePath), { recursive: true })
    const Factory = options.databaseFactory ?? defaultDatabaseFactory()
    this.db = new Factory(databasePath)
  ...
  withTransaction<T>(work: () => T): T {
    // Track nesting depth ourselves rather than trusting the driver's
    // inTransaction/isTransaction flag: the bundled agent runtime does not
    // reliably report an open transaction, so a nested call that issued its own
    // BEGIN would fail with "cannot start a transaction within a transaction"
    // and break every agent operation.
  ...
  reconcileStartup(): StartupReconciliationResult {
    return this.withTransaction(() => {
      ...
      this.db
        .prepare(
          `UPDATE run_attempts SET status = ?, adapter_instance_id = ?, completed_at_ms = COALESCE(completed_at_ms, ?), updated_at_ms = ? WHERE status IN (${placeholders(ACTIVE_ATTEMPT_STATUSES.length)})`
        )
        .run('orphaned', '', now, now, ...ACTIVE_ATTEMPT_STATUSES)
      ...
      for (const runId of orphanedRunIds) {
        this.db
          .prepare(
            'UPDATE runs SET status = ?, completed_at_ms = COALESCE(completed_at_ms, ?), updated_at_ms = ? WHERE run_id = ?'
          )
          .run('orphaned', now, now, runId)
      }

      const clearedAttemptInstanceIds = Number(
        this.db
          .prepare('UPDATE run_attempts SET adapter_instance_id = ? WHERE adapter_instance_id != ?')
          .run('', '').changes
      )
      const clearedBindingInstanceIds = Number(
        this.db
          .prepare(
            'UPDATE adapter_bindings SET adapter_instance_id = NULL WHERE adapter_instance_id IS NOT NULL'
          )
          .run().changes
      )
      ...
      for (const binding of staleBindings) {
        this.db
          .prepare(
            'UPDATE adapter_bindings SET status = ?, adapter_instance_id = NULL, invalidated_at_ms = COALESCE(invalidated_at_ms, ?), updated_at_ms = ? WHERE binding_id = ?'
          )
          .run('stale', now, now, binding.binding_id)
      }
        ...
        eventIds.push(
          this.appendReconciliationEvent({
            ...
            payload: {
              ...
              reason: 'non_resumable_binding_after_restart'
            },
            createdAtMs: now
          })
        )
      ...
      if (expiredContinuationCheckpointIds.length > 0) {
        this.db
          .prepare('DELETE FROM workstream_continuation_checkpoints WHERE expires_at_ms <= ?')
          .run(now)
      }
      ...
      this.db
        ...
        .run(
          ...
          JSON.stringify({ reason: 'daemon_startup_reconciliation' }),
          'pending',
          now
        )
      ...
      for (const run of orphanedDelegatedRuns) {
        const dispatch = this.insertDesktopDispatch({
          ...
          decisionPrompt:
            'A delegated local agent run was interrupted by a daemon restart. Choose whether to inspect, retry, or dismiss it.',
        ...
        for (const delegation of delegations) {
          this.db
            .prepare(
              'UPDATE delegations SET status = ?, completed_at_ms = COALESCE(completed_at_ms, ?) WHERE delegation_id = ?'
            )
            .run('failed', now, text(delegation.delegation_id))
          eventIds.push(
            this.appendReconciliationEvent({
  ...
  insertSession(input: NewAgentSession): AgentSession {
    ...
    const session: AgentSession = {
      ...
      providerBoundary:
        input.providerBoundary ?? providerBoundaryForAdapter(input.defaultAdapterId),
  ...
  resolveDesktopDispatch(
    ...
    return desktopDispatchFromRow(
      this.getRow('SELECT * FROM desktop_dispatches WHERE dispatch_id = ?', [dispatchId])
    )
  }
  ...
  insertDesktopTaskCandidate(input: NewDesktopTaskCandidate): DesktopTaskCandidate {
    ...
    const candidate: DesktopTaskCandidate = {
      ...
      generationReconciled:
        input.generationReconciled ?? (input.accountGeneration === undefined ? 0 : 1),
      status: input.status ?? 'pending',
      deliveryStatus: input.deliveryStatus ?? 'pending',
  ...
  upsertDesktopAttentionOverride(input: NewDesktopAttentionOverride): DesktopAttentionOverride {
    ...
    return desktopAttentionOverrideFromRow(
      this.getRow(
  ...
  private assertCoordinatorScope(input: {
    ...
    if (input.sessionId) {
      const session = this.getOptionalRow('SELECT owner_id FROM sessions WHERE session_id = ?', [
        input.sessionId
      ])
    ...
    if (input.packetId) {
      const packet = this.getOptionalRow(
        'SELECT owner_id FROM desktop_context_packets WHERE packet_id = ?',
        [input.packetId]
      )
    ...
    if (input.dispatchId) {
      const dispatch = this.getOptionalRow(
        'SELECT owner_id FROM desktop_dispatches WHERE dispatch_id = ?',
        [input.dispatchId]
      )
...
/**
 * Resolve the default database path: an explicit state dir if given, else the
 * kernel database under Electron's userData directory.
 */
function defaultDatabasePath(stateDir: string | undefined): string {
  if (stateDir) {
    return databasePathForStateDir(stateDir)
  }
  return join(app.getPath('userData'), DATABASE_FILENAME)
}

function applyConnectionPragmas(db: KernelDatabase): void {
  db.exec(`
...
function createSchemaMigrationsTable(db: KernelDatabase): void {
  db.exec(`
...
function runPhase1Migration(db: KernelDatabase, appliedAtMs: number): void {
  runTransaction(db, () => {
...
function runArtifactLifecycleMigration(db: KernelDatabase, appliedAtMs: number): void {
  runTransaction(db, () => {
...
function tableHasColumn(db: KernelDatabase, tableName: string, columnName: string): boolean {
...
function runDesktopDispatchesMigration(db: KernelDatabase, appliedAtMs: number): void {
  runTransaction(db, () => {
    db.exec(`
...
function runDesktopArtifactDeliveriesMigration(db: KernelDatabase, appliedAtMs: number): void {
  runTransaction(db, () => {
    db.exec(`
...
function runDesktopCandidatesMigration(db: KernelDatabase, appliedAtMs: number): void {
  runTransaction(db, () => {
    db.exec(`
...
function runDesktopContextAccessLogMigration(db: KernelDatabase, appliedAtMs: number): void {
  runTransaction(db, () => {
    db.exec(`
...
function runDesktopAttentionOverridesMigration(db: KernelDatabase, appliedAtMs: number): void {
  runTransaction(db, () => {
    db.exec(`
...
function runSurfaceConversationsMigration(db: KernelDatabase, appliedAtMs: number): void {
  runTransaction(db, () => {
    db.exec(`
...
function runConversationTurnsMigration(db: KernelDatabase, appliedAtMs: number): void {
  runTransaction(db, () => {
    // NOTE (Mac parity): conversation_turns.metadata_json has NO json_valid CHECK,
    // unlike every other *_json column in this schema.
    db.exec(`
...
function runBindingTurnDeliveryMigration(db: KernelDatabase, appliedAtMs: number): void {
  runTransaction(db, () => {
    db.exec(`
...
function runWorkstreamContinuityMigration(db: KernelDatabase, appliedAtMs: number): void {
  runTransaction(db, () => {
    db.exec(`
...
function runActiveAttemptAuthorityMigration(db: KernelDatabase, appliedAtMs: number): void {
  runTransaction(db, () => {
    const repairedAttempts = db
      .prepare(
        `
...
function runSessionExecutionPolicyMigration(db: KernelDatabase, appliedAtMs: number): void {
  runTransaction(db, () => {
    db.exec(`
...
// is safe here. Reads whichever flag the driver exposes (better-sqlite3
// `inTransaction`, node:sqlite `isTransaction`).
function runTransaction<T>(db: KernelDatabase, work: () => T): T {
...
function isInTransaction(db: KernelDatabase): boolean {
  const flags = db as { inTransaction?: boolean; isTransaction?: boolean }
  return Boolean(flags.inTransaction ?? flags.isTransaction)
}

function sessionIdForRun(db: KernelDatabase, runId: string): string {
  const row = db.prepare('SELECT session_id FROM runs WHERE run_id = ?').get(runId) as
    | Row
    | undefined
  if (!row) {
...
function sessionValues(session: AgentSession): SqlBind[] {
  return [
    session.sessionId,
    session.ownerId,
...
function runValues(run: AgentRun): SqlBind[] {
  return [
    run.runId,
    run.sessionId,
    run.parentRunId,
...
function attemptValues(attempt: RunAttempt): SqlBind[] {
  return [
    attempt.attemptId,
    attempt.runId,
    attempt.attemptNo,
...
function bindingValues(binding: AdapterBinding): SqlBind[] {
  return [
    binding.bindingId,
    binding.sessionId,
    binding.adapterId,
...
function eventValues(event: AgentEvent): SqlBind[] {
  return [
    event.eventId,
    event.sessionId,
    event.runId,
...
function artifactValues(artifact: AgentArtifact): SqlBind[] {
  return [
    artifact.artifactId,
    artifact.sessionId,
    artifact.runId,
...
function grantValues(grant: AgentGrant): SqlBind[] {
  return [
    grant.grantId,
    grant.sessionId,
    grant.runId,
...
function desktopContextPacketValues(packet: DesktopContextPacket): SqlBind[] {
  return [
    packet.packetId,
    packet.ownerId,
    packet.sessionId,
...
function desktopDispatchValues(dispatch: DesktopCoordinatorDispatch): SqlBind[] {
  return [
    dispatch.dispatchId,
    dispatch.ownerId,
    dispatch.kind,
...
function desktopArtifactDeliveryValues(delivery: DesktopArtifactDelivery): SqlBind[] {
  return [
    delivery.deliveryId,
    delivery.artifactId,
    delivery.ownerId,
...
function desktopMemoryCandidateValues(candidate: DesktopMemoryCandidate): SqlBind[] {
  return [
...
function desktopTaskCandidateValues(candidate: DesktopTaskCandidate): SqlBind[] {
  return [
    candidate.candidateId,
    candidate.ownerId,
    candidate.sourceSessionId,
...
function desktopContextAccessLogValues(access: DesktopContextAccessLog): SqlBind[] {
  return [
    access.accessId,
    access.ownerId,
    access.packetId,
...
function desktopAttentionOverrideValues(override: DesktopAttentionOverride): SqlBind[] {
  return [
    override.ownerId,
