...
// `normalizeOmiToolName`, `buildToolAvailabilitySnapshot`) plus the surface-patch
// layer. This module is plain data plus pure filter/map functions — no kernel, no
...
> &
  Partial<
    Pick<OmiToolManifestEntry, 'surfaces' | 'capabilityDoc' | 'aliasCapabilityDocs' | 'voice'>
  >
...
function piAndStdio(
  condition: OmiToolCondition = 'always'
): Partial<Record<OmiToolAdapterId, OmiToolAdapterAvailability>> {
  return {
    'pi-mono': {
      advertised: condition !== 'onboardingOnly',
...
function piLocalApiAndScreenContextStdio(): Partial<
  Record<OmiToolAdapterId, OmiToolAdapterAvailability>
> {
  return {
...
function withSurfacePatch(
  entry: OmiToolManifestEntryDraft,
  patch: OmiToolSurfacePatch
): OmiToolManifestEntry {
...
function finalizeManifestEntries(
  drafts: OmiToolManifestEntryDraft[],
  patches: Record<string, OmiToolSurfacePatch>
): OmiToolManifestEntry[] {
...
const swiftToolSurfacePatches: Record<string, OmiToolSurfacePatch> = {
  ...
  semantic_search: {
    ...
    aliasCapabilityDocs: {
      search_screen_history: {
        ...doc('Search Screen History', "Search the user's on-screen history by meaning.", [
  ...
  get_tasks: {
    surfaces: ['realtime_voice'],
    capabilityDoc: doc('Get Tasks', "Read the user's overdue and due-today tasks locally.", [
  ...
  get_conversations: {
    surfaces: ['desktop_chat', 'realtime_voice'],
    capabilityDoc: doc('Get Conversations', 'Retrieve conversations by recency or date range.', [
  ...
  search_memories: {
    surfaces: ['desktop_chat', 'realtime_voice'],
    capabilityDoc: doc('Search Memories', 'Semantic search across user memories.', [
  ...
  create_action_item: {
    surfaces: ['desktop_chat', 'realtime_voice'],
    capabilityDoc: doc('Create Action Item', 'Create a new task, to-do, or reminder.', [
  ...
  create_calendar_event: {
    surfaces: ['realtime_voice'],
    capabilityDoc: doc('Create Calendar Event', 'Create a new Google Calendar event.', [
      ...
      schemaOverride: schema(
        {
          ...
          start_time: {
            type: 'string',
            description:
              'Event start time in ISO-8601 with timezone, e.g. 2026-06-28T14:00:00-04:00.'
          },
          end_time: {
            type: 'string',
  ...
  check_permission_status: {
    ...
    capabilityDoc: doc(
      ...
      'Check whether a required macOS permission has been granted.',
      [
  ...
  request_permission: {
    ...
    capabilityDoc: doc(
      'Request Permission',
      'Open or guide the user through granting a required macOS permission.',
      [
  ...
  scan_files: {
    ...
    capabilityDoc: doc(
      'Scan Files',
      'Scan selected files/folders during onboarding to build local context.',
      ['Onboarding-only.']
    )
  },
  set_user_preferences: {
    ...
    capabilityDoc: doc(
      ...
      'Persist onboarding preferences such as name and language.',
      ['Onboarding-only.']
    )
  },
  ask_followup: {
    ...
    capabilityDoc: doc(
      'Ask Followup',
      'Ask the user a follow-up onboarding question with optional quick replies.',
      ['Onboarding-only.']
    )
  },
  complete_onboarding: {
    ...
    capabilityDoc: doc(
      'Complete Onboarding',
      'Complete onboarding after required goals and context are collected.',
      ['Onboarding-only.']
    )
  },
  get_email_insights: {
  ...
  get_screenshot: {
    ...
    capabilityDoc: doc(
      'Get Screenshot',
      'Fetch a local Rewind screenshot image by screenshot_id.',
      ['Local API only.']
    )
  },
  get_work_context: {
  ...
  screenshot: {
    ...
    voice: {
      realtimeDescription:
        "Capture the user's current screen so you can see what they're looking at."
    }
  },
  point_click: {
    surfaces: ['realtime_voice'],
...
const swiftToolManifestDrafts: OmiToolManifestEntryDraft[] = [
  {
    ...
    latency: 'fast local',
    inputSchema: schema({ query: { type: 'string', description: 'SQL query to execute' } }, [
      'query'
    ]),
  ...
  },
  {
    ...
    label: 'Daily Recap',
    description:
      'Pre-formatted daily activity recap: app usage, conversations, tasks, focus, memories, observations.',
    promptSnippet: 'get_daily_recap - Get a daily activity summary',
    latency: 'fast local',
    inputSchema: schema({
      days_ago: { type: 'number', description: '0=today, 1=yesterday, 7=past week' }
    }),
    annotations: readOnlyLocal,
    timeoutClass: 'normal',
    executor: { kind: 'swiftTool' },
  ...
  },
  {
    ...
    promptSnippet:
      'fill_cloud_connector_form - Fill and optionally submit the visible ChatGPT/Claude MCP connector form',
    promptGuidelines: [
      'Call this first for ChatGPT or Claude cloud MCP connector setup when the connector form is visible.',
      'Do not install browser extensions before trying this tool.',
  ...
  },
  {
    ...
    description:
      'Retrieve user conversations with summaries, action items, metadata. Use for time-based queries or recaps.',
  ...
  },
  {
    ...
    label: 'Get Memories',
    description:
      "Retrieve user memories - facts, preferences, habits. Use for 'what do you know about me?' type questions.",
  ...
  },
  {
    ...
    description:
      'Semantic search across user memories. Find memories about a topic using AI embeddings.',
  ...
  },
  {
    ...
    description:
      'Check whether a required macOS permission has been granted. Use before requesting access, or after a permission request.',
  ...
  },
  {
    ...
    intendedForAgents: true,
    runtimePreconditions: [
      'Requires explicit current-turn user consent; some macOS permissions require the user to toggle Settings manually.'
    ],
    adapters: piAndStdio()
  },
  {
...
export const omiToolManifest: OmiToolManifestEntry[] = [
  ...
  ...agentControlCapabilityManifest.map((tool) => controlEntry(tool)),
...
export function isToolAvailableForContext(
  ...
  if (availability.condition === 'screenContextOrOnboarding')
    return context.screenContext === true || context.onboarding === true
  return true
}
...
export function toolsForAdapter(
  ...
  return omiToolManifest.filter((tool) =>
    isToolAvailableForContext(tool.adapters[adapterId], context)
  )
}
...
export function toolNamesForAdapter(
  ...
  return toolsForAdapter(adapterId, context).map(
    (tool) => tool.adapters[adapterId]?.adapterName ?? tool.name
  )
}
...
export function productManifestEntry(name: string): OmiToolManifestEntry | undefined {
