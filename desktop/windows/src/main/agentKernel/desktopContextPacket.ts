// Desktop context packet builder — Windows port of the macOS agent runtime's
// desktop-context-packet.ts (desktop/macos/agent/src/runtime/desktop-context-packet.ts).
//
// Pure: turns a set of context snippets into the packet row + access-log rows the
// kernel persists, and enforces the two hard context-safety rules on the way:
//  1. Raw screenshot bytes may never be smuggled into a packet (data: URIs and
//     long base64 blobs are rejected).
//  2. A sensitive source (screenshot / rewind timeline / non-low screen_current /
//     any 'sensitive' tier) may only be included when a verified dispatch already
//     authorized it — otherwise the build throws.
//
// The kernel calls this on every main_chat / task_chat / workstream turn that has
// a conversation, so the packet is the scoped context the model is allowed to see.
...
function assertNoScreenshotBytes(snippet: DesktopContextSnippetInput): void {
  const candidates = [
    snippet.content,
    snippet.redactedContent,
    ...(Object.values(snippet.metadata ?? {}) as unknown[])
  ]
  for (const candidate of candidates) {
    if (typeof candidate !== 'string') continue
    if (/^data:image\//i.test(candidate) || /^[A-Za-z0-9+/]{400,}={0,2}$/.test(candidate)) {
      throw new Error(
        `Context snippet ${snippet.snippetId} appears to contain raw screenshot image bytes.`
      )
    }
  }
}
...
export function buildDesktopContextPacket(
  input: DesktopContextPacketBuildInput
  ...
  const tokenEstimate = snippets.reduce(
    (sum, snippet) => sum + snippet.tokenEstimate,
    estimateTokens(input.objective)
  )
