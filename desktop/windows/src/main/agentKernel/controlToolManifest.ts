// Agent-control capability manifest — Windows port of the macOS agent runtime's
// control-tool-manifest.ts (desktop/macos/agent/src/runtime/).
//
// The 18 tools the coordinator advertises, with the prompt copy, capability
// docs, and policy tiers that drive prompts, tool policy, and UI. This module is
// plain data plus one pure schema-shaping helper — no kernel, no store, no
// Electron.
//
// Two coexisting type families on macOS, faithfully preserved here:
//   - the NARROW control-tool-only types in this file (`AgentControlBundle`, 11
//     values; `AgentControlApprovalPolicy`, 3 values), and
//   - the WIDER coordinator-wide types in ./desktopToolPolicy (12 bundles incl.
//     `desktop.permissions.request`; 4 approval policies incl. `deny`), which
//     also cover the non-control "omi" product tools.
// Do not collapse them into one list — `request_permission` (a product tool, not
// a control tool) needs the 12th bundle, and `deny` only exists coordinator-wide.
...
/** Bundle enum advertised to the model on the two policy-evaluation properties. */
const POLICY_BUNDLE_ENUM = [
  'desktop.agent_control.read',
  'desktop.agent_control.manage',
  'desktop.context.local_read',
  'desktop.context.screen_summary',
...
Use a runId returned by list_agent_sessions or a correlated Omi response. Returns the run, session, attempts, adapter bindings, artifact metadata, and optionally events.`,
    ...
    capabilityDoc: controlDoc('Get Agent Run', 'Inspect one canonical Omi agent run.', [
      'Use a runId from list_agent_sessions or a correlated Omi result.',
      'Returns the run, attempts, adapter bindings, events, and artifact metadata.'
    ]),
    latency: 'fast local',
  ...
  },
  {
    ...
    promptSnippet:
      'route_desktop_intent - Decide quick answer, resume, fork, delegate, dispatch, or new run',
    promptGuidelines: [
  ...
  },
  {
    ...
    properties: {
      ...
      selectedBundles: {
        ...
        items: { type: 'string', enum: POLICY_BUNDLE_ENUM }
      },
      requestedBundles: {
        type: 'array',
  ...
  },
  {
    ...
    properties: {
      ...
      recommendedDefault: {
        type: 'string',
        description: 'Optional recommended default decision label.'
      },
      sourceSessionId: {
        type: 'string',
        description: 'Optional source Omi session_id scope guard.'
      },
      sourceRunId: { type: 'string', description: 'Optional source Omi run_id scope guard.' },
      sourceAttemptId: {
        type: 'string',
        description: 'Optional source Omi attempt_id scope guard.'
      },
      sourceArtifactId: {
        type: 'string',
        description: 'Optional source Omi artifact_id scope guard.'
      },
      capability: {
        type: 'string',
        description: 'Capability being requested, e.g. desktop.context.screenshot_image.'
      },
      ...
      payload: {
        type: 'object',
        description: 'Small structured payload.',
        additionalProperties: true
      },
      expiresAtMs: {
        type: 'number',
        description: 'Optional epoch-ms expiration for the decision item.'
      }
    },
  ...
  },
  {
    ...
    properties: {
      ...
      grant: {
        type: 'object',
        description: 'Optional scoped grant to create for an explicit allow resolution.',
        additionalProperties: true
      }
    },
...
This only records artifact metadata state and ordered kernel events. It does not open files, delete files, retain blobs, or read artifact contents.`,
    promptSnippet:
      'update_agent_artifact_lifecycle - Mark an Omi agent artifact retained, dismissed, or opened',
    promptGuidelines: [
    ...
    properties: {
      ...
      sessionId: {
        type: 'string',
        description: 'Optional canonical Omi session_id scope guard.'
      },
      runId: { type: 'string', description: 'Optional canonical Omi run_id scope guard.' },
      attemptId: {
        type: 'string',
        description: 'Optional canonical Omi attempt_id scope guard.'
      },
      ownerId: {
        type: 'string',
        description: 'Owner guard. Defaults to the active signed-in owner.'
      ...
      metadata: {
        type: 'object',
        description: 'Small structured metadata for the lifecycle event.',
        additionalProperties: true
      }
    },
...
Not exposed to agent-facing surfaces.`,
    ...
    properties: {
      ...
      surfaceKind: {
        type: 'string',
        description: 'Optional session surface kind. Default floating_bar.'
      },
      externalRefKind: {
        type: 'string',
        description: 'Optional external reference kind for UI projection.'
      },
      externalRefId: {
        type: 'string',
        description: 'Optional external reference id for UI projection.'
      },
      ownerId: {
        type: 'string',
        description: 'Owner id. Defaults to the active signed-in owner.'
...
] as const satisfies readonly AgentControlManifestTool[]
