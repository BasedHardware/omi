#!/usr/bin/env node
/**
 * Custom ACP entry point that patches ClaudeAcpAgent to support real token
 * usage and USD cost from SDKResultSuccess (via query.next interception).
 *
 * Used instead of the default @zed-industries/claude-agent-acp entry point.
 * Ported from desktop/macos/agent/src/patched-acp-entry.mjs; spawned as a
...
// usage (input_tokens, output_tokens, etc.) but acp-agent.js drops them and
// only returns { stopReason }. We capture them here so that our patched
// prompt() can attach them to the response.
...
ClaudeAcpAgent.prototype.newSession = async function (params) {
  ...
  if (session?.query) {
    ...
    session.query.next = async function (...args) {
      ...
      if (item.value?.type === 'result' && item.value?.subtype === 'success') {
        // total_cost_usd is the CUMULATIVE session cost, not per-turn.
        // We must compute the delta so cost increments are per-turn only.
