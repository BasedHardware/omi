...
type Row = { entry: ModelEntry; status: ModelStatus['state']; bytesOnDisk: number; progress: ModelDownloadProgress | null }
...
export function LocalModelsCard(): React.JSX.Element {
  ...
  const load = useCallback(async (): Promise<void> => {
    const [entries, statuses] = await Promise.all([window.omi.modelsList(), window.omi.modelsStatus()])
    const byId = new Map(statuses.map((s) => [s.entry.id, s]))
    setRows(
      entries.map((entry) => {
        const s = byId.get(entry.id)
        return { entry, status: s?.state ?? 'absent', bytesOnDisk: s?.bytesOnDisk ?? 0, progress: null }
      })
    )
  }, [])
  ...
  useEffect(() => {
    ...
    const off = window.omi.onModelsProgress((p) => {
      setRows((prev) =>
        prev.map((r) => (r.entry.id === p.id ? { ...r, progress: p, status: p.phase === 'done' ? 'installed' : r.status } : r))
      )
    })
    return off
  }, [load])
  ...
  const download = async (id: string): Promise<void> => {
    setRows((prev) => prev.map((r) => (r.entry.id === id ? { ...r, progress: { id, received: 0, total: 0, phase: 'download' } } : r)))
  ...
  return (
    <div className="mt-6">
      ...
      <div className="flex flex-col gap-3">
        {rows.map((r) => {
          const busy = r.progress && r.progress.phase !== 'done' && r.progress.phase !== 'error' && r.progress.phase !== 'cancelled'
          const pct = r.progress ? downloadPct(r.progress.received, r.progress.total || r.entry.sizeBytes) : 0
          return (
            <div key={r.entry.id} className="rounded-xl border border-white/10 bg-white/[0.03] p-3">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <div className="truncate text-sm font-medium text-text-primary">{r.entry.label}</div>
                  ...
                  {deviceRamGb !== undefined && r.entry.minRamGb && deviceRamGb < r.entry.minRamGb && (
                    <div className="mt-0.5 text-xs text-amber-400">
                      This machine has ~{Math.round(deviceRamGb)} GB — below this model’s recommendation.
                    </div>
                  )}
                </div>
                <div className="flex shrink-0 items-center gap-2">
                  {r.status === 'installed' ? (
                    <button onClick={() => void del(r.entry.id)} title="Delete" className="rounded-md bg-white/5 p-2 text-text-tertiary hover:text-white">
                      <Trash2 className="h-4 w-4" />
                    </button>
                  ) : busy ? (
                    <button onClick={() => cancel(r.entry.id)} title="Cancel" className="rounded-md bg-white/5 p-2 text-text-tertiary hover:text-white">
                      <X className="h-4 w-4" />
                    </button>
                  ) : (
                    <button
              ...
              {busy && (
                <div className="mt-2">
                  ...
                  <div className="mt-1 flex items-center gap-1 text-xs text-text-tertiary">
                    ...
                    {r.progress?.phase === 'verify' ? 'Verifying…' : r.progress?.phase === 'cache' ? 'Using local cache…' : `${pct}%`}
                  </div>
                  {r.progress?.phase === 'error' && <div className="mt-1 text-xs text-red-400">{r.progress.error}</div>}
                </div>
              )}
            </div>
          )
