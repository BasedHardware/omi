...
export function SignAvatar({ poseUrl }: SignAvatarProps): React.JSX.Element {
    ...
    async function updatePose(): Promise<void> {
      if (poseUrl) {
        try {
          ...
          console.error('[SignAvatar] updatePose FAILED:', err?.message, '| name:', err?.name);
        ...
        {resolvedUrl ? (
          <pose-viewer
            ...
            onError={(e: React.SyntheticEvent<HTMLElement>) => {
              const detail = (e as unknown as { detail?: { message?: string } }).detail
              const msg = (e as unknown as { message?: string }).message || detail?.message || 'pose-viewer error'
