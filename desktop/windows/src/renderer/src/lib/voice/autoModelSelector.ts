...
// back to GPT-Live-1 (the default lane) only when we have never had a pick. This
...
// Mac's RealtimeOmniProvider rawValues. `gptLive1` is the GPT-Live-1 lane (the
// default); `geminiFlashLive` stays the alternate fallback. The pick is ALWAYS
// one of these concrete ids, never "auto".
export type AutoModelProviderId = 'gptLive1' | 'geminiFlashLive' | 'gptRealtime2'
...
// The no-pick / error default lane: GPT-Live-1 (the shipped default), so a user
// who has never received a pick still connects without a round-trip.
const FALLBACK_PROVIDER_ID: AutoModelProviderId = 'gptLive1'
...
// gptLive1 → 'gpt_live', geminiFlashLive → 'gemini', gptRealtime2 → 'openai' — the
// same mapping macOS RealtimeHubSettings.provider applies to RealtimeOmniProvider.
export function mapProviderIdToVoiceProvider(id: AutoModelProviderId): VoiceProvider {
  if (id === 'gptLive1') return 'gpt_live'
  return id === 'gptRealtime2' ? 'openai' : 'gemini'
}

function isProviderId(raw: unknown): raw is AutoModelProviderId {
  return raw === 'gptLive1' || raw === 'geminiFlashLive' || raw === 'gptRealtime2'
}
 ...
 *  GPT-Live default when we have never had a pick — never clobber a good cache
...
export async function refresh(): Promise<void> {
  try {
    ...
    if (isProviderId(raw)) {
      ...
      store(FALLBACK_PROVIDER_ID)
    }
  } catch {
    if (currentPick() === null) store(FALLBACK_PROVIDER_ID)
  }
}

/** The concrete provider to actually connect with right now — mirrors macOS
 *  RealtimeOmniSettings.effectiveProvider. A concrete setting
 *  ('gpt_live'/'openai'/'gemini') returns itself, bypassing the selector; 'auto'
 *  (the default) resolves via the cached daily pick, falling back to GPT-Live-1
 *  when no pick exists. Always returns a concrete VoiceProvider — never 'auto'. */
export function resolveEffectiveVoiceProvider(): VoiceProvider {
  ...
  if (setting === 'gpt_live' || setting === 'openai' || setting === 'gemini') return setting
  const pick = currentPick()
  return pick ? mapProviderIdToVoiceProvider(pick) : 'gpt_live'
}
