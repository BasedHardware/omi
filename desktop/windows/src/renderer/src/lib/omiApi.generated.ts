...
export async function reject_app_v1_apps__app_id__reject_post(path: { app_id: string }, query: { uid: string }, header: { secret_key: string }, body: AppRejectRequest, init?: OmiApiClientInit): Promise<AppMutationResponse> {
  const _base = init?.baseURL ?? "";
