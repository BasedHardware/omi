...
struct ConversationDetailView: View {
  ...
  var body: some View {
    ...
    .task {
      ...
      if conversation.transcriptSegments.isEmpty {
        ...
        do {
          ...
          if let session = try await TranscriptionStorage.shared.getSessionByBackendId(
            conversation.id)
          {
            let segmentRecords = try await TranscriptionStorage.shared.getSegments(
            ...
            if !segmentRecords.isEmpty {
              ...
              log(
                "ConversationDetail: Loaded \(fullConversation.transcriptSegments.count) segments from API"
              )
            }
          } else {
            ...
            loadedConversation = fullConversation
            log(
              "ConversationDetail: Loaded \(fullConversation.transcriptSegments.count) segments from API (no local session)"
            )
          }
        } catch {
    ...
    .onReceive(
      NotificationCenter.default.publisher(
        for: .desktopAutomationShowConversationTranscriptRequested)
    ) { notification in
        ...
        onSave: { personId, isUser, segmentIndices in
          Task {
            ...
            if success {
              ...
              await updateDisplayedConversation(
                segmentIndices: segmentIndices, isUser: isUser, personId: personId)
            }
  ...
  private var headerView: some View {
    HStack(spacing: 12) {
      ...
      // Emoji
      Text(
        displayConversation.structured.emoji.isEmpty
          ? "\u{1F4AC}" : displayConversation.structured.emoji
      )
      .scaledFont(size: 28)
  ...
  private var inlineActionButtons: some View {
    HStack(spacing: 8) {
      // Public share links are cloud-only.
      ...
      if !folders.isEmpty {
        Menu {
          ...
          Image(systemName: displayConversation.folderId != nil ? "folder.fill" : "folder")
            .scaledFont(size: 14)
            .foregroundColor(
              displayConversation.folderId != nil
                ? OmiColors.purplePrimary : OmiColors.textSecondary
            )
            .frame(width: 28, height: 28)
            .background(
              Circle()
  ...
  private var metadataSection: some View {
    HStack(spacing: 12) {
      ...
      // Category chip
      if !displayConversation.structured.category.isEmpty
        && displayConversation.structured.category != "other"
      {
