...
extension APIClient {
  ...
  func updateAdvice(id: String, isRead: Bool? = nil, isDismissed: Bool? = nil) async throws
    -> ServerAdvice
  {
    struct UpdateRequest: Encodable {
      let is_read: Bool?
...
extension APIClient {
  ...
  func toolGetConversations(
    ...
    if let sd = startDate {
      params +=
        "&start_date=\(sd.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? sd)"
    }
    if let ed = endDate {
      params +=
        "&end_date=\(ed.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? ed)"
    }
  ...
  func toolGetActionItems(
    ...
    if let dsd = dueStartDate {
      params +=
        "&due_start_date=\(dsd.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? dsd)"
    }
    if let ded = dueEndDate {
      params +=
        "&due_end_date=\(ded.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? ded)"
    }
    return try await get(params, customBaseURL: nil)
  }
