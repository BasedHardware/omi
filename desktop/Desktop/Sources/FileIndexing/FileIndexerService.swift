...
actor FileIndexerService {
    ...
    /// Send file summary to Gemini, parse facts, store as memories
    private func analyzeWithGemini(summary: String) async {
        let systemPrompt = """
        You analyze file metadata from a macOS user's computer to extract factual insights about them.
        Output a flat list of factual statements, one per line, prefixed with "- ".
        Each fact must be directly evidenced by the file data.
        Write in third person ("User works with...", "User has projects in...").
        Focus on: profession, technical stack, projects, interests, tools used.
        Under 1500 characters total. Be precise, skip uncertain inferences.
        Do not repeat information. Max 15 facts.
        """

        let prompt = """
        Analyze this file index from a macOS user's computer and extract facts about them.
        ...
        do {
            ...
            let response = try await gemini.sendTextRequest(
                prompt: prompt,
            ...
            guard !facts.isEmpty else {
                log("FileIndexer: Gemini returned no parseable facts")
                return
            }

            log("FileIndexer: Extracted \(facts.count) facts from file analysis")
            ...
            for fact in facts {
                let record = MemoryRecord(
                    ...
                    category: "system",
                    source: "desktop",
                    contextSummary: "Extracted from file index analysis"
                )
                ...
                do {
                    ...
                    Task {
                        do {
                            let backendResponse = try await APIClient.shared.createMemory(
                                content: fact,
                                category: .system,
                                source: "desktop"
                            )
                            try await MemoryStorage.shared.markSynced(
                                id: inserted.id ?? -1,
                                backendId: backendResponse.id
                            )
                        } catch {
                            log("FileIndexer: Backend sync failed for memory: \(error.localizedDescription)")
                        }
                    }
                } catch {
                    log("FileIndexer: Failed to save memory: \(error.localizedDescription)")
                }
            }

            log("FileIndexer: Stored \(facts.count) memories from file analysis")
        } catch {
            log("FileIndexer: Gemini analysis failed: \(error.localizedDescription)")
        }
    }
