...
/// OpenAI-compatible chat completions via the local Codex loopback proxy (ChatGPT subscription tier).
enum CodexLLMClient {
  ...
  enum ClientError: LocalizedError {
    ...
    var errorDescription: String? {
      switch self {
      case .notConfigured:
        return "ChatGPT plan is not active. Sign in with ChatGPT in Settings."
      case .invalidSettings:
        return "Codex proxy settings are invalid."
      case .invalidResponse:
        return "Codex proxy returned an unexpected response."
      case .httpFailure(let status, _):
        return "Codex proxy request failed (HTTP \(status))."
      }
    }
  }
      ...
      if role == "user" {
        ...
        for part in content.parts {
          if let fr = part.functionResponse {
            let toolCallId =
              pendingToolCallIds.isEmpty ? "call_codex_fallback_\(fr.name)" : pendingToolCallIds.removeFirst()
            toolResults.append([
              "role": "tool",
