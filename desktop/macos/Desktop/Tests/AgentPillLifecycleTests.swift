...
final class AgentPillLifecycleTests: XCTestCase {
  ...
  func testRealtimeBargeInUsesProviderInterruptionStrategy() throws {
    ...
    XCTAssertTrue(hubSource.contains("attachGeminiScreenFrameBeforeCommitIfNeeded(session: s)"))
    XCTAssertTrue(hubSource.contains("attached in-turn screen frame before commit"))
    ...
    XCTAssertTrue(hubSource.contains("The current screen frame was already attached to this voice turn before commit"))
