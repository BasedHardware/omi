...
final class DesktopCoordinatorServiceTests: XCTestCase {
  ...
  func testMainChatAddsNonConsumingCoordinatorRouteContextBeforeBridgeQuery() throws {
    ...
    XCTAssertTrue(source.contains(#""ask\s+(an?\s+)?agent\s+to\s+""#))
    XCTAssertTrue(source.contains(#""\b(done|ready|finished|complete|completed|where|open|show|find|saved|file|artifact)\b""#))
