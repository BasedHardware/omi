...
@testable import Omi_Computer
  ...
  final class MemoryExportStatusTests: XCTestCase {
    ...
    func testExistingCodexMCPConfigMarksCodexConnected() async throws {
      ...
      command = "npx"
      args = ["-y", "mcp-remote", "\(MemoryExportDestination.mcpServerURL)", "--header", "Authorization: Bearer ***"]
    ...
    func testClaudeDesktopConfigMarksClaudeNotClaudeCodeConnected() async throws {
      ...
      """.write(
        to: claudeDesktop.appendingPathComponent("claude_desktop_config.json"),
        atomically: true,
        encoding: .utf8
      )
    ...
    func testCommentedHermesMCPConfigDoesNotMarkHermesConnected() async throws {
      ...
      #    command: npx
      #    args: [
      #      "-y", "mcp-remote", "\(MemoryExportDestination.mcpServerURL)",
