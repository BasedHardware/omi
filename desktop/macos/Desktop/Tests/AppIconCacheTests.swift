...
final class AppIconCacheTests: XCTestCase {
  ...
  func testExactNameResolvesSystemApp() {
    let icon = AppIconCache.shared.getIcon(for: "Calculator", size: 24)
  ...
  func testIconIsResizedToTwiceRequestedSize() {
    guard let icon = AppIconCache.shared.getIcon(for: "TextEdit", size: 24) else {
  ...
  func testLowercaseNameResolvesViaCaseInsensitiveScan() {
    let icon = AppIconCache.shared.getIcon(for: "calculator", size: 24)
  ...
  func testSubstringFragmentResolvesViaContainsMatch() {
    let icon = AppIconCache.shared.getIcon(for: "extedi", size: 24)
  ...
  func testUnknownAppReturnsNil() {
    let icon = AppIconCache.shared.getIcon(for: "DefinitelyNotAnInstalledApp2026", size: 24)
    XCTAssertNil(icon)
  }
  ...
  func testCoreServicesAppsResolve() {
    let finder = AppIconCache.shared.getIcon(for: "Finder", size: 24)
    XCTAssertNotNil(finder, "Finder lives in /System/Library/CoreServices")
    let archiveUtility = AppIconCache.shared.getIcon(for: "Archive Utility", size: 24)
  ...
  func testRenamedSystemAppResolvesThroughAlias() {
    let icon = AppIconCache.shared.getIcon(for: "System Preferences", size: 24)
  ...
  func testSecondLookupHitsCache() {
    let first = AppIconCache.shared.getIcon(for: "Calculator", size: 24)
    let second = AppIconCache.shared.getIcon(for: "Calculator", size: 24)
  ...
  func testLogResolutionForCommonNames() {
    let probes = [
    ...
    for name in probes {
      let icon = AppIconCache.shared.getIcon(for: name, size: 24)
      print(
