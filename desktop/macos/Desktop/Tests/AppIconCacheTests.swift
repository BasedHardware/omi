...
final class AppIconCacheTests: XCTestCase {
  ...
  func testExactNameResolvesSystemApp() async {
    let icon = await AppIconCache.shared.getIcon(for: "Calculator", size: 24)
  ...
  func testIconIsResizedToTwiceRequestedSize() async {
    guard let icon = await AppIconCache.shared.getIcon(for: "TextEdit", size: 24) else {
  ...
  func testSubstringFragmentResolvesViaContainsMatch() async {
    let icon = await AppIconCache.shared.getIcon(for: "extedi", size: 24)
  ...
  func testSecondLookupHitsCache() async {
    let first = await AppIconCache.shared.getIcon(for: "Calculator", size: 24)
    let second = await AppIconCache.shared.getIcon(for: "Calculator", size: 24)
