...
final class DataAnswerAssistantTests: XCTestCase {
  private func step(_ json: String) throws -> DataAnswerAssistant.Step {
    try XCTUnwrap(DataAnswerAssistant.decodeStep(json), "undecodable step: \(json)")
  }

  func testPresentAnswerEndsTheSearch() throws {
  ...
  func testPartialFindKeepsItemsAndNamesTheMissing() throws {
    let outcome = DataAnswerAssistant.outcome(
      of: try step(
  ...
  func testAllMissingBecomesNothingWithNames() throws {
    let outcome = DataAnswerAssistant.outcome(
      of: try step(
  ...
  func testEmptyAnswerBecomesNothingFound() throws {
    let outcome = DataAnswerAssistant.outcome(
      of: try step(#"{"action":"present_answer","query":"","title":"Empty","items":[],"missing":[]}"#))
    XCTAssertEqual(outcome, .nothing(reason: "the answer came back empty"))
  }

  func testMissingTitleGetsAFallback() throws {
    let outcome = DataAnswerAssistant.outcome(
      of: try step(
  ...
  func testSearchStepsAreNotTerminal() throws {
    let outcome = DataAnswerAssistant.outcome(
      of: try step(#"{"action":"search_memories","query":"email","title":"","items":[],"missing":[]}"#))
    XCTAssertNil(outcome)
  }

  func testUndecodableStepIsRefused() throws {
  ...
  func testOnlyDeclaredDataToolsMayExplore() throws {
    for allowed in [
...
final class DataAnswerAbsenceGuardTests: XCTestCase {
  ...
  func testALoneApologyIsNotAnAnswer() throws {
    let outcome = DataAnswerAssistant.outcome(
      of: try step(items: [("", "That was not found in your stored memories.")]))
    guard case .nothing = outcome else {
      return XCTFail("an apology must take the panel down, not fill it")
    }
  }

  func testRealValuesStillPresent() throws {
    let outcome = DataAnswerAssistant.outcome(
      of: try step(items: [("Email", "a@b.c"), ("Site", "example.dev")]))
  ...
  func testAValueThatHappensToSoundNegativeIsKept() throws {
    let outcome = DataAnswerAssistant.outcome(
      of: try step(items: [("Diet", "No dairy, no shellfish")]))
    guard case .answer(_, let items, _) = outcome else { return XCTFail("expected an answer") }
  ...
  func testAnApologyBesideARealValueDoesNotDropThePanel() throws {
    let outcome = DataAnswerAssistant.outcome(
      of: try step(items: [("Email", "a@b.c"), ("", "The phone number was not found.")]))
    guard case .answer(_, let items, _) = outcome else { return XCTFail("expected an answer") }
    XCTAssertEqual(items.count, 2)
  }

  func testTheSpokenReasonCarriesWhatTheModelSaid() throws {
    let outcome = DataAnswerAssistant.outcome(
      of: try step(items: [("", "I don't have that in your memories.")]))
