  ...
  private static func isPlaudDevice(advertisementData: [String: Any]) -> Bool {
    ...
    if manufacturerId == DeviceUUIDs.PLAUD.manufacturerId {
      ...
      if manufacturerData.count >= 6 {
        ...
        if payload.count >= 4 && payload[payload.startIndex] == 0x04 && payload[payload.startIndex + 1] == 0x56
          && payload[payload.startIndex + 2] == 0xcf && payload[payload.startIndex + 3] == 0x00
        {
          return true
        }
      }
...
extension BtDevice {
  ...
  static func loadPairedDevice() -> BtDevice? {
    guard let data = UserDefaults.standard.data(forKey: pairedDeviceKey),
      let device = try? JSONDecoder().decode(BtDevice.self, from: data)
    else {
      return nil
    }
    return device
