...
final class BluetoothManager: NSObject, ObservableObject {
  ...
  func triggerPermissionPrompt() {
    ...
    logger.info(
      "Triggering Bluetooth permission prompt (state: \(self.bluetoothStateDescription), auth: \(self.authorizationDescription))"
    )
    log(
      "BLUETOOTH_TRIGGER: state=\(bluetoothStateDescription), stateRaw=\(bluetoothState.rawValue), auth=\(authorizationDescription), authRaw=\(authStatus.rawValue)"
    )
...
extension BluetoothManager: CBCentralManagerDelegate {
  ...
  nonisolated func centralManagerDidUpdateState(_ central: CBCentralManager) {
    Task { @MainActor in
      ...
      self.logger.info(
        "Bluetooth state updated: \(oldStateDesc) -> \(newStateDesc) (raw: \(oldState.rawValue) -> \(central.state.rawValue))"
      )

      // Log detailed state info for debugging
      log(
        "BLUETOOTH_STATE: \(oldStateDesc) -> \(newStateDesc), raw=\(central.state.rawValue), authorization=\(self.authorizationDescription) (\(CBCentralManager.authorization.rawValue))"
      )
  ...
  nonisolated func centralManager(
    ...
    Task { @MainActor in
      ...
      guard
        let resolution = self.connectionLeases.markConnected(
          peripheralID: peripheral.identifier
        )
      else {
  ...
  nonisolated func centralManager(
    ...
    Task { @MainActor in
      self.logger.error(
        "Failed to connect to \(peripheral.name ?? peripheral.identifier.uuidString): \(error?.localizedDescription ?? "unknown error")"
      )

      guard
        let lease = self.connectionLeases.finish(
          peripheralID: peripheral.identifier
        )
      else {
  ...
  nonisolated func centralManager(
    ...
    Task { @MainActor in
      if let error = error {
        self.logger.warning(
          "Disconnected from \(peripheral.name ?? peripheral.identifier.uuidString) with error: \(error.localizedDescription)"
        )
      } else {
