...
package struct OmiButtonStyle: ButtonStyle {
  ...
  /// Disabled is an opacity on the whole control rather than a second palette: a greyed-out fill and
  /// a greyed-out label are two more contrast pairs to keep true, and neither says anything the
  /// dimming does not.
  nonisolated package static let disabledOpacity: Double = 0.45
...
private struct OmiButtonContent: View {
  ...
  var body: some View {
    configuration.label
      ...
      .background(shape.fill(OmiButtonStyle.fill(kind, pressed: pressed)))
      .overlay(shape.strokeBorder(OmiButtonStyle.border(kind), lineWidth: 1))
      .contentShape(shape)
      .opacity(isEnabled ? 1 : OmiButtonStyle.disabledOpacity)
