...
enum AFMJSONSchemaBridge {
  ...
  static func parse(_ schema: LocalInferenceJSONSchema) throws -> AFMJSONSchemaNode {
    ...
    let node = try parseNode(object, name: schema.name, path: schema.name)
  ...
  private static func parseNode(_ raw: [String: Any], name: String, path: String) throws -> AFMJSONSchemaNode {
    ...
    case "object":
      ...
      return try parseObject(raw, name: name, path: path)
    case "array":
      try rejectUnknownKeys(raw, allowed: arrayKeys, path: path)
      return try parseArray(raw, name: name, path: path)
    default:
      throw LocalInferenceError.capabilityUnavailable("unsupported_type:\(type) at \(path)")
    }
  }

  private static func parseObject(_ raw: [String: Any], name: String, path: String) throws -> AFMJSONSchemaNode {
    ...
    for key in propertiesObject.keys.sorted() {
      ...
      let child = try parseNode(childRaw, name: childName, path: "\(path).\(key)")
      properties.append(
        AFMJSONSchemaNode.ObjectProperty(
          name: key,
          node: child,
          isOptional: !requiredSet.contains(key),
          description: childRaw["description"] as? String
        ))
    }
  ...
  private static func parseArray(_ raw: [String: Any], name: String, path: String) throws -> AFMJSONSchemaNode {
    ...
    let item = try parseNode(itemsObject, name: "\(name)_item", path: "\(path).items")
...
indirect enum AFMJSONSchemaNode: Sendable, Equatable {
  ...
  case integer
  case boolean
  case object(name: String, properties: [ObjectProperty])
