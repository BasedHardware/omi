...
enum SpineScreenIndex {
  ...
  /// One day, or an empty day when Rewind has no database yet (capture never enabled, or the pool
  /// is still opening). That is a real state and not an error: the spine simply has no screen rows.
  static func load(day: Date, calendar: Calendar = .current) async -> SpineDayScreen {
    ...
    guard let pool = await RewindDatabase.shared.getDatabaseQueue() else { return .empty }
