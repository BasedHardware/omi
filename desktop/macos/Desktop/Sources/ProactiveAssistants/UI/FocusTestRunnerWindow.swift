...
enum FocusTestRunner {
  ...
  static func runCLITest(lookbackHours: Double, maxScreenshots: Int) async {
      ...
      do {
        ...
        if let result = result {
          ...
          case .focused:
            focusedCount += 1
            log(
              "FocusTestCLI: \(label) \(time) \(screenshot.appName) | \"\(windowTitle)\" → FOCUSED: \(result.description) (\(String(format: "%.1fs", duration)))"
            )
          case .distracted:
            distractedCount += 1
            let msg = result.message.map { " msg=\"\($0)\"" } ?? ""
            log(
              "FocusTestCLI: \(label) \(time) \(screenshot.appName) | \"\(windowTitle)\" → DISTRACTED: \(result.description)\(msg) (\(String(format: "%.1fs", duration)))"
            )
          }
        } else {
          errorCount += 1
          log(
            "FocusTestCLI: \(label) \(time) \(screenshot.appName) | \"\(windowTitle)\" → nil (\(String(format: "%.1fs", duration)))"
          )
        }
      } catch {
        errorCount += 1
    ...
    log(
      "FocusTestCLI: DONE — \(sampled.count) switches (\(skippedCount) skipped, \(analyzed) analyzed), \(focusedCount) focused, \(distractedCount) distracted (\(distractionPct)%), \(errorCount) errors, \(String(format: "%.1fs", totalTime)) total"
    )
  }
}
