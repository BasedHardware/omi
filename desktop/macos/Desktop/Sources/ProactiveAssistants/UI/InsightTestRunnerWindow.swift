    ...
    VStack(spacing: OmiSpacing.lg) {
      HStack {
        VStack(alignment: .leading, spacing: OmiSpacing.xxs) {
          ...
          Text(
            "Replay screenshots through the agentic insight pipeline (activity summary + SQL investigation + insight)"
          )
...
enum InsightTestRunner {
  ...
  static func runCLITest(lookbackHours: Double, maxScreenshots: Int) async {
    ...
    log(
      "InsightTestCLI: Starting test — range \(periodStart) to \(periodEnd), interval \(Int(interval))s, max \(maxScreenshots) windows"
    )
    ...
    for (i, window) in sampled.enumerated() {
      ...
      guard
        let anchor = screenshots.first(where: { ss in
          !ss.appName.isEmpty
        ...
        log(
          "InsightTestCLI: \(label) \(timeFormatter.string(from: window.windowEnd)) — skipped (no non-excluded screenshots)"
        )
        continue
      }
      ...
      do {
        ...
        if let result = result, result.hasInsight, let extractedInsight = result.insight {
          ...
          log(
            "InsightTestCLI: \(label) \(time) \(anchor.appName) | \"\(windowTitle)\" → INSIGHT [\(conf)%] \(extractedInsight.headline ?? "") — \(extractedInsight.insight) (sql=\(sqlCount), \(String(format: "%.1fs", duration)))"
          )
        } else if let result = result, !result.hasInsight {
          noAdviceCount += 1
          log(
            "InsightTestCLI: \(label) \(time) \(anchor.appName) | \"\(windowTitle)\" → no_insight (sql=\(sqlCount), \(String(format: "%.1fs", duration)))"
          )
        } else {
          nilCount += 1
          log(
            "InsightTestCLI: \(label) \(time) \(anchor.appName) | \"\(windowTitle)\" → nil (loop exhausted) (sql=\(sqlCount), \(String(format: "%.1fs", duration)))"
          )
        }
      } catch {
        errorCount += 1
    ...
    log(
      "InsightTestCLI: DONE — \(sampled.count) windows, \(insightCount) advice, \(noAdviceCount) no_advice, \(nilCount) exhausted, \(errorCount) errors, \(totalSql) SQL queries, \(String(format: "%.1fs", totalTime)) total"
    )
  }
}
