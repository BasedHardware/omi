  ...
  private func analyzeAndDeleteDuplicates(tasks: [TaskActionItem], client: GeminiClient) async -> Int {
    ...
    for group in result.duplicateGroups {
      ...
      for deleteId in validDeleteIds {
        ...
        do {
          let deleted: Bool =
            try await TaskLegacyEffectGate.live.perform(
          ...
          deletedCount += 1
          log(
            "TaskDedup: Hard-deleted staged task '\(deletedTask?.description ?? deleteId)' (kept: '\(keptTask?.description ?? group.keepId)') - \(group.reason)"
          )
        } catch {
