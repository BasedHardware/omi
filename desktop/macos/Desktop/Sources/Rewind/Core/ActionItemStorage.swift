...
actor ActionItemStorage {
  ...
  func getLocalActionItems(
    ...
    return try await db.read { database in
      ...
      if let category = category {
        query = query.filter(
          Column("tagsJson").like("%\"\(category)\"%") || Column("category") == category
        )
      }
  ...
  func getFilteredActionItems(
    ...
    return try await db.read { database in
      ...
      if let dateAfter = dateAfter {
        query = query.filter(
          Column("dueAt") >= dateAfter || (Column("dueAt") == nil && Column("createdAt") >= dateAfter)
        )
      }
  ...
  func getFilterCounts() async throws -> (
    ...
    return try await db.read { database in
      ...
      let knownTags = [
        "personal", "work", "feature", "bug", "code", "research", "communication", "finance", "health", "other",
      ]
      ...
      let priorityRows = try Row.fetchAll(
        database,
        sql: """
    ...
    let reconciled = try await authorization.withCommitLease {
      try await db.write { database -> Int in
        ...
        for item in items {
          ...
          let incomingTimestamp = item.updatedAt ?? item.createdAt
          let protectsRecentLocalDueDate =
            Date().timeIntervalSince(record.updatedAt) < 60
  ...
  func insertObservation(
    ...
    log(
      "ActionItemStorage: Inserted observation (id: \(inserted.id ?? -1), app: \(inserted.appName), hasTask: \(inserted.hasTask))"
    )
    return inserted
  }
  ...
  func backfillUnscoredTasks(
    ...
    return try await authorization.withCommitLease {
      try await db.write { database in
        ...
        let unscoredIds = try Int64.fetchAll(
          database,
          sql: """
