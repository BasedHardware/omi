...
class TimeBasedTimelineNSView: NSView {
  ...
  private func needsLayoutRecalculation() -> Bool {
    ...
    return abs(currentRect.width - lastLayoutBounds.width) > 1 || abs(currentRect.height - lastLayoutBounds.height) > 1
  ...
  override func mouseDown(with event: NSEvent) {
    ...
    if let index = indexAtPoint(location) {
      ...
      log(
        "TimelineBar: Click at \(location) not in timeline rect \(rect), bounds=\(bounds), screenshots=\(screenshots.count)"
      )
    }
  }
