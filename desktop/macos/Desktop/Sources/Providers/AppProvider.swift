...
class AppProvider: ObservableObject {
  ...
  func fetchApps() async {
    ...
    do {
      ...
      log(
        "Fetched \(apps.count) apps via v2: \(popularApps.count) featured, \(integrationApps.count) integrations, \(chatApps.count) chat, \(summaryApps.count) summary, \(notificationApps.count) notifications"
      )
  ...
  private func updateCachedEnabledState(appId: String, enabled: Bool) {
    for key in Array(filteredAppsCache.keys) {
      ...
      guard var cached = filteredAppsCache[key],
        let index = cached.apps.firstIndex(where: { $0.id == appId })
      else { continue }
