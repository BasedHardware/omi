...
final class AICloneConfig: ObservableObject {
  ...
  init(defaults: UserDefaults = .standard) {
    ...
    if (try? AICloneKeychain.get(.telegramUserSession)) != nil {
  ...
  func applyDiscovery() {
    ...
    if changed {
      ...
      log(
        "AICloneConfig: auto-discovered plugin at \(discoveryURL) (type=\(discovery.pluginType), devMode=\(discovery.devMode))"
      )
  ...
  func applyUserAccountDiscovery() {
    ...
    self.telegramAccountEnabled = true
    log(
      "AICloneConfig: auto-discovered user-account plugin at \(info.pluginURL) (phone=\(info.phone ?? "?"), name=\(info.name ?? "?"))"
    )
