...
struct AIClonePage: View {
  ...
  private var header: some View {
    VStack(alignment: .leading, spacing: 6) {
      ...
      Text(
        "Replies to people on your behalf in WhatsApp, Telegram, iMessage and more — grounded in your Omi memories, through Beeper Desktop."
      )
      .font(.system(size: 13))
  ...
  private var chatsCard: some View {
    VStack(alignment: .leading, spacing: 10) {
      Text("Chats")
        ...
        .foregroundColor(OmiColors.textPrimary)
      Text(
        "Draft puts the reply in Beeper's compose box. Ask me sends only after you approve. Auto sends by itself when confident — unlocked by a benchmark score of \(service.configuration.autoModeMinimumBenchmarkScore)+."
      )
      .font(.system(size: 12))
      .foregroundColor(OmiColors.textTertiary)
      .fixedSize(horizontal: false, vertical: true)
        ...
        HStack(spacing: 6) {
          ...
          if let result = service.configuration.benchmarkResults[chat.id] {
            Text("matches you \(result.matchScore)%")
              .font(.system(size: 11, weight: .medium))
              .foregroundColor(
                result.matchScore >= service.configuration.autoModeMinimumBenchmarkScore ? .green : OmiColors.warning)
          }
        }
      }
      Spacer()
