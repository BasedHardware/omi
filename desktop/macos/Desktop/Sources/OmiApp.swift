...
class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate, @unchecked Sendable {
  ...
  func applicationDidFinishLaunching(_ notification: Notification) {
    ...
    // Resume the AI Clone listener when the user left it enabled. The reply loop
    // belongs to the service, not to the Settings pane that happens to render
    // it, so a restart must not silently stop replying until that pane is
    // opened. No-op when the clone is off or no Beeper token is stored.
