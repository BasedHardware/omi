...
final class StartupWarmupCoordinator {
    ...
    private func runDatabaseWarmup() async {
        guard await sleepForStartupDelay(remainingStartupDelay(StartupWarmupPolicy.immediateWarmupDelay)) else { return }
        ...
        guard await sleepForStartupDelay(remainingStartupDelay(StartupWarmupPolicy.deferredWarmupDelay)) else { return }
