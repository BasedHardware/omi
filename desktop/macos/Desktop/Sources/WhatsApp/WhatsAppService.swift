...
actor WhatsAppService {
  ...
  static func findWacliBinary() -> String? {
    if let bundled = Bundle.resourceBundle.path(forResource: "wacli", ofType: nil),
  ...
  func pair() async {
    ...
    guard let binary = Self.findWacliBinary() else {
      await setState(.degraded(reason: "wacli not installed"))
      log("WhatsAppService: wacli binary not found")
      return
    }
  ...
  func startFollow() async {
    ...
    guard let binary = Self.findWacliBinary() else {
      await setState(.degraded(reason: "wacli not installed"))
      log("WhatsAppService: wacli binary not found for sync")
      return
    }
