...
struct WhatsAppSettingsSection: View {
      ...
      VStack(alignment: .leading, spacing: 14) {
        VStack(alignment: .leading, spacing: 4) {
          Text("Auto-Send Allowlist")
            .scaledFont(size: 15, weight: .semibold)
            .foregroundColor(OmiColors.textPrimary)
          Text("Auto replies are restricted to these direct-chat JIDs.")
            .scaledFont(size: 12)
            .foregroundColor(OmiColors.textTertiary)
        }

        HStack(spacing: 10) {
          TextField("contact@s.whatsapp.net", text: $allowlistInput)
            .textFieldStyle(.roundedBorder)
          Button("Add") {
            replySettings.addAllowlistedJid(allowlistInput)
            allowlistInput = ""
          }
          .buttonStyle(OnboardingCardButtonStyle(isPrimary: false))
        }
      ...
      VStack(alignment: .leading, spacing: 16) {
        ...
        HStack {
          Text(toneProfile.snapshot?.styleGuide ?? "Tone profile has not been built yet")
            ...
            .lineLimit(3)
          Spacer()
          Button(isRebuildingToneProfile ? "Rebuilding..." : "Rebuild Tone") {
            Task { await rebuildToneProfile() }
          }
          .buttonStyle(OnboardingCardButtonStyle(isPrimary: false))
          .disabled(isRebuildingToneProfile)
        }
      }
    }
  }
        ...
        if entries.isEmpty {
          ...
          ForEach(entries) { entry in
            VStack(alignment: .leading, spacing: 3) {
              Text("\(entry.outcome) - \(entry.chatJid)")
  ...
  private func draftRow(_ draft: WhatsAppDraft) -> some View {
    VStack(alignment: .leading, spacing: 10) {
      Text("To \(draft.senderName ?? draft.senderJid)")
