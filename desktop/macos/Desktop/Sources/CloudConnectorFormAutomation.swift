...
enum CloudConnectorFormAutomation {
  ...
  private typealias AccessibleNode = AXFormNode
  ...
  static func fill(
    ...
    let refreshedNodes = AXFormTree.collectNodes(from: form.root, maxDepth: 12, maxNodes: 700)
    let fields =
      refreshedNodes
    ...
    if submit && missing.isEmpty {
      ...
      let submitNodes = AXFormTree.collectNodes(from: form.root, maxDepth: 12, maxNodes: 700)
  ...
  private static func pressClaudeConnectorAddButtonByOCR(
    ...
    let nodes = focusedAndWindowRoots(for: appElement)
      .flatMap { AXFormTree.collectNodes(from: $0, maxDepth: 10, maxNodes: 500) }
  ...
  private static func findBestForm(
    ...
    for app in candidates where !seen.contains(app.processIdentifier) {
      ...
      for root in roots {
        let nodes = AXFormTree.collectNodes(from: root, maxDepth: 12, maxNodes: 700)
  ...
  private static func focusedAndWindowRoots(for appElement: AXUIElement) -> [AXUIElement] {
    ...
    if let focused = AXFormTree.elementAttribute(appElement, "AXFocusedWindow") {
      roots.append(focused)
    }
    for window in AXFormTree.elementArrayAttribute(appElement, "AXWindows") {
      roots.append(window)
    }
    return roots
  }
  ...
  private static func findClaudeConnectorTargetWithNodes() -> (
    ...
    for app in candidates {
      ...
      for root in focusedAndWindowRoots(for: appElement) {
        let nodes = AXFormTree.collectNodes(from: root, maxDepth: 10, maxNodes: 500)
  ...
  private static func pointIsInVerifiedBrowserContent(
    ...
    let node = AXFormTree.node(from: element)
