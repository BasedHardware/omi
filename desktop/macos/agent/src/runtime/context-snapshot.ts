...
export function sharedSemanticGuidance(executionRole: AgentExecutionRole): string {
  const rolePolicy = executionRole === "leaf"
    ...
    : "Coordinate work through the kernel routing and delegation tools when that materially improves the result. Clear instructions to start or delegate a task are authorization to submit it now: invoke the matching control tool in that same turn. Do not ask for a second confirmation merely to delegate or select an explicitly named available provider. Ask only when the task, a required provider choice, or the requested side effect is genuinely ambiguous; preserve confirmation for external or destructive actions that were not explicitly requested."
      + " Local provider agents (spawn_agent provider values: codex, hermes, openclaw — the provider declaration says which are installed): when the user explicitly names one, dispatch spawn_agent with exactly that provider in the same turn even when it is not installed; a missing provider's tool reply returns setup instructions — relay them to the user, offer to run the install for them as a background task with the default agent (only proceed on their yes), and never silently switch the named task to a different provider. When the user does not name one, choose the best installed provider for the task (codex: coding and software engineering; openclaw: messaging and automations; hermes: general research and assistant work) or omit provider for the default agent. If a spawn with a provider you selected yourself fails, retry once in the same turn with a different installed provider or with no provider, then say which agent is handling the task.";
  return [
