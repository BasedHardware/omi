...
Pass parentRunId to link the new run to a parent.`,
    ...
    promptGuidelines: [
      ...
      "Pass provider='openclaw', provider='hermes', or provider='codex' only when the current user explicitly names that provider. Pass provider='best' when the user asks for the best or any agent (Omi then picks the most capable connected one). Otherwise omit provider so Omi starts its regular managed agent.",
    ...
    properties: {
      ...
      provider: { type: "string", enum: ["openclaw", "hermes", "codex", "best"], description: "Optional local provider override when the current user names one, or 'best' when they ask for the best or any agent; omit for a regular Omi agent." },
