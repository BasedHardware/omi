#!/usr/bin/env python3
"""make-onboarding-sounds.py — synthesizes every audio asset the first-run cinematic ships.
...
audio, so the app can ship them freely. Keeping the generator in-tree is what makes the
committed binaries reproducible and reviewable.
...
Sources/Resources/Sounds/<name>.m4a, with the round-trip verified bit-exact against
the pre-encode WAV before the temp file is discarded. ALAC, not AAC: AAC's encoder
inserts priming/padding samples at the edges of the stream, which would reintroduce
exactly the loop click pad's head/tail crossfade exists to eliminate. Playback decodes
each .m4a into an AVAudioPCMBuffer once (`OmiOnboardingSound.swift`), so the container
costs nothing at loop time — only sample-exactness matters, and only ALAC guarantees
it here.

Run:
    python3 desktop/macos/scripts/make-onboarding-sounds.py

Output (48 kHz, 16-bit PCM, ALAC in .m4a, written to Desktop/Sources/Resources/Sounds/):
    pad.m4a     — looping ambient bed for the first-run cinematic and the chat onboarding
    swoosh.m4a  — transition whoosh (windows swooping in, the prompt stretch)
    click.m4a   — UI click for a dot landing, a keystroke, a beat arriving
    chime.m4a   — completion cue for the final beat
...
# reproducible — re-running this script is a no-op on the decoded audio.
...
def attack_decay_envelope(n_samples, sr, attack_s, decay_tau_s, peak_frac_of_attack=1.0, onset_sample=0):
    ...
    (used to arpeggiate chime.m4a's notes).
    """
...
def ship_alac(name, samples, channels, out_dir, sample_rate=SAMPLE_RATE):
    """Write `samples` to a temp WAV, losslessly encode it to Sounds/<name>.m4a via
    ALAC, verify the round-trip is bit-exact, then delete the temp WAV. Raises if
    afconvert is missing/fails or if the decoded audio does not match exactly — we never
    ship a "probably fine" lossy asset."""
    fd, tmp_wav_path = tempfile.mkstemp(prefix=f"make-onboarding-sounds-{name}-", suffix=".wav")
    os.close(fd)
    ...
    try:
        ...
        fd2, decoded_wav_path = tempfile.mkstemp(prefix=f"make-onboarding-sounds-{name}-decoded-", suffix=".wav")
        os.close(fd2)
        subprocess.run(
...
def make_pad():
    ...
    pseudo-reverb, then a crossfaded loop point so `.loops` on the decoded PCM is
    seamless.

    24s is a deliberate size trade-off: even losslessly encoded, a 60-90s bed is
    too large to commit as generated content. The seam math (crossfade width,
    envelope shape) is unaffected by total length."""
    loop_len_s = 24.0
    crossfade_s = 2.0
    ...
    left = feedback_delay_reverb(
        left, SAMPLE_RATE, delay_ms=390.0, tap_ms=180.0, feedback=0.40, wet=0.27, cutoff_hz=5000.0
    )
...
def make_chime():
    """Completion cue: a short bell-like arpeggio in C major (C5-G5-E5) — consonant
    with the pad's C3/G3/C4/E4 chord — gentle attack, no sustain plateau, tail under
    1.2s."""
    duration_s = 1.15
    n = int(duration_s * SAMPLE_RATE)
    t = np.arange(n) / SAMPLE_RATE
    ...
    for freq, onset_s, pan in notes:
        ...
        env = attack_decay_envelope(
            n, SAMPLE_RATE, attack_s, decay_tau_s, peak_frac_of_attack=1.0, onset_sample=onset_sample
        )
        bell = (
...
def main():
    ...
    # any stray .wav here can never be committed. Sweep it clean on every run.
