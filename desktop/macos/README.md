...
Agent runtime preparation keeps the universal Node binary in a shared cache at `~/Library/Caches/OmiDesktop/node-runtime` and stages it into the worktree with an APFS clone when both are on the same volume. CI builds that binary in the worktree and does not use the shared cache. Set `OMI_AGENT_RUNTIME_FORCE_REBUILD=1` to rebuild and refresh the cache.
