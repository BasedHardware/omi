...
At shortcut release, `VoiceTypeSession` captures the exact Accessibility text element, process, bundle, selection, and value digest before closing transcription. It prefers a verified AX replacement. A verified insertion at a collapsed caret keeps a 30-second, owner-authorized receipt for `Undo Last Dictation`, and invalidates it if the target or value changes or ownership or Accessibility trust is lost. If AX insertion is unavailable, the clipboard fallback rechecks the captured field and posts `Cmd-V` to that same PID; a detected target change degrades to copy. A same-app focus change after the last check but before the paste event is delivered remains a limitation of this fallback; it does not offer Undo.

An offline question remains in an owner-scoped in-memory recovery for five minutes, stays unsent, and is available only while that authorization remains current. Review opens main Chat, where the pending text is appended once the composer hydrates its own draft; Copy remains available without sending. The notch keeps a persistent `SEND` affordance during listening, with a lock icon and `Locked` accessibility state in hands-free mode.
