...
## Dictation, insertion, and recovery

Dictation polish has a finite skip path: only already-clean English utterances from the explicit short acknowledgement and greeting allowlist, with no spelling hints, skip remote polishing; arbitrary prose keeps the model path. The opening `type` check uses one exact-audio decoder entry: an identical opening shares the in-flight decode, while a shorter prefix is never reused for the released turn.

At shortcut release, `VoiceTypeSession` captures the exact Accessibility text element, process, bundle, selection, and value digest before closing transcription. It prefers a verified AX replacement. A verified insertion at a collapsed caret keeps a 30-second, owner-authorized receipt for `Undo Last Dictation`, and revalidates the exact target, value, caret and authorization when Undo executes. Reading menu availability never consumes the receipt; menu actions run after tracking ends. Expiry publishes an availability update. A dispatched AX write whose result cannot be verified is reported as unconfirmed; the app neither overwrites the clipboard nor instructs a blind retry. If AX insertion is unavailable, the clipboard fallback rechecks the captured field and posts `Cmd-V` to that same PID; a detected target change degrades to copy. A same-app focus change after the last check but before the paste event is delivered remains a limitation of this fallback; it reports “Paste requested — check the editor” rather than confirmed insertion and does not offer Undo.

An offline question remains in an owner-scoped in-memory recovery for five minutes, survives ordinary new PTT attempts, stays unsent, and is available only while that authorization remains current. Review opens main Chat, where the pending text is appended once the composer hydrates its own draft; Copy remains available without sending. The notch keeps a persistent `SEND` affordance during listening, with a lock icon and `Locked` accessibility state in hands-free mode.
