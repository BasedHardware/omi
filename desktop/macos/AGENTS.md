...
4. **Automatic beta promotion** (`desktop_promote_beta.yml`) — runs in the unreviewed GitHub `beta` environment, rejects stale automatic targets, honors `DESKTOP_AUTO_BETA_ENABLED=false` as an emergency pause, validates digest-matched evidence, registers the immutable manifest, and atomically advances the explicit beta pointer
...
- Automatic beta is fail-closed: any signed-smoke, digest, static, T2, fault-suite, newest-tag, manifest, or pointer failure leaves the candidate non-live. Set `DESKTOP_AUTO_BETA_ENABLED=false` in Codemagic or the GitHub `beta` environment to pause automatic qualification/promotion without changing stable. Before this workflow can be activated, the `beta` environment needs a dedicated, least-privileged Beta-scoped `GCP_CREDENTIALS` identity and `GCP_PROJECT_ID=based-hardware`; do not copy the protected `prod` credential. Repository-scoped Omi Bot secrets remain available. Keep Stable on the separately protected `prod` environment.
