...
2. **Codemagic** (`codemagic.yaml`, workflow `omi-desktop-swift-release`) — triggered by the tag, runs on Mac mini M4:
   - Resolves one closed, auditable Beta release profile. `nightly-rigorous` runs Dockerless hermetic T2 and its fault suite against a source-built named bundle before any signing/publication; `manual-fast` is a protected manual override that truthfully records the T2 skip. Missing/unknown profiles fail closed.
   - The Dockerless lane uses native Firebase emulators/Java, native Redis, pinned/checksummed native Typesense, the locked native Python backend, and `PROVIDER_MODE=offline`. Python provisioning prefers the repository patch and deterministically falls back to the pinned 3.11 minor with the exact platform lock when uv lacks that standalone patch.
...
3. **Exact-artifact qualification** (`desktop_qualify_beta.yml`) — dispatched only after candidate publication. The administrator-managed M1 Mac Studio is the sole final authority: it sparsely checks out trusted control scripts, downloads the exact canonical ZIP/DMG/app bytes, and runs the fast signed/notarized black-box launch, Keychain, notification, identity, and digest gates. This phase never checks out or builds Swift, Rust, or application source. The removed M4 and Codemagic post-build source-rebuild lanes are not fallbacks.
...
Stable is manual:
- Automatic qualification never promotes Stable. `desktop_promote_prod.yml` remains `workflow_dispatch` only, protected by the `prod` environment, and accepts only digest-bound `nightly-rigorous` evidence with both the pre-sign T2 and trusted-M1 black-box gates passed. `manual-fast` is Beta-only.
