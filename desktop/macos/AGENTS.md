...
The former self-hosted T2 qualification and post-qualification promotion workflows are not part of the release path. Local T2 and fault suites remain available as engineering QA tools, but they do not gate Beta visibility.
...
- Codemagic uploads stable and Beta smoke results with artifact digests and completed checks; the backend compares the Beta result to the exact immutable release assets before changing channels.
