...
- `run.sh` prepends `/opt/homebrew/bin` (Apple Silicon) and `/usr/local/bin` (Intel) when present, because agent and launchd shells may not inherit Homebrew's PATH. This keeps `pkg-config` and other build tools discoverable without requiring a machine-wide shell profile change.
