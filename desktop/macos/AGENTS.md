...
- Automatic beta is fail-closed: any signed-smoke, digest, static, T2, fault-suite, newest-tag, manifest, or pointer failure leaves the candidate non-live. Set `DESKTOP_AUTO_BETA_ENABLED=false` in Codemagic or the GitHub `beta` environment to pause automatic qualification/promotion without changing stable. The `beta` environment needs `GCP_CREDENTIALS` and `GCP_PROJECT_ID=based-hardware`; repository-scoped Omi Bot secrets remain available. Keep Stable on the separately protected `prod` environment.
