...
Beta release candidates are cut **continuously — on every macOS-affecting merge to `main`** (a `push` trigger scoped to the releasable desktop paths, with an hourly `schedule` backstop). Each run plans the newest releasable `desktop/macos/**` (or release-owning root `codemagic.yaml`) change since the last tag, keeping immutable tags bound to the Codemagic configuration that builds them. A candidate advances to beta automatically only after every qualification gate passes:

1. **GitHub Actions** (`desktop_auto_release.yml`) — on every macOS-affecting `push` to `main` (plus the hourly `schedule` backstop and manual `workflow_dispatch`) it batches mainline changes, auto-increments the version, and pushes a `v*-macos` build-candidate tag. A short quiet window (`AUTO_RELEASE_QUIET_SECONDS`) coalesces near-simultaneous merges; the one-active-release fence (Codemagic build status on the latest tag) admits at most one candidate at a time, so bursts collapse to the newest releasable SHA. It fails closed before changelog or tag mutation unless `Release Eligibility`, `Desktop Swift Build & Tests`, and `Desktop Swift Release Compile` all completed successfully for the exact newest queued releasable desktop SHA. Later backend/docs-only commits do not replace that immutable source with a SHA where desktop CI was skipped; the tag includes the newly consolidated release notes.
...
3. **Qualification** (`desktop_qualify_beta.yml`) — dispatched by Codemagic after candidate publication. A Codemagic primary lane plus independent per-machine self-hosted fallback lanes. The `verdict` job **fails open on infra, fails closed on genuine failure**: each lane self-classifies as `qualified` (ran + published evidence), `failed` (the qualification step RAN and returned failure — a genuinely bad build), or `unavailable` (offline / skipped / setup-failed before qualifying). A lane that genuinely `failed` blocks promotion; otherwise a `qualified` lane promotes; otherwise (every lane `unavailable` — machines down/unresponsive/broke before qualifying) the run **still succeeds** so unreachable infra cannot block the release (a single self-hosted machine proved to be a release-blocking single point of failure in the Jul 22 2026 outage — failure class `FC-single-machine-release-gate`). Note: an infra failure that happens *inside* the qualification step (e.g. a mid-run timeout) still classifies as `failed`; only failures before/around the run fail open.
