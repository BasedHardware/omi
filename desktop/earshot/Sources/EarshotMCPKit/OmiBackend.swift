...
public enum OmiKeySource: Sendable {
    ...
    public var label: String {
        switch self {
        case .environment: return "the EARSHOT_OMI_MCP_KEY environment variable"
        case .appSupportFile: return "~/Library/Application Support/Earshot/mcp-key"
...
///
/// `earshot-mcp` is spawned per Claude session and lives for seconds, so OAuth is not an option:
...
enum OmiKeyResolver {
    static let environmentVariable = "EARSHOT_OMI_MCP_KEY"

    /// `~/Library/Application Support/Earshot/mcp-key` — written 0600 by the app after sign-in.
    ...
    private static func fromKeyFile() -> String? {
        ...
        // Read it either way — refusing a readable key would only make Earshot look broken — but say
    ...
    /// The user already has a working key in their Claude config; reusing it means Earshot answers
...
// MARK: - Wire shapes
//
// Only the fields Earshot renders. Everything is optional and every decode is defensive: one
...
public final class OmiBackend: @unchecked Sendable {
    ...
    private func get<T: Decodable & Sendable>(
        ...
        request.setValue("earshot-mcp/1.0", forHTTPHeaderField: "User-Agent")
