...
/// The credential set Earshot keeps between launches.
///
...
/// Earshot's own login-Keychain item — the only secret this app stores.
///
...
/// it gets the login-keychain password sheet ("Earshot wants to use your confidential information"),
/// which is both alarming and unanswerable. A separate service name means Earshot only ever touches
...
enum SessionStore {
    static let service = "com.omi.earshot.session"
    ...
    static func load() -> OmiSession? {
        ...
        case errSecSuccess:
            ...
            guard let session = try? JSONDecoder().decode(OmiSession.self, from: data) else {
                ...
                EarshotLog.error("Stored session is unreadable; discarding it", "auth")
                clear()
                return nil
            }
            return session
        case errSecItemNotFound:
            return nil
        default:
            EarshotLog.error("Keychain read unavailable (OSStatus \(status))", "auth")
            return nil
        }
    }
    ...
    static func save(_ session: OmiSession) -> Bool {
        guard let data = try? JSONEncoder().encode(session) else {
            EarshotLog.error("Could not encode session for storage", "auth")
            return false
        }
        ...
        if updateStatus != errSecItemNotFound {
            EarshotLog.error("Keychain update failed (OSStatus \(updateStatus))", "auth")
            return false
        }
        ...
        addQuery[kSecAttrLabel as String] = "Omi Earshot"
        ...
        if addStatus == errSecDuplicateItem {
            ...
            EarshotLog.error("Keychain add/update race failed (OSStatus \(retry))", "auth")
            return false
        }
        EarshotLog.error("Keychain add failed (OSStatus \(addStatus))", "auth")
        return false
    }
    ...
    static func clear() {
        ...
        if status != errSecSuccess, status != errSecItemNotFound {
            EarshotLog.error("Keychain delete failed (OSStatus \(status))", "auth")
        }
    }
