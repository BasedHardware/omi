...
enum OmiAuthError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .missingAPIKey:
            return "This build of Earshot has no Firebase API key, so it cannot finish signing in."
        case .cancelled:
            return "Sign-in didn't finish."
        case .stateMismatch:
            return "That sign-in response didn't match this request, so Earshot ignored it. Try again."
...
/// Signs Earshot in to the user's real Omi account and keeps a Firebase ID token available.
///
/// No Firebase SDK. Earshot is a small menu-bar app and the SDK would drag in a method-swizzling
...
final class OmiAuth: ObservableObject {
    ...
    func restore() {
        guard let stored = SessionStore.load() else {
            EarshotLog.info("No stored Omi session", "auth")
            return
        }
    ...
    func signIn(provider: OmiAuthProvider) async throws {
        guard !isSigningIn else {
            EarshotLog.info("Sign-in already in progress; ignoring duplicate request", "auth")
            return
        }
        isSigningIn = true
        ...
        do {
            ...
            throw OmiAuthError.badResponse("Earshot couldn't open a local port to receive the sign-in response.")
        }
        ...
        guard
            ...
            throw OmiAuthError.badResponse("Earshot couldn't build the sign-in URL.")
        }

        EarshotLog.info("Opening \(provider.rawValue) sign-in; awaiting callback on 127.0.0.1:\(server.port)", "auth")
        ...
        guard SessionStore.save(stored) else {
            throw OmiAuthError.badResponse(
                "Signed in, but Earshot couldn't store the session in your Keychain. Try again.")
        }
        adopt(stored, email: exchange.email ?? Self.claim("email", in: tokens.idToken))
        EarshotLog.info("Signed in as \(stored.tokenUserId) via \(provider.rawValue)", "auth")
    }
    ...
    func signOut() {
        ...
        isSignedIn = false
        SessionStore.clear()
        EarshotLog.info("Signed out", "auth")
    }
    ...
    private func performRefresh(refreshToken: String) async throws -> String {
        ...
        guard let url = URL(string: "https://securetoken.googleapis.com/v1/token?key=\(apiKey)") else {
            throw OmiAuthError.badResponse("Earshot couldn't build the token refresh URL.")
        }
        ...
        guard response.statusCode == 200 else {
            ...
            throw OmiAuthError.http(response.statusCode, "Earshot couldn't refresh your Omi session.")
        }
        ...
        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
            ...
            throw OmiAuthError.badResponse("Earshot couldn't read the refreshed Omi session.")
        }
        ...
        if !SessionStore.save(refreshed) {
            ...
            EarshotLog.error("Refreshed session couldn't be stored; keeping it in memory for this launch", "auth")
        }
        adopt(refreshed, email: email)
        EarshotLog.info("Refreshed Omi session for \(ownerId)", "auth")
        return idToken
    }
    ...
    private func exchangeAuthorizationCode(
        ...
        guard let url = URL(string: Self.apiBaseURL + "v1/auth/token") else {
            throw OmiAuthError.badResponse("Earshot couldn't build the token exchange URL.")
        }
        let body = Self.formEncoded([
            ...
            // Earshot mint a real Firebase ID token without linking the SDK.
    ...
    private func signInWithCustomToken(_ customToken: String, apiKey: String) async throws -> FirebaseTokens {
        guard
            ...
            throw OmiAuthError.badResponse("Earshot couldn't build the Firebase sign-in URL.")
        }
    ...
    private func post(url: URL, contentType: String, body: Data) async throws -> (Data, HTTPURLResponse) {
        ...
        guard let http = response as? HTTPURLResponse else {
            throw OmiAuthError.badResponse("The server sent a response Earshot couldn't read.")
        }
        return (data, http)
    }
    ...
    private static func firebaseAPIKey() throws -> String {
        ...
        EarshotLog.error("No Firebase API key in the environment or Info.plist", "auth")
    ...
    private static func randomBytes(_ count: Int) -> [UInt8] {
        ...
        guard SecRandomCopyBytes(kSecRandomDefault, count, &bytes) == errSecSuccess else {
            ...
            EarshotLog.error("SecRandomCopyBytes failed; falling back to the system generator", "auth")
    ...
    private static func authError(from error: Error) -> OmiAuthError {
        ...
        switch serverError {
        case .stateMismatch:
            EarshotLog.error("Callback state did not match; refusing to exchange the code", "auth")
            return .stateMismatch
        case .provider(let reason):
            return .badResponse("The sign-in provider returned an error: \(reason)")
        case .timedOut:
            EarshotLog.info("Sign-in callback timed out; closed the listener", "auth")
            return .cancelled
        case .stopped:
            return .cancelled
        case .socketCreationFailed, .bindFailed, .listenFailed, .portLookupFailed:
            return .badResponse("Earshot couldn't listen for the sign-in response.")
        }
    }
