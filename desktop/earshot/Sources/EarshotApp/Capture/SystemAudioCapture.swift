...
final class SystemAudioCapture: AudioSource, @unchecked Sendable {
    ...
    private let audioQueue = DispatchQueue(label: "com.omi.earshot.systemaudio")
        ...
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
            audioQueue.async { [weak self] in
                ...
                guard !self.capturing else {
                    EarshotLog.info("Already capturing system audio", Self.logCategory)
                    continuation.resume()
                    return
                }
    ...
    func stop() {
        audioQueue.async { // strong self: the object must outlive its own HAL teardown
            ...
            self.chunkHandler = nil
            self.levelHandler = nil
            EarshotLog.info("Stopped capturing system audio", Self.logCategory)
        }
    }
    ...
    private func startOnQueue() throws {
        ...
        tapDescription.name = "Earshot System Audio Tap"
        ...
        EarshotLog.info("Created process tap \(tapID)", Self.logCategory)
        ...
        let aggregateDescription: [String: Any] = [
            kAudioAggregateDeviceNameKey as String: "Earshot System Audio Tap Device",
        ...
        EarshotLog.info("Created aggregate device \(aggregateDeviceID)", Self.logCategory)
        ...
        capturing = true
        EarshotLog.info("Started capturing system audio", Self.logCategory)
    }
    ...
    private func handleAudioInput(_ inputData: UnsafePointer<AudioBufferList>?) {
        ...
        if let error {
            EarshotLog.error("Conversion failed: \(error.localizedDescription)", Self.logCategory)
            return
        }
    ...
    static func primePermission() -> Bool {
        ...
        let tapDescription = CATapDescription(stereoGlobalTapButExcludeProcesses: [])
        tapDescription.uuid = tapUUID
        tapDescription.name = "Earshot System Audio Tap (permission prime)"
        tapDescription.muteBehavior = .unmuted
        ...
        teardown()
        EarshotLog.info("primePermission complete, tap and aggregate device destroyed", logCategory)
        return true
    }
}
