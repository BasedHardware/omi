...
final class ScreenWatcher {
    ...
    func start(interval: TimeInterval = 3.0) {
        ...
        if !Self.hasPermission() {
            EarshotLog.info("Screen Recording not granted yet — watcher will idle until it is", "screen")
        }
        ...
        EarshotLog.info("Screen watcher started (every \(period)s)", "screen")
    }
    ...
    func stop() {
        ...
        cachedContent = nil
        cachedContentAt = nil
        EarshotLog.info("Screen watcher stopped", "screen")
    }
    ...
    private func tick() async {
        ...
        let capturedAt = EarshotTime.now
    ...
    private func noteSkip(_ reason: String) {
        ...
        EarshotLog.info("Skipping capture: \(reason)", "screen")
    }
    ...
    private func noteError(_ message: String) {
        ...
        EarshotLog.error(message, "screen")
    }
}
...
private enum ScreenPipeline {
    ...
    static func recognizeText(in image: CGImage) -> String? {
        ...
        do {
            ...
            EarshotLog.error("OCR failed: \(error.localizedDescription)", "screen")
            return nil
        }
        if let failure = sink.failure {
            EarshotLog.error("OCR failed: \(failure.localizedDescription)", "screen")
            return nil
        }
    ...
    static func writeJPEG(_ image: CGImage, capturedAt: Double) -> String? {
        ...
        guard let data = jpegData(from: sized) else {
            EarshotLog.error("Failed to encode frame JPEG", "screen")
            return nil
        }

        let directory = EarshotPaths.framesDirectory(for: capturedAt)
        ...
        do {
            ...
            EarshotLog.error("Failed to write frame JPEG: \(error.localizedDescription)", "screen")
            return nil
        }
    }
