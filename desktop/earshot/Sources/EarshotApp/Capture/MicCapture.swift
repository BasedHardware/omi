...
final class MicCapture: AudioSource, @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.omi.earshot.mic.device")
    private let listenerQueue = DispatchQueue(label: "com.omi.earshot.mic.listener")
        ...
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
            queue.async { [weak self] in
                ...
                guard !self.isCapturing else {
                    EarshotLog.info("start ignored, already capturing", "mic")
                    continuation.resume()
                    return
                }
    ...
    private func resolveInputDeviceID() throws -> AudioDeviceID {
        if let fallback = fallbackDeviceID {
            ...
            EarshotLog.info("fallback input device went away, using system default", "mic")
            fallbackDeviceID = nil
        }
    ...
    private func handleAudioInput(_ inputData: UnsafePointer<AudioBufferList>?) {
        ...
        if let error {
            EarshotLog.error("conversion failed: \(error.localizedDescription)", "mic")
            return
        }
    ...
    private func checkSilentMicWindow() {
        ...
        queue.async { [weak self] in
            ...
            if wasBluetooth, let builtIn = Self.findBuiltInMicDeviceID() {
                ...
                EarshotLog.info("falling back to the built-in mic", "mic")
            }
    ...
    private func retryOrGiveUp(retryCount: Int, reason: String) {
        guard retryCount < Self.maxRetries else {
            EarshotLog.error("gave up rebuilding after \(retryCount + 1) attempts: \(reason)", "mic")
        ...
        EarshotLog.info("rebuild attempt \(retryCount + 1) failed (\(reason)), retrying in \(delay)s", "mic")
