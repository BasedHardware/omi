...
/// `/Applications/Earshot.app` rather than running in place.
enum LoginItem {
    ...
    static func enable() -> Bool {
        do {
            ...
            if SMAppService.mainApp.status == .requiresApproval {
                ...
                EarshotLog.info("Launch at login registered, waiting on approval in System Settings", logCategory)
            } else {
                EarshotLog.info("Launch at login enabled", logCategory)
            }
            return true
        } catch {
            EarshotLog.error("Could not enable launch at login: \(error.localizedDescription)", logCategory)
            return false
        }
    }
    ...
    static func disable() {
        do {
            ...
            EarshotLog.info("Launch at login disabled", logCategory)
        } catch {
            EarshotLog.error("Could not disable launch at login: \(error.localizedDescription)", logCategory)
        }
    }
}
