...
/// Points Claude Code and Claude Desktop at the bundled `earshot-mcp` server.
///
...
enum ClaudeRegistrar {
    ...
    private static let binaryName = "earshot-mcp"

    /// Absolute path to the MCP server Claude should launch.
    ///
    /// The canonical answer is `Earshot.app/Contents/MacOS/earshot-mcp`, which is what a signed
    ...
    static func register() -> Result {
        ...
        guard FileManager.default.isExecutableFile(atPath: binary) else {
            ...
            return Result(
                ...
                message: "I couldn't find my MCP server at \(displayPath(binary)), so I haven't changed anything. "
                    + "Reinstall Earshot and connect again.")
        }
        ...
        EarshotLog.info(message, logCategory)
        return Result(
    ...
    /// The exact inverse, for when the user wants Earshot out of Claude again. Removes the entry
    ...
    private static func modify(_ surface: Surface, _ transform: ([String: Any]) -> [String: Any]?) -> Outcome {
        ...
        do {
            ...
            EarshotLog.info("Rewrote \(ClaudeConfig.serverName) in \(displayPath(url))", logCategory)
            return .changed
        } catch {
