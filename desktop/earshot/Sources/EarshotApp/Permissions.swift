...
/// The three things Earshot has to be allowed to do. Nothing else is ever asked for.
...
enum Permissions {
    ...
    static func openSettings(for c: Capability) {
        ...
        let open = {
            ...
            EarshotLog.info("Opened the \(c.rawValue) pane in System Settings", "permissions")
        }
    ...
    /// made while Earshot is running applies to the *next* Earshot, never this one.
    ...
    static func relaunchApp() -> Never {
        ...
        NSWorkspace.shared.openApplication(at: url, configuration: configuration) { app, error in
            if let error {
                EarshotLog.error("Relaunch failed: \(error.localizedDescription)", "permissions")
            } else {
                EarshotLog.info("Relaunched \(url.lastPathComponent) as pid \(app?.processIdentifier ?? -1)", "permissions")
            }
            launch.signal()
        }
    ...
    private static func requestMicrophone() async -> Bool {
        ...
        case .notDetermined:
            ...
            EarshotLog.info("Microphone prompt answered: \(granted ? "granted" : "denied")", "permissions")
            return granted
        default:
    ...
    private static func requestScreen() async -> Bool {
        ...
        if screenNeedsRelaunch {
            EarshotLog.info("Screen recording granted; capture stays dead until Earshot is reopened", "permissions")
        }
    ...
    private static func cachedSystemAudioGrant() -> Bool {
        ...
        if !cached, EarshotTime.now - defaults.double(forKey: Key.systemAudioProbedAt) > systemAudioProbeInterval {
    ...
    private static func scheduleSystemAudioProbe() {
        ...
        defaults.set(EarshotTime.now, forKey: Key.systemAudioProbedAt)
    ...
    private static func recordSystemAudio(_ granted: Bool) {
        ...
        if previous != granted {
            EarshotLog.info("System audio consent is now \(granted ? "granted" : "denied")", "permissions")
        }
    }
    ...
    private enum Key {
        static let systemAudioGranted = "earshot.permission.systemAudio.granted"
        static let systemAudioProbedAt = "earshot.permission.systemAudio.probedAt"
        static let screenPendingRelaunch = "earshot.permission.screen.pendingRelaunch"

        static func prompted(_ c: Capability) -> String {
            "earshot.permission.\(c.rawValue).prompted"
        }
    }
