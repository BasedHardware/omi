...
///
/// **Why `POST v1/conversations/from-segments` and not the `/v4/listen` websocket.** Earshot has
...
final class ConversationUploader: ObservableObject {
    ...
    private init() {
        ...
        OmiAuth.shared.$isSignedIn
            ...
            .filter { $0 }
            .sink { [weak self] _ in
                EarshotLog.info("Signed in; draining the upload queue", ConversationUploader.category)
    ...
    func enqueue(sessionId: Int64) {
        Task { [weak self] in
            ...
            do {
                ...
                EarshotLog.info("Session \(sessionId) queued for upload", Self.category)
            } catch {
    ...
    func drain() async {
        ...
        while !Task.isCancelled {
            ...
            guard OmiAuth.shared.isSignedIn else {
                EarshotLog.info("Signed out mid-drain; the rest stays queued", Self.category)
                break
            }
    ...
    private func handle(_ error: Error, sessionId: Int64, attempts: Int) async -> Outcome {
        ...
        case .notSignedIn:
            ...
            EarshotLog.info("Deferred session \(sessionId): not signed in", Self.category)
            return .stopped
        ...
        case .http(let status, let detail):
            ...
            case 401, 403:
                ...
                EarshotLog.error("Deferred session \(sessionId): HTTP \(status)", Self.category)
                return .stopped
        ...
        do {
            try await database.perform {
                try UploadQueue.markFailed(
                    $0, sessionId: sessionId, error: reason, retryAt: EarshotTime.now + seconds)
            }
        } catch {
            EarshotLog.error(
    ...
    private func postpone(_ sessionId: Int64, seconds: Double, reason: String) async {
        try? await database.perform {
            try UploadQueue.postpone(
                $0, sessionId: sessionId, until: EarshotTime.now + seconds, reason: reason)
        }
    }

    private func skip(_ sessionId: Int64, reason: String) async {
        EarshotLog.info("Skipping session \(sessionId): \(reason)", Self.category)
    ...
    private func nextDue(excluding blocked: Set<Int64>) async -> UploadQueue.Entry? {
        do {
            let due = try await database.perform {
                try UploadQueue.pending($0, dueAt: EarshotTime.now, limit: 20)
            }
            return due.first { !blocked.contains($0.sessionId) }
        } catch {
            EarshotLog.error("Could not read the upload queue: \(error.localizedDescription)", Self.category)
            return nil
        }
    }
    ...
    private func reconcile() async {
        do {
            ...
            if added > 0 {
                EarshotLog.info("Queued \(added) closed session(s) found on disk", Self.category)
            }
        } catch {
            EarshotLog.error("Reconcile failed: \(error.localizedDescription)", Self.category)
        }
    }
    ...
    private func stamp(_ key: UploadQueue.MetaKey, at value: Double = EarshotTime.now) async {
    ...
    private func throttleWait() async -> Double {
        ...
        return max(0, min(Self.throttleInterval, last + Self.throttleInterval - EarshotTime.now))
    }
    ...
    private func scheduleNextWake() async {
        ...
        let delay = dueAt - EarshotTime.now
...
private enum UploadPayload {
            ...
            for line in chunk {
                ...
                segments.append(
                    UploadSegmentBody(
                        ...
                        // attribution the rest of Earshot makes, in the shape the API expects.
    ...
    /// - the session's start, so that deleting `earshot.db` and starting over — which resets the
    ...
    static func clientSessionId(
        ...
        let base = "earshot-\(deviceIdHash)-s\(sessionId)-\(stamp)"
...
///
/// A second `EarshotStore` rather than a share of the engine's: the engine keeps its writer inside
...
private final class UploadDatabase: @unchecked Sendable {
    private let queue = DispatchQueue(label: "com.omi.earshot.upload", qos: .utility)
    private var store: EarshotStore?

    /// Runs `body` against the queue's store, off the main thread.
    func perform<T: Sendable>(_ body: @escaping @Sendable (EarshotStore) throws -> T) async throws -> T {
    ...
    private func opened() throws -> EarshotStore {
        if let store { return store }
        let opened = try EarshotStore()
        try UploadQueue.prepare(opened)
        store = opened
        EarshotLog.info("Upload queue ready at \(opened.databaseURL.lastPathComponent)", "upload")
        return opened
    }
}
