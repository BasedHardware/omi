...
final class ScreenActivityUploader: ObservableObject {
    ...
    private static let cursorKey = "earshot.screenSync.lastId"
    private static let installIdKey = "earshot.screenSync.installId"
    ...
    func start() {
        ...
        EarshotLog.info("Screen sync started (cursor=\(lastSyncedId))", Self.category)
        ...
        loop = Task { [weak self] in
            while !Task.isCancelled {
                ...
                if self.isTrialExpired {
                    self.loop = nil
                    EarshotLog.info("Screen sync loop stopped: desktop trial expired", Self.category)
                    return
                }
    ...
    func stop() {
        ...
        loop?.cancel()
        loop = nil
        EarshotLog.info("Screen sync stopped (cursor=\(lastSyncedId))", Self.category)
    }
    ...
    private func drain() async {
        ...
        guard OmiAuth.shared.isSignedIn else {
            if !didLogSignedOut {
                ...
                EarshotLog.info("Signed out — screen frames stay local until sign-in", Self.category)
            }
            return
        }
        ...
        for _ in 0..<Self.maxBatchesPerTick {
            ...
            case .accepted(let synced):
                ...
                lastError = nil
                lastSyncedAt = EarshotTime.now
                if consecutiveFailures > 0 {
                    EarshotLog.info("Screen sync recovered after \(consecutiveFailures) failures", Self.category)
                }
    ...
    private func noteCaughtUp() {
        ...
        if hadPendingWork {
            ...
            EarshotLog.info("Screen sync caught up (cursor=\(lastSyncedId))", Self.category)
        }
    }
    ...
    private func loadPending() async throws -> [PendingRow] {
        ...
        let (opened, rows) = try await Task.detached(priority: .utility) {
            () async throws -> (EarshotStore, [PendingRow]) in
            let store: EarshotStore
            if let existing {
                store = existing
            } else {
                store = try EarshotStore(readOnly: true)
            }
    ...
    private nonisolated static func fetch(
        _ store: EarshotStore, after cursor: Int64, limit: Int
    ) throws -> [PendingRow] {
        let sql = """
