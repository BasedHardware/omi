...
/// The only way Earshot talks to the Omi backend.
///
/// One shared instance, one shared `URLSession`, one header set, one retry policy. The retry policy
/// is the reason this is centralised: Earshot runs all day, so a client that reacts to a 429 by
...
final class OmiAPI: @unchecked Sendable {
    ...
    static func resolveBaseURL(_ override: String?) -> URL {
        ...
        guard
            ...
            EarshotLog.error("Ignoring unusable OMI_PYTHON_API_URL; using api.omi.me", logCategory)
    ...
    /// sent, kept identical so Earshot's traffic is legible in the same dashboards.
    ...
    private func authorization(forceRefresh: Bool) async throws -> String {
        ...
        do {
            ...
            EarshotLog.error("Token refresh failed", Self.logCategory)
    ...
    private func perform(
        ...
        do {
            ...
            guard let response = urlResponse as? HTTPURLResponse else {
                EarshotLog.error("\(method) \(path) → non-HTTP response", Self.logCategory)
            ...
            EarshotLog.error("\(method) \(path) failed: \(reason)", Self.logCategory)
    ...
    /// half random). The jitter matters because Earshot's callers are timer-driven — without it,
    ...
    static func normalized(_ path: String) -> String {
        ...
        EarshotLog.error("Path \"\(path)\" has a leading slash; paths are relative to the base URL", logCategory)
    ...
    private func decode<T: Decodable>(_ type: T.Type, from data: Data, path: String) throws -> T {
        do {
            ...
            EarshotLog.error("\(path) → could not decode \(type)", Self.logCategory)
    ...
    private static let fallbackDeviceIdKey = "earshot.device-id"
    ...
    private static func makeDeviceIdHash() -> String {
        ...
        EarshotLog.error("No IOPlatformUUID; using the persisted device id instead", logCategory)
