...
/// state the menu bar and `earshot-mcp` read.
///
...
final class Engine: ObservableObject {
    ...
    /// Wall-clock seconds of today that Earshot actually covered. The one number in the menu bar.
    ...
    private let heartbeatQueue = DispatchQueue(label: "com.omi.earshot.heartbeat", qos: .utility)
    ...
    func start() {
        ...
        observeTermination()
        EarshotLog.info("Engine started", "engine")
    }
    ...
    func pause() {
        ...
        EarshotLog.info("Capture paused", "engine")
    ...
    func resume() {
        ...
        EarshotLog.info("Capture resumed", "engine")
        publishState()
    }
    ...
    private nonisolated static func writeHeartbeat(_ state: CaptureState) {
        do {
            ...
            EarshotLog.error("Heartbeat write failed: \(error.localizedDescription)", "engine")
        }
    }
    ...
    private func handleTermination() {
        ...
        EarshotLog.info("Engine stopped", "engine")
    }
}

// MARK: - Store writer

/// Owns the one writable `EarshotStore`, the open session, and nothing else.
///
...
private final class EngineStore: @unchecked Sendable {
    private let queue = DispatchQueue(label: "com.omi.earshot.store", qos: .utility)
        ...
        await withCheckedContinuation { (continuation: CheckedContinuation<String?, Never>) in
            queue.async {
                ...
                do {
                    ...
                    EarshotLog.info("Store opened at \(opened.databaseURL.path)", "store")
    ...
    func record(_ line: TranscriptLine, appHint: String?) {
        queue.async {
            ...
            do {
                ...
                EarshotLog.error("Dropped a transcript line: \(error.localizedDescription)", "store")
            }
        }
    }
    ...
    func record(_ frame: Frame) {
        queue.async {
            ...
            do {
                ...
                EarshotLog.error("Dropped a screen frame: \(error.localizedDescription)", "store")
            }
        }
    }
    ...
    func pruneFrames(olderThanDays days: Int) {
        queue.async {
            ...
            do {
                ...
                if removed > 0 {
                    EarshotLog.info("Pruned \(removed) frames older than \(days) days", "store")
                }
            } catch {
                EarshotLog.error("Frame retention sweep failed: \(error.localizedDescription)", "store")
            }
        }
    }

    /// Wall-clock seconds of today covered by a session. Sessions never overlap, so summing their
    /// clipped spans is the honest answer to "how much of today did Earshot actually see".
    ...
    private func closeSessionsLeftOpen(_ store: EarshotStore) {
