...
///
/// This lives in `EarshotCore` rather than in the app because the state belongs beside the sessions
...
public enum UploadQueue {
    ...
    public static func prepare(_ store: EarshotStore) throws {
    ...
    public static func markPending(_ store: EarshotStore, sessionId: Int64) throws {
    ...
    public static func markPartUploaded(
        _ store: EarshotStore, sessionId: Int64, partsDone: Int, conversationId: String
    ) throws {
        let now = EarshotTime.now
        try store.write { db in
    ...
    public static func markUploaded(
        _ store: EarshotStore, sessionId: Int64, conversationIds: [String]
    ) throws {
        let now = EarshotTime.now
    ...
    public static func markFailed(
        _ store: EarshotStore, sessionId: Int64, error: String, retryAt: Double
    ) throws {
        let now = EarshotTime.now
        try store.write { db in
            try db.execute(
    ...
    public static func postpone(
        _ store: EarshotStore, sessionId: Int64, until: Double, reason: String
    ) throws {
        let now = EarshotTime.now
        try store.write { db in
            try db.execute(
    ...
    public static func markSkipped(_ store: EarshotStore, sessionId: Int64, reason: String) throws {
        let now = EarshotTime.now
        try store.write { db in
            try db.execute(
                sql: """
    ...
    public static func pending(
        _ store: EarshotStore, dueAt: Double = EarshotTime.now, limit: Int = 50
    ) throws -> [Entry] {
    ...
    public static func pendingCount(_ store: EarshotStore) throws -> Int {
        try store.read { db in
    ...
    public static func nextDueAt(_ store: EarshotStore) throws -> Double? {
        try store.read { db in
    ...
    public static func entry(_ store: EarshotStore, sessionId: Int64) throws -> Entry? {
        try store.read { db in
            guard try db.tableExists("uploads") else { return nil }
            guard
    ...
    /// conversation over.
    public static func content(_ store: EarshotStore, sessionId: Int64) throws -> SessionUpload? {
        try store.read { db in
            guard
    ...
    @discardableResult
    public static func reconcile(_ store: EarshotStore, limit: Int = 100) throws -> Int {
        let now = EarshotTime.now
    ...
    public static func timestamp(_ store: EarshotStore, _ key: MetaKey) throws -> Double? {
        try store.read { db in
    ...
    public static func setTimestamp(_ store: EarshotStore, _ key: MetaKey, _ value: Double) throws {
        try store.write { db in
            try db.execute(
                sql: "INSERT OR REPLACE INTO upload_meta (key, value) VALUES (?, ?)",
