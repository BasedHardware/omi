...
/// Registration of the `earshot` stdio MCP server into Claude Code and Claude Desktop.
///
...
/// keys we know nothing about. Earshot is a guest adding one key to one dictionary inside it. So
...
public enum ClaudeConfig {
    ...
    /// `~/.claude.json`
    ///
    /// Resolved from the home directory rather than a container-aware API because Earshot is not
    ...
    /// Removes only the `earshot` entry. Every other server, and every other key, is untouched.
    ...
    public static func writeDocument(_ document: [String: Any], to url: URL) throws {
        ...
        let tempURL = directory.appendingPathComponent(
            ".\(url.lastPathComponent).earshot-\(UUID().uuidString).tmp"
        )
...
public enum ClaudeConfigError: LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .malformed(let detail):
            return "The Claude config is not valid JSON (\(detail)). Earshot left it untouched."
        case .notAnObject(let detail):
            return "The Claude config at \(detail) is not a JSON object. Earshot left it untouched."
        }
    }
}
