...
The point: you stop explaining context to Claude. It asks Earshot instead.
...
That builds both products, assembles `Earshot.app`, signs it, installs it to `/Applications`, and
...
session that was already open will not see Earshot until it is relaunched.
...
```
Earshot.app                     earshot-mcp
  mic ─┐                          (stdio, spawned by Claude)
  sys ─┼→ Parakeet (on device) ─┐        │
 screen ┴→ Vision OCR ──────────┼→ earshot.db (SQLite, WAL, FTS5) ←┘ read-only
...
Exact visual values: `docs/earshot-design-system.md`.
