...
final class MCPServerTests: XCTestCase {
    ...
    func testInitializeAnnouncesTheProtocolAndServerIdentity() throws {
        ...
        XCTAssertEqual(result["serverInfo"]?["name"]?.stringValue, "earshot")
    ...
    private func makeStore() throws -> EarshotStore {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("earshot-mcp-tests-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        addTeardownBlock { try? FileManager.default.removeItem(at: root) }
        return try EarshotStore(url: root.appendingPathComponent("earshot.db"))
    }
