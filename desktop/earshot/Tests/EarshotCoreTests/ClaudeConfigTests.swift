...
/// Earshot adds one key to one dictionary inside it, so the tests that matter are the ones proving
...
final class ClaudeConfigTests: XCTestCase {
    private let binary = "/Applications/Earshot.app/Contents/MacOS/earshot-mcp"
    ...
    func testConfigLocationsAreTheOnesClaudeReads() {
        XCTAssertEqual(ClaudeConfig.serverName, "earshot")
    ...
    func testMergeWritesExactlyTheStdioEntry() throws {
        ...
        let entry = try object(try object(merged["mcpServers"])["earshot"])
    ...
    func testMergeRepointsAStaleEntry() throws {
        let stale = ClaudeConfig.merged(into: [:], mcpBinaryPath: "/old/path/earshot-mcp")
    ...
    func testIsRegisteredIsAnExactPathMatch() throws {
        ...
        XCTAssertFalse(ClaudeConfig.isRegistered(in: registered, mcpBinaryPath: "/old/path/earshot-mcp"))
        ...
        XCTAssertFalse(ClaudeConfig.isRegistered(
            in: ["mcpServers": ["earshot": ["type": "stdio"]]], mcpBinaryPath: binary))
