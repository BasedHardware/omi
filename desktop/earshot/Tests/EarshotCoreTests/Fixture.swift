...
/// A throwaway `EarshotStore` under the system temp directory, plus the deterministic corpus the
/// query tests read.
///
...
final class Fixture {
    ...
    init(seeded: Bool = true) throws {
        ...
        store = try EarshotStore(url: root.appendingPathComponent("earshot.db"))
        if seeded { try seed() }
    }

    var databaseURL: URL { root.appendingPathComponent("earshot.db") }
...
/// Compares by case name rather than `==`: `EarshotStoreError` is not declared `Equatable`, and a
/// test should not be the reason a source file has to grow a conformance.
func XCTAssertStoreError(
    _ error: Error,
    _ expected: EarshotStoreError,
    file: StaticString = #filePath,
    line: UInt = #line
) {
    guard let actual = error as? EarshotStoreError else {
        XCTFail("expected EarshotStoreError.\(expected), got \(error)", file: file, line: line)
        return
    }
