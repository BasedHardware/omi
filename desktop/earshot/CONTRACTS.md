...
5. Log via `EarshotLog.info/error(...)` (owned by the support agent). Never `print()` in
   `EarshotApp`; **never write anything to stdout in `EarshotMCP`** except JSON-RPC frames — stdout
   is the protocol channel there, so diagnostics go to stderr only.

## Already written — read, do not edit

- `Sources/EarshotCore/Models.swift` — `Session`, `SegmentSource`, `Segment`, `Frame`, `Hit`,
  `SessionSummary`, `ActivityBlock`, `CapabilityReport`, `StatusInfo`, `EarshotTime`.
- `Sources/EarshotCore/Paths.swift` — `EarshotPaths`, `CaptureState`.
- `Package.swift` — targets `EarshotCore`, `EarshotMCPKit`, `EarshotMCP`, `EarshotApp`, and two test
  targets.
...
|---|---|---|
| `EarshotCore` | Foundation, GRDB | Storage, queries, and pure policy. **No AppKit, no SwiftUI, no AVFoundation** — it has to link into the MCP binary. |
| `EarshotMCPKit` | Foundation, EarshotCore | JSON-RPC + tool dispatch. No UI, no capture. |
| `EarshotMCP` | EarshotMCPKit | `main.swift` only: wire stdin/stdout to `MCPServer`. |
| `EarshotApp` | everything | The app: capture, transcription, UI, integration. |

---

## EarshotCore

### `Sources/EarshotCore/Store.swift` — owner: **store agent**
...
public enum EarshotStoreError: Error { case readOnly, notInitialized }
```
...
throw `EarshotStoreError.notInitialized` so callers can report "not capturing yet".

### `Sources/EarshotCore/Queries.swift` — owner: **queries agent**

```swift
public enum Queries {
    public static func recall(_ store: EarshotStore, query: String, since: Double? = nil,
                              until: Double? = nil, limit: Int = 40) throws -> [Hit]
    public static func recent(_ store: EarshotStore, minutes: Int = 30, limit: Int = 120) throws -> [Hit]
    public static func sessions(_ store: EarshotStore, since: Double? = nil, until: Double? = nil,
                                limit: Int = 30) throws -> [SessionSummary]
    public static func transcript(_ store: EarshotStore, sessionId: Int64) throws -> [Hit]
    public static func screen(_ store: EarshotStore, since: Double? = nil, until: Double? = nil,
                              app: String? = nil, limit: Int = 60) throws -> [Hit]
    public static func activity(_ store: EarshotStore, since: Double, until: Double) throws -> [ActivityBlock]
    public static func status(_ store: EarshotStore) throws -> StatusInfo
}
```
...
- `status` reads `CaptureState.read()`; if it is nil or `isStale`, report `capturing: false` with
  `pausedReason: "Earshot is not running"`.

### `Sources/EarshotCore/Policy.swift` — owner: **policy agent**
...
### `Sources/EarshotCore/ClaudeConfig.swift` — owner: **registrar agent (pure half)**
...
public enum ClaudeConfig {
    ...
    /// Merges an `earshot` stdio entry into an existing config document without disturbing
    ...
    /// True when `existing` already points `earshot` at exactly `mcpBinaryPath`.
...
### `Sources/EarshotMCPKit/JSONRPC.swift` — owner: **mcp protocol agent**
...
### `Sources/EarshotMCPKit/Tools.swift` — owner: **mcp tools agent**
...
Earshot has not captured anything yet — never an exception.

### `Sources/EarshotMCPKit/MCPServer.swift` — owner: **mcp protocol agent**
...
Implements `initialize` (protocolVersion `"2024-11-05"`, `serverInfo` name `"earshot"`,
...
### `Sources/EarshotMCP/main.swift` — owner: **mcp protocol agent**

Opens `EarshotStore(readOnly: true)`, tolerating `notInitialized` by passing `nil`, then
...
### `Sources/EarshotApp/Support/Log.swift` — owner: **support agent**

```swift
public enum EarshotLog {
    static func info(_ message: String, _ category: String = "app")
    static func error(_ message: String, _ category: String = "app")
}
```
`os.Logger` with subsystem `com.omi.earshot`, plus a mirror to `/tmp/earshot.log` when
`EARSHOT_DEBUG=1` — that file is how the build/verify loop sees what happened.

### `Sources/EarshotApp/Capture/AudioSource.swift` — owner: **mic agent**
...
### `Sources/EarshotApp/Capture/MicCapture.swift` — owner: **mic agent**
...
### `Sources/EarshotApp/Capture/SystemAudioCapture.swift` — owner: **system audio agent**
...
### `Sources/EarshotApp/Capture/ScreenWatcher.swift` — owner: **screen agent**
...
(quality 0.5, longest side ≤ 1600) into `EarshotPaths.framesDirectory(for:)` and set `imagePath`.
Record `appName` and `windowTitle` on every frame even when OCR is skipped.

### `Sources/EarshotApp/Transcribe/Transcriber.swift` — owner: **transcription agent**
...
### `Sources/EarshotApp/Engine.swift` — owner: **engine agent**
...
```
Owns `MicCapture`, `SystemAudioCapture`, two `Transcriber`s, `ScreenWatcher`, and the `EarshotStore`
...
### `Sources/EarshotApp/Permissions.swift` — owner: **permissions agent**
...
### `Sources/EarshotApp/Integration/ClaudeRegistrar.swift` — owner: **registrar agent (I/O half)**

```swift
enum ClaudeRegistrar {
    struct Result { let claudeCode: Bool; let claudeDesktop: Bool; let message: String }
    static var mcpBinaryPath: String { get }        // Bundle.main/Contents/MacOS/earshot-mcp
...
### `Sources/EarshotApp/Integration/LoginItem.swift` — owner: **registrar agent**
...
### `Sources/EarshotApp/EarshotApp.swift` — owner: **app shell agent**

`@main struct EarshotApp: App` with a single `MenuBarExtra` scene, `.menuBarExtraStyle(.window)`.
...
`UserDefaults.standard.bool(forKey: "earshot.onboarded")` is false. Menu bar icon: the eight-dot Omi
mark drawn in code (see `Ink.swift`), faintly animated while capturing.

### `Sources/EarshotApp/MenuBar/StatusView.swift` — owner: **menu bar agent**
...
- `Sources/EarshotApp/Onboarding/Ink.swift` — **design system agent.** Colours, fonts, the stadium
  button, the permission row, the eight-dot mark. Exact values in `docs/earshot-design-system.md`.
- `Sources/EarshotApp/Onboarding/Backdrop.swift` + `RandomizedText.swift` — **atmosphere agent.**
  The nine-blob gradient and the word-by-word reveal.
- `Sources/EarshotApp/Onboarding/OnboardingWindow.swift` + `OnboardingView.swift` — **flow agent.**
  The borderless floating window, the `NSVisualEffectView` backing, and the four steps.

### `scripts/build.sh` — owner: **build agent**

`swift build -c release` both products, assemble `Earshot.app`, copy `earshot-mcp` into
`Contents/MacOS/`, copy `Resources/Fonts`, generate `Info.plist` (`LSUIElement`, usage strings,
`CFBundleIdentifier = com.omi.earshot`), sign with `Omi Local Dev Signing` and the entitlements —
**never ad-hoc, which resets Screen Recording for every Omi app on this machine** — then install to
`/Applications/Earshot.app`. Idempotent, and it must not touch `/Applications/Omi.app` or
`Omi Beta.app`.
