#!/bin/bash
#
# Builds, signs, and installs Earshot (menu-bar app + the earshot-mcp stdio server).
#
# Idempotent: run it as often as you like. Every run rebuilds the bundle from scratch under
# build/, signs it with the local dev identity, and replaces /Applications/Earshot.app.
#
# Usage:
...
#
# SAFETY: this script only ever touches Earshot. It never reads, writes, signs, launches, or kills
...
SIGN_IDENTITY="${EARSHOT_SIGN_IDENTITY:-Omi Local Dev Signing}"

BUILD_DIR="$PKG_DIR/build"
APP_BUNDLE="$BUILD_DIR/$APP_NAME.app"
INSTALL_PATH="/Applications/$APP_NAME.app"
INFO_PLIST_TEMPLATE="$PKG_DIR/Resources/Info.plist"
ENTITLEMENTS="$PKG_DIR/Resources/$APP_NAME.entitlements"
...
# Build
# ---------------------------------------------------------------------------------------------
log "building EarshotApp (release)"
swift build -c release --package-path "$PKG_DIR" --product EarshotApp

log "building earshot-mcp (release)"
swift build -c release --package-path "$PKG_DIR" --product earshot-mcp
...
[[ -x "$BIN_DIR/EarshotApp" ]] || die "EarshotApp binary missing at $BIN_DIR/EarshotApp"
[[ -x "$BIN_DIR/earshot-mcp" ]] || die "earshot-mcp binary missing at $BIN_DIR/earshot-mcp"

# ---------------------------------------------------------------------------------------------
# Assemble build/Earshot.app
# ---------------------------------------------------------------------------------------------
...
# CFBundleExecutable is "Earshot", so the app binary is installed under that name; earshot-mcp
# rides along in the same directory because Claude launches it by absolute path.
cp -f "$BIN_DIR/EarshotApp" "$APP_BUNDLE/Contents/MacOS/$APP_NAME"
cp -f "$BIN_DIR/earshot-mcp" "$APP_BUNDLE/Contents/MacOS/earshot-mcp"
...
log "signing nested earshot-mcp"
codesign --force --options runtime --entitlements "$ENTITLEMENTS" \
    --sign "$SIGN_IDENTITY" "$APP_BUNDLE/Contents/MacOS/earshot-mcp"
...
if [[ "$DO_INSTALL" -eq 0 ]]; then
    ...
    log "MCP binary: $APP_BUNDLE/Contents/MacOS/earshot-mcp"
    if [[ "$DO_RUN" -eq 1 ]]; then
...
# Quit only a running Earshot, matched on its exact executable path, and re-checked per pid.
...
MCP_PATH="$INSTALL_PATH/Contents/MacOS/earshot-mcp"
...
echo "$MCP_PATH"
echo
echo "  claude mcp add earshot \"$MCP_PATH\""
