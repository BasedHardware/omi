#!/bin/bash
#
# Removes Earshot from this machine.
#
# Usage:
#   scripts/uninstall.sh                remove /Applications/Earshot.app and the login item
#   scripts/uninstall.sh --purge-data   also delete ~/Library/Application Support/Earshot
#
...
APP_NAME="Earshot"
BUNDLE_ID="com.omi.earshot"
MCP_SERVER_NAME="earshot"
...
assert_not_production() {
    ...
    if [[ "$matched" -eq 1 ]]; then
        die "refusing to continue: $what resolves to a production Omi target ('$value'). This script only ever touches Earshot."
    fi
}
...
# Quit a running Earshot, matched on its exact executable path and re-checked per pid.
