...
enum UpdatePolicy {
    ...
    enum Refusal: String, Equatable, Sendable {
        ...
        /// `SUFeedURL` is missing, is not `https`, or is Omi's feed. Plain `http` is refused outright
        /// rather than warned about: the EdDSA signature makes tampering detectable, not
        /// unobservable.
    ...
    /// A feed this app may fetch: present, parseable, and `https`.
    ///
    /// Also refuses Omi's own appcast, by host *and* path. Sparkle does not know these are two
    /// products — it would download `Omi.zip`, verify it against whatever key this bundle carries,
    /// and if that key ever matched, replace Context for Claude with Omi Computer under this app's
    /// name and icon. That is not a plausible typo, but it is a catastrophic one, so it is refused
    /// where the refusal can be tested rather than left to review.
    static func isUsableFeedURL(_ feedURL: String?) -> Bool {
        ...
        if host.hasSuffix("omi.me") && url.path.contains("/desktop/appcast") { return false }
        return true
    }
