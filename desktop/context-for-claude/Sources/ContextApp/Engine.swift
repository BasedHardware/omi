...
final class Engine: ObservableObject {
  ...
  /// the conversation uploader. All of it is a no-op while signed out — captures queue locally and go up
  ...
  private func startLocalFallbackIfNeeded(
    ...
    guard
      TranscriptOwnership.shouldStartLocalFallback(
        when: cloudState, isSignedIn: OmiAuth.shared.isSignedIn)
    else { return nil }
    ...
    let localStart = Task { [weak self] in
      ...
      guard !Task.isCancelled,
        ...
        self.transcribers[component] != nil
      else { return }
    ...
    retentionQueue.asyncAfter(deadline: .now() + delay) { [weak self] in
      ...
      let strategy: StorageStrategy = {
        guard let raw = defaults.string(forKey: "context.settings.storageStrategy"),
          let value = StorageStrategy(rawValue: raw)
        else { return .off }
        return value
      }()
