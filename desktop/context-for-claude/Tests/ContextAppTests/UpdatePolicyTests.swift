...
final class UpdatePolicyTests: XCTestCase {
    ...
    private static let shippingFeed =
    ...
    func testOmisOwnAppcastIsRefusedAsAFeedForThisApp() {
    ...
    func testTheFeedMustBePresentAndHTTPS() {
        for feed in [nil, "", "not a url at all", "http://example.com/appcast.xml"] {
            XCTAssertEqual(
    ...
    func testTheShippedPlistCarriesTheKeysThePolicyReadsAndNoPrivateKey() throws {
        ...
        XCTAssertTrue(
            ...
            "SUFeedURL must be an https feed that is not Omi's")
