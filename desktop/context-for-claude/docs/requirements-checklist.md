...
| G2 | Opens a page in the browser as the first step. **NOT Wikipedia** — direct user instruction | **DECIDED OUT** — direct user instruction, superseding the original: "I dont like Ling's website." The step is gone, not re-pointed. Opening any third-party commercial page in someone's browser thirty seconds after asking them to let this app watch their screen is wrong on taste and on trust, and it taught the product on content that was not theirs. The capture beat now runs on **whatever the user already has open**, which is the more convincing demonstration as well as the more honest one: the frames it waits for are frames of their own work. `TutorialEnvironment` has no browser-opening seam left, and `testTheFlowWalksEveryBeatInOrder` fails if a beat that opens anything is put back |
...
| G8 | Coach mark with a left-scrolling animation: "scroll left to see the past" | **DONE, and now actually detected.** Direct user report: "The two finger dragging didnt work." Two independent causes, both real: (1) **nothing listened** — the hint was decorative and the step's gate was `.userAction`, so a perfect gesture changed nothing; (2) **the gesture moved nothing** — the track's scroll called `RewindModel.panTrack`, which pans the *visible window* and is clamped to the loaded day, and `resetTrackWindow` opens with `trackSpan` = the whole day, so `bounds.end - trackSpan == bounds.start` and `trackStart` could not move by any number of seconds. Every scroll on a freshly opened timeline was arithmetically a no-op. Fixes: `TutorialDrag` (a pure recogniser over scroll deltas, 12 tests in `TutorialDragTests`) behind a **local** `NSEvent` monitor gates the step as `TutorialGate.realGesture`; and `RewindModel.travel(by:)` replaces `panTrack`, moving the **playhead** so the picture changes and the window follows via `keepPlayheadVisible()`. The recogniser has no phase filter (momentum counts), no direction rule (which way is "back" is the user's own natural-scrolling setting, which this app does not get to read) and no trackpad requirement (a notched wheel scales by `pointsPerLine`). The card's arrow is `arrow.left.and.right` for the same reason, and `TutorialScrollHint` still collapses under Reduce Motion |
