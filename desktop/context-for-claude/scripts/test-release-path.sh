...
printf '%s\n' "$DRY_RUN_OUTPUT" | rg -q 'product=context-for-claude tag=context-for-claude-v1.1.0 version=1.1.0 build=1001000' \
    || fail "tag/version/build derivation"
printf '%s\n' "$DRY_RUN_OUTPUT" | rg -q 'no build, signing, notarization, GitHub release' \
...
rg -q 'CONTEXT_SPARKLE_PUBLIC_KEY' "$SCRIPT_DIR/build.sh" \
    || fail "build key injection contract"
rg -q 'CONTEXT_SPARKLE_PRIVATE_KEY' "$SCRIPT_DIR/release-context.sh" \
    || fail "Context private key naming contract"
rg -q 'build: \$BUILD_NUMBER' "$SCRIPT_DIR/release-micro-app.sh" \
    || fail "generic feed build metadata"
rg -q 'edSignature: \$ED_SIGNATURE' "$SCRIPT_DIR/release-micro-app.sh" \
    || fail "generic feed signature metadata"
rg -q 'context-for-claude-v\*' "$REPO_ROOT/codemagic.yaml" \
