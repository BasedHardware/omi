...
- **Valdi:** source checkout contains a native hello-world with C++/Objective-C/Kotlin examples. The isolated CLI and Bazelisk setup succeeded. Full `agent-check --module hello_world --json` resolved the graph, then failed during build because the host's Apple toolchain resolves to nonexistent `Beta.app/Contents/Developer`; the test target also rejects a `.png.bin` asset and lint reports formatting failures. Valdi is not marked built.
