...
function facts() {
  ...
  return {
    ...
    launchers: {
      macos: { status: "pass", exitCode: 0 },
      ios: { status: "pass", exitCode: 0 },
    },
...
const reportFor = (input) => buildReport(input, { readinessRecord: readiness(input) });
...
test("RED-PROOF ios:null is an explicit shell failure even with sixteen rows", () => {
  ...
  const report = reportFor(input);
  assert.equal(report.result, "fail");
  assert.match(named(report, "two_shell_results").detail, /ios launcher outcome is missing/);
});

test("RED-PROOF reports and receipts cannot copy forbidden producer content or readiness secrets", () => {
  const input = facts();
  const readinessRecord = readiness(input);
  readinessRecord.devToken = "secret-readiness-token";
  readinessRecord.ownerAccountId = "secret-owner-account";
  ...
  const report = buildReport(input, { readinessRecord });
  assert.equal(report.result, "fail");
  ...
  assert.doesNotMatch(serialized, /127\.0\.0\.1|baseUrl/);
});

test("RED-PROOF final facts reject missing launcher outcome keys", () => {
  const input = facts();
  delete input.launchers.ios.exitCode;
  const report = reportFor(input);
  assert.equal(report.result, "fail");
  assert.match(named(report, "facts_integrity").detail, /missing field.*exitCode/);
});
