...
import {
  ...
  type FormationWorkInputStageRequest,
...
const contentHash = (input: StagedFormationWorkInput): string => sha256CanonicalContent({
  contract_version: "formation-work-staged-input-content-v1",
  staged_input: input,
});

const parseRow = (row: InputRow, expectedJob?: DurableMemoryWorkJob): StagedFormationWorkInput => {
...
): Promise<StagedFormationWorkInput | null> => {
  const rows = await connection.query<InputRow>({
    name: "formation.input.read",
    text: "SELECT * FROM omi_memory.read_formation_work_input($1, $2)",
    ...
    return await withAuthorizedSerializableConnectionTransaction(
      ...
      async ({ authority, connection }) => {
        await connection.query({
          name: "formation.input.lock_key",
          text: "SELECT pg_advisory_xact_lock(hashtextextended($1, 0))",
          values: [`formation-input:${authority.account_id}:${request.pending_job.job_id}`],
        });
        ...
        const rows = await connection.query<{ inserted: boolean }>({
          name: "formation.input.insert",
          text: `
SELECT omi_memory.insert_formation_work_input(
    ...
    return await withAuthorizedSerializableConnectionTransaction(
      ...
      async ({ authority, connection, dbNowEpochSeconds }) => {
        const rows = await connection.query<LeaseRow>({
          name: "formation.input.current_lease",
          text: `
...
export interface PostgresFormationWorkInputOptions {
  readonly pool: PostgresTransactionPool;
  readonly observability?: PostgresTransactionObservability;
}

/** Inert sensitive input adapter. It adds no model, scheduler, route, or runtime default. */
export const createPostgresFormationWorkInputRepository = (
  options: PostgresFormationWorkInputOptions,
): FormationWorkInputRepository => defineFormationWorkInputRepository({
