  ...
  WITH selected AS MATERIALIZED (
    ...
    SELECT s.conversation_sequence AS sequence,s.session_id,s.conversation_id,s.source,u.session_id IS NOT NULL AS device,
      ...
      CASE WHEN u.session_id IS NULL THEN (
        SELECT coalesce(left(btrim(string_agg(left(btrim(segment.text_content,v_ws),240),' ' ORDER BY segment.ordinal) FILTER (WHERE length(btrim(segment.text_content,v_ws))>0),v_ws),240),'')
        ...
        SELECT coalesce(left(btrim(string_agg(left(btrim(segment.value->>'text',v_ws),240),' ' ORDER BY segment.ordinal) FILTER (WHERE length(btrim(segment.value->>'text',v_ws))>0),v_ws),240),'')
