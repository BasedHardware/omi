CREATE TABLE omi_memory.conversation_union_cursor_positions (
...
CREATE INDEX conversation_union_cursor_expiry ON omi_memory.conversation_union_cursor_positions(account_id,expires_at);
REVOKE ALL ON omi_memory.conversation_union_cursor_positions FROM PUBLIC,omi_platform_application;

CREATE FUNCTION omi_memory.read_listen_conversation_union_page(p_limit integer,p_cursor_hash text,p_binding_digest text,p_revision bigint,p_chat_snapshot_sequence bigint)
...
CREATE FUNCTION omi_memory.save_conversation_union_cursor(
...
AS $function$
  ...
  FROM (VALUES
    ...
    ('product_projections', 'conversation_union_cursor_positions'),
