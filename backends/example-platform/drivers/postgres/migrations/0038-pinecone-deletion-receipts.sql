-- P7 retained, content-free receipts for account-filtered Pinecone cleanup.
-- Provider credentials and vector identifiers never enter PostgreSQL.

CREATE TABLE omi_memory.account_pinecone_deletion_receipts (
  ...
  resource_role text NOT NULL CHECK (resource_role IN (
    'conversation_vectors', 'memory_vectors', 'screen_activity_vectors',
    'action_item_vectors', 'transcript_chunk_vectors', 'x_post_vectors',
    'workstream_association_vectors'
  )),
  index_name text NOT NULL CHECK (index_name = 'memories-backend'),
  namespace_name text NOT NULL,
  CHECK ((resource_role, namespace_name) IN (
  ...
  pre_delete_content_hash text NOT NULL CHECK (pre_delete_content_hash ~ '^[0-9a-f]{64}$'),
...
CREATE FUNCTION omi_memory.load_pinecone_deletion_receipt(
  p_account_id text,
...
AS $function$
DECLARE
  v_receipt omi_memory.account_pinecone_deletion_receipts%ROWTYPE;
BEGIN
  IF p_operation_ref !~ '^opref1_[0-9a-f]{64}$'
    OR p_eligibility_digest !~ '^[0-9a-f]{64}$'
    OR p_registry_digest !~ '^[0-9a-f]{64}$'
    OR p_index_name IS DISTINCT FROM 'memories-backend'
    OR (CASE p_resource_role
      WHEN 'conversation_vectors' THEN p_namespace_name IS DISTINCT FROM 'ns1'
      WHEN 'memory_vectors' THEN p_namespace_name IS DISTINCT FROM 'ns2'
      WHEN 'screen_activity_vectors' THEN p_namespace_name IS DISTINCT FROM 'ns3'
      WHEN 'action_item_vectors' THEN p_namespace_name IS DISTINCT FROM 'ns4'
      WHEN 'transcript_chunk_vectors' THEN p_namespace_name IS DISTINCT FROM 'ns_tchunks'
      WHEN 'x_post_vectors' THEN p_namespace_name IS DISTINCT FROM 'ns_x'
      WHEN 'workstream_association_vectors' THEN
        p_namespace_name IS DISTINCT FROM 'workstream-association-v1'
      ELSE true
    END) THEN
    RAISE EXCEPTION USING ERRCODE = '22023', MESSAGE = 'pinecone_receipt_input_invalid';
  END IF;
  ...
  FOR SHARE;
  IF NOT FOUND THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'pinecone_receipt_coordinate_denied';
  END IF;

  SELECT * INTO v_receipt
  FROM omi_memory.account_pinecone_deletion_receipts r
  ...
  IF v_receipt.control_revision <> p_control_revision
    ...
    OR v_receipt.index_name <> p_index_name
    OR v_receipt.namespace_name <> p_namespace_name THEN
    RAISE EXCEPTION USING ERRCODE = '23505', MESSAGE = 'pinecone_receipt_conflict';
  END IF;
  ...
  RETURN QUERY SELECT
    ...
    v_receipt.resource_role, v_receipt.index_name, v_receipt.namespace_name,
    v_receipt.result, v_receipt.pre_delete_count, v_receipt.pre_delete_content_hash,
    v_receipt.provider_receipt_digest, v_receipt.receipt_digest;
END
$function$;

CREATE FUNCTION omi_memory.record_pinecone_deletion_receipt(
  p_account_id text,
  p_control_revision bigint,
  p_deletion_epoch bigint,
  p_operation_ref text,
  ...
  p_pre_delete_content_hash text,
...
BEGIN
  IF p_operation_ref !~ '^opref1_[0-9a-f]{64}$'
    ...
    OR p_pre_delete_content_hash !~ '^[0-9a-f]{64}$'
  ...
  INSERT INTO omi_memory.account_pinecone_deletion_receipts (
    account_id, deletion_epoch, control_revision, operation_ref,
    eligibility_digest, registry_digest, resource_role, index_name, namespace_name,
    ...
    p_eligibility_digest, p_registry_digest, p_resource_role, p_index_name, p_namespace_name,
    p_result, p_pre_delete_count, p_pre_delete_content_hash, p_provider_receipt_digest,
    p_receipt_digest
  ...
  IF v_receipt.control_revision <> p_control_revision
    ...
    OR v_receipt.registry_digest <> p_registry_digest
    OR v_receipt.index_name <> p_index_name
    OR v_receipt.namespace_name <> p_namespace_name
    OR v_receipt.result <> p_result
    OR v_receipt.pre_delete_count <> p_pre_delete_count
    OR v_receipt.pre_delete_content_hash <> p_pre_delete_content_hash
...
REVOKE ALL ON TABLE omi_memory.account_pinecone_deletion_receipts
  FROM PUBLIC, omi_platform_application, omi_platform_cleanup;
REVOKE ALL ON FUNCTION omi_memory.load_pinecone_deletion_receipt(
  text, bigint, bigint, text, text, text, text, text, text
) FROM PUBLIC, omi_platform_application;
REVOKE ALL ON FUNCTION omi_memory.record_pinecone_deletion_receipt(
  text, bigint, bigint, text, text, text, text, text, text, text, bigint, text, text, text
) FROM PUBLIC, omi_platform_application;
GRANT EXECUTE ON FUNCTION omi_memory.load_pinecone_deletion_receipt(
  text, bigint, bigint, text, text, text, text, text, text
) TO omi_platform_cleanup;
GRANT EXECUTE ON FUNCTION omi_memory.record_pinecone_deletion_receipt(
  text, bigint, bigint, text, text, text, text, text, text, text, bigint, text, text, text
) TO omi_platform_cleanup;
