...
DECLARE v_seg jsonb; v_duration double precision; v_i integer; v_n integer; v_start double precision; v_end double precision; v_text text; v_chars integer:=0; v_bytes integer:=0;
...
BEGIN
  ...
  FOR v_i IN 0..v_n-1 LOOP
    ...
    IF v_text IS NULL OR btrim(v_text,v_ws)='' OR octet_length(convert_to(v_text,'UTF16BE'))>3000 OR position(chr(0) IN v_text)>0
