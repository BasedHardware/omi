...
The production entry releases acquired Firebase, pool and HTTP resources when factory construction, socket binding or readiness fails. After runtime construction, the runtime owns pool shutdown; the entry closes the pool directly only if construction never completed. Explicit shutdown attempts every remaining cleanup even if another cleanup fails, is idempotent, and rejects failed or undrained runtime outcomes. Startup failure remains the primary cause, with cleanup failures retained alongside it. `bun run check:deployed` covers these paths, including a real occupied socket. Early SIGTERM/SIGINT before startup resolves and the 45-second startup watchdog still exit without this cleanup path; this change does not establish graceful startup interruption or alter the supervisor's existing deadlines.
