...
On 2026-09-07, the separate migration identity applied verified migration 51 to `omi-platform-dev-pg18` through the existing authenticated Cloud SQL proxy. The checksummed runner verified 50 existing migration receipts, applied only 51, and read back exactly 51 receipts. No account activation, application grant or entitlement records were synthesized. Before application, the local PostgreSQL 18.4 gate passed 23 tests and 958 assertions, restored 51 migrations and 119 tables, and passed the pinned Bun and Node adapter qualification.
