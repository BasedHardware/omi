...
On 2026-09-24, from head `68ef253618`, the migration identity applied only migrations 55–58 through the authenticated proxy (58 receipts read back), and a full logical restore drill against this instance produced byte-identical source and restored receipt projections; the generation release ran through `omi_memory.release_postgres_restore_generation_v2` as a dedicated restore-operator login. Digests, derivations and receipts are in the private operator audit log (`~/.local/share/omi-v5-dev/operator/generation-release-20260924.log`). Measured `max_connections` on the provisioned instance is 100; the manifest allocation (3 revisions x 2 instances x 4 pool + 4 migration + 8 operator = 36) is within the approved 70. The linux/amd64 image built from `68ef253618` is pushed to `us-central1-docker.pkg.dev/based-hardware-dev/omi-platform` as `service@sha256:ffc1e02bd04878dfcd821589682e8e91e980a0fa2c5f64cc60ebebbff54b1121`, and `release-candidate.ts` prepared initial-dev revision `omi-platform-dev-r-e2366a680f5c16d0` in the operator release directory. The eight runtime IAM grants remain unapplied: the operator identity (`undivisible@vk.com`, `roles/editor`) receives 403 on project/secret `setIamPolicy` and `iam.roles.create`, matching the documented 2026-09-07 limitation; the fresh OpenTofu plan reproduces the reviewed eight-add grant set and requires a project owner to apply before `gcloud run services replace`.

On 2026-09-25, after the project owner granted the operator ownership of the full-access bot service account, the eight reviewed runtime IAM grants were applied through the impersonated identity and the fresh OpenTofu plan reported no changes. `gcloud run services replace` created the 2026-09-24 revision, but its startup probe failed: version 1 of `jit-qa-gateway-token` carried a trailing newline, so deployed-config validation rejected the gateway token. Version 2 was created with the verified 64-character hexadecimal payload (version 1 remains enabled and unreferenced). Because the only revision had never become ready, blue-green mode could not express a release over it; the never-ready service was deleted and `release-candidate.ts` prepared initial-dev revision `omi-platform-dev-r-849d5c5a8bf69c32` from the same image and generation digest, binding the gateway token to version 2. The revision became ready, the service denied anonymous `/ready` with 403, an authorized Cloud Run caller received `{"status":"ready"}`, `/v1/memories` returned 401 without valid Firebase credentials, and the signed-out `/v1/settings` body matched its optional-auth contract. `bun run check:deployed` passed 321 tests with three real-chat-grant skips; the diagnostic Cloud Run jobs from the failure investigation were deleted. Operator artifacts are in `~/.local/share/omi-v5-dev/operator/release-20260925-initial/`.

On 2026-09-25, the refactor wave from head `fe3ed2300b` (output-preserving deduplication across the chat streaming route, agent-run ordering, generation-supervisor tables, consolidate state-machine key list and failure tail, deletion-dominance plan builders, a shared chat test-fixture module, a shared bearer-token parser, and the React Native load-more/search/transport-error helpers) released blue-green. `release-candidate.ts` prepared candidate `omi-platform-dev-r-e398b5a8f58e76c2` from image `service@sha256:fa2fd36cf6c877534ba331925eb238d5fa61fb6c02ba5feb8a8e7ae8c3fb0226` with the same database generation digest and secret-version pins, over resident `omi-platform-dev-r-849d5c5a8bf69c32` (aggregate admission 28 of 36). `gcloud run services replace` applied the candidate at zero traffic while the previous revision kept 100%. The candidate tag URL returned GFE 401 to a valid audience-matched owner-impersonated ID token while the main URL accepted the identical mint pattern, so acceptance ran after promotion on the main URL, as this procedure permits. Promotion moved 100% of traffic to the new revision; anonymous `/ready` is 403, an authorized caller receives `{"status":"ready"}`, `/v1/memories`, `/v1/tasks` and `/v1/chat-messages` return their pinned 401 bodies with garbage Firebase credentials, signed-out `/v1/settings` matches its optional-auth contract, and `bun run check:deployed` passed 321 tests with three real-chat-grant skips. The full `bun run check` gate passed before release. Operator artifacts are in `~/.local/share/omi-v5-dev/operator/release-20260925-refactor/`. Correction to the initial-release record above: the `omi-diag-2` diagnostic Cloud Run job was not deleted; it remains in the project.
...
The production entry releases acquired Firebase, pool and HTTP resources when factory construction, socket binding or readiness fails. After runtime construction, the runtime owns pool shutdown; the entry closes the pool directly only if construction never completed. Explicit shutdown attempts every remaining cleanup even if another cleanup fails, is idempotent, and rejects failed or undrained runtime outcomes. Startup failure remains the primary cause, with cleanup failures retained alongside it. `bun run check:deployed` covers these paths, including a real occupied socket. SIGTERM/SIGINT handlers are installed before startup. Cancellation waits for pending resource acquisitions to settle, closes their returned handles and prevents further acquisition; PostgreSQL readiness receives the cancellation signal. Signals retain an eight-second hard shutdown deadline, and startup retains its existing 45-second hard limit. A dependency that never settles can still require forced process exit; the watchdog does not promise completed cleanup.
