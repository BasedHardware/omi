...
import { defineDerivedGroupDreamWorkDispatch } from "./derived-group-dream-work-dispatch";
...
const repository = (lease: DurableMemoryWorkLeaseNextOutcome) =>
  defineDurableMemoryWorkExecutionRepository({
    leaseNext: async (_authorized, request) => {
      // The lease filter must name derived_group_dream alone, so this port can
      // never consume another kind's backlog.
      expect(request.work_kinds).toEqual(["derived_group_dream"]);
      return lease;
    },
...
describe("derived group dream work one-shot dispatch", () => {
  test("idle never invokes the consolidation executor", async () => {
    ...
    const dispatch = defineDerivedGroupDreamWorkDispatch({
      ...
      derived_group_dream: { run: async () => { calls += 1; throw new Error("must not run"); } },
    });
  ...
  test("one dream lease invokes one executor and returns only a content-safe summary", async () => {
    let calls = 0;
    const dispatch = defineDerivedGroupDreamWorkDispatch({
      ...
      derived_group_dream: {
        run: async () => {
          ...
          return {
            kind: "succeeded",
            outcome: { kind: "committed", job: leased() },
            producer_calls: 1,
  ...
  test("a recorded failure is summarized without leaking the job", async () => {
    const dispatch = defineDerivedGroupDreamWorkDispatch({
      execution_repository: repository({ kind: "leased", job: leased() }),
      derived_group_dream: {
        run: async () => ({
    ...
    expect(JSON.stringify(outcome)).not.toMatch(/job:|account:/);
  });
  ...
  test("authority, storage, and dependency failures remain closed", async () => {
    ...
    const denied = defineDerivedGroupDreamWorkDispatch({
      execution_repository: repository({ kind: "none_available" }),
      derived_group_dream: { run: async () => { throw new Error("must not run"); } },
    });
    ...
    const executionFailure = defineDerivedGroupDreamWorkDispatch({
      execution_repository: repository({ kind: "leased", job: leased() }),
      derived_group_dream: { run: async () => { throw new Error("provider secret"); } },
    });
