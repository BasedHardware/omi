...
| Request | Result |
| --- | --- |
| `GET /v1/device-sessions/ownership` | 200 `{ownership:{ownerKey,receipt}}` after `listen.capture.write`; 401/403 for missing admission; 503 `capture_ownership_unavailable` without a signing key |
