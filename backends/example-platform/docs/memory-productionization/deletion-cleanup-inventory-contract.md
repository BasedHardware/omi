...
Status: implemented production-neutral P7 v5 contract. No scanner, persistence,
...
- the complete legacy product-memory generation; and
...
post-cutover data. Version 5 removes `stranded_product_data` as a duplicate
physical surface: ADR-014 defines it as the same new-generation PostgreSQL data
already covered by the authoritative, work, projection, index, migration, and
external surfaces. Its separate 30-day recovery manifest is not a second store
and does not need a second delete receipt. Earlier versions are not accepted because they could report
