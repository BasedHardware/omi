...
- **hosted MCP OAuth** (`routers/mcp_sse.py`) — ChatGPT Apps OAuth for `/v1/mcp/sse`. Configure with `MCP_AUTHORIZATION_SERVER_URL`, `MCP_OAUTH_CHATGPT_CLIENT_ID`, `MCP_OAUTH_CHATGPT_CLIENT_SECRET` or `MCP_OAUTH_CHATGPT_CLIENT_SECRET_SHA256`, `MCP_OAUTH_CHATGPT_REDIRECT_URIS`, optional `MCP_RESOURCE_URL`, and token TTL env vars.
