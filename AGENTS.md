...
- Desktop release pipeline: `.github/workflows/desktop_auto_release.yml` batches `main` into at most one weekday `v*-macos` candidate; Codemagic builds/signs/notarizes it with `isLive: false`. Run `desktop/macos/scripts/bless-release.sh <tag>` for the T2 harness and beta promotion. That promotion registers the immutable manifest, advances the explicit beta pointer, and maintains the legacy bridge. Developers who need head-of-main use `desktop/macos/scripts/omi-main`. Stable/prod requires `desktop/macos/docs/agent-prod-promotion-runbook.md`, then manual `.github/workflows/desktop_promote_prod.yml` with `release_tag` and `confirm=promote-stable`; it is roll-forward only and advances the explicit stable pointer only after backend verification. Desktop Rust backend deploys require environment-scoped `DESKTOP_BACKEND_BASE_API_URL` so OAuth callbacks set runtime `BASE_API_URL`.
