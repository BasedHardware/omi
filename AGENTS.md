...
- **parakeet** (`parakeet/`) — GPU STT service for streaming and pre-recorded transcription. Batch: `/v1/transcribe`, `/v2/transcribe` (REST). Streaming: `/v4/stream` (WebSocket, NeMo CacheAwareRNNTPipeline, PCM16 binary protocol). Single GPU worker thread with separate batch/stream queues and fairness cap (`PARAKEET_MAX_STREAM_DRAIN`). Called by backend when `HOSTED_PARAKEET_API_URL` is set and parakeet is selected.
