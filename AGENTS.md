...
- **Pyright strictness ratchet** — directories graduate to stricter per-directory rules via `executionEnvironments` in `backend/pyrightconfig.json` (e.g. `utils/sync` enforces `reportMissingParameterType: error`). Add fully-annotated directories there; never remove one or downgrade a rule.
...
- **PR size is bounded** (manifest check `pr-scope`, local pre-push + CI) — over 1,500 changed production-source lines (tests/docs/l10n/generated excluded) warns; over 3,000 fails unless a maintainer applies the `scope-approved` label (locally: `OMI_SCOPE_APPROVED=1`). Revert PRs are exempt. Split large work into independently reviewable PRs; review effectiveness collapses past this size.
...
- **Hermetic tests must run in CI.** Put new hermetic tests where the component's runner discovers them and confirm they execute in a full local run; if a test requires a live service, keep it out of CI and document how you ran it in the PR. Backend enforces this mechanically: `backend/scripts/check_unit_test_discovery.py` (CI) fails on any test file no runner discovers.
- **New fail-closed gates ship with a legacy-principal test** — any new auth/consent/rollout gate includes a test where the caller is an existing/unmigrated principal (no state doc, old API key, already-shipped client) asserting the intended fallback. Gates without one have shipped as day-one breakage for existing users.
- **A test rewritten in the same PR as the code it asserts is suspect** — reviewers reject the change unless the PR body cites an external source (platform doc, wire contract, measurement) for the new expected value; an assertion rewritten to mirror a new constant deletes the guard (this shipped a prod timeout regression).
