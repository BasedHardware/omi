...
- **`run.sh` build lock is per-worktree, build-phase only.** It serializes same-checkout builds that share `Desktop/.build/` + `build/$APP_NAME.app`, then releases after `/Applications` install — never a per-user global mutex, and never held across the long-running backend wait. Cross-worktree `./run.sh` must not block each other.
