...
- **PR size is bounded in CI** — over 1,500 changed production-source lines (tests/docs/l10n/generated excluded) warns; over 3,000 fails unless a maintainer applies the `scope-approved` label (`.github/scripts/check_pr_scope.py`). Split large work into independently reviewable PRs; review effectiveness collapses past this size.
...
- **A test rewritten in the same PR as the code it asserts is suspect** — tests encode external invariants (platform limits, wire contracts), not the implementation's current constants; changing an assertion to mirror a new constant deletes the guard (this shipped a prod timeout regression).
