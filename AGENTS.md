...
- **PR size is bounded** (manifest check `pr-scope`, local pre-push + CI) — over 1,500 changed production-source lines (tests/docs/l10n/generated excluded; moves count both sides) warns; over 3,000 fails unless a maintainer applies the `scope-approved` label (locally: `OMI_SCOPE_APPROVED=1`; emergency reverts use the same overrides). Split large work into independently reviewable PRs; review effectiveness collapses past this size.
...
- **Hermetic tests must run in CI.** Put new hermetic tests where the component's runner discovers them and confirm they execute in a full local run; if a test requires a live service, keep it out of CI and document how you ran it in the PR. Backend enforces this mechanically: manifest check `backend-test-discovery` fails on any test file no verified runner discovers.
