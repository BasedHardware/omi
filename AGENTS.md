...
- **memory-maintenance-job** (`modal/memory_maintenance_job.py`) — Cloud Run Job for canonical ST→LT maintenance (TTL → consolidation → promotion). Manual deploy via `.github/workflows/gcp_memory_maintenance_job.yml`; auto-dev on push to `main` via `gcp_memory_maintenance_job_auto_dev.yml`. Gated by `MEMORY_CANONICAL_PROMOTION_CRON_ENABLED` + cohort whitelist; prod stays off until Gate 3.
