...
def postprocess_conversation(
    ...
    try:
        ...
        # Store enhanced diarization results if applied successfully
        if enhancement_metrics.get("status") == "success":
            conversations_db.store_model_segments_result(
                uid, conversation.id, 'pyannote_enhanced', [seg.dict() for seg in enhanced_segments]
            )
            print(f'📊 Enhanced diarization metrics stored for conversation {conversation.id}')
