...
describe("computeProfitability cost-per-user honesty", () => {
  ...
  it("computes a real rate when the platform has active users", async () => {
    ...
    const fetchMock = vi.fn(async (url: any, init: any) => {
      ...
      // The per-day desktop active-users query; every other query (mobile
      // actives, uid lookups) returns nothing.
      const rows =
        q.includes("= 'macOS'") && q.includes("count(DISTINCT distinct_id)")
          ? dates.map((d) => [d, 10])
          : [];
      return {
        ok: true,
