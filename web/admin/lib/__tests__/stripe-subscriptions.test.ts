...
describe("monthlyAmount", () => {
  ...
  it("divides an annual price by twelve", () => {
    expect(
      monthlyAmount(
        sub([{ unit_amount: 12000, recurring: { interval: "year" } }])
      )
    ).toBeCloseTo(10);
  });
  ...
  it("normalises weekly and daily intervals", () => {
    expect(
      monthlyAmount(
        sub([{ unit_amount: 100, recurring: { interval: "week" } }])
      )
    ).toBeCloseTo(1 * (52 / 12));
    expect(
      monthlyAmount(sub([{ unit_amount: 100, recurring: { interval: "day" } }]))
    ).toBeCloseTo(1 * (365 / 12));
  });
  ...
  it("sums multiple items", () => {
    const bundle = sub([
      { unit_amount: 1000 },
      { unit_amount: 12000, recurring: { interval: "year" } },
    ]);
    expect(monthlyAmount(bundle)).toBeCloseTo(20);
  });

  it("ignores one-off items with no recurring interval", () => {
    expect(monthlyAmount(sub([{ unit_amount: 5000, recurring: null }]))).toBe(
      0
    );
  });
...
describe("monthlyAmount honesty guards", () => {
  ...
  it("warns loudly instead of silently contributing zero for an unknown interval", () => {
    ...
    const exotic = sub([
      { unit_amount: 3000, recurring: { interval: "fortnight" } },
    ]);
  ...
  it("counts what it skipped so a route can report it", () => {
    const subscriptions = [
      ...
      sub([{ unit_amount: 1000 }, { unit_amount: 1000, currency: "gbp" }], {
        id: "sub_mixed",
      }),
    ];

    expect(subscriptions.map(isNonUsdSubscription)).toEqual([
      false,
      true,
      true,
    ]);
...
describe("isOmiPlanSubscription", () => {
  it("returns true if the subscription has an item with an Omi plan product", () => {
    ...
    expect(
      isOmiPlanSubscription(sub([{ unit_amount: 1000, product: planProduct }]))
    ).toBe(true);
  });

  it("returns false if no items match an Omi plan product", () => {
    expect(
      isOmiPlanSubscription(sub([{ unit_amount: 1000, product: "prod_other" }]))
    ).toBe(false);
  });
...
describe("isAppSubscription", () => {
  it("is true only when the backend stamped an app_id", () => {
    expect(
      isAppSubscription(
        sub([{ unit_amount: 500 }], { metadata: { app_id: "app_123" } })
      )
    ).toBe(true);
    expect(
      isAppSubscription(
        sub([{ unit_amount: 500 }], {
          metadata: { uid: "u1", sub_type: "unlimited" },
        })
      )
    ).toBe(false);
...
describe("isAnnual", () => {
  it("classifies by billing interval, not by price id", () => {
    expect(
      isAnnual(sub([{ unit_amount: 12000, recurring: { interval: "year" } }]))
    ).toBe(true);
...
describe("productIdOf", () => {
  it("reads a string or expanded product", () => {
    expect(
      productIdOf(sub([{ unit_amount: 1, product: "prod_a" }]).items.data[0])
    ).toBe("prod_a");
    expect(
      productIdOf(
        sub([{ unit_amount: 1, product: { id: "prod_b" } }]).items.data[0]
      )
    ).toBe("prod_b");
  });
});
...
describe("groupByProduct", () => {
  it("groups every product without a hardcoded list", () => {
    ...
    expect(
      groups.map((g) => [g.productName, g.subscriptionCount, g.mrr])
    ).toEqual([
  ...
  it("falls back to the product id when no name is known", () => {
    const [group] = groupByProduct([
      sub([{ unit_amount: 1000, product: "prod_new" }]),
    ]);
...
function stripeWithPages(pages: Record<string, Stripe.Subscription[][]>) {
  const list = vi.fn(async (params: Stripe.SubscriptionListParams) => {
    ...
    const index = params.starting_after
      ? Number(String(params.starting_after).split("#")[1]) + 1
      : 0;
...
describe("listSubscriptions", () => {
  it("follows pagination to the last page", async () => {
    const page = (n: number) => [
      sub([{ unit_amount: 1000 }], { id: `sub#${n}` }),
    ];
    const { stripe, list } = stripeWithPages({
      active: [page(0), page(1), page(2)],
    });
    ...
    const { stripe } = stripeWithPages({
      active: [
        [
          ...
          sub([{ unit_amount: 9900, product: "prod_internal_test" }], {
            id: "sub#1",
          }),
          sub([{ unit_amount: 500, product: planProduct }], {
            id: "sub#2",
            metadata: { app_id: "app_1" },
          }),
        ],
      ],
      past_due: [[]],
...
describe("fetchOmiSubscriptions", () => {
    ...
    const { stripe } = stripeWithPages({
      active: [
        [
          ...
          sub([{ unit_amount: 500, product: plan }], {
            id: "sub#1",
            metadata: { app_id: "app_1" },
          }),
        ],
  ...
  it("degrades to partial when one status leg fails", async () => {
    const list = vi.fn(async (params: Stripe.SubscriptionListParams) => {
      ...
      return {
        data: [
          sub(
            [{ unit_amount: 2000, product: Object.keys(OMI_PLAN_PRODUCTS)[0] }],
            { id: "sub#0" }
          ),
        ],
        has_more: false,
  ...
  it("throws rather than reporting zero when every leg fails", async () => {
    ...
    await expect(fetchOmiSubscriptions(stripe)).rejects.toBeInstanceOf(
      AllSubscriptionSourcesFailedError
    );
  });
});
