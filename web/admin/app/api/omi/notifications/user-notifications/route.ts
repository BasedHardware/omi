...
function deriveKey(uid: string): Buffer {
  ...
  return Buffer.from(
    crypto.hkdfSync(
      ...
      Buffer.from("user-data-encryption"),
      32
    )
  );
}
  ...
  try {
    ...
    if (!uid || uid.length < 10) {
      return NextResponse.json(
        { error: "Valid uid parameter required" },
        { status: 400 }
      );
    }
    ...
    for (const doc of recentConvosSnap.docs) {
      ...
      const createdAt =
        ...
        (data.created_at ? new Date(data.created_at) : new Date(0));
    ...
    const goals = goalsSnap.docs.map(
      (doc: FirebaseFirestore.QueryDocumentSnapshot) => {
        ...
        return {
          ...
          checkpoints: (data.checkpoints || []).map(
            (cp: any) => cp.title || cp.description || ""
          ),
        };
      }
    );
    ...
    const notificationFrequency = userDoc.exists
      ? userDoc.data()?.mentor_notification_frequency ?? 3
      : 3;
    ...
    const notifications = rawMessages.map((msg: any) => {
      ...
      if (linkedConversations.length === 0 && allConversations.length > 0) {
        ...
        if (bestMatch) {
          linkedConversations = [
            buildConvoContext(bestMatch.data, bestMatch.id, uid),
          ];
        }
      }
