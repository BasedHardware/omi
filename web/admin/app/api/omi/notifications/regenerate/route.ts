...
const GATE_SCHEMA = {
  ...
  properties: {
    is_relevant: {
      ...
      description:
        "True ONLY if there is a specific, concrete insight the user would genuinely benefit from hearing right now. Most conversations are NOT relevant — default to false.",
    },
    relevance_score: {
      type: "number" as const,
      description:
        "0.90+: preventing a concrete mistake or time-sensitive opportunity. 0.75-0.89: non-obvious connection. 0.60-0.74: somewhat useful. Below 0.60: not worth interrupting.",
    },
    reasoning: {
      type: "string" as const,
      description:
        "What specific thing in the conversation warrants a notification. Must cite a concrete detail.",
    },
    context_summary: {
      type: "string" as const,
...
const RESULT_SCHEMA = {
  ...
  properties: {
      ...
      anyOf: [
        {
          ...
          properties: {
            notification_text: {
              type: "string" as const,
              description:
                "The advice. Max 100 chars. Start with the actionable part. No filler words.",
            },
            reasoning: {
              type: "string" as const,
              description:
            ...
            category: {
              ...
              description:
                "One of: productivity, mistake_prevention, goal_connection, dot_connecting",
            },
          },
          required: [
            "notification_text",
    ...
    context_summary: {
      type: "string" as const,
      description:
        "Brief summary of what user is discussing (1 sentence). Always provided.",
    },
    current_activity: {
...
const CRITIC_SCHEMA = {
  ...
  properties: {
    approved: {
      type: "boolean" as const,
      description:
        "True ONLY if you would genuinely want to receive this notification yourself. Most should be rejected.",
    },
    reasoning: {
      type: "string" as const,
...
async function runGate(
  ...
  const prompt = GATE_PROMPT.replace(/{user_name}/g, userName)
  ...
  const content = await invokeAdminLlmGateway({
...
async function runGenerate(
  ...
  const frequencyGuidance =
    FREQUENCY_GUIDANCE[frequency] || FREQUENCY_GUIDANCE[3];
  ...
  const content = await invokeAdminLlmGateway({
    ...
    responseFormat: {
      type: "json_schema",
      json_schema: {
        name: "proactive_notification_result",
        strict: true,
        schema: { ...RESULT_SCHEMA, additionalProperties: false },
      },
    },
    temperature: 0.3,
  });
...
async function runCritic(
  ...
  const prompt = CRITIC_PROMPT.replace(/{user_name}/g, userName)
...
async function processItem(
  ...
  // Step 1: Gate
  const gate = await runGate(userName, userFacts, goalsText, item);
  ...
  try {
    ...
    if (!prompt_template || !items || items.length === 0) {
      return NextResponse.json(
        { error: "prompt_template and items are required" },
        { status: 400 },
      );
    }
      ...
      for (const result of batchResults) {
        if (result.status === "fulfilled") {
          ...
          results.push({
            ...
            has_advice: false,
            context_summary: "Error processing",
          });
        }
      }
    }
