...
// PostHog — active mobile users per day (unique distinct_ids with any event on
// iOS/Android). Primary source: the old Mixpanel $ae_session segmentation went
// silent (returns zeros for the whole window) while PostHog keeps receiving
// mobile events, so the mobile half of the actives series flatlined at 0.
async function fetchMobileActivePerDay(
  ...
  if (apiKey && projectId) {
    const query = `
      SELECT toDate(timestamp) as day, count(DISTINCT distinct_id) as users
      FROM events
      WHERE properties.$os_name IN ('iOS', 'Android', 'iPadOS')
        AND timestamp >= now() - interval ${days} day
      GROUP BY day
      ORDER BY day
    `;
    try {
      ...
      if (response.ok) {
        ...
        console.error(
          "PostHog mobile actives returned no rows; falling back to Mixpanel"
        );
      } else {
        console.error(
          "PostHog mobile actives failed:",
          response.status,
          "falling back to Mixpanel"
        );
      }
    } catch (err) {
      console.error(
        "PostHog mobile actives exception:",
        err,
        "falling back to Mixpanel"
      );
    }
  ...
  // Legacy fallback — kept because the merged platform map (mobile uids for
  // revenue attribution) still comes from Mixpanel engage; if it works there
  // it may work here. $ae_session segmentation has been returning zeros.
