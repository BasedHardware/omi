    ...
    WITH entrant_actors AS (
      ...
      FROM (
        SELECT
          ...
          argMin(toString(properties.${ENTRY_PROPERTY_NAME}), timestamp) AS first_event_property,
      ...
      WHERE first_event_name = '${ENTRY_EVENT_NAME}'
        AND first_event_property = '${ENTRY_PROPERTY_VALUE}'
        AND first_event_at >= now() - INTERVAL ${days} DAY
    )
    SELECT
      COALESCE(person_id, distinct_id) AS actor_id,
      event,
      toString(properties.${ENTRY_PROPERTY_NAME}) AS property_value
    FROM events
    WHERE event IN (${escapedEventNames})
      AND properties.$os = 'macOS'
...
export async function computeOnboarding(days: number) {
  ...
  const body = {
    query: {
      ...
      query: withRowLimit(funnelQuery(days)),
    },
  };
  ...
  return {
    ...
    totalUsers,
    methodology:
      'First-ever entrants into the current macOS Second Brain onboarding flow, using users whose earliest recorded onboarding event is the "promise" step inside the selected window.',
    steps,
    truncated,
  };
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    const cached = await getPayload<
      Awaited<ReturnType<typeof computeOnboarding>>
    >(key);
    if (cached) {
    ...
    if (error instanceof PostHogError) {
      return NextResponse.json(
        { error: `PostHog API error: ${error.status}` },
        { status: 502 }
      );
    }
    if (
