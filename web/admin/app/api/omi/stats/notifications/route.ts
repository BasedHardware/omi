...
const MARKETPLACE_MENTOR_APP_ID =
  "omi-your-mentor-and-teacher-01JCPRSZ7FS40FHFNSJZEWR8R1";
...
export async function computeNotifications(days: number) {
  ...
  const hourlyTimeline: Record<
    string,
    { mentor: number; marketplace: number }
  > = {};
    ...
    try {
      ...
      for (const doc of mentorSnap.docs) {
        ...
        if (dayBuckets[dayKey]) {
          ...
          dayBuckets[dayKey].uniqueUsersMentor.add(uid);
        }
      ...
      for (const doc of marketplaceSnap.docs) {
        ...
        if (dayBuckets[dayKey]) {
          ...
          dayBuckets[dayKey].uniqueUsersMarketplace.add(uid);
        }
        const hk = createdAt.toISOString().slice(0, 13);
  ...
  const fetchFloatingBarCtr = async () => {
    ...
    const buildCtr = (
      ...
      const dailyData = dateKeys.map((date) => {
        ...
        return {
          ...
          ctr:
            row.sent > 0 ? Math.round((row.clicked / row.sent) * 1000) / 10 : 0,
        };
      });

      const [sent = 0, clicked = 0, dismissed = 0, uniqueClickers = 0] =
        summaryRow ?? [];
      return {
        dailyData,
        summary: {
    ...
    const surfaceDailyRows: [string, number, number, number][] =
      surfaceDailyResult?.results ?? [];
    const surfaceSummaryRow: [number, number, number, number] | undefined =
      surfaceSummaryResult?.results?.[0];
    const surfaceCtr = buildCtr(
      surfaceDailyRows,
      surfaceSummaryRow,
      "surface_tagged"
    );
    ...
    const [fallbackDailyResult, fallbackSummaryResult] = await Promise.all([
      queryPostHog(
        ...
        posthogProjectId,
        posthogApiKey,
        fallbackDailyCtrQuery
      ),
      queryPostHog(
        posthogHost,
        posthogProjectId,
        posthogApiKey,
        fallbackSummaryQuery
      ),
    ]);

    const fallbackDailyRows: [string, number, number, number][] =
      fallbackDailyResult?.results ?? [];
    const fallbackSummaryRow: [number, number, number, number] | undefined =
      fallbackSummaryResult?.results?.[0];
    return buildCtr(
      fallbackDailyRows,
      fallbackSummaryRow,
      "all_notifications_fallback"
    );
  };
  ...
  for (const key of dateKeys) {
    ...
    weekBuckets[weekKey].marketplaceMentorSent +=
      dayBuckets[key].marketplaceMentorSent;
    dayBuckets[key].uniqueUsersMentor.forEach((u) =>
      weekBuckets[weekKey].uniqueUsersMentor.add(u)
    );
    dayBuckets[key].uniqueUsersMarketplace.forEach((u) =>
      weekBuckets[weekKey].uniqueUsersMarketplace.add(u)
    );
  }

  const weeklyData = Object.keys(weekBuckets)
    .sort()
    .map((weekKey) => ({
      week: weekKey,
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const cached = await getPayload<
      Awaited<ReturnType<typeof computeNotifications>>
    >(key);
    if (cached) {
