...
export function HeaderLinks() {
  ...
  // Calculate right offset based on which panels are open
  const rightOffset = 12 + (isChatOpen ? 404 : 0) + (isNotificationOpen ? 404 : 0);

  return (
    <motion.div
      animate={{ right: rightOffset }}
      transition={{ type: 'spring', stiffness: 300, damping: 30 }}
      className="fixed top-3 z-[9998] flex items-center gap-2"
    >
      <a
