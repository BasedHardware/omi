...
import { storeKey, importKey, computeKeyHash, storeKeyHash } from '@/lib/e2ee';
      ...
      <div className="mx-4 w-full max-w-md rounded-2xl bg-zinc-900 p-6">
        ...
        <div className="mb-4 flex rounded-lg bg-zinc-800 p-1">
          <button
            ...
            className={`flex-1 rounded-md py-2 text-sm font-medium transition ${mode === 'scan' ? 'bg-zinc-700 text-white' : 'text-zinc-400 hover:text-white'}`}
          >
            📷 Scan QR
          </button>
          <button
            onClick={() => { stopScanning(); setMode('paste'); }}
            className={`flex-1 rounded-md py-2 text-sm font-medium transition ${mode === 'paste' ? 'bg-zinc-700 text-white' : 'text-zinc-400 hover:text-white'}`}
          >
            📋 Paste Key
          </button>
        </div>
        ...
        {mode === 'scan' ? (
          ...
          <div>
            <p className="mb-3 text-sm text-zinc-400">Paste your recovery key from the Omi app</p>
            <input
              type="password"
              value={pasteValue}
        ...
        {error && (
          <div className="mt-3 rounded-lg bg-red-500/10 px-4 py-2 text-sm text-red-400">{error}</div>
        )}

        <p className="mt-4 text-center text-xs text-zinc-600">
          Your key stays in this browser and is never sent to the server
        </p>
      </div>
    </div>
  );
