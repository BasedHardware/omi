...
export function ConversationDetail({ conversation, userName, onConversationUpdate }: ConversationDetailProps) {
  ...
  const getInitialTab = (): TabId => {
    if (structured.overview) return 'summary';
    if (hasActionItems) return 'actions';
    ...
    <div className="max-w-4xl mx-auto">
      ...
      >
        <div className="flex items-start gap-4">
          {/* Emoji */}
          <div className="w-16 h-16 rounded-2xl bg-bg-tertiary flex items-center justify-center text-3xl flex-shrink-0">
            {structured.emoji || '💬'}
          </div>
      ...
      {enabledTabs.length > 0 && (
        ...
        >
          {/* Tab buttons */}
          <div className="flex gap-1 p-1 mb-6 rounded-xl bg-bg-secondary border border-bg-tertiary">
            {enabledTabs.map((tab) => (
              <button
                key={tab.id}
      ...
      {enabledTabs.length === 0 && (
        <div className="text-center py-12 text-text-tertiary">
          ...
          <p>No content available for this conversation</p>
        </div>
      )}
    </div>
  );
