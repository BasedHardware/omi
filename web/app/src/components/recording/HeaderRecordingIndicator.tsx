...
const BYPASS_MOBILE_CHECK_PATHS = ['/record/popout', '/record/popout/transcript', '/apps'];
const MOBILE_DISMISSED_KEY = 'omi_mobile_overlay_dismissed';
...
export function HeaderRecordingIndicator() {
  ...
  const [isMobileBlocked, setIsMobileBlocked] = useState(true);
  const dropdownRef = useRef<HTMLDivElement>(null);

  // Check if mobile overlay should be blocking
  useEffect(() => {
    ...
    const checkMobileState = () => {
      ...
      const isDismissed = sessionStorage.getItem(MOBILE_DISMISSED_KEY) === 'true';
      setIsMobileBlocked(isMobile && !isDismissed);
    };

    checkMobileState();
    window.addEventListener('resize', checkMobileState);
    window.addEventListener('storage', checkMobileState);
    return () => {
      window.removeEventListener('resize', checkMobileState);
      window.removeEventListener('storage', checkMobileState);
    };
  }, [pathname]);
  ...
  // Hide when mobile overlay is showing (only in idle state - keep visible during active recording)
  if (isMobileBlocked && isIdle) {
    return null;
  }
              ...
              {showModeSelector && (
                ...
                >
                  ...
                  <div className="space-y-2 mb-4">
                    ...
                    {/* Check if system audio (display media) is supported */}
                    {typeof navigator !== 'undefined' && navigator.mediaDevices && 'getDisplayMedia' in navigator.mediaDevices ? (
                      ...
                      <div className="w-full flex items-center gap-3 p-3 rounded-lg bg-bg-tertiary/50 border border-transparent opacity-60 cursor-not-allowed">
                        <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-bg-secondary">
                          <Monitor className="w-4 h-4 text-text-quaternary" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] bg-bg-quaternary px-1.5 py-0.5 rounded text-text-quaternary">
                              Not Supported
                            </span>
                          </div>
                          <p className="text-xs text-text-quaternary">
                            Browser doesn&apos;t support system audio
                          </p>
                        </div>
                      </div>
                    )}
