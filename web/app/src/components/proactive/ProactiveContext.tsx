...
export function ProactiveProvider({ children }: { children: ReactNode }) {
    ...
    // Env API Key logic removed - server side only
    // const envApiKey = process.env.NEXT_PUBLIC_GEMINI_API_KEY || null;
    // const activeApiKey = settings.apiKey || envApiKey || '';
    ...
    const updateSettings = useCallback((updates: Partial<ProactiveSettings> | ((prev: ProactiveSettings) => Partial<ProactiveSettings>)) => {
        setSettings((prev) => {
            ...
            // Handle nested updates if needed, but for now simple merge
            // Deep merge might be safer for nested objects:
            const updated = { ...prev, ...newUpdates };

            // If explicit sub-objects passed, they replace. We should probably accept partials of sub-objects.
            // But for simplicity, let's assume the caller handles deep merging if updating a sub-object
            // Or we can manually spread active/focus objects if they are in updates
