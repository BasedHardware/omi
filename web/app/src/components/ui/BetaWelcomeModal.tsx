...
export function BetaWelcomeModal({ onDismiss }: BetaWelcomeModalProps) {
  ...
  const handleClose = () => {
    ...
    const frame = () => {
      ...
      confetti({
        ...
        spread: 55,
        origin: { x: 0, y: 0.6 },
        colors: ['#FFFFFF', '#E5E5E5', '#B0B0B0', '#737373'],
      });
      // Right side
      confetti({
        particleCount: 2,
  ...
  return (
    ...
    >
      {/* Modal. The enter/exit offsets are `y` and `scale` numbers, not a
          ...
          screen-filling backdrop. `relative` keeps the close button in the
          dialog's corner, and the height cap keeps the button reachable on a
          phone. */}
      <motion.div
        initial={{
          opacity: 0,
