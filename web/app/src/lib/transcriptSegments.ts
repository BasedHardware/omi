 ...
 */
export function upsertTranscriptSegment<T extends { id: string }>(
  segments: readonly T[],
  segment: T,
): T[] {
  const existingIndex = segments.findIndex((s) => s.id === segment.id);
  if (existingIndex >= 0) {
    if (segments[existingIndex] === segment) return segments as T[];
...
export function applyLiveTranscriptSegment<T extends { id: string }>(
  segments: readonly T[],
  segment: T,
