...
export function useConversations(
  ...
  // Track previous date params to detect changes
  ...
  // Initial fetch and refetch when date params change
  useEffect(() => {
    ...
    // Fetch if dates changed or this is initial load
    if (startDateChanged || endDateChanged || conversations.length === 0) {
      setOffset(0);
      setHasMore(true);
      fetchConversations(0, false);
    }
  }, [params.startDate, params.endDate, fetchConversations]);
