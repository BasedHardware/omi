...
- **Formula injection:** Values starting with `=`, `+`, `-`, or `@` are prefixed with `'` so spreadsheet applications render them as plain text instead of executing them as formulas.
