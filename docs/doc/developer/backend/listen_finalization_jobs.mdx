...
* Eligible: `requires_byok`, status `queued` or `leased` with an expired lease,
  ...
  `LISTEN_FINALIZATION_BYOK_ABANDONED_SECONDS` (default seven days, clamped to a
  one-day floor and a 90-day ceiling). A row whose age cannot be established is
...
`false` in every environment. Outcomes surface on
