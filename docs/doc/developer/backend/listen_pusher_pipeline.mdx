...
> Last updated: 2026-03-24 (speech profile sharing: shared speaker IDs)
>
...
sequenceDiagram
    ...
    Backend->>Firestore: Load active shared owners for receiver
    Backend->>Backend: Add shared embeddings as person_id = "shared:{owner_uid}"
    ...
    Backend-->>Client: {type: "speaker_label_suggestion", person_id, person_name, distance}
    Note over Backend: shared:{owner_uid} assignments are not forwarded to pusher speaker-sample extraction
```

### Shared Profile Revoke Semantics

- Share/revoke/remove is persisted in Firestore under `users/{owner_uid}/shared_speech_profiles/{target_uid}`.
- `/v4/listen` loads shared embeddings once at session start.
- Current behavior: revoke/remove does not retroactively evict already-loaded shared embeddings from an active session; effect is guaranteed on next session start.
