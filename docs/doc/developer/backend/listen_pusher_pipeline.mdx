...
`RECORDING_SESSION_MODE=enforce` (the default) makes the durable binding
authoritative. `shadow` records and reports mapping conflicts through the shared
fallback telemetry while preserving the old proposed route during a bounded
rollout. The resource contains only IDs, phase, sequence, schema version, and
