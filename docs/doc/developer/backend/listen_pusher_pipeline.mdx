...
> Last updated: 2026-05-21 (AssemblyAI background STT MVP)
>
...
selects Deepgram by default and can select AssemblyAI only when
`ASSEMBLYAI_BACKGROUND_STT_ENABLED=true` and the workload is listed in
`ASSEMBLYAI_BACKGROUND_STT_WORKLOADS`.
...
sequenceDiagram
    ...
    alt AssemblyAI fails, times out, or exhausts retries for sync/postprocess
    ...
    Note over ProviderService: AssemblyAI background workload fails closed;<br/>it does not fall back to Deepgram during desktop cloud batch.
