...
- It is an async coordinator, driven on a shared daemon event-loop thread rather than a
  ...
  and starve memory extraction, action items or vector indexing.
...
- Verified assignments are written through `update_conversation_segments`. The refuted and
  ...
  `speaker_label_suggestions` (`{speaker_id, person_name, evidence_quote, confidence,
  segment_ids}`). This is not the live `speaker_label_suggestion` WebSocket event — the
  listen session is already gone by finalization.
