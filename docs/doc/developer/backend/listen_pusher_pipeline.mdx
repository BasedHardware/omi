...
For cloud batch, the desktop client buffers roughly 15s PCM chunks with 0.5s
overlap, drops chunks that do not contain sustained speech activity before
uploading, and uses backpressure instead of stopping recording when AssemblyAI
is slower than realtime. AssemblyAI batch requests use the selected single
language rather than `multi` to avoid provider-side language-detection failures
on low-speech audio.
