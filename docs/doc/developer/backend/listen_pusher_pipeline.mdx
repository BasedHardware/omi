...
> Last updated: 2026-07-14 (durable recording-session bindings and ordered lifecycle envelopes)
>
...
`RECORDING_SESSION_MODE=dual_write` is the safe default: the durable binding
is returned while the legacy pointer continues to be maintained. `shadow`
records and reports mapping conflicts through shared fallback telemetry while
preserving the old proposed route; `enforce` rejects durable-store failures.
...
bind a new conversation without mutating the terminal generation.
