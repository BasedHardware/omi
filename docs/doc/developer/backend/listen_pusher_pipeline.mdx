...
- The pass is started by `schedule_speaker_resolution` *after* the private-cloud
  ...
  there are none.
- It is an async coordinator, driven on a shared daemon event-loop thread rather than a
  `postprocess_executor` worker. Each blocking step inside borrows a db/llm/storage thread
  for that step alone, so a burst of finalized conversations cannot pin postprocess slots
  and starve memory extraction, action items or vector indexing. Each pass is a tracked
  task (`utils.async_tasks.create_named_task`) and `drain_speaker_resolution` — registered
  with `atexit`, like `shutdown_executors` — waits it out at process exit, so a shutdown
  cannot abandon a pass midway through creating a Person or writing segments.
