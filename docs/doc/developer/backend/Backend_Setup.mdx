...
<Steps>
  ...
  <Step title="Authenticate with Google Cloud" icon="key">
    ...
    keys are the #6800 anti-pattern. Prefer user ADC for local work, or impersonate a
    lower-privilege SA when you need production-adjacent roles:

    ```bash
    gcloud auth application-default login \
      --impersonate-service-account=dev-readonly@<project-id>.iam.gserviceaccount.com
    ```
...
<Tabs>
  <Tab title="Required" icon="asterisk">
    ...
    | `GOOGLE_APPLICATION_CREDENTIALS` | Optional. Leave unset to use ADC from `gcloud auth application-default login`. Do not point this at a long-lived service-account JSON key (#6800). |
