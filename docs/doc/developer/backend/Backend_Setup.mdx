...
<Card title="Required Services" icon="key">
  ...
  | Modulate | Speech-to-text fallback | Provision `MODULATE_API_KEY` |
  | Parakeet | Self-hosted speech-to-text | Configure `HOSTED_PARAKEET_API_URL` |
...
<AccordionGroup>
  <Accordion title="Pusher Service (for webhooks)" icon="bell">
    ...
    The pusher imports the same `database`/`utils` modules as the main backend
    (`database.redis_db` reads `REDIS_DB_HOST` at import time), and
    `uvicorn --env-file` loads exactly one file, so run it against `backend/.env`
    rather than `pusher/.env`, which only carries `SERVICE_ACCOUNT_JSON`.

    Firebase initializes from the `GOOGLE_APPLICATION_CREDENTIALS` file you created
    in step 1. To use an inline service account instead, add a
    `SERVICE_ACCOUNT_JSON=<credentials JSON>` line to `backend/.env`.

    Start the service from `backend/` (port 8080 matches the deployed pusher image
    and leaves 8000 free for the main backend):
    ```bash
    uvicorn pusher.main:app --reload --env-file .env --port 8080
    ```
...
<Tabs>
  <Tab title="Required" icon="asterisk">
    ...
    | `MODULATE_API_KEY` | Modulate API key for transcription fallback |
    | `HOSTED_PARAKEET_API_URL` | Parakeet endpoint for transcription |
  ...
  <Tab title="Optional" icon="puzzle-piece">
    ...
    | `HUGGINGFACE_TOKEN` | Hugging Face token for the pyannote models. Read only by the self-hosted `backend/diarizer`, `backend/parakeet` and `backend/modal` services, never by the API backend — which is why it is absent from `.env.template` and present in `.env.dev.template`. |
  </Tab>
</Tabs>
