...
Cloud Run env and secret bindings are declared in `backend/deploy/runtime_env.yaml` and rendered by `backend/scripts/render-backend-runtime-env.py`. After changing the manifest, run `./scripts/pre-deploy-check.sh`.
