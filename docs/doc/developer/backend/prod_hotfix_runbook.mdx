...
The dev `backend`, `backend-sync`, and `backend-integration` services own their routed pre-recorded STT selection in `backend/deploy/runtime_env.yaml`. When Parakeet is selected, they must use the isolated dev endpoint `http://parakeet.omiapi.com`—never the production `parakeet.omi.me` endpoint. The runtime validator rejects a Parakeet selection without `HOSTED_PARAKEET_API_URL` for rendered and live Cloud Run revisions.
