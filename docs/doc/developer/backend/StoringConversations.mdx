...
Live capture retains fresh unmerged input until its transcript transaction reads
the current protected manual receipt. Selected IDs and speaker-wide defaults
block every concatenation and partial sentence transfer involving covered speech,
including clears and training opt-outs. The accepted transcript and deleted-ID
delta return together for client delivery. This also protects older receipts.
