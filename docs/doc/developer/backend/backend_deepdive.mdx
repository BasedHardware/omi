...
10. **Processing the Memory (`utils/memories/process_memory.py`):**
    ...
    - **Storage in Firestore:** The fully processed memory, including the transcript, structured data, app results, and other metadata, is stored in Firebase Firestore (a NoSQL database) for
      persistence.
...
- `enable_app`, `disable_app`, `get_enabled_apps`: For handling app enable/disable states.
- `get_app_reviews`: Retrieves reviews for a app.
