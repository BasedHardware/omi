...
<Warning>
The Deepgram-specific sections in this historical architecture deep dive are retired. Serving transcription uses Parakeet and Modulate only; see [Transcription](/doc/developer/backend/transcription) for the current provider policy.
</Warning>
...
- The LLM modules use a provider-agnostic model system (see `utils/llm/model_config.py`) with models from OpenAI (`gpt-5.4`, `gpt-5.4-mini`, `gpt-4.1-mini`), Anthropic (`claude-sonnet-4-6`), and others, selected per task via QoS profiles (`premium`, `max`, `fair_use`).
...
communication with the Omi app and Deepgram for accurate and efficient transcription.
...
Omi uses Deepgram as its Speech-to-Text (STT) service:

- **Deepgram** (Nova-3) - Primary service for all languages

The backend uses `process_audio_dg()` in `utils/stt/streaming.py` for all transcription.
...
2. **Backend Receives Audio:** The backend's `websocket_endpoint` function receives the audio chunks.
3. **Backend Forwards to STT:** The backend immediately forwards audio chunks to Deepgram using `process_audio_dg`.
...
async def websocket_endpoint(websocket: WebSocket, uid: str, language: str = 'en', ...):
    ...
    # Start STT transcription via Deepgram
