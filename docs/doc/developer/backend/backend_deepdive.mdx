...
- The LLM modules use a provider-agnostic model system (see `utils/llm/model_config.py`) with models from OpenAI (`gpt-4.1-mini`, `gpt-5.1`), Anthropic (`claude-sonnet-4-6`), and others, selected per task via QoS profiles.
