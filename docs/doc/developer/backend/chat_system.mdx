...
<Note>
Models are selected via QoS profiles (`premium`, `max`, `fair_use`, `byok`) in `backend/utils/llm/model_config.py`. Managed desktop chat is overridden after legacy profile resolution by `generated_route_overrides.yaml` and uses the OpenAI-compatible Luna route. The `omi-sonnet` and `claude-sonnet-4-6` values remain compatibility inputs, not the active managed provider. The `fair_use` profile uses `gpt-5.1` as a fallback. See the config files for the current authoritative set.
</Note>
