...
| `claude-sonnet-4-6` | Chat agent (Anthropic) | `chat_agent` profile |
| `gemini-2.5-flash-lite` | Session titles, followups (Gemini) | Light tasks |
| `text-embedding-3-large` | Vector embeddings (3,072 dims) | Pinecone queries |

<Note>
Models are selected via QoS profiles in `backend/utils/llm/model_config.py`. Premium users and BYOK accounts may route to different models. See the config file for the current set.
</Note>
