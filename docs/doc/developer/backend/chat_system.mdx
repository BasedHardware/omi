...
| `gpt-5.6-luna` | Managed desktop chat agent | `omi:auto:chat-agent` gateway lane |
...
<Note>
Models are selected via QoS profiles (`premium`, `max`, `fair_use`, `byok`) in `backend/utils/llm/model_config.py`. Managed desktop chat is classified by the `chat_agent` feature lane and its capability contract after legacy profile resolution. The `omi-sonnet` and `claude-sonnet-4-6` values remain compatibility inputs for released clients, not the active managed provider. The `fair_use` profile uses `gpt-5.1` as a fallback. See the config files for the current authoritative set.
</Note>
