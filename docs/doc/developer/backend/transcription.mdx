...
policy; an environment edit cannot re-enable a disabled provider. The first
configured model that supports the requested language wins. Serving defaults are
`parakeet,modulate-velma-2`; retired or unknown model tokens are ignored. If
neither supported provider can serve a language, the request fails closed.
...
provider depends on language capability. `DEEPGRAM_*` settings are not part of
