...
Errors: `413` audio exceeds the duration/size limits, `503` model loading or service busy (retry later), `502` upstream failure. Rate limited per user (`stt:transcribe` policy). Implementation: `backend/routers/stt.py`.
