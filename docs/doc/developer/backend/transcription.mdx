...
policy. The live default is `parakeet,modulate-velma-2,dg-nova-3`:
single-channel sessions in the 25 TDT v3 languages, including default multilingual sessions, use Parakeet first, with Modulate and
hosted Deepgram available for connection and mid-session fallback. Unsupported
languages and multi-channel sessions retain a supported vendor route. PTT uses
`parakeet,modulate-velma-2`; batch retains that same order with different model
capabilities. Supported explicit-provider and BYOK requests retain their existing policy.
Self-hosted Deepgram is a distinct,
...
| **Deepgram cloud** | Model-dependent | `dg-nova-3` | Live streaming fallback |

Parakeet realtime uses `HOSTED_PARAKEET_STREAM_API_URL`; batch uses
`HOSTED_PARAKEET_API_URL`. The realtime service runs in `PARAKEET_SERVICE_MODE=stream`
on dedicated GPU capacity, with model warmup readiness and per-pod admission.
The deployment must provision and verify this capacity before publishing the
primary routing configuration. See the [implementation and qualification
record](https://github.com/BasedHardware/omi/tree/main/backend/docs/plans/parakeet-primary).
