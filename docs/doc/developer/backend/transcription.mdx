...
The selected streaming provider comes from the ordered `STT_SERVICE_MODELS`
...
flowchart TD
    ...
    Lang -->|Supported by first configured model| Selected[Deepgram / Modulate / Parakeet]
    Lang -->|Unsupported| Fallback[Next configured model]
```
...
|----------|-----------|-------|----------|
| **Deepgram** | 50+ | `nova-3` | Broad language coverage |
...
The manual production backend workflow deploys a no-traffic, uniquely tagged
`backend` candidate and proves the real
`/v2/voice-message/transcribe` route before shifting any Cloud Run traffic. The
tag resolver requires the exact candidate revision recorded in Cloud Run
`status.traffic`; it cannot accidentally exercise the current serving revision
or the latest-created revision. The full-route probe requires HTTP 200,
`outcome=success`, the exact normalized known-audio phrase, and the configured
provider/model identity. It emits only redacted booleans and status codes.

Configure these protected production-environment values before the first gated
deploy; missing configuration fails the deploy before promotion:

| Value | GitHub protection | Purpose |
| --- | --- | --- |
| `OMI_TRANSCRIPTION_SYNTHETIC_AUDIO_URL` | Secret | Protected known-audio fixture URL |
| `OMI_TRANSCRIPTION_SYNTHETIC_BEARER_TOKEN` | Secret | Test account bearer token |
| `OMI_TRANSCRIPTION_SYNTHETIC_EXPECTED_PHRASE` | Secret | Exact expected normalized text |
| `OMI_TRANSCRIPTION_SYNTHETIC_LANGUAGE` | Environment variable | Language submitted in the real multipart request |
| `OMI_TRANSCRIPTION_SYNTHETIC_EXPECTED_PROVIDER` | Environment variable | Expected selected provider |
| `OMI_TRANSCRIPTION_SYNTHETIC_EXPECTED_MODEL` | Environment variable | Expected selected model |

The Cloud Run candidate itself makes the private VPC hop to Parakeet. A direct
Parakeet probe remains an optional diagnostic for a VPC-connected runner; it is
not the production promotion authority. After a passing or failed candidate
gate, the workflow removes its temporary tag; a failed gate never shifts
traffic.
