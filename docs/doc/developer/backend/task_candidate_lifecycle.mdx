...
description: Universal backend contracts, resolution rules, and released-client compatibility for smart tasks.
---
...
indexes and wait for readiness before retrying deploy. Generated single-field exemptions are intentionally not part
of that create-only operation: the workflow's automatic lane detects unapplied exemptions and fails with remediation.
To apply them to the shared production data plane, dispatch the same workflow on `main` with environment `prod`,
operation `field-exemptions`, and `APPLY_FIRESTORE_FIELD_EXEMPTIONS`; the dedicated reconciler previews the exact
fields, disables only those automatic indexes, and verifies the live field configs afterward. The development verification job then calls
