...
SHA and the two signed artifact digests. Automatic Beta promotion admits only
that successful tag-bound qualification run, registers one immutable manifest,
and advances the Beta pointer. The manifest retains the exact asset URLs and
digests, Sparkle signature, qualification evidence identity/digest, source
SHA, and compatibility/backend requirements.
