...
SHA and the two signed artifact digests. It attaches that evidence as the
canonical `qualification-evidence-<tag>.json` release asset. The Beta workflow
...
release, immutable tag, trusted completed qualification run, evidence, signed
assets, and GitHub SHA-256s before it atomically registers one immutable
