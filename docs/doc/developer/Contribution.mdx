...
defines constructor seams, session generations and the single owners of recovery,
foreground service and socket lifecycle. New singleton wakes and unowned capture
resources fail a changed-file ratchet; its diagnostic names the request interface
to use. Run `TZ=UTC bash app/test.sh` for the protected contracts. Pending tests
