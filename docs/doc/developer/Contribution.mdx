...
terminal outcomes. Migrate one API file and its consumers at a time; new nullable
HTTP calls or null-swallowing patterns fail the changed-file ratchet. The protected
contracts run in `TZ=UTC bash app/test.sh`, using synthetic loopback fixtures.
