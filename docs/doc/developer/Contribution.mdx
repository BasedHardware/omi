...
<Steps>
  ...
  <Step title="Test Your Changes">
    ...
    `make preflight`, then run `scripts/pr-preflight --pr-body-file /path/to/draft-pr-body.md` when invariant citations apply (use `scripts/pr-preflight --suggest` to get the paste-ready invariant ID block). The manifest-backed checks are identical locally and in CI.
  </Step>
