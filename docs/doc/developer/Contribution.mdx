...
The [typed API result contract](https://github.com/BasedHardware/Omi/blob/main/app/lib/backend/http/API_RESULTS.md)
keeps transport/auth failures distinct from empty data and defines locked and
terminal outcomes. Migrate endpoint families with their direct consumers; batch
small files and split large ones. Declare completed file/rule adoption in
`app/contracts/api-result/boundary-baseline.json`. Only adopted files and new files
are constrained; ordinary legacy-file growth stays unblocked. Protected contracts
run in `TZ=UTC bash app/test.sh`, using synthetic loopback fixtures.
