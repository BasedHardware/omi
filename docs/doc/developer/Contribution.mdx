...
- **Bug fixes** should include tests that would have caught the bug or give high confidence it will not regress.
- **Small features** can go straight to PR when they are well scoped, include useful automated tests, and describe any manual testing performed.
- **Large or ambiguous features** should start as a GitHub issue so maintainers can discuss scope and direction before major implementation work begins.
- Prefer unit, widget, provider, and hermetic tests that run reliably in CI. Live-service integration tests are useful for validation, but should not be the only proof that a PR works.
