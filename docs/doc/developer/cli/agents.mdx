...
agents. The browser OAuth flow exists for humans on a laptop, uses a
loopback callback with PKCE, and stores short-lived Firebase ID tokens that
auto-refresh between calls.
