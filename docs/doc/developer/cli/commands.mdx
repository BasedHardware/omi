...
├── memory        list | export | get | create | update | delete
...
| `omi --json memory export [--max-items N --categories CSV]` | paginated `GET /v1/dev/user/memories` |
...
```bash
omi --json memory export --max-items 10000 > memories.json
omi --json memory export --categories work,skills > work-memories.json
```

Requires the global `--json` flag. Retrieves pages of up to 1,000 records and emits
one array with all returned fields. `--max-items` defaults to 10,000 and accepts
1–100,000. Exceeding it is an error, not a successful truncated export. API errors
also leave stdout empty. Records are buffered in memory until every page succeeds.
Use a new output filename: shell redirection truncates existing files even if the
command fails.

The result covers the Developer API's visible memory view, not a full account
backup. Offset pagination can miss or repeat records during concurrent changes.
Category filtering currently scans at most 5,000 underlying memories on the
server. Conversations, recordings, and archived data are outside this export.
...
| `omi action-item update ID [--description ... --completed/--open --due-at ... --clear-due-at]` | `PATCH /v1/dev/user/action-items/{id}` |
