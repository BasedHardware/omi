...
`omi conversation list` supports the same read filters as the Developer API: `--limit`, `--offset`, `--start-date`, `--end-date`, `--categories`, `--folder-id`, `--starred`, `--not-starred`, and `--include-transcript`.
