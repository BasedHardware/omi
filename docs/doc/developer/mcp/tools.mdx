...
| `get_screen_activity` | `start_date?`, `end_date?`, `app?` | Deprecated: returns a fixed message that screen activity is no longer available from cloud chat; captures stay on the user's computer in the Omi desktop app (Rewind) |
| `get_daily_summaries` | `start_date?`, `end_date?`, `limit?` (default 30), `offset?` | List Omi's daily summaries, newest first |

Dates use `YYYY-MM-DD`.

Screen activity is local-only: cloud chat never queries it. The deprecated
`get_screen_activity` tool exists only so existing clients get an explicit
unavailable message instead of an error, and it never returns screen rows.
