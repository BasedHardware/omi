...
| `create_memory`, `create_memories` | Create one memory, or up to 25 in a bounded batch |
...
| `get_conversation_by_id`, `get_conversations_by_ids` | Get full conversations with transcripts — one, or up to 20 in a bounded batch |
...
| `rename_person` | Correct a recognized person's display name while retaining the old name as an alias |
| `dismiss_person` | Soft-dismiss a false-positive person from normal People reads |
