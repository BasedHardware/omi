...
<Tabs>
  <Tab title="Claude Desktop / Claude Code" icon="robot">
    Claude's cloud connector supports OAuth as described above. For a manual-key
    fallback, current Claude Desktop builds that support remote MCP servers
    accept a hosted HTTP entry in `claude_desktop_config.json`:
    ...
    **Config file location:**
    ...
    <Note>
    Older Claude Desktop builds only support local stdio servers — use the
    [local stdio fallback](#local-stdio-server-with-uvx-or-python-deprecated)
    there, or the claude.ai OAuth connector flow instead.
    </Note>
