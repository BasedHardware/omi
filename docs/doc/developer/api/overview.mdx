...
<CardGroup cols={4}>
  ...
  <Card title="E2EE at Rest" icon="lock" color="#ef4444" href="/doc/developer/api/e2ee">
...
<AccordionGroup>
  <Accordion title="HTTP Status Codes" icon="circle-exclamation" defaultOpen={true}>
    ...
    | `403 Forbidden` | API key doesn't have required permissions, or [E2EE key hash](/doc/developer/api/e2ee) required |
...
<Info>
If a user has **E2EE at rest** enabled, data endpoints return `403` unless you include the key hash. See the [E2EE documentation](/doc/developer/api/e2ee) for details.
</Info>
