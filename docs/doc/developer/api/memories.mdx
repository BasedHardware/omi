...
<Card title="GET /v1/dev/user/memories/{memory_id}" icon="download" color="#22c55e" horizontal>
  Retrieve a single memory by ID
</Card>

Returns the same memory shape as the list endpoint. Returns `404` when the
memory does not exist (or cannot be represented in the current shape).

<Tabs>
  <Tab title="cURL">
    ```bash
    curl -H "Authorization: Bearer $API_KEY" \
      "https://api.omi.me/v1/dev/user/memories/mem_789ghi"
    ```
  </Tab>
  <Tab title="Python">
    ```python
...
<AccordionGroup>
  ...
  <Accordion title="Duplicates and retries" icon="rotate" defaultOpen={true}>
    A memory is identified by its text. Sending text the user already has returns that memory unchanged, with the same `id`, so a create is safe to retry without any extra header.

    | Request | Result |
    |---------|--------|
    | Text the user does not have yet | A new memory |
    | Text the user already has | `200` with the existing memory; nothing new is stored |
    | Same text with different `category`, `visibility`, or `tags` | `200` with the existing memory as stored; the new fields are **not** applied |
    | Same text after the memory was deleted | A new memory with a new `id` |

    Matching ignores leading and trailing whitespace and is case-sensitive. To change a memory you already created, use [Update Memory](#update-memory) with its `id`.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Request Body" icon="code" defaultOpen={true}>
    ...
    Items follow the same duplicate rule: text the user already has returns the existing memory at that position, so retrying a batch never stores anything twice. `created_count` is the number of memories returned, including existing ones.
  </Accordion>
</AccordionGroup>
