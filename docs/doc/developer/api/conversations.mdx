...
<AccordionGroup>
  <Accordion title="Query Parameters" icon="sliders" defaultOpen={true}>
    ...
    | `folder_id` | string | - | Filter by folder ID. Each conversation belongs to at most one folder, so this is a single-value filter. Must be a non-empty string if provided. Use `GET /v1/dev/user/folders` to retrieve folder IDs. |
