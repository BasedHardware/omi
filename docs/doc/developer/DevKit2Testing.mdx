...
<Info>
**Prerequisites:** Python packages from `requirements.txt`, plus `ffmpeg` and `opus` for audio decoding. The test scripts live in [`omi/firmware/scripts/devkit/`](https://github.com/BasedHardware/omi/tree/main/omi/firmware/scripts/devkit).
</Info>
...
<Steps>
  ...
  <Step title="Check File Sizes" icon="list">
    Run `get_info_list.py` — replace the `device_id` value with your device ID. This lists the size of each audio file on the SD card.
  </Step>
  <Step title="Download Audio" icon="download">
    Run `get_audio_file.py` — replace the `device_id` value with your device ID. Audio data streams to `my_file.txt` in the current directory.
  </Step>
  ...
  <Step title="(Optional) Delete Audio" icon="trash">
    Run `delete_audio_files.py` — replace the `device_id` value with your device ID. This clears audio data from the specified file.
  </Step>
</Steps>
...
| `2` | Double tap |
| `6` | Triple tap |
| `5` | Button released (follows every tap gesture, and any hold shorter than the power-off long press) |
| — | Long press (~3 seconds) powers the device off; nothing is sent |

Codes `3` (long press) and `4` (button pressed down) are reserved for older firmware and are no longer emitted.
Taps must be shorter than 300 ms and separated by less than 350 ms to count as one gesture; single and double taps are
reported once that window closes, triple taps on the third release. The recogniser lives in
`omi/firmware/common/button_gesture.c` and is host-tested by `omi/firmware/common/test/run.sh`.

In the app, single/double/triple tap each run the action configured under Settings → Device Settings (Ask a Question, Mute/Unmute,
End Conversation, Star Conversation, or Do Nothing). The long-press power-off gesture is fixed in firmware.
