...
The checked-in project config uses maintained provider endpoints for GitHub, Notion, and Figma. Export `GITHUB_TOKEN` before starting Cursor to enable GitHub; Cursor completes the provider OAuth flow for Notion and Figma when first used. Browser automation runs Microsoft's Playwright MCP package at the exact version recorded in the config. Do not replace these entries with guessed `@modelcontextprotocol/server-*` names or floating npm versions.
