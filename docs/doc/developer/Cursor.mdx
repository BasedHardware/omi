...
Every entry runs a published npm package pinned to an exact version, so contributor setup never executes a floating dependency. Export `NOTION_TOKEN` and `FIGMA_ACCESS_TOKEN` before starting Cursor to enable those servers; browser automation needs no credentials and is given none. A repository contract (`.github/scripts/check_cursor_mcp_config.py`) enforces this: it rejects the four `@modelcontextprotocol/server-*` names that were never published, any unpinned or floating npm spec, a plaintext credential where a `${env:NAME}` reference belongs, and credentials forwarded into the browser server. Bump a version deliberately rather than reaching for `@latest`.
