...
8. **Update `pi-mono-adapter.test.ts`**
   ...
   - pi-mono `callSwiftTool` serializes request/client/run/attempt correlation from `OMI_*` environment variables or the active context channel onto the daemon relay message.
