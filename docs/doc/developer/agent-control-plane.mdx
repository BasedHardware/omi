...
The Agent VM REST surface is intentionally narrower than the backend's internal tool registry. `GET /v1/agent/tools` omits `fetch_url_tool`, and `POST /v1/agent/execute-tool` rejects it with `404`; clients must treat that absence and rejection as the stable removal contract. Agent VMs must not use a cloud execution path for URLs or content that a desktop user supplied for local-only handling; URL fetching remains available only through the current-turn, user-provided allowlist enforced by the backend agent path.
