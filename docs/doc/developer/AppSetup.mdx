...
<Steps>
  ...
  <Step title="Setup Firebase" icon="fire">
    ...
    ([#9404](https://github.com/BasedHardware/omi/issues/9404), profiles clarified in [#11273](https://github.com/BasedHardware/omi/pull/11273)):

    | Profile | API | Firebase identity |
    | --- | --- | --- |
    | Community local / emulator (default `setup.sh`) | local backend | `app/setup/prebuilt/*-local` (e.g. `demo-omi-local`) |
    | Mobile beta / remote staging (`setup.sh ios beta`) | `https://api.omiapi.com/` | `based-hardware` via FlutterFire + service account |

    The live `api.omiapi.com` backend verifies Firebase tokens against **`based-hardware`**.
    Tokens from **`based-hardware-dev`** are rejected with **401** after an apparently successful login.

    <Warning>
    `setup.sh` / `setup.ps1` copy the `*-local` trio, then **fail closed** if those three files disagree on project id or still target `based-hardware-dev`.
    Beta FlutterFire must use `--project=based-hardware` (never `based-hardware-dev`).
    Do **not** text-replace project IDs in prebuilt configs — see closed PR [#5945](https://github.com/BasedHardware/omi/pull/5945).
...
<AccordionGroup>
  ...
  <Accordion title="Firebase auth not working / 401 after sign-in" icon="fire">
    - Default `setup.sh` uses the local/emulator Firebase trio + local API. Remote staging (`api.omiapi.com`) requires Firebase **`based-hardware`** (beta FlutterFire path) — tokens from `based-hardware-dev` are rejected with 401 ([#9404](https://github.com/BasedHardware/omi/issues/9404)).
