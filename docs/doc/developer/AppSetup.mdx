...
<Steps>
  ...
  <Step title="Setup Firebase" icon="fire">
    ...
    Until maintainers regenerate the prebuilt trio via FlutterFire against `based-hardware` (do **not** text-replace project IDs — see closed PR [#5945](https://github.com/BasedHardware/omi/pull/5945)), use a local backend:

    ```bash
    API_BASE_URL=http://127.0.0.1:8000/ OMI_ALLOW_FIREBASE_MISMATCH=1 bash setup.sh ios
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Firebase auth not working / 401 after sign-in" icon="fire">
    ...
    - For isolated local backends, point `API_BASE_URL` at your server and set `OMI_ALLOW_FIREBASE_MISMATCH=1` only when intentionally using `based-hardware-dev` / emulator configs
  </Accordion>
