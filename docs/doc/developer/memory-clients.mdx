...
The QA repair tool can accept a complete accounting window with zero matching
attempts only after the claim lease and safety margin expire. That receipt says
`no_recorded_attempt`; it is not evidence of zero provider spend or a successful
memory sweep. The operator procedure lives in the backend runbook
