...
| `App Session Started` | macOS desktop | Once on cold launch and once after each inactive-to-active transition: [`AppSessionLifecycle`](../../desktop/macos/Desktop/Sources/Analytics/AppSessionLifecycle.swift) | Opaque `app_session_id` and closed `session_start_kind` (`cold_start` or `foreground_resume`), plus global macOS app/version context. | Not alerted; macOS session/retention denominator. |
