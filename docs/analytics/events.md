...
- Every Flutter mobile event carries bootstrap-owned `platform`, `app_version`,
  `app_build`, and `release_channel` properties. These dimensions override any
  ad-hoc call-site value so Android/iOS release-cohort charts stay comparable.
...
| `Device Disconnected` | Flutter mobile BLE (iOS/Android) | Exactly once when [`DeviceProvider.setConnectedDevice`](../../app/lib/providers/device_provider.dart) owns a connected → disconnected transition; duplicate callbacks cannot double count it. | Local/native `connection_duration_seconds`; closed `disconnect_type`, `reason`, `app_state`, and `rssi_trend`; valid RSSI/HCI code when available; closed `device_vendor`, normalized `model` and `firmware_revision`. No raw error, device ID, or device name. | [Specified: weekly unique people and connected/disconnected ratio](posthog-alerts.md#connection-balance-contract); not yet provisioned. |
...
| Event | Owning surface | Emission contract and authoritative emitter | Key properties / person properties | Alert |
|---|---|---|---|---|
| `Device Session Ended` | Flutter mobile BLE (iOS/Android) | When `setConnectedDevice(null)` closes a session that has a recorded start: [`DeviceProvider.setConnectedDevice`](../../app/lib/providers/device_provider.dart) | Native `duration_seconds`, bounded `reason`, and raw `hci_reason_code` when available; local duration/`unknown` fallback; closed `device_vendor`, normalized `model` and `firmware_revision`. No person properties. | Not alerted; reliability trend segmented by platform/vendor/model/firmware. |
| `Transcription Error` | Flutter mobile | At a terminal STT, capture, or exhausted-reconnect boundary owned by [`CaptureController`](../../app/lib/services/capture/capture_controller.dart). | Closed `source` and `error_code`; bounded `provider`, retryability/outcome/count when known. Never includes transcript text or raw error messages. | Not alerted initially; trend by platform/version/source after two stable release weeks. |
| `Transcribe Later Recording Captured` | Flutter mobile batch/offline capture | When the native batch writer finalizes a recording and [`LocalRecordingsProvider`](../../app/lib/providers/local_recordings_provider.dart) observes the immutable file. | `duration_seconds`, closed `source` and `codec`, non-negative `file_size_bytes`; no file name/path or device ID. | Not alerted; offline-capture denominator. |
| `Transcribe Later Recording Processed` | Flutter mobile batch/offline upload | Once per user-triggered upload attempt after its immediate accepted/failed/rate-limited/busy outcome is known. Deferred jobs remain accepted here; their eventual conversation is measured by `Memory Created`. | Boolean `success`, closed `outcome`, closed `source`; no job ID, file name, or raw backend error. | Not alerted; ratio against captured recordings, segmented by platform/version/source. |
