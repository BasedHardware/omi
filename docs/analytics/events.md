...
| `App Session Started` | Flutter mobile and macOS desktop | Once on cold launch and once after each background/inactive to foreground transition: mobile lifecycle observer and macOS [`AppSessionLifecycle`](../../desktop/macos/Desktop/Sources/Analytics/AppSessionLifecycle.swift) | Opaque `app_session_id` and closed `session_start_kind` (`cold_start` or `foreground_resume`), plus global app platform/version context. | Not alerted; canonical cross-platform session/retention event. |
...
| `chat_agent_query_completed` | macOS desktop chat | Once the final agent response is visible: [`ChatVisibleTurnCompletion`](../../desktop/macos/Desktop/Sources/Chat/ChatQueryTelemetry.swift), populated by [`ChatProvider`](../../desktop/macos/Desktop/Sources/Providers/ChatProvider.swift) | Effective session `model_name`, harness/adapter context, duration, response shape, tool dimensions, and measured agent cost. No prompt or response text. | Not alerted; model/cost dashboard source. |
