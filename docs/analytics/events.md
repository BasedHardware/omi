...
| `Transcription Error` | Windows desktop live mic | At a terminal STT or exhausted-reconnect boundary owned by [`startLiveMicSession`](../../desktop/windows/src/renderer/src/capture/liveMicSession.ts). | Closed `source=desktop_mic` and bounded `error_class`, outcome, and retry count. Never includes transcript text or raw error messages. | Not alerted initially; trend by Windows app version after two stable release weeks. |
| `Transcription Started` / `Transcription Ended` | Windows desktop live mic | Exactly once per logical live-transcription conversation owned by [`startLiveMicSession`](../../desktop/windows/src/renderer/src/capture/liveMicSession.ts). Reconnects resume the same lifecycle; a save/backend boundary starts the next; Ended is never a per-segment signal. | Closed `source=desktop_mic`, bounded outcome/error class/retry count and non-negative duration. No transcript text, conversation ID, or raw error. | Not alerted initially; Windows live-capture reliability funnel. |
