...
| 2026-06-19 | Oracle P0-2 developer API fallback explicit read-decision semantics | **Complete for narrow developer fallback-caller-family slice; production rollout still BLOCKED**: converted the developer default-list and vector adapter from implicit `None` downgrade semantics to `V17DeveloperMemorySearchResult` with explicit `read_decision` and `fallback_reason`. Developer default list (`GET /v1/dev/user/memories` with no category filter) and developer vector search (`GET /v1/dev/user/memories/vector/search`) now pass the persisted developer rollout decision into the adapter, return V17 results only for `USE_V17`, and deny `DENY_MEMORY`/`SHADOW_ONLY` before legacy/default/vector fallback. Enabled/granted rollout preserves hydrated V17 behavior; missing/malformed/no-grant/disabled rollout avoids V17 vector search and `users/{uid}/memory_items` reads and no longer silently calls legacy `memories_db.get_memories(...)`. Category-filtered developer list remains legacy-only for T22/T23 compatibility but is explicitly classified as `USE_LEGACY_SAFE` (`developer_category_legacy_safe_fallback_explicit`) instead of relying on implicit `None`. Archive remains default-unavailable. | RED: `cd backend && pytest tests/unit/test_v17_developer_memory_adapter.py -q` → collection failed with `ImportError: cannot import name 'V17DeveloperMemorySearchResult' from 'utils.memory.v17_developer_memory_adapter'`; GREEN: `pytest tests/unit/test_v17_developer_memory_adapter.py -q` → `11 passed in 0.06s`; format/focused/regression: `black --line-length 120 --skip-string-normalization backend/utils/memory/v17_developer_memory_adapter.py backend/routers/developer.py backend/tests/unit/test_v17_developer_memory_adapter.py && cd backend && pytest tests/unit/test_v17_developer_memory_adapter.py tests/unit/test_v17_default_read_rollout_decision.py tests/unit/test_v17_vector_search_service.py -q && pytest tests/unit/test_v17_*.py -q` → `1 file reformatted, 2 files left unchanged`, `23 passed in 0.13s`, then `189 passed, 1 warning in 1.18s`; async scan from repo root: `python3 backend/scripts/scan_async_blockers.py` → exit 0 with pre-existing findings only (`HIGH async helpers with blocking: 41`, `STRUCTURAL mixed await+sync DB: 10`). | Remaining P0-2/P0: MCP list remains legacy-only/not V17-default-read-wired; do not promote it without explicit decisions. Next recommended Oracle P0 slice: P0-3 external write/read split-brain (developer/MCP create/edit/delete) or, if staying read-only, P0-4 mandatory vector freshness/purge fences. |
