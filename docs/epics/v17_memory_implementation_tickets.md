...
| 2026-06-19 | Oracle P0-4 stale vector repair/purge candidate seam | **Complete for narrow fake-injectable repair/purge seam; production rollout still BLOCKED**: added `VectorRepairPurgeReason` taxonomy and `SearchVectorHit.vector_id` plumbing so Pinecone match IDs can flow through V17 vector hydration. `hydrate_and_filter_vector_hits(...)` now returns repair/purge candidates for missing authoritative items, stale projection commits, missing vector freshness metadata, stale account generation, cross-user metadata, stale item revision, stale source commit, stale content hash, and stale vector timestamp. `fetch_default_v17_vector_memory_search(...)` accepts optional `repair_purge_callback` and dispatches one batch only after hydration rejects stale-ID candidates; missing-fence paths still fail before vector query, `memory_items` reads, or callbacks. Returned items remain hydrated valid `memory_items` only; access-policy rejects (including stale Short-term and Archive default denial) are not purge candidates; Archive remains default-unavailable. This is an outbox/worker contract seam only and does **not** claim real Pinecone delete, Firestore outbox writes, tombstone-precedence worker behavior, shared-`ns2` proof, benchmark evidence, or production approval. | RED: `cd backend && pytest tests/unit/test_v17_vector_search_service.py -q` → collection failed with `ImportError: cannot import name 'VectorRepairPurgeReason'`; GREEN: `pytest tests/unit/test_v17_vector_search_service.py -q` → `6 passed in 0.06s`; format: `black --line-length 120 --skip-string-normalization backend/models/v17_memory_search_gateway.py backend/database/v17_vector_metadata.py backend/utils/memory/v17_vector_search_service.py backend/tests/unit/test_v17_vector_search_service.py` → `1 file reformatted, 3 files left unchanged`; focused/regression: `pytest tests/unit/test_v17_search_gateway.py tests/unit/test_v17_mcp_memory_adapter.py tests/unit/test_v17_developer_memory_adapter.py tests/unit/test_v17_chat_memory_adapter.py tests/unit/test_v17_product_memory_router.py tests/unit/test_v17_default_read_rollout_decision.py tests/unit/test_v17_vector_search_service.py -q && pytest tests/unit/test_v17_*.py -q` → `78 passed in 0.24s`, then `208 passed, 1 warning in 1.21s`; async scan from repo root: `python3 backend/scripts/scan_async_blockers.py` → exit 0 with pre-existing findings only (`HIGH async helpers with blocking: 41`, `STRUCTURAL mixed await+sync DB: 10`). | Remaining P0-4: wire candidates to durable Firestore outbox and real Pinecone delete/repair worker, prove stale IDs are deleted/repaired against Pinecone/Firestore/emulator or cloud fixtures, add tombstone precedence/idempotency/retry telemetry, then continue shared-`ns2` isolation, overfetch/refill/budgets, app/key/scope auth, central telemetry, and real benchmark/cloud evidence. |
