# memory T20 Vector Readiness / Remaining Gates

**Date:** 2026-06-19  
**Scope:** T20-R shared-namespace memory vector search over the existing `ns2` memory namespace, with authoritative `memory_items` hydration and concrete default-vector callers.  
...
The milestone is substantially complete for default memory vector search surfaces that were already selected. No new vector surfaces or explicit Archive vector routes were added in this slice. Production cutover remains blocked on the remaining gates below, especially Oracle review, real cloud/Pinecone/Firestore validation, benchmark validation, production metrics aggregation, and the explicit Archive vector policy decision.

## Commit chain

| Commit | Added |
|---|---|
| `0f22ed289` | Existing-namespace memory vector metadata/filter gateway seam: deterministic memory vector IDs, metadata builders, default vs explicit-Archive Pinecone filters, fail-closed vector-hit parsing, and `query_memory_vector_candidates(...)` over existing `ns2`; legacy `search_memories_by_vector(...)` untouched. |
| `aaee67639` | Hydrated memory vector search service/gateway: `fetch_default_vector_memory_search(...)` queries vector candidates, loads authoritative `memory_items`, hydrates/filter/ranks results, surfaces rejected counts, and keeps Archive default-invisible. |
| `4e11d7be8` | Concrete product route: `GET /memory/memory/vector/search`, gated by persisted Omi-chat default-read rollout state before vector or `memory_items` reads. |
| `fe67f2380` | Omi chat vector caller: `search_memory_default_chat_memories_vector_text(...)` and LangChain `search_memories_tool` wiring before legacy vector fallback. |
| `010b7306e` | MCP REST vector caller: `search_default_mcp_memories_vector(...)` wired into `/v1/mcp/memories/search` before legacy vector fallback. |
...
| `a8aac6806` | Developer API vector caller/endpoint: `search_memory_default_developer_memories_vector(...)` and `GET /v1/dev/user/memories/vector/search`, gated by developer default-read rollout/grant before hydrated vector search. |
...
- **Legacy fallback untouched:** legacy vector behavior remains the fallback/unchanged path for non-enabled callers and existing legacy functions are not repurposed as authoritative memory paths.
...
```text
0f22ed289 feat: add memory vector metadata filter seam
aaee67639 feat: add memory hydrated vector search service
4e11d7be8 feat: add memory product vector search route
fe67f2380 feat: wire memory chat vector memory search
010b7306e feat: wire memory MCP vector memory search
e09aafc20 feat: wire memory MCP SSE vector memory search
a8aac6806 feat: wire T20 developer API vector search
```

```bash
cd /root/workspace/omi-memory-ingestion-pipeline/backend
pytest tests/unit/test_memory_developer_memory_adapter.py tests/unit/test_memory_vector_search_service.py tests/unit/test_default_read_rollout_decision.py -q
```
...
- Oracle milestone review has now run and is recorded in `docs/epics/memory_t20_oracle_milestone_review.md`; verdict is **BLOCK production rollout** with P0 fixes required before any production read/vector cutover.
...
1. **Oracle milestone review:** complete and recorded in `docs/epics/memory_t20_oracle_milestone_review.md`; apply all Oracle P0 fixes before any production read/vector cutover.
...
3. **Projection/vector consistency:** validate memory outbox/upsert/delete/tombstone/idempotency behavior against real vector projection flows, including stale-version rejection and delete/tombstone precedence.
...
5. **Benchmark validation:** run vector recall/precision/latency comparison anchored to Base Omi and memory default read policy; verify Archive returned-by-default remains zero.
...
Treat T20 default vector implementation as reviewed by Oracle and **blocked for production launch**. The next implementation slice should address Oracle P0 findings before production cutover, starting with shared memory route authorization/Archive capability, explicit read-decision semantics, T22/T23 write/read convergence, mandatory vector fences, shared-namespace validation, third-party scope enforcement, and production-shaped vector hydration/overfetch.
