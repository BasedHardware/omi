...
- `backend/tests/unit/test_memory_developer_memory_adapter.py`
- `backend/tests/unit/test_memory_vector_search_service.py`
...
1. **Product default read is not rollout-gated; Archive capability is client-self-granted.** Every memory product route must call one shared server-side authorization decision before any `memory_items` access. Archive must require both persisted capability and explicit Archive query.
2. **“Fail closed” is conflated with unsafe legacy downgrade.** Replace Boolean/`None` rollout contracts with an explicit decision such as `USE_MEMORY`, `USE_LEGACY_SAFE`, `DENY_MEMORY`, and optionally `SHADOW_ONLY`; legacy downgrade must require proven reconciliation/generation/epoch safety and explicit policy.
3. **Reads are memory while MCP/developer writes and deletes remain legacy.** Before authoritative read cutover, route applicable create/edit/delete through memory, disable those writes for pilots and prove reconciliation, or implement tested durable dual-write/outbox; deletion must update authoritative state and compatibility/vector projections before success.
...
5. **Shared `ns2` isolation from legacy search is unproven.** Prove with real Pinecone/`ns2` data that legacy queries exclude memory schema records and retain baseline recall, or add a legacy schema filter / separate namespace before production memory inserts.
...
1. Gate every memory route with the same versioned rollout decision and add a global emergency kill switch independent of per-user Firestore reads.
...
6. Prove shared-namespace isolation against real Pinecone data for memory and legacy callers.
...
2. Fallback semantics: deny, error, last-known-safe, merge, or legacy only when reconciliation is proven for missing/malformed control state, grant revocation, vector outage, or empty memory results.
...
6. Read/write sequencing: whether T22 memory writes/deletes must land before read cutover or pilot external writes are disabled.
7. API strategy: additive switch on existing `/v3`, MCP, developer endpoints vs private/experimental `/memory` until compatibility is complete.
...
- Added `backend/utils/memory/v3_get_dependency_seam.py` with a pure adapter composition contract for future GET `/v3/memories` dependencies. The seam executes authenticated subject binding first, rejects client uid override before control/read selection, preserves non-enrolled legacy-primary-only without memory/legacy merge, validates enrollment/control/config/cursor/projection source, and applies rate-limit/backpressure before allowing projection reads.
- Added `backend/scripts/p1_3_v3_get_dependency_seam_readiness.py` as a read-only BLOCKED readiness artifact plus unit coverage in `backend/tests/unit/test_v3_get_dependency_seam.py` and `backend/tests/unit/test_p1_3_v3_get_dependency_seam_readiness.py`.
...
- Added shared explicit `MemoryReadDecision` semantics in `backend/utils/memory/default_read_rollout.py`: `USE_MEMORY`, `USE_LEGACY_SAFE`, `DENY_MEMORY`, and `SHADOW_ONLY`.
- Classified missing, malformed, uid-mismatched, disabled, unsupported, and no-default-memory-grant control states as `DENY_MEMORY` for default memory reads instead of silently implying legacy fallback.
- Added an explicit `legacy_safe_default_read_rollout_decision(...)` constructor for callers that are intentionally legacy-safe by policy, so safe legacy behavior is opt-in rather than derived from bad memory control state.
- Classified shadow-enabled/read-disabled granted state as `SHADOW_ONLY`.
- Applied the explicit decision to product `/memory/memory/search` and `/memory/memory/vector/search`: both require persisted server-owned Omi-chat rollout/grant state to return `USE_MEMORY` before any `users/{uid}/memory_items` or vector read; non-`USE_MEMORY` returns 403 with rollout observability.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED import failure for missing explicit decision type, GREEN focused tests, full `pytest tests/unit/test_memory_*.py -q` (`175 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.
...
- Added shared persisted Archive capability parsing in `backend/utils/memory/default_read_rollout.py` through `read_archive_read_rollout(...)` / `normalize_archive_read_rollout_decision(...)`.
- Archive reads now require the same persisted Omi-chat memory default-read authorization (`USE_MEMORY`) plus default-memory grant and a distinct server-owned Archive capability (`grants.omi_chat.archive=true`, `grants.chat.archive=true`, or boolean top-level Omi-chat aliases). Non-boolean Archive capability values fail closed as malformed.
- Applied the decision to the explicit product `/memory/memory/archive/search` route: it still requires explicit `include_archive=true`, but that flag alone no longer grants Archive access. Missing control state, disabled reads, no default grant, missing Archive grant, or malformed Archive capability all return 403 before `users/{uid}/memory_items` reads.
- Default `/memory/memory/search` and `/memory/memory/vector/search` remain Archive-default-unavailable (`archive_capability=false`), and no Archive vector route/exposure was added.

Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED import failure for missing `read_archive_read_rollout`, first GREEN attempt exposing fallback reason precedence (`2 failed, 18 passed`), focused GREEN (`20 passed`), full `pytest tests/unit/test_memory_*.py -q` (`178 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.
...
- Continue rolling explicit `USE_MEMORY` / `USE_LEGACY_SAFE` / `DENY_MEMORY` / `SHADOW_ONLY` semantics across fallback callers and deny unsafe downgrades.
- Add the global emergency kill switch independent of per-user Firestore reads.
- Expand shared route authorization to any remaining memory product/caller surfaces before production rollout.
- Production rollout remains **BLOCKED / NO-GO** until all Oracle P0s and required real-service evidence are complete.

### 2026-06-19 — global memory product-read gate / emergency kill switch slice
...
- Added `GLOBAL_READ_GATE_PATH = memory_control/global_read_gate` plus `read_global_read_gate(...)` / `normalize_global_read_gate(...)` in `backend/utils/memory/default_read_rollout.py`.
- The global gate is independent of per-user `users/{uid}/memory_control/state`; product routes read it first and require boolean `memory_reads_enabled=true` and boolean `kill_switch_active=false`.
- Missing global config, malformed config, disabled global reads, and active kill switch all return explicit `DENY_MEMORY` reasons (`missing_global_read_gate`, `malformed_global_read_gate`, `global_memory_reads_disabled`, `global_memory_read_kill_switch_active`). This intentionally fails safe/closed under the Oracle-risk posture.
- Applied the gate before per-user rollout reads, vector calls, or `users/{uid}/memory_items` reads on product `/memory/memory/search`, `/memory/memory/vector/search`, and explicit `/memory/memory/archive/search?include_archive=true`.
- Enabled global gate preserves existing per-user `USE_MEMORY` default and Archive decisions; Archive remains default-unavailable and still requires explicit intent plus persisted Archive capability.

Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED import failure for missing `GLOBAL_READ_GATE_PATH`, GREEN focused tests (`24 passed`), formatted/focused regression (`26 passed`), full `pytest tests/unit/test_memory_*.py -q` (`182 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.
...
- Continue P0-2 by rolling explicit `USE_MEMORY` / `USE_LEGACY_SAFE` / `DENY_MEMORY` / `SHADOW_ONLY` semantics across remaining fallback callers and denying unsafe legacy downgrades.
...
- Added `ChatMemorySearchResult` in `backend/utils/memory/chat_memory_adapter.py` so the Omi chat vector adapter returns explicit `MemoryReadDecision` semantics plus fallback reason instead of using `None` as an ambiguous downgrade signal.
...
- Missing, malformed, no-grant, disabled, or shadow-only Omi-chat rollout decisions now avoid memory vector search and `users/{uid}/memory_items` reads and return a safe no-memory response instead of silently calling legacy `vector_db.find_similar_memories(...)`.
- Enabled/granted Omi-chat rollout continues using hydrated memory vector search with default Archive unavailable; no Archive exposure was broadened.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED import failure for missing `ChatMemorySearchResult`, GREEN chat adapter tests (`8 passed`), focused regression (`20 passed`), full `pytest tests/unit/test_memory_*.py -q` (`183 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.
...
- Added `McpMemorySearchResult` in `backend/utils/mcp_memories.py` so the MCP hydrated vector adapter returns explicit `MemoryReadDecision` semantics plus fallback reason instead of using `None` as an ambiguous downgrade signal.
- Wired MCP REST `/v1/mcp/memories/search` and streamable HTTP/SSE `search_memories` to pass the persisted MCP rollout decision into the adapter and branch on `USE_MEMORY` / `USE_LEGACY_SAFE` / `DENY_MEMORY` / `SHADOW_ONLY` before any legacy vector/default fallback.
- Missing, malformed, no-grant, disabled, or shadow-only MCP rollout decisions now avoid memory vector search and `users/{uid}/memory_items` reads and return an empty safe memory response instead of silently calling legacy `vector_db.find_similar_memories(...)` or the legacy default read fallback.
- Enabled/granted MCP rollout continues using hydrated memory vector search with default Archive unavailable; no Archive exposure was broadened.
- Intentional legacy fallback is preserved only for an explicit `USE_LEGACY_SAFE` adapter classification.

Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED import failure for missing `McpMemorySearchResult`, GREEN MCP adapter tests (`11 passed`), focused regression (`23 passed`), full `pytest tests/unit/test_memory_*.py -q` (`186 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only. An additional attempted legacy MCP route test collection is blocked in this local environment by missing `fastapi`.

This closes the Oracle P0-2 subpoint for MCP REST/SSE vector search fallback downgrade semantics. Remaining P0-2/P0 work:

- MCP list (`GET /v1/mcp/memories`) is still legacy-only/not memory-default-read-wired; if it becomes a memory default/list path, convert it through explicit decisions before rollout.
...
- Added `DeveloperMemorySearchResult` in `backend/utils/memory/developer_memory_adapter.py` so developer default-list and vector adapters return explicit `MemoryReadDecision` semantics plus fallback reason instead of using `None` as an ambiguous downgrade signal.
- Wired developer default list (`GET /v1/dev/user/memories` with no category filter) and developer vector search (`GET /v1/dev/user/memories/vector/search`) to pass the persisted developer rollout decision into the adapter and branch on `USE_MEMORY` / `USE_LEGACY_SAFE` / `DENY_MEMORY` / `SHADOW_ONLY` before any legacy fallback.
- Missing, malformed, no-grant, disabled, or shadow-only developer rollout decisions now avoid memory vector search and `users/{uid}/memory_items` reads and return 403 instead of silently calling legacy `memories_db.get_memories(...)` on the memory-enabled list path.
...
- Enabled/granted developer rollout continues using hydrated memory default/vector reads with default Archive unavailable; no Archive exposure was broadened.

Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED import failure for missing `DeveloperMemorySearchResult`, GREEN developer adapter tests (`11 passed`), focused regression (`23 passed`), full `pytest tests/unit/test_memory_*.py -q` (`189 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.
...
- MCP list (`GET /v1/mcp/memories`) remains legacy-only/not memory-default-read-wired; if it becomes a memory default/list path, convert it through explicit decisions before rollout.
...
- Added `LegacyMemoryWriteGuardDecision` and `assert_legacy_memory_write_allowed_for_default_read_decision(...)` in `backend/utils/memory/default_read_rollout.py`.
...
- The guard uses the same persisted `developer_api` default-read rollout decision as developer memory reads and blocks legacy `memories` mutation with HTTP 409 for `USE_MEMORY`, `SHADOW_ONLY`, missing, malformed, or uid-mismatched control state unless an explicit server-owned `allow_write_convergence=True` policy is passed.
- memory-disabled reads and explicit convergence preserve the legacy create path; Archive remains default-unavailable and no read surface was broadened.

Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED missing guard import, RED fail-safe coverage catch, GREEN developer adapter tests (`15 passed`), focused regression (`27 passed`), full `pytest tests/unit/test_memory_*.py -q` (`193 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.
...
- Decide and implement durable memory write convergence / dual-write outbox before any authoritative external read cutover.
- Production rollout remains **BLOCKED / NO-GO** until all Oracle P0s and required real-service evidence are complete.
...
- The guard uses the same persisted `developer_api` default-read rollout decision as developer memory reads and blocks legacy batch mutation with HTTP 409 for `USE_MEMORY`, `SHADOW_ONLY`, missing, malformed, or uid-mismatched control state unless an explicit server-owned convergence policy is passed through the shared guard.
- memory-disabled reads preserve existing developer batch-create behavior; Archive remains default-unavailable and no read surface was broadened.

Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED batch-route guard-order failure (`1 failed, 16 passed`), GREEN developer adapter tests (`17 passed`), focused regression (`29 passed`), full `pytest tests/unit/test_memory_*.py -q` (`195 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.
...
- The guard uses the same persisted `developer_api` default-read rollout decision as developer memory reads and blocks legacy mutation/delete with HTTP 409 for `USE_MEMORY`, `SHADOW_ONLY`, missing, malformed, or uid-mismatched control state unless an explicit server-owned convergence policy is passed through the shared guard.
- memory-disabled reads preserve existing developer edit/delete behavior; Archive remains default-unavailable and no read surface was broadened.

Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED edit/delete guard-order failures (`2 failed, 18 passed`), GREEN developer adapter tests (`20 passed`), focused regression (`32 passed`), full `pytest tests/unit/test_memory_*.py -q` (`198 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.
...
- The guard uses the same persisted `mcp` default-read rollout decision as MCP memory reads and blocks legacy mutation/delete for `USE_MEMORY`, `SHADOW_ONLY`, missing, malformed, or uid-mismatched control state unless a server-owned convergence policy explicitly allows the write.
- REST surfaces return HTTP 409 with the shared guard detail; SSE tools return a safe MCP tool error (`code=-32009`) with the same guard detail. memory-disabled reads preserve existing behavior.
- Archive remains default-unavailable and no read surface was broadened.

Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED MCP write-surface guard-order failures (`2 failed, 12 passed`), GREEN MCP adapter/source tests (`14 passed`), focused regression (`26 passed`), full `pytest tests/unit/test_memory_*.py -q` (`201 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.
...
Concretized the remaining Oracle P0-3 durable write convergence gate as a narrow fail-closed backend seam, without claiming full external memory write convergence or changing the production rollout verdict:

- Added server-owned convergence gate path `memory_control/write_convergence_gate`, `WriteConvergencePolicy`, and `read_write_convergence_gate(...)` in `backend/utils/memory/default_read_rollout.py`.
- Updated `assert_legacy_memory_write_allowed_for_default_read_decision(...)` so `USE_MEMORY`, `SHADOW_ONLY`, and fail-safe read-decision states still block legacy external writes unless the convergence gate is explicitly ready.
...
- Missing, malformed, or partial convergence config fails safe; the old legacy boolean override is not sufficient for memory/shadow read consumers.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED missing convergence gate import, focused GREEN across MCP/developer/default-read tests (`47 passed`), focused regression (`49 passed`), full `pytest tests/unit/test_memory_*.py -q` (`204 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.

This closes only the fail-closed convergence readiness seam. Remaining P0-3/P0 work:

- Implement the actual durable external memory write service / dual-write outbox worker and transaction contract.
...
- memory vector hydration now requires a server-owned `vector_projection_commit_id` from persisted default-read rollout state plus the current account generation.
- `fetch_default_vector_memory_search(...)` requires `required_projection_commit_id` and `required_account_generation`; missing or malformed values fail fast.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED vector-service tests (`2 failed, 2 passed`), focused fixture failure while adapting mandatory fences (`5 failed, 68 passed`), GREEN focused suite (`76 passed`), full `pytest tests/unit/test_memory_*.py -q` (`206 passed, 1 warning`), and formatting (`14 files left unchanged`).
...
- `fetch_default_vector_memory_search(...)` accepts an optional fake-injectable `repair_purge_callback` and dispatches one batch after hydration only when repair/purge candidates exist. Returned memory results are still only hydrated valid `memory_items`; access-policy rejects such as stale Short-term or Archive default denial are not purge candidates.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED import failure for missing `VectorRepairPurgeReason`, GREEN vector-service seam tests (`6 passed`), focused memory vector/caller regression (`78 passed`), full `pytest tests/unit/test_memory_*.py -q` (`208 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.
...
- Added `backend/database/memory_vector_repair_outbox.py` with `build_vector_repair_purge_outbox_records(...)` and fake-friendly `write_vector_repair_purge_outbox_records(...)`.
...
- `fetch_default_vector_memory_search(...)` now builds outbox records after authoritative hydration and calls an injected `repair_purge_outbox_writer` exactly once only when records exist. Missing/no candidates writes nothing.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED missing outbox module (`ModuleNotFoundError`), GREEN vector-service tests (`8 passed`), focused memory vector/caller regression (`80 passed`), full `pytest tests/unit/test_memory_*.py -q` (`210 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.
...
- Validated `write_vector_repair_purge_outbox_records(...)` against a fake Firestore document seam: repeated writes set the same stable `users/{uid}/memory_outbox/{record_id}` document with the same `record_id`/`idempotency_key`.
...
- Wired only the product `/memory/memory/vector/search` surface to pass the real Firestore outbox writer into `fetch_default_vector_memory_search(...)`, and only when that server flag is true.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED flag/wiring tests (`2 failed, 23 passed` with missing `vector_repair_outbox_enabled`), GREEN focused tests (`25 passed`), focused memory vector/caller regression (`83 passed`), full `pytest tests/unit/test_memory_*.py -q` (`213 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.
...
Continued Oracle P0-4 by adding and running a local Firebase emulator validation gate for memory vector repair outbox persistence, without changing the production rollout verdict:

- Added `backend/scripts/v17_vector_repair_outbox_emulator_test.py` and `npm run test:memory-vector-repair-outbox:emulator`.
- The emulator harness uses the real Python Firestore client against `FIRESTORE_EMULATOR_HOST`, builds a deterministic `vector_repair_purge` outbox record, writes it twice through `write_vector_repair_purge_outbox_records(...)`, and verifies the stable `users/{uid}/memory_outbox/{record_id}` document with unchanged `record_id`/`idempotency_key`.
...
- Added `npm run test:memory-vector-repair-outbox-rules:emulator` as the explicit rules-side companion command for the existing Security Rules emulator harness, which denies signed-in client direct read/create/update/delete on `memory_outbox` and other protected memory collections. Backend/Admin context remains required; direct client writes are not assumed.
- Updated `docs/epics/memory_firestore_iam_deployment.md` with exact commands, prerequisites, pass/fail criteria, IAM/rules assumptions, non-claims, and remaining worker/Pinecone/shared-namespace gates.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED harness test (`1 failed`), GREEN harness test (`1 passed`), real Firestore emulator outbox persistence command exit 0 with idempotent path `users/memory-vector-repair-outbox-emulator-user/memory_outbox/memvrp_a9f8abf2b6c7f8409d23f3bc63de76cf`, real rules emulator command exit 0 with client-denial PASS, plus focused/regression/async outputs from this commit.
...
- Added `backend/database/vector_repair_outbox_worker.py` with `process_vector_repair_purge_outbox_records(...)`.
...
- Added `backend/tests/unit/test_vector_repair_outbox_worker.py` covering missing item delete, live item repair, tombstone precedence delete, terminal/in-progress/duplicate idempotency skips, and retry/dead-letter failure handling; added the test file to `backend/test.sh`.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED missing worker module (`ModuleNotFoundError`), GREEN worker tests (`5 passed`), focused vector/outbox/product regression (`31 passed`), full `pytest tests/unit/test_memory_*.py -q` (`219 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.
...
- Added `lease_vector_repair_purge_outbox_records(...)` in `backend/database/vector_repair_outbox_worker.py` for `users/{uid}/memory_outbox/*` records.
...
- Added `ack_vector_repair_purge_outbox_record(...)` to apply the worker's ack/retry/dead-letter patches (`in_progress`, `completed`, `pending`, `dead_letter`, `attempt_count`, `last_error`, `action`) to the deterministic outbox document path.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED missing lease/ack imports (`ImportError`), GREEN worker seam tests (`8 passed`), focused vector/outbox/product regression (`34 passed`), full `pytest tests/unit/test_memory_*.py -q` (`222 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.
...
- Updated `lease_vector_repair_purge_outbox_records(...)` so each pending document claim uses a Firestore transaction when the injected client supports `transaction()`: the claim re-reads the document inside the transaction, verifies it is still `event_type="vector_repair_purge"`, `status="pending"`, and due, then updates the same document to `in_progress` with lease metadata in the transaction.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED transactional unit expectation (`1 failed, 8 passed`), RED missing emulator lease harness/package script (`2 failed`), GREEN focused outbox tests (`11 passed`), real Firestore emulator contention command (`npm run test:memory-vector-repair-outbox-lease:emulator` → PASS, `claimed=1`), focused vector/outbox/product regression (`36 passed`), full `pytest tests/unit/test_memory_*.py -q` (`224 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.
...
- Added `backend/database/v17_vector_repair_pinecone_adapter.py` with worker-compatible `make_pinecone_vector_deleter(...)` and `make_pinecone_vector_repairer(...)` factories.
...
- The repair adapter calls injected `embed_text(content)` and `upsert_vectors(vectors=[...], namespace="ns2")` functions, builds the deterministic memory vector id and memory metadata from the live authoritative `MemoryItem` plus the outbox `required_projection_commit_id`, and raises `VectorRepairNotReady` before embedding/upsert if content, source commit, content hash, or required projection fence is missing.
- Namespace isolation is explicit at the seam (`VECTOR_REPAIR_PINECONE_NAMESPACE = "ns2"`), matching the existing memory-vector namespace in `backend/database/vector_db.py`; real shared-`ns2` coexistence proof is still not claimed.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED missing Pinecone adapter module (`ModuleNotFoundError`), GREEN worker/adapter tests (`13 passed`), focused vector/outbox/product regression (`40 passed`), full `pytest tests/unit/test_memory_*.py -q` (`228 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.
...
Continued Oracle P0-4 by adding the first explicit Cloud Run/Tasks-or-scheduler execution contract seam for memory vector repair outbox processing, without registering a production scheduler or changing the production rollout verdict:

- Added `VectorRepairOutboxWorkerTickConfig` and `run_vector_repair_outbox_worker_tick(...)` in `backend/database/vector_repair_outbox_worker.py`.
...
- An enabled tick leases due pending `vector_repair_purge` records for one uid, processes them through the existing fake-injectable worker with injected authoritative item loader plus delete/repair adapter functions, and applies ack/retry/dead-letter patches through `ack_vector_repair_purge_outbox_record(...)`.
...
- Updated `docs/epics/memory_firestore_iam_deployment.md` with the explicit proposed Cloud Run/Tasks contract, service identity/config/env vars, enablement flag, telemetry/alert needs, remaining IAM/Pinecone/shared-`ns2` gates, and non-claims.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED missing worker tick imports (`ImportError`), GREEN worker/adapter tests (`17 passed`), focused vector/outbox/product/docs regression (`45 passed`), full `pytest tests/unit/test_memory_*.py -q` (`232 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.
...
- Added `backend/scripts/vector_repair_outbox_worker_entrypoint.py`, a small fake-injectable entrypoint that reads explicit env/config and prints one deterministic JSON summary suitable for Cloud Run/Tasks logs.
- The wrapper fails closed/no-ops when `MEMORY_VECTOR_REPAIR_OUTBOX_WORKER_ENABLED` is absent, empty, or `false`; malformed enablement, missing enabled uid, missing enabled stable worker/lease-owner ID, or invalid numeric bounds exit nonzero before any tick/lease call.
- Enabled execution requires injected Firestore client, authoritative item loader, vector deleter, vector repairer, and invokes exactly one `run_vector_repair_outbox_worker_tick(...)` for one explicit uid. There is no unbounded scan and no scheduler enqueue side effect.
- Added `backend/tests/unit/test_vector_repair_outbox_worker_entrypoint.py` and registered it in `backend/test.sh`. Tests cover disabled no-op, malformed config denial, required uid/lease-owner denial, enabled fake tick summary, dependency/action failure summary, and source-level no scheduler enqueue side effects.
- Updated `docs/epics/memory_firestore_iam_deployment.md` with the proposed command, env vars, OIDC/IAM assumptions, uid-shard/backlog ownership rule, telemetry/alert needs, failure modes, and explicit non-claims.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED missing entrypoint import (`ImportError`), GREEN entrypoint tests (`6 passed`), disabled CLI JSON smoke output, focused vector/outbox/docs regression (`26 passed`), full `pytest tests/unit/test_memory_*.py -q` (`238 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.
...
- Added `VectorRepairOutboxProductionDependencies` and `build_vector_repair_outbox_production_dependencies(...)` to `backend/scripts/vector_repair_outbox_worker_entrypoint.py`.
...
- When dependency env is present, the resolver lazily constructs Admin Firestore via `database._client.db`, an authoritative `users/{uid}/memory_items/{memory_id}` loader returning `MemoryItem`, and Pinecone `delete`/`upsert` repair adapters using `utils.llm.clients.embeddings.embed_query` with explicit namespace `ns2`.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED dependency-factory tests (`4 failed, 6 passed`), GREEN entrypoint tests (`10 passed`), focused wrapper/outbox/doc regression (`30 passed`), disabled CLI smoke JSON, full `pytest tests/unit/test_memory_*.py -q` (`242 passed, 1 warning`), and async scan exit 0 with pre-existing findings only.
...
- Added `docs/epics/memory_vector_repair_outbox_cloud_deployment_contract.yaml`, a static Cloud Run/Cloud Tasks/Cloud Scheduler contract with explicit command/image shape, env/secrets, dedicated worker/scheduler service accounts, OIDC `serviceAccountEmail`/`audience`, IAM proof targets, retry/backoff/dead-letter placeholders, server-owned uid shard parameterization, and pass/fail proof commands.
- The contract is intentionally disabled: Cloud Run env has `MEMORY_VECTOR_REPAIR_OUTBOX_WORKER_ENABLED="false"`, Cloud Scheduler is `state: PAUSED`, Cloud Run invoker IAM remains required, Cloud Tasks concurrency is one, and enablement requires later production gates plus an explicit patch to `MEMORY_VECTOR_REPAIR_OUTBOX_WORKER_ENABLED=true`.
- Added `backend/tests/unit/test_memory_vector_repair_outbox_deployment_contract.py` and registered it in `backend/test.sh` to assert the static contract contains the required disabled/OIDC/IAM/retry/dead-letter fields and does not claim production IAM, deployed resources, or Pinecone deletion proof.
- Updated `docs/epics/memory_firestore_iam_deployment.md` to link the artifact and record the key readiness caveat: the current worker entrypoint is CLI one-tick code, so an HTTP shim (or a deliberate Cloud Run Job + OAuth trigger design) must exist before applying the HTTP Cloud Tasks/Scheduler shape.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED missing contract artifact (`1 failed`), GREEN static contract test (`1 passed`), focused wrapper/outbox/doc regression, disabled CLI smoke JSON, full `pytest tests/unit/test_memory_*.py -q`, and async scan exit 0 with pre-existing findings only.
...
- Added `create_vector_repair_outbox_worker_app(...)` and `run_vector_repair_outbox_worker_http_tick(...)` in `backend/scripts/vector_repair_outbox_worker_entrypoint.py`.
- The module-level `app` exposes `POST /memory-vector-repair-outbox-worker/tick` for a Cloud Run service command such as `uvicorn scripts.vector_repair_outbox_worker_entrypoint:app --host 0.0.0.0 --port 8080`.
- The HTTP shim is fail-closed by default: absent/false `MEMORY_VECTOR_REPAIR_OUTBOX_WORKER_ENABLED` returns the deterministic no-op JSON summary and does not build Firestore/Pinecone/embedding dependencies or lease/process/ack records.
...
- Updated the static deployment YAML and Firestore/IAM deployment doc to use the HTTP shim service command and to remove the prior “HTTP shim must exist” caveat. The artifact remains disabled (`MEMORY_VECTOR_REPAIR_OUTBOX_WORKER_ENABLED=false`, Scheduler `PAUSED`).
- No Cloud Run service, Cloud Tasks queue, Cloud Scheduler job, IAM binding, production Firestore rules validation, Pinecone delete/upsert, shared-`ns2` proof, telemetry alert, benchmark, or production approval was created or claimed.

Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED HTTP shim tests (`5 failed, 10 passed` for missing `create_vector_repair_outbox_worker_app` and Cloud Run IAM documentation), RED deployment contract test (`1 failed` for missing `uvicorn`/HTTP shim contract), GREEN focused trigger tests (`36 passed`), disabled CLI smoke JSON, full `pytest tests/unit/test_memory_*.py -q` (`248 passed, 1 warning`), and async scan exit 0 with pre-existing findings only.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED missing proof-runner/static reference tests (`2 failed`), GREEN proof-runner test (`2 passed`), readiness command outputs (`NOT_RUN` and safe `--execute` prerequisite failure), focused vector/outbox/docs regression (`38 passed`), full memory regression (`250 passed, 1 warning`), and async scan exit 0 with pre-existing findings only.
...
Continued Oracle P0-4 by adding a safe production Firestore IAM/deployed Security Rules validation runner for the memory vector repair outbox paths, without claiming real cloud proof or changing the production rollout verdict:
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED missing runner/doc tests (`2 failed`), GREEN static test (`2 passed`), readiness command outputs (`NOT_RUN` and safe `--execute` prerequisite failure), focused vector/outbox/docs regression (`40 passed`), full memory regression (`252 passed, 1 warning`), and async scan exit 0 with pre-existing findings only.
...
- Execute mode is gated by explicit credentials/config (`PINECONE_API_KEY`, `PINECONE_INDEX_NAME`, `PINECONE_INDEX_HOST`) plus `--allow-throwaway-mutation`, a non-`ns2` `--test-namespace`, a long `memory-proof-...` `--throwaway-prefix`, exact `--confirm-throwaway-prefix`, and optional `--shared-ns2-readonly`; the runner refuses shared `ns2` mutation.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED missing runner/doc tests (`3 failed`), GREEN static test (`3 passed`), readiness command outputs (`NOT_RUN` and safe `--execute` prerequisite failure), focused vector/outbox/docs regression, full memory regression, and async scan exit 0 with pre-existing findings only.
...
- Produce read-only shared `ns2` coexistence evidence proving legacy queries exclude memory schema records and baseline legacy recall remains intact, or choose a separate namespace/filter before production inserts.
...
Continued Oracle P0-4 by adding a fake-injectable low-cardinality telemetry and alert contract for the memory vector repair outbox worker, without claiming production metrics wiring or changing the production rollout verdict:

- Added `backend/database/memory_vector_repair_outbox_telemetry.py` with `VectorRepairOutboxTelemetryConfig` and `emit_vector_repair_outbox_worker_telemetry(...)`.
...
- `run_vector_repair_outbox_worker_tick(...)` now accepts an optional injected telemetry emitter/config/backlog/duration and records telemetry emission failures under `summary["telemetry"]` without masking lease/process/ack cleanup results.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED missing telemetry module (`ModuleNotFoundError`), GREEN telemetry tests (`4 passed`), focused vector/outbox/doc regression, full memory regression, and async scan exit 0 with pre-existing findings only.
...
### 2026-06-19 — P0-1 shared memory product authorization decision seam
...
- Added `backend/utils/memory/product_authorization.py` with `ProductAuthorizationContext`, `ProductAuthorizationDecision`, and `authorize_memory_product_memory_route(...)`.
...
- Wired product `/memory/memory/search`, `/memory/memory/vector/search`, and `/memory/memory/archive/search` through the shared seam before memory/vector reads while preserving existing default Archive-unavailable behavior.

Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED import failure for missing `utils.memory.v17_product_authorization`, GREEN seam tests (`5 passed`), focused product/default rollout regression (`34 passed`), full `pytest tests/unit/test_memory_*.py -q` (`264 passed, 1 warning`), and async scan exit 0 with pre-existing findings only.
...
### 2026-06-19 — P0-1/P0-6 app/key/scope memory memory grant contract seam

Continued Oracle P0-1/P0-6 with the first fake-injectable app/key/scope grant contract for external memory memory access, without wiring production route behavior or changing the production rollout verdict:

- Extended `backend/utils/memory/product_authorization.py` with `MemoryGrantOperation`, `AppKeyScopeGrantDecision`, and `authorize_app_key_scope_memory_grant(...)`.
...
- Inspected current route/auth inventory: developer keys carry stored `scopes` but only return uid from route dependencies; MCP REST/SSE API key models currently do not persist scopes; MCP SSE advertises OAuth `memories.read`/`memories.write`; product `/memory` routes are first-party `omi_chat`. Therefore this slice is a contract/helper plus tests, not route enforcement for MCP/developer yet.

Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED missing grant-operation import, GREEN grant/product authorization tests (`11 passed`), focused product/default rollout regression (`40 passed`), full `pytest tests/unit/test_memory_*.py -q` (`270 passed, 1 warning`), and async scan exit 0 with pre-existing findings only.
...
- Carry key id/app id/scopes through developer and MCP REST/SSE auth dependencies, including MCP key scope storage or OAuth token introspection, then compose this seam before memory memory reads/writes.
...
- Added `backend/database/app_key_memory_grants.py` with canonical path constants for `users/{uid}/memory_control/app_key_memory_grants`, a fake-injectable `read_app_key_memory_grants_state(uid, db_client)`, and a pure contract-shape builder for admin/test tooling.
- The persisted document intentionally stores the exact nested authorization shape consumed by `authorize_app_key_scope_memory_grant(...)`: `grants.<consumer>.apps.<app_id>.keys.<key_id>`.
- Missing documents return absent state with `missing_app_key_memory_grants_state`; malformed top-level state returns `malformed_app_key_memory_grants_state` and then fails closed through the grant authorization helper.
...
- Extended the local Firestore rules emulator harness to assert that a signed-in client cannot read/create/update/delete `users/memory-emulator-user/memory_control/app_key_memory_grants` with an attempted `grants.developer_api.apps.client-app.keys.client-key` self-grant. The local emulator proof passed.
- Added `docs/epics/memory_app_key_memory_grants_readiness.md` with path/schema, conversion rules, emulator command, route dependency blockers, and explicit non-claims.

Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED missing storage module, RED emulator harness static test for missing self-grant denial target, GREEN storage/static tests, local emulator PASS, focused memory regression, full memory regression, and async scan exit 0 with pre-existing findings only.

This closes only the storage/read helper plus local rules-emulator denial subpoint. Remaining P0-1/P0-6 work:

- Wire developer/MCP/third-party route dependencies to carry authenticated `app_id`, `key_id`, and verified scopes into this storage+authorization seam before any memory read/write.
...
Continued Oracle P0-1/P0-6 by carrying authenticated Developer API key context into the stored app/key grant authorization seam for one narrow memory read path, without changing the production rollout verdict:
...
- Added `get_developer_memory_default_memory_read_context(...)` to translate verified Developer API scopes (`memories:read`, `memories:write`) into the memory app/key grant scope vocabulary (`memories.read`, `memories.write`) and build `ProductAuthorizationContext` for `consumer='developer_api'` / `surface='developer_default_memory_read'`.
- Added `authorize_memory_external_default_memory_read(...)`, which reads `users/{uid}/memory_control/app_key_memory_grants` and composes it with `authorize_app_key_scope_memory_grant(..., operation=DEFAULT_READ)`.
- Wired Developer API default memory list (`GET /v1/dev/user/memories` without category filters) to require that composition before memory default-list reads. Missing app/key identity, missing/wrong scope, missing/malformed grant state, or missing persisted default-read grant returns 403 before memory `memory_items` access.
...
- Apply the same app/key/scope grant composition to Developer API vector search before memory vector reads.
...
Continued Oracle P0-1/P0-6 by applying the same Developer API app/key/scope memory grant composition to the concrete Developer API vector search route, without changing the production rollout verdict:

- Changed `GET /v1/dev/user/memories/vector/search` from the uid-only `get_uid_with_memories_read` dependency to `get_developer_memory_default_memory_read_context(...)`.
- The route now derives `uid` from the authenticated memory product authorization context and calls `authorize_memory_external_default_memory_read(auth_context, db_client=db)` before reading developer rollout state or calling `search_memory_default_developer_memories_vector(...)`.
- Missing app/key identity, missing/wrong authenticated `memories.read` scope, missing/malformed persisted app/key grant state, disabled grant, missing persisted scope, or missing `default_read=true` returns 403 before any memory vector query, repair/outbox callback, or `users/{uid}/memory_items` hydration.
- Valid app/key/scope grant continues to the existing memory vector adapter and mandatory projection/account-generation fences. Default-read vector policy keeps `archive_capability=false`; no Archive vector path or default Archive exposure was added.

Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED route-order/static test (`1 failed, 19 passed`), GREEN focused tests (`35 passed`), full `pytest tests/unit/test_memory_*.py -q` (`280 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.

This closes the narrow Developer API vector-route P0-1/P0-6 app/key/scope composition subpoint. Remaining P0-1/P0-6 work:

- MCP REST/SSE still need persisted key scopes or OAuth token introspection plus route execution context carrying app/key/scope identity before memory reads/tools execute.
...
Continued Oracle P0-1/P0-6 with the first MCP memory authorization context/readiness helper, without route enforcement or changing the production rollout verdict:

- Added `McpVerifiedAuth`, `MCP_MEMORY_DEFAULT_MEMORY_READ_SURFACE`, and `build_mcp_default_memory_read_context(...)` in `backend/utils/mcp_memories.py`.
- The helper can carry `uid`, stable `app_id`, `key_id`, verified MCP scopes, `consumer='mcp'`, and `surface='mcp_default_memory_read'` into `ProductAuthorizationContext` for the existing `authorize_memory_external_default_memory_read(...)` composition seam.
...
- Missing app/key identity or missing verified `memories.read` scope fails closed through deterministic shared grant reasons before any future memory read can proceed; valid injected MCP context plus stored `grants.mcp.apps.{app_id}.keys.{key_id}` default-read grant allows a default-read policy with `archive_capability=false`.
- Added `docs/epics/memory_mcp_app_key_scope_readiness.md`, listing MCP REST routes/tools, SSE tool security schemes/OAuth advertised scopes, current returned values, MCP key model/storage gaps, required future route execution context, safe composition point, RED tests needed, blockers, and explicit non-claims.

Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED missing MCP helper import, GREEN helper tests (`5 passed`), focused auth/MCP/product regression (`32 passed`), full `pytest tests/unit/test_memory_*.py -q` (`285 passed, 1 warning`), and async blocker scan exit 0 with pre-existing findings only.
...
- Wire MCP REST `/v1/mcp/memories/search` and streamable HTTP/SSE `search_memories` to deny before memory vector/default reads when app/key/scope grant composition fails.
- Decide whether `get_memories` becomes memory default-list; if so, wire the same context before `memory_items` access.
- Deployed Firestore rules/IAM proof against a real target project remains not run.
- Production rollout remains **BLOCKED / NO-GO** until all Oracle P0s and required real-service evidence are complete.
...
- Added `get_mcp_api_key_auth(...)` and `get_mcp_memory_default_memory_read_context(...)`; the memory helper only builds a MCP `ProductAuthorizationContext` when persisted `memories.read`, `app_id`, and `key_id` are present. Old keys and keys without persisted scopes fail closed for memory authorization rather than inheriting advertised MCP/OAuth scopes.
...
- Updated `docs/epics/memory_mcp_app_key_scope_readiness.md` to reflect the storage/cache/dependency contract, migration/default behavior, OAuth-introspection gap, and remaining REST/SSE route wiring blockers.
...
- Wire MCP REST `/v1/mcp/memories/search` and streamable HTTP/SSE `search_memories` to pass the verified context into `authorize_memory_external_default_memory_read(...)` and deny before memory vector/default reads when composition fails.
...
- Changed REST `GET /v1/mcp/memories/search` from uid-only `get_uid_from_mcp_api_key` to `get_mcp_memory_default_memory_read_context(...)`.
- The route now calls `authorize_memory_external_default_memory_read(auth_context, db_client=db)` before reading MCP rollout state, calling `search_default_mcp_memories_vector(...)`, touching legacy vector fallback, or hydrating `users/{uid}/memory_items`.
- Missing app/key identity, missing/wrong persisted `memories.read` scope, missing/malformed `users/{uid}/memory_control/app_key_memory_grants`, disabled grant, missing persisted scope, or missing `default_read=true` returns 403 before memory reads/vector side effects.
- Valid persisted MCP key scope plus a matching stored `grants.mcp.apps.{app_id}.keys.{key_id}` default-read grant reaches the existing memory MCP vector adapter; the resulting default-read policy keeps `archive_capability=false`.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED route-order/static test (`1 failed, 14 passed`), GREEN route/static tests (`15 passed`), focused MCP/auth/product regression (`37 passed, 2 warnings`), full memory regression (`290 passed, 3 warnings`), and async blocker scan exit 0 with pre-existing findings only.
...
- Wire streamable HTTP/SSE `search_memories` through verified app/key/scope context and stored grant composition before tool memory reads.
...
- `MCPSession`, `handle_mcp_message(...)`, and `execute_tool(...)` now thread an optional `ProductAuthorizationContext` into tool calls while keeping uid-only helper compatibility available for untouched/non-memory direct paths.
- The `search_memories` tool calls `authorize_memory_external_default_memory_read(auth_context, db_client=db)` before reading MCP rollout state, calling `search_default_mcp_memories_vector(...)`, touching legacy vector fallback, or hydrating `memory_items` candidates.
- Missing app/key identity, missing/wrong persisted `memories.read` scope, missing/malformed `users/{uid}/memory_control/app_key_memory_grants`, disabled grant, missing persisted grant scope, or missing `default_read=true` fails closed with MCP tool error `-32009` before memory reads/vector side effects.
- Valid persisted MCP key scope plus a matching stored `grants.mcp.apps.{app_id}.keys.{key_id}` default-read grant reaches the existing memory MCP vector adapter; default-read policy keeps `archive_capability=false` and no Archive path/default exposure was added.

Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED SSE route/static tests (`2 failed, 14 passed`), GREEN route/static tests (`16 passed`), focused MCP/auth/product regression (`38 passed, 2 warnings`), full memory regression (`291 passed, 3 warnings`), and async blocker scan exit 0 with pre-existing findings only.
...
- Updated `docs/epics/memory_mcp_app_key_scope_readiness.md` with commands, prerequisites, pass/fail criteria, and non-claims.

Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED missing-runner/doc failures (`4 failed`), GREEN runner/doc tests (`4 passed`), default readiness run `python3 backend/scripts/v17_mcp_api_key_scope_readiness.py` produced JSON `status: "NOT_RUN"`, focused MCP/auth/product regression and full memory regression were rerun, and async scan remained pre-existing findings only.
...
- Deployed Firestore rules/IAM proof against a real target project remains not run.
- App/key memory grants at `users/{uid}/memory_control/app_key_memory_grants` still require server-owned product/admin assignment separate from MCP key scopes.
- Production rollout remains **BLOCKED / NO-GO** until all Oracle P0s and required real-service evidence are complete.
...
- Extended `backend/scripts/v17_firestore_rules_iam_proof.py` pass/fail inventory to include `mcp_api_keys/{key_id}` and `users/{uid}/memory_control/app_key_memory_grants` alongside the existing memory vector repair outbox/control paths.
...
### 2026-06-19 — P0-1/P0-6 memory app/key memory grant assignment readiness runner

Continued Oracle P0-1/P0-6 by adding a safe admin-readiness runner for server-owned memory app/key memory grant assignment at `users/{uid}/memory_control/app_key_memory_grants`, without changing the production rollout verdict:
...
- Writes are unreachable unless both `--execute` and `--allow-write` are supplied with a deterministic assignment file; write plans target only `users/{uid}/memory_control/app_key_memory_grants` at `grants.<consumer>.apps.<app_id>.keys.<key_id>`.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED missing-runner/doc failures (`5 failed`), GREEN runner/doc tests, default readiness run `python3 backend/scripts/v17_app_key_memory_grant_assignment_readiness.py` produced JSON `status: "NOT_RUN"`, focused P0-1/P0-6 tests passed, full memory regression passed, and async scan remained pre-existing findings only.
...
### 2026-06-19 — P0-5 shared ns2 legacy/memory isolation local readiness

Continued Oracle P0-5 by adding a safe local guard/readiness slice for shared `ns2` legacy/memory vector isolation, without changing the production rollout verdict:
...
- The runner inventories legacy `ns2` memory search paths (`database.vector_db.find_similar_memories`, `database.vector_db.search_memories_by_vector`), required memory metadata barriers, stale/deleted physical-ID risk, and P0-7 overfetch/refill implications.
- Added `build_legacy_memory_vector_filter(...)` in `backend/database/vector_db.py` and wired legacy `find_similar_memories(...)` plus `search_memories_by_vector(...)` to include exact uid filtering and `memory_schema_version: {"$exists": false}` before top-k, preserving subject filtering where applicable.
- This local guard prevents memory schema vectors from consuming legacy top-k slots in the legacy adapters before hydration. It does not prove real Pinecone coexistence, stale/deleted physical-ID cleanup, baseline recall retention, or provider behavior.

Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED missing-runner/filter tests (`4 failed`), GREEN readiness/filter tests (`4 passed`), default readiness run `python3 backend/scripts/v17_shared_ns2_legacy_isolation_readiness.py` produced JSON `status: "NOT_RUN"`, execute readiness returned exit 2 due missing Pinecone prerequisites, focused vector/filter tests passed, full memory regression passed, and async scan remained pre-existing findings only.
...
- Stale/deleted physical-ID and duplicate memory physical-ID proof remains provider-backed work.
...
Started Oracle P0-7 with a narrow local algorithmic hardening slice for memory default vector search, without changing the production rollout verdict:

- `fetch_default_vector_memory_search(...)` now uses configurable, fake-injectable bounded overfetch/refill parameters: `overfetch_factor` defaults to 3 and is capped at 10; `max_candidates` defaults to 50 and is hard capped at `MAX_MEMORY_VECTOR_SEARCH_LIMIT=100`.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED vector-service tests failed on the missing `overfetch_factor` parameter (`2 failed, 9 passed`); GREEN vector-service tests passed (`11 passed`); focused product/chat/MCP/developer vector caller regression passed (`60 passed`); full memory regression passed (`306 passed, 3 warnings`); async scan remained pre-existing findings only.
...
### 2026-06-19 — P0-7 memory vector-search low-cardinality telemetry seam

Continued Oracle P0-7 with a narrow local central telemetry seam for hydrated memory vector search, without changing the production rollout verdict:

- Added `backend/utils/memory/v17_vector_search_telemetry.py` with `VectorSearchTelemetryConfig` and `emit_memory_vector_search_telemetry(...)`.
- Wired optional `telemetry_emitter` / `telemetry_config` into `fetch_default_vector_memory_search(...)`; missing or disabled telemetry is a no-op.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED collection failure for missing telemetry module (`ModuleNotFoundError`), GREEN vector-service tests passed (`13 passed`), focused product/chat/MCP/developer vector caller regression passed (`60 passed`), full memory regression passed (`308 passed, 3 warnings`), and async scan remained pre-existing findings only.
...
### 2026-06-19 — P0-7 memory vector-search timeout/rate-limit control seam

Continued Oracle P0-7 with explicit fake-injectable local timeout/rate-limit controls for hydrated memory vector search, without changing the production rollout verdict:

- `fetch_default_vector_memory_search(...)` now accepts bounded `max_vector_queries`, `max_candidate_hydration_reads`, `timeout_seconds`, and an injected monotonic `clock` for deterministic tests without sleeps.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED vector-service tests failed on missing control kwargs (`4 failed, 13 passed`); GREEN vector-service tests passed (`17 passed`); focused product/chat/MCP/developer vector caller regression passed (`60 passed`); full memory regression passed (`312 passed, 3 warnings`); async scan remained pre-existing findings only.
...
### 2026-06-19 — P0-7 memory vector-search provider-proof readiness artifact

Continued Oracle P0-7 with `backend/scripts/vector_search_provider_readiness.py`, a safe-by-default provider-proof/readiness artifact for the real Pinecone/Firestore evidence still required before production rollout:
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED readiness tests failed on missing runner/docs (`4 failed`); intermediate GREEN for runner failed only on missing docs (`1 failed, 3 passed`); final GREEN readiness tests passed; default readiness output returned `NOT_RUN` with missing prerequisites; execute without prerequisites exited 2 with `NOT_RUN`; full memory regression and async scan remained green/pre-existing only.
...
### 2026-06-19 — Oracle P0-8 memory cutover evidence readiness/checklist artifact

Started Oracle P0-8 with `backend/scripts/cutover_evidence_readiness.py`, a safe-by-default production-cutover evidence readiness checklist. It does not attempt cloud/provider execution and does not claim approval:
...
- The artifact explicitly references existing local route/adapter/test surfaces: `backend/routers/memories.py GET /v3/memories`, `backend/database/memories.py get_memories`, `backend/utils/memory/product_memory_read_service.py`, `backend/utils/memory/developer_memory_adapter.py`, `backend/utils/memory/mcp_memories.py`, `backend/utils/memory/chat_memory_adapter.py`, `backend/routers/mcp.py`, `backend/routers/mcp_sse.py`, `backend/routers/developer.py`, and existing memory read/caller tests.
...
Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED matrix tests failed on missing runner/docs/cutover links (`4 failed`); final GREEN/static/readiness/regression/async passed. This slice does **not** execute production traffic, Pinecone/Firestore/cloud calls, mutate shared `ns2` or Firestore, collect benchmarks, wire central telemetry, claim T21 completion, or approve cutover. Production rollout remains **BLOCKED / NO-GO**.
...
- Default and `--execute` CLI modes emit `status=BLOCKED`, `read_only=true`, `mutation_allowed=false`, `network_or_provider_calls_executed=false`, `provider_calls_executed=false`, `benchmark_evidence_collected=false`, `approval_claimed=false`, and `production_rollout_approved=false`.
- The proof matrix remains `NOT_RUN` with empty evidence arrays for external create/edit/delete/list/search write/read convergence; Developer API write/read paths; MCP REST/SSE write/read/list/search paths; chat/tool/agent caller coverage; dual-write/outbox or memory-write convergence plan; delete/review/import compatibility; no legacy unsafe fallback after memory writes; app/key/scope grant enforcement; Archive default-unavailable; response-shape compatibility; and rollback/disable behavior.
- The artifact explicitly references existing local route/adapter/test surfaces: `backend/routers/memories.py`, `backend/routers/developer.py`, `backend/routers/mcp.py`, `backend/routers/mcp_sse.py`, `backend/routers/tools.py`, `backend/routers/agent_tools.py`, `backend/database/memories.py`, the legacy write guard, external default-read authorization, memory Developer/MCP/chat/product adapters, and existing guard/caller tests.
- The production cutover checklist now points the T22/T23 gate at this matrix and requires proof of external create/edit/delete/list/search write/read convergence, Developer API write/read paths, MCP REST/SSE write/read/list/search paths, chat/tool/agent caller coverage, durable memory-write convergence or dual-write/outbox evidence, delete/review/import compatibility, no legacy unsafe fallback after memory writes, app/key/scope grant enforcement, Archive default-unavailable, response-shape compatibility, and rollback/disable behavior.

Verification recorded in `docs/epics/memory_implementation_tickets.md`: RED matrix tests failed on missing runner/docs/cutover links (`4 failed`); final GREEN/static/readiness/regression/async passed. This slice does **not** execute production traffic, Pinecone/Firestore/cloud calls, mutate shared `ns2` or Firestore, collect benchmarks, execute external writes, wire central telemetry, claim T22/T23 completion, or approve cutover. Production rollout remains **BLOCKED / NO-GO**.
...
Do not enable memory as an authoritative read path for customers, MCP, developer API, or chat. A tightly isolated shadow test that cannot affect returned results may continue, but the current code should not be treated as fail-closed production behavior.

## P0 issues

### P0-1 — Product default read is not rollout-gated; Archive capability is client-self-granted

`GET /memory/memory/search` authenticates the user and immediately reads `memory_items`; it never checks `memory_control/state`, rollout mode, allowlisting, or the Omi-chat grant.

`GET /memory/memory/archive/search` treats `include_archive=true` as sufficient to construct `archive_capability=True`. That is an explicit query, but it is **not** a server-authorized capability. It does not check a persisted Archive opt-in, app policy, rollout state, or global Archive kill switch.
...
**Required fix:** Every memory product route must call one shared server-side authorization decision before any `memory_items` access. Archive must require both a persisted capability and an explicit Archive query.

---

### P0-2 — “Fail closed” is conflated with an unsafe legacy downgrade

The rollout helper collapses several materially different states into “memory disabled”:
...
* A malformed or unavailable control document can fall back to legacy after memory-only writes have begun, hiding new memories or resurrecting deleted/superseded ones.
* `disabled_default_read_rollout_decision` marks legacy reads authoritative without proving `fallback_projection_ready`, reconciliation, generation, or epoch safety.
...
| Surface          | Disabled/malformed/no grant | Enabled but memory returns zero           |
| ---------------- | --------------------------- | -------------------------------------- |
| Product vector   | 403                         | Empty result                           |
| Developer vector | 403                         | Empty result                           |
| Developer list   | Legacy fallback             | memory result                             |
| Chat             | Legacy fallback             | “No memory vector memories…”; no fallback |
| MCP REST         | Legacy fallback             | Empty list; no fallback                |
| MCP SSE          | Legacy fallback             | Empty list; no fallback                |

Thus an empty or under-indexed memory vector result silently suppresses legacy memories, while a control-plane failure can silently re-enable legacy.
...
### P0-3 — Reads are memory while MCP/developer writes and deletes remain legacy

The shown MCP and developer create/edit/delete routes still mutate the legacy memory collection and legacy vectors. Meanwhile, search or list may read authoritative memory `memory_items`.

Consequences include:

* a newly created memory is not visible in memory search;
* an edit can leave memory content unchanged;
* deleting a legacy memory can leave the memory item and memory vector searchable;
* a category parameter on the developer list bypasses memory and forces the legacy path;
...
1. move all applicable create/edit/delete routes through the memory write/deletion service, or
...
The memory query has schema and tier filters, but the legacy vector functions are explicitly left untouched. With both schemas in `ns2`, an unchanged legacy top-k query may:

* retrieve memory vectors and lose useful legacy slots;
* produce different results for non-enabled users;
* accidentally hydrate a local legacy record using a memory hit with the same logical ID;
* be influenced by Archive or stale memory embeddings even when the item is later dropped.

“Legacy code untouched” is not evidence that legacy behavior remains unchanged. No real Pinecone validation or legacy-plus-memory coexistence benchmark was run.

**Required fix:** Prove with real `ns2` data that legacy queries explicitly exclude memory schema records and retain baseline recall. Otherwise add a legacy schema filter or separate namespace before inserting production memory vectors.

**2026-06-19 local readiness slice:** Added `backend/scripts/v17_shared_ns2_legacy_isolation_readiness.py`, a safe-by-default shared-namespace artifact that inventories the legacy `ns2` memory search paths (`find_similar_memories`, `search_memories_by_vector`), the required memory metadata barriers (`memory_schema_version`, `uid`, `memory_tier`, `status`, `source_state`, `restricted_sensitivity`, account/revision/source/content/projection fences), and remaining stale/deleted physical-ID plus overfetch/refill risks. Default output is `status=NOT_RUN`, `read_only=true`, `mutation_allowed=false`; `--execute` only checks provider prerequisites for a future read-only inventory and performs no Pinecone query or mutation in this slice.

Also added a narrow local code guard: legacy queries exclude memory schema before top-k selection via `{'memory_schema_version': {'$exists': False}}`, so legacy queries exclude memory schema records instead of letting memory Short-term/Long-term/Archive/stale/tombstoned candidates consume legacy result slots. No real Pinecone shared `ns2` proof, recall benchmark, provider query output, production coexistence validation, or rollout approval is claimed; real shared-`ns2` data validation and baseline recall evidence remain required.

---

### P0-6 — Third-party authorization is not shown at app/key granularity

The memory grant is stored per user and broad consumer—`mcp` or `developer_api`—rather than per MCP app, developer key, OAuth client, or installation. Enabling it appears to enable every corresponding key for the user.
...
`fetch_default_vector_memory_search` queries only `limit` vector candidates and then loads the entire authoritative `memory_items` collection into a dictionary.
...
* MCP `search_memories` may use memory, while MCP `get_memories` remains legacy.
...
**2026-06-19 local compatibility-contract/readiness slice:** Added safe-by-default `backend/scripts/p1_3_caller_api_compatibility_readiness.py` plus `backend/tests/unit/test_p1_3_caller_api_compatibility_readiness.py` to inventory the explicit Oracle P1-3 caller/API compatibility contract before external rollout. The artifact remains `status=BLOCKED`, read-only, no mutation, no Firestore reads/writes, no network/provider/cloud calls, no benchmarks, and no approval claimed. It codifies required local decisions for Product memory routes, `/v3`, Developer API default/category/vector paths, MCP REST `search_memories` and `get_memories`, MCP SSE `search_memories` and `get_memories`, chat `get_memories_tool`/`search_memories_tool`, tools REST memory endpoints, and agent execute-tool callers. Required decisions include MCP search_memories vs get_memories consistency, MCP REST vs SSE shape/fallback consistency, Developer category filtering must not force unsafe legacy, Developer response shape must not fabricate private/reviewed/edited/category defaults, disabled rollout semantics per surface: 403, empty, or legacy-safe, enabled-but-empty semantics, Archive default-unavailable, `/v3` external compatibility, and tools and agent callers. No broad runtime behavior changed; Oracle remains **BLOCKED / NO-GO** pending real compatibility proof and product-approved decisions.

Verification: RED `cd backend && pytest tests/unit/test_p1_3_caller_api_compatibility_readiness.py -q` → `4 failed in 0.08s` (missing runner/docs/test runner). Interim GREEN blocked on docs/test runner: after adding the runner, same command → `1 failed, 3 passed in 0.06s` (Oracle/ticket/test.sh links missing). Final GREEN/static: `cd backend && black --line-length 120 --skip-string-normalization scripts/p1_3_caller_api_compatibility_readiness.py tests/unit/test_p1_3_caller_api_compatibility_readiness.py && pytest tests/unit/test_p1_3_caller_api_compatibility_readiness.py -q` → `1 file reformatted, 1 file left unchanged`, `4 passed in 0.04s`. Readiness from repo root: `python3 backend/scripts/p1_3_caller_api_compatibility_readiness.py --execute` → `BLOCKED True False False False False False False False False 13 11` (`status`, read-only, no mutation, no network/provider calls, no provider calls, no Firestore reads, no Firestore writes, no benchmark evidence, no production approval, no approval claimed, 13 surfaces, 11 behavior cases). Full memory regression: `cd backend && pytest tests/unit/test_memory_*.py -q` → `344 passed, 3 warnings in 2.05s`. Async scan: `python3 backend/scripts/scan_async_blockers.py` from backend returned `Error: backend/routers not found` (wrong working directory); rerun from repo root `python3 backend/scripts/scan_async_blockers.py` → exit 0 with pre-existing `HIGH async helpers with blocking: 41`, `STRUCTURAL mixed await+sync DB: 10`. Docs hygiene: no truncation-marker literals or SUB chars under `docs/epics/memory*`. This slice does **not** run production traffic, Firestore/Pinecone/cloud/provider calls, Firestore reads/writes, migration execution, benchmark, telemetry sink integration, or rollout approval.

**2026-06-19 local MCP REST/SSE get/search parity hardening slice:** Added `list_default_mcp_memories(...)` plus `McpMemoryListResult` and wired MCP REST `GET /v1/mcp/memories` and MCP SSE `get_memories` through the same explicit memory rollout decision contract used by MCP `search_memories`. Missing/malformed/no-grant/disabled/shadow states now return an empty get/list result and do not touch legacy memory reads unless the shared decision is explicitly `USE_LEGACY_SAFE`; enabled-but-empty memory get/list returns an empty memory result without legacy fallback. The get/list adapter uses the shared product default-memory read service with MCP policy and `archive_capability=False`, preserving Archive default-unavailable and preventing stale Short-term default visibility through the default visibility policy. This is still a narrow local hardening slice: it does not resolve all MCP category/review/manual/filter response-shape choices, does not claim route-level production proof, and does not run production traffic, Firestore/Pinecone/cloud/provider calls, Firestore reads/writes, benchmarks, telemetry sink integration, or approval. Oracle remains **BLOCKED / NO-GO** pending remaining P1-3 compatibility decisions.

Verification: RED adapter import `cd backend && pytest tests/unit/test_memory_mcp_memory_adapter.py -q` → collection error `ImportError: cannot import name 'McpMemoryListResult'`; RED route parity after adapter implementation: same command → `2 failed, 19 passed in 0.16s` because REST/SSE get paths did not call `list_default_mcp_memories(...)`. GREEN/format/focused: `black --line-length 120 --skip-string-normalization utils/mcp_memories.py routers/mcp.py routers/mcp_sse.py tests/unit/test_memory_mcp_memory_adapter.py && pytest tests/unit/test_memory_mcp_memory_adapter.py -q` → `1 file reformatted, 3 files left unchanged`, `21 passed in 0.13s`. Focused non-memory MCP search regression attempted `pytest tests/unit/test_mcp_search_memories.py -q` and is environment-blocked at collection with `ModuleNotFoundError: No module named 'fastapi'`. Full memory regression: `cd backend && pytest tests/unit/test_memory_*.py -q` → `349 passed, 3 warnings in 2.15s`. P1-3 readiness from repo root: `python3 backend/scripts/p1_3_caller_api_compatibility_readiness.py --execute` → `BLOCKED True False False False False False False False False 13 11`. Async scan: `python3 backend/scripts/scan_async_blockers.py` from repo root exited 0 with pre-existing `HIGH async helpers with blocking: 41`, `STRUCTURAL mixed await+sync DB: 10`.

**2026-06-19 local Developer API category/response-shape compatibility hardening slice:** Hardened the first Developer API P1-3 runtime seam. `GET /v1/dev/user/memories` no longer forces category-filtered requests into a legacy-safe fallback solely because `categories` is present; it now performs the same app/key/scope grant authorization and server-owned rollout read for filtered and unfiltered Developer default-list calls, then passes normalized category values to `search_memory_default_developer_memories(...)`. The adapter filters memory default-readable results locally without touching legacy memory reads unless a shared rollout decision explicitly returns `USE_LEGACY_SAFE`. Developer memory formatting now propagates authoritative memory `visibility` from `MemoryItem` through the product read result (`visibility_source="memory_item.visibility"`) and marks compatibility-derived fields instead of silently fabricating them: `category=other` has `category_source="developer_memory_compatibility_default_no_source_category"`, `reviewed=False` has `reviewed_source="developer_memory_compatibility_default_no_review_state"`, `edited=False` has `edited_source="developer_memory_compatibility_default_no_edit_state"`, and `manually_added=False` has an explicit source marker. Archive remains default-unavailable through the existing Developer memory policy (`archive_capability=False`), and stale Short-term remains filtered by the shared product default-memory visibility policy. This is still a narrow local hardening slice: it does not prove all Developer legacy client compatibility, does not make `/v3`, tools, agent, chat, or remaining MCP shape decisions, does not run production traffic, Firestore/Pinecone/cloud/provider calls, Firestore reads/writes, benchmarks, telemetry sink integration, or approval. Oracle remains **BLOCKED / NO-GO** pending remaining P1-3 compatibility decisions and production proof.

Verification: RED `cd backend && pytest tests/unit/test_memory_developer_memory_adapter.py -q` → `3 failed, 20 passed in 0.15s` (`developer_category_legacy_safe_fallback_explicit` still present, `search_memory_default_developer_memories()` rejected `categories`, and memory Developer formatting returned fabricated `visibility='private'` instead of source `public`). GREEN/format/focused/regression: `cd backend && black --line-length 120 --skip-string-normalization utils/memory/v17_developer_memory_adapter.py utils/memory/memory_read_api.py routers/developer.py tests/unit/test_memory_developer_memory_adapter.py && pytest tests/unit/test_memory_developer_memory_adapter.py -q && pytest tests/unit/test_memory_*.py -q` → `4 files left unchanged`, `23 passed in 0.08s`, `352 passed, 3 warnings in 2.06s`. P1-3 readiness from repo root remains safe/read-only and blocked: `python3 backend/scripts/p1_3_caller_api_compatibility_readiness.py --execute` → status `BLOCKED`, `read_only=true`, `mutation_allowed=false`, `network_or_provider_calls_executed=false`, `provider_calls_executed=false`, `firestore_reads_executed=false`, `firestore_writes_executed=false`, `benchmark_evidence_collected=false`, `production_rollout_approved=false`, `approval_claimed=false`, with 13 surfaces and 11 behavior cases. Async scan: `python3 backend/scripts/scan_async_blockers.py` from repo root exited 0 with pre-existing `HIGH async helpers with blocking: 41`, `STRUCTURAL mixed await+sync DB: 10`. Docs hygiene: no truncation-marker literals or SUB chars under `docs/epics/memory*`.

**2026-06-19 local MCP category/review/manual response-shape source hardening slice:** Hardened the next MCP P1-3 runtime seam. memory MCP list/search formatting now exposes explicit compatibility-derived source semantics instead of silently fabricating unsupported legacy fields: `category=other` is accompanied by `category_source="mcp_memory_compatibility_default_no_source_category"`, `reviewed=False` by `reviewed_source="mcp_memory_compatibility_default_no_review_state"`, and `manually_added=False` by `manually_added_source="mcp_memory_compatibility_default_no_manual_state"`. MCP REST `CleanerMemory` exposes those additive fields plus memory/default policy metadata so REST responses do not strip the markers, and MCP SSE tool responses continue to return adapter dictionaries directly. `list_default_mcp_memories(...)` now accepts category/review/manual filters and applies them to the explicit compatibility fields; REST `GET /v1/mcp/memories` and SSE `get_memories` pass normalized filters into the shared memory list adapter. Archive remains default-unavailable through MCP policy (`archive_capability=False`), stale Short-term remains filtered by the shared product default-memory visibility policy, and malformed/no-grant/disabled rollout states still fail closed without unsafe legacy reads unless the shared decision explicitly returns `USE_LEGACY_SAFE`. This is still a narrow local hardening slice: it does not prove all MCP legacy client compatibility, does not resolve `/v3`, tools, agent, chat, production telemetry, or benchmark evidence, and does not run production traffic, Firestore/Pinecone/cloud/provider calls, Firestore reads/writes, benchmarks, telemetry sink integration, or approval. Oracle remains **BLOCKED / NO-GO** pending remaining P1-3 compatibility decisions and production proof.

Verification: RED `cd backend && pytest tests/unit/test_memory_mcp_memory_adapter.py -q` → `3 failed, 21 passed in 0.17s` (`category_source` missing, `list_default_mcp_memories()` rejected `categories`, and REST `CleanerMemory` did not expose source fields). GREEN/format/focused/regression: `cd backend && black --line-length 120 --skip-string-normalization utils/mcp_memories.py routers/mcp.py routers/mcp_sse.py tests/unit/test_memory_mcp_memory_adapter.py && pytest tests/unit/test_memory_mcp_memory_adapter.py -q && pytest tests/unit/test_memory_*.py -q` → `1 file reformatted, 3 files left unchanged`, `24 passed in 0.13s`, `355 passed, 3 warnings in 2.06s`. P1-3 readiness from repo root remains safe/read-only and blocked: `python3 backend/scripts/p1_3_caller_api_compatibility_readiness.py --execute` → status `BLOCKED`, `read_only=true`, `mutation_allowed=false`, `network_or_provider_calls_executed=false`, `provider_calls_executed=false`, `firestore_reads_executed=false`, `firestore_writes_executed=false`, `benchmark_evidence_collected=false`, `production_rollout_approved=false`, `approval_claimed=false`, with 13 surfaces and 11 behavior cases. Async scan: `python3 backend/scripts/scan_async_blockers.py` from repo root exited 0 with pre-existing `HIGH async helpers with blocking: 41`, `STRUCTURAL mixed await+sync DB: 10`. Docs hygiene: no truncation-marker literals or SUB chars under `docs/epics/memory*`.

**2026-06-19 local `/v3` external compatibility readiness slice:** Added safe-by-default `backend/scripts/p1_3_v3_external_compatibility_readiness.py` plus `backend/tests/unit/test_p1_3_v3_external_compatibility_readiness.py` and registered the test in `backend/test.sh`. The artifact pins the exact `/v3` route surfaces that remain unresolved before broad runtime changes: `GET /v3/memories`, `POST /v3/memories`, `POST /v3/memories/batch`, `PATCH /v3/memories/{memory_id}`, `DELETE /v3/memories/{memory_id}`, and missing single-read/search surfaces (`GET /v3/memories/{memory_id}`, `GET /v3/memories/search`). It explicitly blocks on disabled/malformed/no-grant semantics, enabled-empty semantics, response shape/source metadata, Archive default-unavailable, category/filter behavior, unsafe legacy fallback after memory writes, and cursor pagination stability. Default and `--execute` modes remain `status=BLOCKED`, read-only, no mutation, no Firestore reads/writes, no network/provider/cloud calls, no benchmarks, no production approval, and no approval claimed. No `/v3` runtime behavior changed; Oracle remains **BLOCKED / NO-GO** pending product-approved compatibility decisions and production proof.

Verification: RED `cd backend && pytest tests/unit/test_p1_3_v3_external_compatibility_readiness.py -q` → `4 failed in 0.07s` (missing runner/test.sh/docs). GREEN/format/focused/readiness/regression/async/docs hygiene: `cd backend && black --line-length 120 --skip-string-normalization scripts/p1_3_v3_external_compatibility_readiness.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py && pytest tests/unit/test_p1_3_v3_external_compatibility_readiness.py -q` → `2 files reformatted`, `4 passed in 0.05s`; readiness from repo root `python3 backend/scripts/p1_3_v3_external_compatibility_readiness.py --execute` → `BLOCKED True False False False False False False False False 7 7`; P1-3 readiness from repo root `python3 backend/scripts/p1_3_caller_api_compatibility_readiness.py --execute` → `BLOCKED True False False False False False False False False 13 11`; full memory regression `cd backend && pytest tests/unit/test_memory_*.py -q` → `359 passed, 3 warnings in 2.20s`; async scan `python3 backend/scripts/scan_async_blockers.py` from repo root exited 0 with pre-existing `HIGH async helpers with blocking: 41`, `STRUCTURAL mixed await+sync DB: 10`; docs hygiene found no truncation-marker literals or SUB chars under `docs/epics/memory*`.

**2026-06-20 local `/v3` canary approval lifecycle/evidence-bundle readiness slice:** Added safe-by-default `backend/scripts/p1_3_v3_canary_approval_lifecycle_readiness.py` plus `backend/tests/unit/test_p1_3_v3_canary_approval_lifecycle_readiness.py`, registered it in `backend/test.sh`, and linked `canary_approval_lifecycle_readiness_proof` into source-selection, production read-proof, observability approval, `/v3` external compatibility, and GET runtime-wiring readiness. The contract defines required human/ops approval ticket/id, bounded owner groups and approver roles, issued/approved/expires_at expiry and rotation constraints, rollback owner/steps, monitoring gate ids, production read proof reference, IAM/emulator proof reference, telemetry/runbook reference, and exact `GET /v3/memories` route scope before any artifact can be considered production-usable. It inventories fail-closed semantics for missing/stale approval evidence, missing rollback, missing monitoring gates, missing production read proof, missing IAM/emulator proof, and route mismatch while preserving no production calls, no route wiring, and no approval claim.

Verification: RED `cd backend && env -u VIRTUAL_ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin python3 -m pytest tests/unit/test_p1_3_v3_canary_approval_lifecycle_readiness.py -q` → `5 failed in 0.10s` (missing runner/readiness links/docs). GREEN/focused/full/readiness/async/docs outputs are recorded in the handoff for this commit; readiness remains **BLOCKED / NO-GO**.

**2026-06-20 local `/v3` canary/approval production read-proof readiness slice:** Added production-safe backend service-principal read proof runner `backend/scripts/p1_3_v3_canary_approval_production_readiness.py` plus `backend/tests/unit/test_p1_3_v3_canary_approval_production_readiness.py`, registered it in `backend/test.sh`, and linked `canary_approval_production_readiness_proof` into source-selection, observability approval, `/v3` external compatibility, and GET runtime-wiring readiness. The runner defaults to `NOT_RUN`/`BLOCKED`; `--execute` still requires explicit project id, backend service account email, credentials, and `V3_CANARY_APPROVAL_PROD_READ_ALLOW=1` before a single read-only backend service-principal document read of `system/v3_canary_approvals/routes/get_v3_memories`. It validates any returned artifact with the existing `validate_memory_v3_canary_approval_artifact(...)` semantics for exact `GET /v3/memories` route scope, bounded owner/cohort metadata, rollback plan, monitoring gates, metadata-only approval, and no high-cardinality/sensitive fields. Even a valid future artifact only proves read/existence/shape; production rollout approval remains separate and unclaimed. Runtime remains **BLOCKED / NO-GO**; no `backend/routers/memories.py` change, `/v3` behavior change, production write, vector/provider call, telemetry sink call, PII/raw-content/secret/cursor logging, Archive default visibility, stale Short-term default visibility, or legacy fallback/merge is claimed.

Verification: RED `cd backend && env -u VIRTUAL_ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin python3 -m pytest tests/unit/test_p1_3_v3_canary_approval_production_readiness.py -q` → `5 failed in 0.10s` (missing runner). GREEN/focused/full/readiness/async/docs outputs are recorded in the handoff for this commit; readiness remains **BLOCKED / NO-GO**.

**2026-06-20 local `/v3` observability/telemetry approval readiness slice:** Added safe-by-default `backend/scripts/p1_3_v3_observability_approval_readiness.py` plus `backend/tests/unit/test_p1_3_v3_observability_approval_readiness.py`, registered it in `backend/test.sh`, and linked `observability_approval_readiness_proof` into both `/v3` external compatibility readiness and GET runtime-wiring readiness. The artifact inventories the required future `GET /v3/memories` telemetry labels for read source, route decision, low-cardinality failure reason, control/projection/account generation, cursor validation result/reason, canary cohort/enrollment, no-legacy-fallback, projection source/generation, request limit/cursor/no-offset shape, Archive/default visibility, rollback/read-disable gate, and approval owner/status. It links existing repository mechanisms honestly (`backend/routers/metrics.py` + `backend/utils/metrics.py`, `backend/utils/log_sanitizer.py`, and memory read-decision model concepts) while marking them **not wired** to `/v3` GET. Blockers remain for a real memory `/v3` telemetry sink, structured event sink choice, server-owned canary artifact, rollback/read-disable gate wiring, and explicit product/privacy/operational approval. Static guardrails preserve no PII/raw memory content, no high-cardinality exception/user/session labels, no secret/cursor token logging, no production calls by default, and no approval claimed. Runtime remains **BLOCKED / NO-GO**; no `backend/routers/memories.py` behavior changed.

**2026-06-20 local `/v3` canary/approval artifact source-selection and ownership/IAM readiness slice:** Added safe-by-default `backend/scripts/p1_3_v3_canary_approval_source_readiness.py` plus `backend/tests/unit/test_p1_3_v3_canary_approval_source_readiness.py`, registered it in `backend/test.sh`, and linked `canary_approval_source_readiness_proof` into observability approval, `/v3` external compatibility, and GET runtime-wiring readiness. The artifact selects the future server-owned route-scoped approval source `firestore:system/v3_canary_approvals/routes/get_v3_memories`, pins bounded owner groups (`product_privacy_ops`, `memory_platform_oncall`), forbids client-supplied approval artifacts, requires direct-client-deny rules/IAM or emulator proof and backend service-principal read proof before runtime wiring, now inventories static/emulator-harness readiness for signed-in client read/create/update/delete denial plus Admin-context service-principal read fixture coverage via `backend/scripts/v17_firestore_rules_emulator_test.mjs` / `npm run test:memory-v3-canary-approval-source:emulator`, and keeps production artifact existence/read/approval as missing/not-run. No `backend/routers/memories.py` change, runtime `/v3` behavior change, production Firestore/cloud/provider/vector/network call, telemetry sink production call, rollout approval, PII/raw-content/secret/cursor logging, Archive default visibility, stale Short-term default visibility, or legacy fallback/merge is claimed.

Verification: RED `cd backend && env -u VIRTUAL_ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin python3 -m pytest tests/unit/test_p1_3_v3_canary_approval_source_readiness.py -q` → `5 failed in 0.08s` (missing runner/test.sh/docs/readiness links). GREEN/focused/full/async/docs outputs are recorded in the handoff for this commit; readiness remains **BLOCKED / NO-GO**.

Verification: RED `cd backend && env -u VIRTUAL_ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin python3 -m pytest tests/unit/test_p1_3_v3_observability_approval_readiness.py -q` → `6 failed in 0.09s` (missing runner/test.sh/docs/readiness links). GREEN/focused/full/async/docs outputs are recorded in the handoff for this commit; no production telemetry sink, Firestore, cloud, provider, Pinecone, or vector calls were executed.

**2026-06-19 local `/v3` compatibility decision-blocker evidence tightening slice:** Tightened the existing safe `/v3` readiness artifact instead of adding runtime behavior. The artifact now carries exact code-route evidence from `backend/routers/memories.py`: `GET /v3/memories` uses `response_model=List[MemoryDB]`, accepts only `limit`/`offset`, overrides the first page to `limit=5000`, and directly calls `memories_db.get_memories(uid, limit, offset)` with no route-local memory read decision, no source metadata contract, no category/cursor/include_archive/source query support, and no enabled-empty memory seam. It also pins write-side blockers for `POST /v3/memories` and `DELETE /v3/memories/{memory_id}` as legacy DB/vector write paths without memory write/tombstone convergence, so read cutover cannot safely broaden legacy fallback after memory writes. Added an explicit runtime-decision matrix for disabled, malformed, missing, no-default-memory-grant, enabled-empty, Archive-default, response-shape/source metadata, and cursor-pagination states; all unsafe legacy fallback remains disallowed unless a separate explicit legacy-safe product decision is made. Product dependencies are now enumerated for disabled/malformed/no-grant policy, enabled-empty policy, response-shape/source metadata, cursor pagination, and write convergence before read cutover. The artifact remains `status=BLOCKED`, read-only, no mutation, no Firestore reads/writes, no network/provider/cloud calls, no benchmarks, no production approval, and no approval claimed. No stale Short-term default visibility or Archive default exposure was introduced; no `/v3` runtime behavior changed. Oracle remains **BLOCKED / NO-GO**.

Verification: RED `cd backend && pytest tests/unit/test_p1_3_v3_external_compatibility_readiness.py -q` → `3 failed, 3 passed in 0.07s` (`KeyError: 'route_decorator'`, `KeyError: 'runtime_decision_matrix'`, missing summary counts). GREEN/format/focused: `cd backend && black --line-length 120 --skip-string-normalization scripts/p1_3_v3_external_compatibility_readiness.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py && pytest tests/unit/test_p1_3_v3_external_compatibility_readiness.py -q` → `1 file reformatted, 1 file left unchanged`, `6 passed in 0.05s`. Readiness from repo root: `python3 backend/scripts/p1_3_v3_external_compatibility_readiness.py --execute` → `BLOCKED True False False False False False False False False 7 7 8 5`. Full memory regression: `cd backend && pytest tests/unit/test_memory_*.py -q` → `383 passed, 3 warnings in 2.34s`. Async scan: `python3 backend/scripts/scan_async_blockers.py` from repo root exited 0 with pre-existing `HIGH async helpers with blocking: 41`, `STRUCTURAL mixed await+sync DB: 10`.

**2026-06-19 local `/v3` pure compatibility decision-service seam:** Added the first Oracle-prescribed pure/local `/v3` memory compatibility decision seam in `backend/utils/memory/v3_compatibility.py` with unit proof in `backend/tests/unit/test_v3_compatibility.py` and registered it in `backend/test.sh`. The service is not wired into `backend/routers/memories.py` and performs no app startup, no Firestore/Pinecone/cloud/provider/network calls, and no mutations. It models Oracle defaults only: non-enrolled users remain legacy-primary; enrolled missing/malformed/uid-mismatch/unsupported-schema/control-timeout states fail closed with 503 and no legacy fallback; absent default-memory grant denies with 403 as privacy/consent default and remains product-overridable only by explicit future decision; enabled-empty/projection-empty returns `200 []` from the memory compatibility projection with no legacy fallback; write convergence or projection unreadiness fails closed before memory read cutover; Archive remains default-unavailable; response body contract stays `List[MemoryDB]` with source/read-decision metadata as additive headers only; and memory cursor mode is additive, opaque/signed/keyset/generation/projection-bound with no offset or first-page `limit=5000` override. The `/v3` readiness artifact now links this local proof as `decision_service_proof` while preserving `status=BLOCKED` / **NO-GO** and no production rollout approval.

Verification: RED `cd backend && pytest tests/unit/test_v3_compatibility.py -q` → collection error `ModuleNotFoundError: No module named 'utils.memory.v3_compatibility'`; RED readiness/docs/test-runner proof `cd backend && pytest tests/unit/test_p1_3_v3_external_compatibility_readiness.py -q` → `3 failed, 6 passed in 0.07s` (`KeyError: 'decision_service_proof'`, missing summary key, missing test.sh registration/docs links). GREEN verification is recorded with the commit summary for this slice; no runtime `/v3` route behavior changed and Oracle remains **BLOCKED / NO-GO**.

**2026-06-19 local `/v3` cursor signing/parsing seam:** Added the next Oracle-prescribed pure/local `/v3` compatibility seam in `backend/utils/memory/v3_cursor.py` with unit proof in `backend/tests/unit/test_memory_v3_cursor.py` and registered it in `backend/test.sh`. The service is not wired into `backend/routers/memories.py` and performs no app startup, no Firestore/Pinecone/cloud/provider/network calls, and no mutations. It creates opaque HMAC-signed keyset cursors over `created_at desc` plus `memory_id desc`, binds `uid`, `account_generation`, `projection_generation`, `filter_hash`, `source`, and `read_mode`, enforces expiry, rejects tampering and mismatched caller context fail-closed, and explicitly disallows offset plus the legacy first-page `limit=5000` override in memory cursor mode. The `/v3` readiness artifact now links this local proof as `cursor_service_proof` while preserving `status=BLOCKED` / **NO-GO** and no production rollout approval. Archive remains default-unavailable and no stale Short-term default-visible behavior was introduced.

Verification: RED `cd backend && pytest tests/unit/test_memory_v3_cursor.py -q` → collection error `ModuleNotFoundError: No module named 'utils.memory.v3_cursor'`; RED readiness/docs/test-runner proof `cd backend && pytest tests/unit/test_p1_3_v3_external_compatibility_readiness.py -q` → `3 failed, 6 passed in 0.08s` (`cursor_service_proof`, summary key, missing test.sh/docs links). GREEN/format/focused/readiness/regression/async/docs hygiene: `cd backend && black --line-length 120 --skip-string-normalization utils/memory/v3_cursor.py tests/unit/test_memory_v3_cursor.py scripts/p1_3_v3_external_compatibility_readiness.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py && pytest tests/unit/test_memory_v3_cursor.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py -q` → `4 files left unchanged`, `13 passed in 0.07s`; focused/regression `pytest tests/unit/test_v3_compatibility.py tests/unit/test_memory_v3_cursor.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py -q && pytest tests/unit/test_memory_*.py -q` → `21 passed in 0.10s`, `398 passed, 3 warnings in 2.32s`; `/v3` readiness summarized `BLOCKED True False False False False False False False False 7 7 8 5 8 2 13 True True`; P1-3 caller readiness summarized `BLOCKED True False False False False False False False False 13 11`; async scan from repo root exited 0 with pre-existing `HIGH async helpers with blocking: 41`, `STRUCTURAL mixed await+sync DB: 10`; docs hygiene `docs_hygiene 16 BAD=[]`. No runtime `/v3` route behavior changed and Oracle remains **BLOCKED / NO-GO**.

**2026-06-19 local `/v3` projection-readiness seam:** Added the next Oracle-prescribed pure/local memory-derived compatibility projection readiness seam in `backend/utils/memory/v3_projection_readiness.py` with unit proof in `backend/tests/unit/test_v3_projection_readiness.py` and registered it in `backend/test.sh`. The service is not wired into `backend/routers/memories.py` and performs no app startup, no Firestore/Pinecone/cloud/provider/network calls, and no mutations. It requires external create/update/delete convergence, expected/current account generation match, current projection generation, source `memory_derived_compatibility_projection`, current tombstone/delete fences, and source/projection commit/version/freshness fences before memory `/v3` read cutover. Missing, stale, inconsistent, ad hoc mapping, or legacy direct-read projection states fail closed and disallow memory cutover with no legacy fallback. Enabled-empty can return `[]` only when all readiness checks pass. The `/v3` readiness artifact now links this local proof as `projection_readiness_proof` while preserving `status=BLOCKED` / **NO-GO** and no production rollout approval. Archive remains default-unavailable and no stale Short-term default-visible behavior was introduced.

Verification: RED `cd backend && pytest tests/unit/test_v3_projection_readiness.py -q` → collection error `ModuleNotFoundError: No module named 'utils.memory.v3_projection_readiness'`; RED readiness/docs/test-runner proof `cd backend && pytest tests/unit/test_p1_3_v3_external_compatibility_readiness.py -q` → `3 failed, 6 passed in 0.08s` (`projection_readiness_proof`, summary key, missing test.sh/docs links). GREEN/format/focused/readiness/regression/async/docs hygiene: `cd backend && black --line-length 120 --skip-string-normalization utils/memory/v3_projection_readiness.py tests/unit/test_v3_projection_readiness.py scripts/p1_3_v3_external_compatibility_readiness.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py` → `4 files left unchanged`; `pytest tests/unit/test_v3_projection_readiness.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py -q` → `17 passed in 0.07s`; focused/regression `pytest tests/unit/test_v3_projection_readiness.py tests/unit/test_v3_compatibility.py tests/unit/test_memory_v3_cursor.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py -q && pytest tests/unit/test_memory_*.py -q` → `29 passed in 0.13s`, `406 passed, 3 warnings in 2.36s`; `/v3` readiness summarized `BLOCKED True False False False False False False False False 7 7 8 5 8 2 13 True True True`; P1-3 caller readiness summarized `BLOCKED True False False False False False False False False 13 11`; async scan from repo root exited 0 with pre-existing `HIGH async helpers with blocking: 41`, `STRUCTURAL mixed await+sync DB: 10`; docs hygiene `docs_hygiene 16 BAD=[]`. No runtime `/v3` route behavior changed and Oracle remains **BLOCKED / NO-GO**.

**2026-06-19 local `/v3` memory read-service composition seam:** Added the next Oracle-prescribed pure/local `/v3` compatibility read-service seam in `backend/utils/memory/v3_memory_read_service.py` with unit proof in `backend/tests/unit/test_memory_v3_memory_read_service.py` and registered it in `backend/test.sh`. The service composes only caller-supplied local inputs plus the existing decision, cursor, and projection-readiness seams. It is not wired into `backend/routers/memories.py` and performs no app startup, no Firestore/Pinecone/cloud/provider/network calls, and no mutations. Non-enrolled users receive a legacy-primary plan marker only; enrolled missing/malformed/no-grant/write-convergence-not-ready/projection-not-ready states fail closed or deny without legacy fallback; projection-ready empty returns `200 []` with no legacy fallback; projection-ready pages pass through the caller-supplied `List[MemoryDB]`-compatible body and only add read-source/read-decision/next-cursor/Link headers. memory cursor mode validates the signed cursor context, rejects offset and the legacy first-page `limit=5000` override, and never downgrades to offset or legacy. The `/v3` readiness artifact now links this local proof as `memory_read_service_proof` while preserving `status=BLOCKED` / **NO-GO** and no production rollout approval. Archive remains default-unavailable and no stale Short-term default-visible behavior was introduced.

Verification: RED `cd backend && pytest tests/unit/test_memory_v3_memory_read_service.py -q` → collection error `ModuleNotFoundError: No module named 'utils.memory.v3_memory_read_service'`; RED readiness/docs proof after implementation `cd backend && pytest tests/unit/test_memory_v3_memory_read_service.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py -q` → `1 failed, 15 passed in 0.13s` (missing docs link for `backend/utils/memory/v3_memory_read_service.py`). GREEN/format/focused/readiness/regression/async/docs hygiene: `cd backend && black --line-length 120 --skip-string-normalization utils/memory/v3_memory_read_service.py tests/unit/test_memory_v3_memory_read_service.py scripts/p1_3_v3_external_compatibility_readiness.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py && pytest tests/unit/test_memory_v3_memory_read_service.py tests/unit/test_v3_projection_readiness.py tests/unit/test_v3_compatibility.py tests/unit/test_memory_v3_cursor.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py -q && pytest tests/unit/test_memory_*.py -q` → `4 files left unchanged`, `36 passed in 0.16s`, `413 passed, 3 warnings in 2.38s`; `/v3` readiness summarized `BLOCKED True False False False False False False False False 7 7 8 5 8 2 13 True True True True`; P1-3 caller readiness summarized `BLOCKED True False False False False False False False False 13 11`; async scan from repo root exited 0 with pre-existing `HIGH async helpers with blocking: 41`, `STRUCTURAL mixed await+sync DB: 10`; docs hygiene `docs_hygiene 16 BAD=[]`. No runtime `/v3` route behavior changed and Oracle remains **BLOCKED / NO-GO**.

**2026-06-19 local `/v3` external write-convergence contract seam:** Added the next Oracle-prescribed pure/local `/v3` compatibility seam in `backend/utils/memory/v3_write_convergence.py` with unit proof in `backend/tests/unit/test_memory_v3_write_convergence.py` and registered it in `backend/test.sh`. The service accepts only caller-supplied local convergence evidence and is not wired into `backend/routers/memories.py`; it performs no app startup, no Firestore/Pinecone/cloud/provider/network calls, and no mutations. It models create/update/delete convergence before memory `/v3` read cutover: enrolled memory accounts require a memory-authoritative write path, durable outbox fence, current account/projection generation, and projection update commit before create/update success/read cutover; delete additionally requires tombstone commit, projection removal commit, and vector cleanup/outbox fence before delete success/read cutover. Missing, stale, partial, swallowed-failure, independently dual-written without durable outbox, generation-mismatched, missing projection commit, or unavailable memory-authoritative write path states fail closed. Disabled external writes are safe only when reads remain blocked for the cohort or no external write surface is active. There is no enrolled memory legacy direct-write fallback knob. The `/v3` readiness artifact now links this local proof as `write_convergence_proof` while preserving `status=BLOCKED` / **NO-GO** and no production rollout approval. Archive remains default-unavailable and no stale Short-term default-visible behavior was introduced.

Verification: RED `cd backend && pytest tests/unit/test_memory_v3_write_convergence.py -q` → collection error `ModuleNotFoundError: No module named 'utils.memory.v3_write_convergence'`; RED readiness/test-runner/docs proof `cd backend && pytest tests/unit/test_p1_3_v3_external_compatibility_readiness.py -q` → `4 failed, 5 passed` (missing `write_convergence_proof`, summary key, implementation-shape link, and test.sh/docs links). GREEN verification is recorded with the commit summary for this slice; no runtime `/v3` route behavior changed and Oracle remains **BLOCKED / NO-GO**.

**2026-06-19 local `/v3` response-shape adapter proof:** Added the next Oracle-prescribed pure/local `/v3` request/response shape adapter proof in `backend/utils/memory/v3_response_adapter.py` with unit proof in `backend/tests/unit/test_memory_v3_response_adapter.py` and registered it in `backend/test.sh`. The adapter consumes only a caller-supplied read-service envelope plus caller-supplied MemoryDB-compatible items; it is not wired into `backend/routers/memories.py` and performs no app startup, no Firestore/Pinecone/cloud/provider/network calls, and no mutations. It preserves the legacy `List[MemoryDB]` body contract: successful projection pages return the exact caller-supplied body, enabled-empty returns `[]` with memory read-source/read-decision headers, and fail-closed/denied envelopes return no body data and no legacy fallback marker. memory diagnostics are constrained to additive headers (`X-Omi-Memory-Read-Source`, `X-Omi-Memory-Read-Decision`, `X-Omi-Memory-Next-Cursor`, and `Link rel=next`); ad hoc memory source/policy/cursor/read-decision/archive/stale Short-term fields in body items are rejected before exposure. The `/v3` readiness artifact now links this local proof as `response_adapter_proof` while preserving `status=BLOCKED` / **NO-GO** and no production rollout approval. Archive remains default-unavailable and no stale Short-term default-visible behavior was introduced.

Verification: RED `cd backend && pytest tests/unit/test_memory_v3_response_adapter.py -q` → collection error `ModuleNotFoundError: No module named 'utils.memory.v3_response_adapter'`; RED readiness/docs proof `cd backend && pytest tests/unit/test_memory_v3_response_adapter.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py -q` → `1 failed, 14 passed in 0.12s` (missing docs link for `backend/utils/memory/v3_response_adapter.py`). GREEN/format/focused/readiness/regression/async/docs hygiene: `cd backend && black --line-length 120 --skip-string-normalization utils/memory/v3_response_adapter.py tests/unit/test_memory_v3_response_adapter.py scripts/p1_3_v3_external_compatibility_readiness.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py` → `1 file reformatted, 3 files left unchanged`; `pytest tests/unit/test_memory_v3_memory_read_service.py tests/unit/test_v3_projection_readiness.py tests/unit/test_v3_compatibility.py tests/unit/test_memory_v3_cursor.py tests/unit/test_memory_v3_write_convergence.py tests/unit/test_memory_v3_response_adapter.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py -q` → `47 passed in 0.23s`; `pytest tests/unit/test_memory_*.py -q` → `424 passed, 3 warnings in 2.40s`; `/v3` readiness summarized `BLOCKED True False False False False False False False False 7 7 8 5 8 2 13 True True True True True True`; P1-3 caller readiness summarized `BLOCKED True False False False False False False False False 13 11`; async scan from repo root exited 0 with pre-existing `HIGH async helpers with blocking: 41`, `STRUCTURAL mixed await+sync DB: 10`; docs hygiene `docs_hygiene 16 BAD=[]`. No runtime `/v3` route behavior changed and Oracle remains **BLOCKED / NO-GO**.
...
* memory/legacy/deny path selection;
...
**2026-06-19 local chat prompt-boundary hardening slice:** Hardened the memory Omi chat memory adapter so default and vector chat memory output now carries an explicit untrusted-evidence boundary notice, default-memory policy marker (`archive_default_visible=False`, `raw_provenance=False`), per-item `memory_id`, source markers (`memory_default_memory` or `vector_memory`), JSON-quoted `content_quoted=...`, and a per-item content cap before text reaches the LangChain chat tool. Prompt-injection-like memory content is no longer emitted as raw bullet text, while relevance/tier/date metadata and existing explicit memory read decisions are preserved. Archive remains default-unavailable, stale Short-term remains filtered by the shared default visibility policy, and malformed/no-grant/disabled rollout states still fail closed without unsafe legacy reads unless the explicit legacy-safe wrapper is requested. This is a narrow local P1-5 hardening seam only: it does not prove full router/tool production behavior, does not add central telemetry/benchmarks, and does not run production traffic, Firestore/Pinecone/cloud/provider calls, Firestore reads/writes beyond unit fakes, or approval.

Verification: RED `cd backend && pytest tests/unit/test_memory_chat_memory_adapter.py -q` → `4 failed, 6 passed in 0.13s` (adapter still emitted raw prompt text and lacked quoted content/source markers/policy boundary). GREEN/format/focused/regression: `cd backend && black --line-length 120 --skip-string-normalization utils/memory/v17_chat_memory_adapter.py tests/unit/test_memory_chat_memory_adapter.py && pytest tests/unit/test_memory_chat_memory_adapter.py -q && pytest tests/unit/test_memory_*.py -q` → `1 file reformatted, 1 file left unchanged`, `10 passed in 0.09s`, `361 passed, 3 warnings in 2.12s`. P1-3 readiness from repo root remains safe/read-only and blocked: `python3 backend/scripts/p1_3_caller_api_compatibility_readiness.py --execute` exited 0 with `status=BLOCKED`, read-only, no mutation, no network/provider calls, no Firestore reads/writes, no benchmark, no production approval, no approval claimed, 13 surfaces, 11 behavior cases. `/v3` readiness from repo root remains safe/read-only and blocked: `python3 backend/scripts/p1_3_v3_external_compatibility_readiness.py --execute` exited 0 with `status=BLOCKED`, read-only, no mutation, no network/provider calls, no Firestore reads/writes, no benchmark, no production approval, no approval claimed, 7 surfaces, 7 gaps. Async scan `python3 backend/scripts/scan_async_blockers.py` from repo root exited 0 with pre-existing `HIGH async helpers with blocking: 41`, `STRUCTURAL mixed await+sync DB: 10`. Docs hygiene found no truncation-marker literals or SUB chars under `docs/epics/memory*`.

**2026-06-19 local chat memory tool caller boundary guard:** Added a fail-closed caller guard for the actual Anthropic chat tool execution seam. `backend/utils/retrieval/agentic.py` now routes `search_memories_tool` and `get_memories_tool` results through `preserve_chat_memory_tool_result_boundary(...)` before they are appended as `tool_result.content`. Compliant memory memory evidence is preserved only when `content_quoted=...` and `source_marker=memory_default_memory` or `source_marker=vector_memory` keep the untrusted-evidence boundary notice, `policy=default_memory archive_default_visible=False raw_provenance=False`, and `archive_default_visible=False`; memory-like partial output that drops those markers is replaced with `No memories available for this request.` before model context. Denied and enabled-empty strings remain stable and do not introduce an unsafe legacy fallback. This is still a narrow local P1-5 seam: it does not prove tools REST or agent execute-tool wrappers, does not resolve `get_memories_tool` memory list semantics, and does not run production traffic, Firestore/Pinecone/cloud/provider calls, telemetry, benchmarks, or approval.

Verification: RED `cd backend && pytest tests/unit/test_memory_chat_memory_tool_caller.py -q` → `ModuleNotFoundError: No module named 'utils.retrieval.tool_result_boundaries'` (`1 error in 0.14s`). GREEN/format/focused: `cd backend && black --line-length 120 --skip-string-normalization utils/retrieval/tool_result_boundaries.py utils/retrieval/agentic.py tests/unit/test_memory_chat_memory_tool_caller.py && pytest tests/unit/test_memory_chat_memory_tool_caller.py -q` → `1 file reformatted, 2 files left unchanged`, `4 passed in 0.07s`. Focused/regression: `pytest tests/unit/test_memory_chat_memory_adapter.py tests/unit/test_memory_chat_memory_tool_caller.py -q && pytest tests/unit/test_memory_*.py -q` → `14 passed in 0.10s`, `365 passed, 3 warnings in 2.16s`. P1-3 readiness from repo root remains safe/read-only and blocked: `python3 backend/scripts/p1_3_caller_api_compatibility_readiness.py --execute` exited 0 with `status=BLOCKED`, read-only, no mutation, no network/provider calls, no Firestore reads/writes, no benchmark, no production approval, no approval claimed, 13 surfaces, 11 behavior cases. `/v3` readiness from repo root remains safe/read-only and blocked: `python3 backend/scripts/p1_3_v3_external_compatibility_readiness.py --execute` exited 0 with `status=BLOCKED`, read-only, no mutation, no network/provider/cloud/provider calls, no Firestore reads/writes, no benchmark, no production approval, no approval claimed, 7 surfaces, 7 gaps. Async scan `python3 backend/scripts/scan_async_blockers.py` from repo root exited 0 with pre-existing `HIGH async helpers with blocking: 41`, `STRUCTURAL mixed await+sync DB: 10`. Docs hygiene found no truncation-marker literals or SUB chars under `docs/epics/memory*`.

**2026-06-19 local tools REST, agent execute-tool, and chat get/list parity hardening slice:** Extended the P1-5 fail-closed memory memory evidence-boundary guard to the tools REST memory endpoints and `POST /v1/agent/execute-tool`, so memory-like memory output through those wrappers is preserved only when the quoted-evidence boundary, `content_quoted=...`, source marker, default-memory policy marker, and `archive_default_visible=False` remain intact; malformed/partially unwrapped memory-like output collapses to `No memories available for this request.` before wrapper clients can treat it as instructions. Added `list_default_chat_memories_decision_text(...)` and wired `get_memories_tool` through the same explicit Omi chat rollout decision family used by `search_memories_tool`: disabled/malformed/no-grant states deny without legacy fallback, enabled-empty returns `No memory default memories found.`, and enabled list results are emitted as quoted memory default evidence with `source_marker=memory_default_memory`, content caps, the untrusted-evidence boundary notice, and `archive_default_visible=False`. Legacy fallback is still only through explicit `USE_LEGACY_SAFE`; no stale Short-term or Archive default visibility was introduced. This remains a narrow local hardening slice: tools REST still needs a runtime memory read adapter parity decision beyond boundary guarding, route-level FastAPI response-model proof is not complete, `/v3` compatibility remains blocked, and no production traffic, Firestore/Pinecone/cloud/provider calls, Firestore reads/writes beyond unit fakes, benchmarks, telemetry sink integration, or approval are claimed. Oracle remains **BLOCKED / NO-GO**.

Verification: RED `cd backend && pytest tests/unit/test_memory_chat_memory_adapter.py tests/unit/test_memory_chat_memory_tool_caller.py -q` → collection error `ImportError: cannot import name 'list_default_chat_memories_decision_text' from 'utils.memory.v17_chat_memory_adapter'` (`1 error in 0.19s`). GREEN/format/focused/regression: `cd backend && black --line-length 120 --skip-string-normalization utils/memory/v17_chat_memory_adapter.py utils/retrieval/tools/memory_tools.py routers/tools.py routers/agent_tools.py tests/unit/test_memory_chat_memory_adapter.py tests/unit/test_memory_chat_memory_tool_caller.py && pytest tests/unit/test_memory_chat_memory_adapter.py tests/unit/test_memory_chat_memory_tool_caller.py -q && pytest tests/unit/test_memory_*.py -q` → `6 files left unchanged`, `17 passed in 0.10s`, `368 passed, 3 warnings in 2.12s`. P1-3 readiness from repo root remains safe/read-only and blocked: `python3 backend/scripts/p1_3_caller_api_compatibility_readiness.py --execute` exited 0 with `status=BLOCKED`, `read_only=true`, `mutation_allowed=false`, `network_or_provider_calls_executed=false`, `provider_calls_executed=false`, `firestore_reads_executed=false`, `firestore_writes_executed=false`, `benchmark_evidence_collected=false`, `production_rollout_approved=false`, `approval_claimed=false`, with 13 surfaces and 11 behavior cases. `/v3` readiness from repo root remains safe/read-only and blocked: `python3 backend/scripts/p1_3_v3_external_compatibility_readiness.py --execute` exited 0 with `status=BLOCKED`, `read_only=true`, `mutation_allowed=false`, `network_or_provider_calls_executed=false`, `provider_calls_executed=false`, `firestore_reads_executed=false`, `firestore_writes_executed=false`, `benchmark_evidence_collected=false`, `production_rollout_approved=false`, `approval_claimed=false`, with 7 surfaces and 7 gaps. Async scan `python3 backend/scripts/scan_async_blockers.py` from repo root exited 0 with pre-existing `HIGH async helpers with blocking: 41`, `STRUCTURAL mixed await+sync DB: 10`.

**2026-06-19 local tools REST and agent execute-tool route response-shape proof:** Added route-level/FastAPI-adjacent proof for the tools REST memory wrappers and `POST /v1/agent/execute-tool`. `backend/tests/unit/test_memory_tools_agent_route_response_shape.py` imports the actual router modules behind safe in-process dependency stubs, invokes the route functions through their real wrapper envelopes, validates `ToolResponse` and the new agent `ExecuteToolResponse` Pydantic response model, and pins quoted memory memory evidence preservation versus fail-closed collapse for partial/unbounded memory-like output. The tests cover tools REST get/search and agent execute-tool result shapes, source markers (`memory_default_memory` and `vector_memory`), `content_quoted=...`, `archive_default_visible=False`, prompt-injection-like text preserved only as quoted data, and stable `No memories available for this request.` collapse when required boundaries are missing. This is still a narrow local proof: it does not implement full tools REST runtime memory read adapter parity, does not run FastAPI `TestClient` with installed production dependencies, does not change memory memory read decisions, and does not resolve `/v3` compatibility. No production traffic, Firestore/Pinecone/cloud/provider calls, Firestore reads/writes, benchmarks, telemetry sink integration, or approval are claimed. Oracle remains **BLOCKED / NO-GO**.

Verification: RED `cd backend && pytest tests/unit/test_memory_tools_agent_route_response_shape.py -q` → `2 failed, 2 passed in 0.12s` because `routers.agent_tools` had no `ExecuteToolResponse`/route response model. GREEN/format/focused/regression: `cd backend && black --line-length 120 --skip-string-normalization routers/agent_tools.py tests/unit/test_memory_tools_agent_route_response_shape.py && pytest tests/unit/test_memory_chat_memory_adapter.py tests/unit/test_memory_chat_memory_tool_caller.py tests/unit/test_memory_tools_agent_route_response_shape.py -q && pytest tests/unit/test_memory_*.py -q` → `1 file reformatted, 1 file left unchanged`, `21 passed in 0.17s`, `372 passed, 3 warnings in 2.20s`. P1-3 readiness remains safe/read-only and blocked: `python3 backend/scripts/p1_3_caller_api_compatibility_readiness.py --execute` exited 0 with `status=BLOCKED`, `read_only=true`, `mutation_allowed=false`, `network_or_provider_calls_executed=false`, `provider_calls_executed=false`, `firestore_reads_executed=false`, `firestore_writes_executed=false`, `benchmark_evidence_collected=false`, `production_rollout_approved=false`, `approval_claimed=false`, 13 surfaces and 11 behavior cases. `/v3` readiness remains safe/read-only and blocked: `python3 backend/scripts/p1_3_v3_external_compatibility_readiness.py --execute` exited 0 with `status=BLOCKED`, same safe/non-claim flags, 7 surfaces and 7 gaps. Async scan `python3 backend/scripts/scan_async_blockers.py` exited 0 with pre-existing `HIGH async helpers with blocking: 41`, `STRUCTURAL mixed await+sync DB: 10`.

**2026-06-19 local tools REST runtime memory read adapter seam:** Wired tools REST memory read services through the same explicit memory Omi chat read-decision adapters used by the LangChain chat memory tools. `backend/utils/retrieval/tool_services/memories.py` now calls `list_default_chat_memories_decision_text(...)` for `get_memories_text` and `search_memory_default_chat_memories_vector_decision_text(...)` for `search_memories_text` before any legacy memory DB/vector path. Enabled memory results keep the shared quoted-evidence boundary, source markers (`memory_default_memory` / `vector_memory`), content caps, and `archive_default_visible=False`; enabled-empty states return stable memory empty text; denied/malformed/no-grant/missing-vector states return `No memories available for this request.` and do not invoke legacy DB/vector fallbacks. Legacy fallback remains reachable only if the adapter explicitly returns `USE_LEGACY_SAFE`. Added `backend/tests/unit/test_memory_tools_rest_memory_runtime_adapter.py` and registered it in `backend/test.sh`, covering get/search parity, prompt-injection-like payloads preserved only as quoted data, Archive default-unavailable markers, enabled-empty states, and no legacy calls on denied memory decisions. This is still a narrow local runtime seam: it does not run production traffic, Firestore/Pinecone/cloud/provider calls, Firestore reads/writes beyond unit stubs, benchmarks, telemetry sink integration, real FastAPI `TestClient` production-dependency proof, or approval. `/v3` remains blocked and Oracle remains **BLOCKED / NO-GO**.

Verification: RED `cd backend && pytest tests/unit/test_memory_tools_rest_memory_runtime_adapter.py -q` → `4 failed, 1 warning in 0.10s` because the tools REST service module had no memory read adapter seam attributes. GREEN/format/focused/regression: `cd backend && black --line-length 120 --skip-string-normalization utils/retrieval/tool_services/memories.py tests/unit/test_memory_tools_rest_memory_runtime_adapter.py` → `1 file reformatted, 1 file left unchanged`; `pytest tests/unit/test_memory_chat_memory_adapter.py tests/unit/test_memory_chat_memory_tool_caller.py tests/unit/test_memory_tools_agent_route_response_shape.py tests/unit/test_memory_tools_rest_memory_runtime_adapter.py -q && pytest tests/unit/test_memory_*.py -q` → `25 passed, 1 warning in 0.19s`, `376 passed, 3 warnings in 2.22s`. P1-3 readiness remains safe/read-only and blocked: `python3 backend/scripts/p1_3_caller_api_compatibility_readiness.py --execute` summarized `BLOCKED True False False False False False False False False 13 11`. `/v3` readiness remains safe/read-only and blocked: `python3 backend/scripts/p1_3_v3_external_compatibility_readiness.py --execute` summarized `BLOCKED True False False False False False False False False 7 7`. Async scan from repo root `python3 backend/scripts/scan_async_blockers.py` exited 0 with pre-existing `HIGH async helpers with blocking: 41`, `STRUCTURAL mixed await+sync DB: 10`. Docs hygiene found no literal truncation markers or SUB chars under `docs/epics/memory*`.

**2026-06-19 local tools REST/agent FastAPI TestClient readiness gap:** Added a safe readiness/proof gap artifact for the remaining P1-5/P1-3 real FastAPI `TestClient` route proof. `backend/scripts/v17_p1_5_tools_fastapi_testclient_readiness.py` and `backend/tests/unit/test_memory_p1_5_tools_fastapi_testclient_readiness.py` pin `backend/routers/tools.py` `GET /v1/tools/memories`, `backend/routers/tools.py` `POST /v1/tools/memories/search`, and `backend/routers/agent_tools.py` `POST /v1/agent/execute-tool` as the exact route surfaces still requiring production-dependency TestClient proof. FastAPI `TestClient` production-dependency proof remains BLOCKED/NOT_RUN because `fastapi` is not importable in the local Python verification environment; this artifact remains read-only and records no route execution, app startup, mutation, network/provider/cloud call, Firestore/Pinecone call, benchmark, telemetry sink integration, or approval. It inventories required future behavior cases for response-model serialization, quoted memory evidence boundary preservation, denied/no-grant fail-closed output, enabled-empty stability, prompt-injection payload preservation as quoted data, and Archive plus stale Short-term default-unavailable behavior. Existing local non-TestClient proof remains limited to `backend/tests/unit/test_memory_tools_agent_route_response_shape.py` and `backend/tests/unit/test_memory_tools_rest_memory_runtime_adapter.py`. Oracle remains **BLOCKED / NO-GO**.

Verification: RED `cd backend && pytest tests/unit/test_memory_p1_5_tools_fastapi_testclient_readiness.py -q` → `4 failed in 0.15s` (missing readiness runner, test.sh registration, and docs links). GREEN/format/focused/readiness/regression: `cd backend && black --line-length 120 --skip-string-normalization scripts/v17_p1_5_tools_fastapi_testclient_readiness.py tests/unit/test_memory_p1_5_tools_fastapi_testclient_readiness.py && pytest tests/unit/test_memory_p1_5_tools_fastapi_testclient_readiness.py -q` → `2 files left unchanged`, `4 passed in 0.12s`; readiness from repo root `python3 backend/scripts/v17_p1_5_tools_fastapi_testclient_readiness.py --execute` summarized `BLOCKED NOT_RUN True False False False False False False False False False 3 6`; focused route/runtime regression `pytest tests/unit/test_memory_chat_memory_adapter.py tests/unit/test_memory_chat_memory_tool_caller.py tests/unit/test_memory_tools_agent_route_response_shape.py tests/unit/test_memory_tools_rest_memory_runtime_adapter.py tests/unit/test_memory_p1_5_tools_fastapi_testclient_readiness.py -q` → `29 passed, 1 warning in 0.30s`; full memory regression `pytest tests/unit/test_memory_*.py -q` → `380 passed, 3 warnings in 2.33s`. P1-3 readiness remained safe/read-only and blocked (`BLOCKED True False False False False False False False False 13 11`); `/v3` readiness remained safe/read-only and blocked (`BLOCKED True False False False False False False False False 7 7`). Async scan `python3 backend/scripts/scan_async_blockers.py` exited 0 with pre-existing `HIGH async helpers with blocking: 41`, `STRUCTURAL mixed await+sync DB: 10`. Docs hygiene found no literal truncation markers or SUB chars under `docs/epics/memory*`.
...
1. **Gate every memory route** with the same versioned rollout decision; add a global emergency kill switch independent of the per-user Firestore read.
...
6. **Prove shared-namespace isolation** against real Pinecone data for both memory and legacy callers.
7. **Replace full-collection hydration** with candidate-ID batch hydration and measured overfetch/refill.
8. **Enforce scopes and app/key-specific grants** on MCP REST, MCP streamable HTTP/SSE, and developer keys.
...
2. **Fallback semantics:** On missing/malformed control state, revoked grant, vector outage, or empty memory results, should the system deny memory, return an error, use last-known-safe state, merge legacy and memory, or fall back only when reconciliation is proven?
...
6. **Read/write sequencing:** Must T22 memory writes and deletes land before read cutover, or will pilot accounts have external memory writes disabled?
7. **API strategy:** Should existing `/v3`, MCP, and developer endpoints switch behavior additively, or should `/memory` endpoints remain private/experimental until compatibility is complete?
...
- Added `backend/scripts/p1_3_v3_fastapi_route_contract.py`, a controlled isolated FastAPI mini-app proof for `GET /v3/memories` using the production `MemoryDB` response model. It deliberately does not import `backend/routers/memories.py` or the production app, and stubs only the `database._client.document_id_from_seed` import side effect needed to load the model without constructing Firestore clients.
- Added `backend/tests/unit/test_p1_3_v3_fastapi_route_contract.py` and registered it in `backend/test.sh`.
- Linked the proof from `backend/scripts/p1_3_v3_external_compatibility_readiness.py` as `fastapi_route_contract_proof` while keeping `/v3` runtime status `BLOCKED`.
...
- The `/v3` proof covers `List[MemoryDB]` legacy-compatible body serialization, additive headers without body mutation, enabled-empty `[]`, fail-closed denied `403` with no body data and no legacy fallback marker, and filtering of memory-only body fields from the `List[MemoryDB]` response body.
...
- RED: `backend/venv/bin/python -m pytest backend/tests/unit/test_p1_3_v3_fastapi_route_contract.py -q` -> `4 failed` before adding the runner/docs/readiness/test.sh registration.
- GREEN focused route proof: `cd backend && venv/bin/python -m pytest tests/unit/test_p1_3_v3_fastapi_route_contract.py -q` -> `4 passed in 1.09s`.
- Normal-env focused `/v3` proof suite: `env -u VIRTUAL_ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin python3 -m pytest tests/unit/test_memory_v3_memory_read_service.py tests/unit/test_v3_projection_readiness.py tests/unit/test_v3_compatibility.py tests/unit/test_memory_v3_cursor.py tests/unit/test_memory_v3_write_convergence.py tests/unit/test_memory_v3_response_adapter.py tests/unit/test_memory_v3_request_adapter.py tests/unit/test_memory_v3_route_planner.py tests/unit/test_p1_3_v3_route_signature_integration.py tests/unit/test_p1_3_v3_fastapi_route_contract.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py tests/unit/test_memory_p1_5_tools_fastapi_testclient_readiness.py -q` -> `74 passed in 2.73s`.
- Normal-env full memory regression: `env -u VIRTUAL_ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin python3 -m pytest tests/unit/test_memory_*.py -q` -> `446 passed, 3 warnings in 4.85s`.
- Venv proof summary: `venv/bin/python scripts/p1_3_v3_fastapi_route_contract.py --execute` -> `BLOCKED PASSED True 5`.
...
- Added `backend/scripts/p1_3_v3_real_router_dependency_map.py`, which uses static AST inspection plus a repo-venv subprocess probe to import the real `backend/routers/memories.py` only after explicit stubs are installed for unsafe dependencies: `database.memories`, `database.review_queue`, `database.vector_db`, `database._client`, `utils.executors`, `utils.apps`, and `utils.other.endpoints`.
- Added `backend/tests/unit/test_p1_3_v3_real_router_dependency_map.py` and registered it in `backend/test.sh`.
- Linked the proof from `backend/scripts/p1_3_v3_external_compatibility_readiness.py` as `real_router_dependency_map_proof` while keeping `/v3` runtime status `BLOCKED`.
...
- RED: `cd backend && env -u VIRTUAL_ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin python3 -m pytest tests/unit/test_p1_3_v3_real_router_dependency_map.py -q` -> `4 failed in 0.07s` before adding the runner/readiness/docs/test.sh registration.
- GREEN focused route proof: `cd backend && env -u VIRTUAL_ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin python3 -m pytest tests/unit/test_p1_3_v3_real_router_dependency_map.py -q` -> `4 passed in 0.83s`.
- Normal-env focused `/v3` proof suite: `cd backend && env -u VIRTUAL_ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin python3 -m pytest tests/unit/test_memory_v3_memory_read_service.py tests/unit/test_v3_projection_readiness.py tests/unit/test_v3_compatibility.py tests/unit/test_memory_v3_cursor.py tests/unit/test_memory_v3_write_convergence.py tests/unit/test_memory_v3_response_adapter.py tests/unit/test_memory_v3_request_adapter.py tests/unit/test_memory_v3_route_planner.py tests/unit/test_p1_3_v3_route_signature_integration.py tests/unit/test_p1_3_v3_fastapi_route_contract.py tests/unit/test_p1_3_v3_real_router_dependency_map.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py tests/unit/test_memory_p1_5_tools_fastapi_testclient_readiness.py -q` -> `78 passed in 3.59s`.
- Normal-env full memory regression: `cd backend && env -u VIRTUAL_ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin python3 -m pytest tests/unit/test_memory_*.py -q` -> `450 passed, 3 warnings in 5.53s`.
- Venv real-router proof test: `cd backend && venv/bin/python -m pytest tests/unit/test_p1_3_v3_real_router_dependency_map.py -q` -> `4 passed in 0.85s`.
- Real-router dependency-map summary: `venv/bin/python scripts/p1_3_v3_real_router_dependency_map.py --execute` -> `BLOCKED True True 7 3 False`.
- Existing FastAPI route-contract summary: `venv/bin/python scripts/p1_3_v3_fastapi_route_contract.py --execute` -> `BLOCKED PASSED True 5`.
...
- Added `backend/scripts/p1_3_v3_real_router_get_testclient.py`, which launches repo-managed `backend/venv/bin/python`, installs explicit import stubs, imports the real `routers.memories` module, builds a minimal `FastAPI()` app, overrides auth to `stubbed-test-uid`, includes the real router, and executes only `GET /v3/memories` plus `GET /v3/memories?limit=17&offset=3`.
- Added `backend/tests/unit/test_p1_3_v3_real_router_get_testclient.py` and registered it in `backend/test.sh`.
- Linked the proof from `backend/scripts/p1_3_v3_external_compatibility_readiness.py` as `real_router_get_testclient_proof` while keeping `/v3` runtime status `BLOCKED`.
...
- The response serializes through the real route response model as `List[MemoryDB]` compatible data; POST/DELETE/vector/persona/executor mutation paths remain unexecuted; memory request adapter, route planner, and response adapter are not invoked yet.
...
- RED: `cd backend && env -u VIRTUAL_ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin python3 -m pytest tests/unit/test_p1_3_v3_real_router_get_testclient.py -q` -> `4 failed in 0.07s` before adding the runner/readiness/docs/test.sh registration.
- Parent recovery fixed the sibling-proof import by loading `p1_3_v3_real_router_dependency_map.py` via `Path(__file__).with_name(...)`, then tightened the response assertion around behavior rather than optional `None` fields added by `response_model` serialization.
- Focused GREEN: `black --line-length 120 --skip-string-normalization scripts/p1_3_v3_real_router_get_testclient.py tests/unit/test_p1_3_v3_real_router_get_testclient.py scripts/p1_3_v3_external_compatibility_readiness.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py && env -u VIRTUAL_ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin python3 -m pytest tests/unit/test_p1_3_v3_real_router_get_testclient.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py -q` -> `1 file reformatted, 3 files left unchanged`, `13 passed in 1.47s`.
- Venv proof: `venv/bin/python -m pytest tests/unit/test_p1_3_v3_real_router_get_testclient.py -q` -> `4 passed in 1.39s`; `venv/bin/python scripts/p1_3_v3_real_router_get_testclient.py --execute` -> `BLOCKED True True True True False 2`.
- Normal-env focused `/v3` proof suite: `82 passed in 4.99s`.
- Normal-env full memory regression: `454 passed, 3 warnings in 7.18s`.
...
- Added `backend/scripts/p1_3_v3_get_runtime_wiring_readiness.py`, a read-only BLOCKED inventory of the exact remaining real-service/runtime gates before future `GET /v3/memories` memory cutover.
- Added `backend/tests/unit/test_p1_3_v3_get_runtime_wiring_readiness.py` and registered it in `backend/test.sh`.
- Linked the new artifact from `backend/scripts/p1_3_v3_external_compatibility_readiness.py` as `get_runtime_wiring_readiness_proof` while keeping `/v3` runtime status `BLOCKED`.
- The gate inventory covers real server-side memory cohort/enrollment/control fail-closed reads, real memory-derived MemoryDB-compatible compatibility projection reads including empty projection state, external create/update/delete write-convergence/source-of-truth evidence, real cursor signing secret/config and validation integration, real route-level dependency overrides/auth/rate-limit TestClient behavior, non-enrolled legacy compatibility including `offset=0 -> limit=5000`, enrolled no-grant/projection-not-ready/write-not-ready no-fallback behavior, Archive default-unavailable and stale Short-term not default-visible proof, observability/telemetry, and explicit approval gates.
- It ties each gate to existing local proof artifacts (`v3_compatibility`, cursor, projection readiness, memory read service, write convergence, response/request adapters, route planner, route-signature integration, FastAPI route contract, real-router dependency map, and real-router GET TestClient) while marking real service/runtime evidence still missing.
...
- RED: `cd backend && env -u VIRTUAL_ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin python3 -m pytest tests/unit/test_p1_3_v3_get_runtime_wiring_readiness.py -q` -> `6 failed in 0.08s` before adding the runner/test.sh/docs/external-readiness link.
...
- Added `backend/scripts/p1_3_v3_get_dependency_auth_readiness.py`, which launches repo-managed `backend/venv/bin/python`, imports the real `routers.memories` only after explicit stubs, and builds minimal FastAPI apps without importing `backend/main.py`.
- Added `backend/tests/unit/test_p1_3_v3_get_dependency_auth_readiness.py` and registered it in `backend/test.sh`.
- Linked `get_dependency_auth_readiness_proof` from both `backend/scripts/p1_3_v3_external_compatibility_readiness.py` and `backend/scripts/p1_3_v3_get_runtime_wiring_readiness.py`.
...
- With auth override, the route still calls stubbed legacy `memories_db.get_memories(uid, limit, offset)`, preserving the non-enrolled legacy baseline. No memory cohort/control dependency is present or invoked.
...
- RED: `cd backend && env -u VIRTUAL_ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin python3 -m pytest tests/unit/test_p1_3_v3_get_dependency_auth_readiness.py -q` -> `6 failed in 0.08s` before adding the runner/test.sh/docs/readiness links.
- Focused GREEN: `cd backend && env -u VIRTUAL_ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin python3 -m pytest tests/unit/test_p1_3_v3_get_dependency_auth_readiness.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py tests/unit/test_p1_3_v3_get_runtime_wiring_readiness.py -q` -> `21 passed in 1.51s`.
- Venv proof: `venv/bin/python -m pytest tests/unit/test_p1_3_v3_get_dependency_auth_readiness.py -q` -> `6 passed in 1.41s`; readiness summary `PARTIAL PARTIAL True True True True False 1 False False False`.
- Normal-env focused `/v3` proof suite: `89 passed in 5.08s`; normal-env full memory regression: `466 passed, 3 warnings in 8.63s`.
...
Added the next real-service-adjacent gate as a safe readiness/local contract artifact for the memory-derived compatibility projection read API/store needed before future `GET /v3/memories` cutover:

- Added `backend/scripts/p1_3_v3_projection_store_readiness.py`, a read-only BLOCKED inventory of the exact production store/API requirements for a memory-derived compatibility projection that can feed `/v3/memories`.
- Added `backend/tests/unit/test_p1_3_v3_projection_store_readiness.py` and registered it in `backend/test.sh`.
- Linked `projection_store_readiness_proof` from both `backend/scripts/p1_3_v3_external_compatibility_readiness.py` and `backend/scripts/p1_3_v3_get_runtime_wiring_readiness.py`.
- The artifact records that the canonical projection path/API is still blocked until chosen, while pinning required MemoryDB materialization fields without memory-only body leakage, account/projection generation and freshness fences, source commit/version/evidence fences, delete/tombstone/vector cleanup fences, enabled-empty `[]` with no legacy fallback, Archive default-unavailable and stale Short-term not default-visible requirements, cursor pagination plus non-enrolled legacy offset compatibility, and a fake-injectable reader interface shape for future route wiring.
...
- RED: `cd backend && pytest tests/unit/test_p1_3_v3_projection_store_readiness.py -q` -> `7 failed in 0.09s` before adding the runner/test.sh/docs/readiness links.
- Focused GREEN and full verification outputs are recorded with the local commit summary for this slice.
- Production rollout remains **BLOCKED / NO-GO** until all Oracle P0/P1 gates and required real-service evidence are complete.

### 2026-06-19 — P1-3 `/v3` control reader readiness

Added the next real-service-adjacent gate as a safe readiness/local contract artifact for the server-side memory cohort/enrollment/control reader needed before future `GET /v3/memories` cutover:

- Added `backend/scripts/p1_3_v3_control_reader_readiness.py`, a read-only BLOCKED inventory of exact production control reader requirements.
- Added `backend/tests/unit/test_p1_3_v3_control_reader_readiness.py` and registered it in `backend/test.sh`.
- Linked `control_reader_readiness_proof` from both `backend/scripts/p1_3_v3_external_compatibility_readiness.py` and `backend/scripts/p1_3_v3_get_runtime_wiring_readiness.py`.
...
- Non-enrolled users preserve legacy-primary behavior including `offset=0 -> limit=5000` only on the legacy path; enrolled memory users must not fall back to legacy on memory gate failures.
...
- RED: `cd backend && env -u VIRTUAL_ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin python3 -m pytest tests/unit/test_p1_3_v3_control_reader_readiness.py -q` -> `8 failed in 0.10s` before adding the runner/test.sh/docs/readiness links.
- Focused GREEN: `cd backend && black --line-length 120 --skip-string-normalization scripts/p1_3_v3_control_reader_readiness.py tests/unit/test_p1_3_v3_control_reader_readiness.py scripts/p1_3_v3_external_compatibility_readiness.py scripts/p1_3_v3_get_runtime_wiring_readiness.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py tests/unit/test_p1_3_v3_get_runtime_wiring_readiness.py && env -u VIRTUAL_ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin python3 -m pytest tests/unit/test_p1_3_v3_control_reader_readiness.py tests/unit/test_p1_3_v3_projection_store_readiness.py tests/unit/test_p1_3_v3_external_compatibility_readiness.py tests/unit/test_p1_3_v3_get_runtime_wiring_readiness.py -q` -> `2 files reformatted, 4 files left unchanged`, `30 passed in 0.21s`.
- Normal-env focused `/v3` proof suite: `104 passed in 5.08s`; normal-env full memory regression: `481 passed, 3 warnings in 8.54s`.
...
Added the next real-service-adjacent readiness artifact for future Firestore-emulator/API-backed validation of the server-side memory `/v3` control reader, without requiring cloud credentials or changing runtime behavior:

- Added `backend/scripts/p1_3_v3_control_reader_emulator_readiness.py`, a safe default `BLOCKED`/`NOT_RUN` inventory that never starts Firestore emulators, reads or writes Firestore cloud/emulator data, calls cloud/provider/network services, mutates state, implements a production reader, wires routes, or claims approval.
- Added `backend/tests/unit/test_p1_3_v3_control_reader_emulator_readiness.py` and registered it in `backend/test.sh`.
...
- Required proof cases are pinned to `v3_control_reader_contract.py`: non-enrolled legacy boundary, enrolled memory projection allowed only when all gates are ready, and fail-closed/no-fallback cases for missing control doc, stale generation, no grant, projection not ready, write convergence not ready, invalid/missing cursor secret, Archive not allowed, and stale Short-term default-hidden.
...
- Canonical source is resolved to `users/{uid}/memory_control/state` via `MemoryCollections(uid).memory_control_state`.
- Existing rollout/migration primitives are `MemoryRolloutMode.off/shadow/write/read`, `MemoryRolloutState`, `stage_gates`, `mode_epoch`, `cutover_epoch`, `account_generation`, `fallback_projection_ready`, `persistent_memory_writes_started`, `writes_blocked`, and `grants`.
- Added `backend/utils/memory/v3_control_state_adapter.py`, which maps the existing persisted rollout state into the `/v3` control contract without reading `memory_items`, wiring routers, starting cloud/emulator services, or mutating state.
- Updated `backend/utils/memory/v3_control_reader_contract.py` to use a `V3ControlReadResult` envelope so non-enrolled users can skip control reads while enrolled users with missing/malformed state fail closed.
...
- RED: focused adapter/contract tests failed before implementation with missing `utils.memory.v3_control_state_adapter` and missing `V3ControlReadResult` imports (`2 errors in 0.16s`).
...
- Full normal-env memory regression: `509 passed, 3 warnings in 8.97s`.
...
Implemented and ran the next narrow local Firestore-emulator proof for the server-side memory `/v3` control-reader seam against the accepted canonical path `users/{uid}/memory_control/state`, without wiring runtime `/v3` routes or touching production Firestore:

- Added `backend/scripts/p1_3_v3_control_reader_emulator_test.py`, an emulator-only Python/Admin-context fixture harness. It refuses to run unless `FIRESTORE_EMULATOR_HOST` is set, seeds only local emulator docs, and maps emulator-read rollout fixtures through `read_v3_control(...)`, `V3ControlState`, and `decide_v3_control_route(...)`.
...
- Added `npm run test:memory-v3-control-reader:emulator`, which starts the local Firestore emulator and runs both the client-denial rules proof and Admin-context control-reader mapping proof.
- Fixture schema matches the persisted rollout doc fields: `uid`, `schema_version`, `mode`, `mode_epoch`, `cutover_epoch`, `account_generation`, `fallback_projection_ready`, `persistent_memory_writes_started`, `writes_blocked`, `stage_gates`, and `grants`. Synthetic `cohort_enrolled`, `control_generation`, `projection_ready`, and Short-term freshness/default-visible control fields remain absent.
- Emulator proof cases cover memory projection success plus missing, malformed, no-grant, projection-not-ready, write-convergence-not-ready, and global-gate-closed fail-closed outcomes with no legacy fallback. Non-enrolled no-read boundary remains covered by pure adapter tests.
...
- Emulator: `npm run test:memory-v3-control-reader:emulator` starts the local Firestore emulator, prints expected client `PERMISSION_DENIED` logs, then `PASS: signed-in client read/write denial asserted for 8 memory collections, users/{uid}/memory_control/state, and memory app/key memory grant self-grant path` and `PASS: emulator Admin-context fixture read from users/{uid}/memory_control/state mapped through read_v3_control/decide_v3_control_route for 7 cases; no production Firestore or runtime /v3 wiring used`.
- Focused/linked, full memory, async scan, docs hygiene, and commit SHA are recorded in the subagent handoff for this slice.
...
Implemented Oracle's prescribed server-only memory compatibility projection reader without route wiring:

- Added `backend/utils/memory/v3_projection_reader_contract.py` and `backend/database/v3_compatibility_projection.py` for a fenced reader over server-owned `users/{uid}/v3_compatibility_projection/state` and `users/{uid}/v3_compatibility_projection_items/{memory_id}`.
- Added `backend/tests/unit/test_v3_compatibility_projection.py` with strict fail-closed coverage for missing/malformed state, schema/uid/source mismatch, caller-supplied expected account-generation mismatch, projection generation/commit/fence mismatch, incomplete write/delete/tombstone convergence, invalid payload whole-page failure, Archive/deleted/tombstone/stale Short-term default exclusion, stable `created_at DESC`, document-id DESC keyset pagination, and offset rejection.
- Added `backend/scripts/p1_3_v3_projection_reader_emulator_test.py` and `npm run test:memory-v3-projection-reader:emulator`. The emulator proof asserts signed-in client denial for projection state/items via the shared rules harness, then proves Admin-context ready-empty, generation mismatch, stale commit/fence, Archive/tombstone/stale Short-term exclusion, and two-page keyset ordering.
- Updated `backend/database/v17_collections.py`, `firestore.rules`, `backend/scripts/v17_firestore_rules_emulator_test.mjs`, `backend/test.sh`, `package.json`, and `backend/scripts/p1_3_v3_projection_store_readiness.py`. Projection store readiness now records local implementation/emulator evidence while overall runtime remains **BLOCKED / NO-GO**.

Verification for this slice:

- RED: `cd backend && env -u VIRTUAL_ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin python3 -m pytest tests/unit/test_v3_compatibility_projection.py -q` initially failed with `ModuleNotFoundError: No module named 'database.v3_compatibility_projection'`.
...
- Non-claims preserved: no `backend/routers/memories.py`, `backend/main.py`, POST/PATCH/DELETE route change, no runtime `/v3` behavior change, no production rollout approval, no production Firestore/cloud/provider/vector calls, no Archive default visibility, no stale Short-term default visibility, no legacy fallback/merge for memory projection failures, and no client-supplied generation trust.

### 2026-06-20 — P1-3 `/v3` trusted account-generation source/readiness

Added the next safe memory `/v3` gate for trusted account-generation sourcing without changing `backend/routers/memories.py` or runtime behavior:

- Added `backend/utils/memory/v3_account_generation_source.py`, a fake-injectable reader for the independent server-owned `users/{uid}/memory_state/head` account-generation source.
- Added `backend/tests/unit/test_memory_v3_account_generation_source.py` covering missing/malformed state head, uid mismatch, source mismatch, unsupported schema, malformed generation, read failure, and distinct state-head/control/projection documents.
- Added `backend/scripts/p1_3_v3_account_generation_readiness.py` plus `backend/tests/unit/test_p1_3_v3_account_generation_readiness.py`; registered both new tests in `backend/test.sh`.
- Linked `account_generation_readiness_proof` from `backend/scripts/p1_3_v3_external_compatibility_readiness.py` and `backend/scripts/p1_3_v3_get_runtime_wiring_readiness.py`. Runtime readiness now has an explicit `real_trusted_account_generation_source` BLOCKED gate.
...
Verification for this slice is recorded in `docs/epics/memory_implementation_tickets.md` and the local commit summary. Preserved non-claims: no route wiring, no production Firestore/cloud/provider/vector calls, no production rollout approval, no client-supplied generation trust, and no legacy fallback/merge for memory failures.
...
- Updated `backend/database/memory_apply_store.py` so committed memory apply writes `users/{uid}/memory_state/head` from committed server-owned `MemoryControlState` in the same transaction as `memory_control/state`, `memory_commits/{commit_id}`, `memory_items/*`, and `memory_outbox/*`.
- The state-head contains `schema_version`, `uid`, `source='memory_state_head'`, `account_generation`, `head_commit_id`, `commit_sequence`, and `updated_at`, matching `read_memory_v3_trusted_account_generation(...)`.
- Extended `backend/tests/unit/test_memory_firestore_apply_store.py` to prove the written state-head is readable by the trusted generation reader.
...
- Extended `backend/scripts/v17_firestore_python_apply_emulator_test.py` and added `npm run test:memory-v3-state-head:emulator` to prove Admin/server apply writes the state-head on the local Firestore emulator and the trusted reader returns the committed account generation/head commit.
...
- RED: `pytest tests/unit/test_memory_firestore_apply_store.py -q` -> `1 failed, 5 passed`, because `users/u1/memory_state/head` was not written.
- Focused GREEN: `52 passed in 0.30s`.
- Full normal-env memory regression: `549 passed, 3 warnings in 8.87s`.
- Emulator proof: `npm run test:memory-v3-state-head:emulator` -> client PERMISSION_DENIED logs expected, `PASS: signed-in client read/write denial asserted for 10 memory collections, users/{uid}/memory_control/state, users/{uid}/memory_state/head, and memory app/key memory grant self-grant path`; `PASS: Python apply_long_term_patch_firestore committed and replayed memory docs on Firestore emulator including users/{uid}/memory_state/head trusted account-generation state-head (...)`.
...
- Added `backend/scripts/p1_3_v3_real_router_fail_closed_matrix.py` and `backend/tests/unit/test_p1_3_v3_real_router_fail_closed_matrix.py`; registered the test in `backend/test.sh`.
- Linked `real_router_fail_closed_matrix_proof` from `backend/scripts/p1_3_v3_external_compatibility_readiness.py` and `backend/scripts/p1_3_v3_get_runtime_wiring_readiness.py`.
- Current real-router baseline remains legacy-only under stubs via the existing TestClient proof: `offset=0 -> limit=5000`, explicit nonzero `limit`/`offset` preserved, no memory adapters invoked, and no mutating routes executed.
...
- Runtime remains **BLOCKED / NO-GO**. No `backend/routers/memories.py` change, no runtime `/v3` behavior change, no production rollout approval, no production Firestore/cloud/provider/vector call, no Archive default visibility, no stale Short-term default visibility, and no legacy fallback/merge for memory failures is claimed.

### 2026-06-20 — P1-3 `/v3` write-convergence/delete/tombstone pre-runtime matrix proof

Added the next safe pre-runtime proof for the write-convergence/delete/tombstone gates future `GET /v3/memories` must require before returning memory projection data:

- Added `backend/scripts/p1_3_v3_write_convergence_tombstone_matrix.py` and `backend/tests/unit/test_p1_3_v3_write_convergence_tombstone_matrix.py`; registered the test in `backend/test.sh`.
- Linked `write_convergence_tombstone_matrix_proof` from `backend/scripts/p1_3_v3_external_compatibility_readiness.py` and `backend/scripts/p1_3_v3_get_runtime_wiring_readiness.py`.
- The matrix uses only pure route-planner/projection/write-convergence seams with fake caller contexts and fake in-memory readers. It proves memory projection success requires create, update, and delete convergence plus matching account/projection/tombstone/freshness fences.
- Fail-closed cases cover create convergence false, update convergence false, delete convergence false, tombstone fence missing, tombstone generation stale/mismatched, and enabled-empty with missing tombstone fence. These cases call no fake legacy reader, call no fake projection reader, and disallow legacy fallback or memory/legacy merge.
...
- Added `backend/scripts/p1_3_v3_cursor_secret_readiness.py` and `backend/tests/unit/test_p1_3_v3_cursor_secret_readiness.py`; registered the test in `backend/test.sh`.
- Linked `cursor_secret_readiness_proof` from `backend/scripts/p1_3_v3_external_compatibility_readiness.py` and `backend/scripts/p1_3_v3_get_runtime_wiring_readiness.py`.
- The readiness artifact does not read environment secret values and does not invent production secret material. It records the exact blocker: no existing runtime-owned memory `/v3` cursor signing secret/config source is wired; a server-owned `V3_CURSOR_SIGNING_SECRET` or managed secret must be injected before runtime route changes.
...
- Added `backend/scripts/p1_3_v3_cursor_secret_production_readiness.py` and `backend/tests/unit/test_p1_3_v3_cursor_secret_production_readiness.py`; registered the test in `backend/test.sh`.
- Linked `cursor_secret_production_readiness_proof` from `backend/scripts/p1_3_v3_cursor_secret_readiness.py`, `backend/scripts/p1_3_v3_external_compatibility_readiness.py`, and `backend/scripts/p1_3_v3_get_runtime_wiring_readiness.py`.
...
- The selected source shape is route-scoped (`GET /v3/memories`, `memory-v3-get-cursor-signing-secret`) with no uid/session/memory/request/raw-cursor dimensions. The proof validates only Secret Manager/config metadata labels/resource shape and never accesses, prints, or logs secret material or raw cursor tokens.
...
Added the next non-canary production-safe projection read source contract/readiness proof for future memory-backed `GET /v3/memories`, while keeping route wiring blocked:

- Added `backend/scripts/p1_3_v3_projection_read_source_readiness.py` and `backend/tests/unit/test_p1_3_v3_projection_read_source_readiness.py`; registered the test in `backend/test.sh`.
- Linked `projection_read_source_readiness_proof` from `backend/scripts/p1_3_v3_external_compatibility_readiness.py` and `backend/scripts/p1_3_v3_get_runtime_wiring_readiness.py`.
...
- Extended `backend/utils/memory/v3_canary_approval.py` with `V3CanaryApprovalArtifactReader` and `read_memory_v3_canary_approval_artifact_decision(...)`; the function accepts only an injected reader/fake and delegates artifact validation to `validate_memory_v3_canary_approval_artifact(...)`.
- Extended `backend/tests/unit/test_memory_v3_canary_approval_artifact.py` to prove missing reader/source, reader exception/timeout-ish sentinel, malformed artifact, route mismatch, unsupported or mismatched cohort, stale artifact, pending/rejected approval, and sensitive/high-cardinality metadata all fail closed.
- The approved fake-artifact path yields only bounded labels through `build_v3_canary_approval_telemetry_labels(...)` and does not expose uid/session/memory content/cursor/secret/request payload metadata.
- `backend/scripts/p1_3_v3_observability_approval_readiness.py` now links `v3_canary_approval_artifact_reader_seam`; external/runtime readiness inherit the proof through `observability_approval_readiness_proof`, but remain **BLOCKED**.
...
- Added `backend/utils/memory/v3_canary_approval.py` and `backend/tests/unit/test_memory_v3_canary_approval_artifact.py`.
...
- `backend/scripts/p1_3_v3_observability_approval_readiness.py` now links the local proof as `v3_canary_approval_artifact_schema_seam` while keeping readiness **BLOCKED** and preserving no production rollout approval.
...
- Added `backend/utils/memory/v3_local_telemetry.py` and `backend/tests/unit/test_memory_v3_local_telemetry.py`.
...
- The rollback/read-disable seam is pure/config-shaped: enrolled memory users fail closed for missing, malformed, disabled, or emergency-disabled server-owned config, while non-enrolled callers remain outside the memory read seam.
- `backend/scripts/p1_3_v3_observability_approval_readiness.py` now links this local proof as `v3_local_telemetry_and_rollback_seam`; remaining blockers are real route wiring, real telemetry sink/config source, canary enrollment artifact, rollback gate wiring, and explicit product/privacy/ops approval.
...
Added `backend/scripts/p1_3_v3_canary_approval_aggregate_readiness.py` as a local-only pre-runtime aggregate decision report for `GET /v3/memories` canary approval. It reports `status=BLOCKED`, `decision=NO_GO`, `runtime_wiring_changed=false`, and no production calls/writes by default or with `--execute`.
...
- Added `backend/scripts/p1_3_v3_route_dependency_contract_readiness.py`, a static/read-only BLOCKED contract for future `GET /v3/memories` route dependency evidence.
- Added `backend/tests/unit/test_p1_3_v3_route_dependency_contract_readiness.py` and registered it in `backend/test.sh`.
- Linked `route_dependency_contract_readiness_proof` from both `backend/scripts/p1_3_v3_external_compatibility_readiness.py` and `backend/scripts/p1_3_v3_get_runtime_wiring_readiness.py`, increasing the runtime local-proof inventory while keeping every runtime gate BLOCKED.
- The contract pins required evidence for authenticated subject binding before reads, legacy token/API-key/MCP behavior inventory, rejection of client uid overrides, non-enrolled legacy vs enrolled memory boundary, an explicit GET rate-limit/backpressure hook, and fail-closed missing/invalid auth/control/cursor/config behavior.
...
- Added `backend/scripts/p1_3_v3_projection_write_convergence_readiness.py` as a read-only readiness proof for the evidence required before `GET /v3/memories` can trust the memory compatibility projection source.
- Added `backend/tests/unit/test_p1_3_v3_projection_write_convergence_readiness.py` and registered it in `backend/test.sh`.
...
- The contract requires route-scoped server-owned evidence; durable outbox/equivalent acknowledgement; dual-write/projection writer readiness; create/update/delete and tombstone convergence; idempotency-key replay/mismatch semantics; aligned account/projection/freshness/tombstone/vector-cleanup fences; and rollback fail-closed behavior before memory projection reads are trusted.
...
Added `backend/scripts/p1_3_v3_archive_short_term_visibility_readiness.py`, `backend/utils/memory/v3_archive_visibility_readiness.py`, and `backend/tests/unit/test_memory_v3_archive_visibility_readiness.py` as a local/read-only readiness contract for future GET `/v3/memories`: Archive is unavailable by default, stale Short-term is not default-visible, archive/historical context requires explicit opt-in, fresh source-backed Short-term and active stable Long-term may be default-visible, and unknown visibility/lifecycle/freshness fails closed. Runtime remains BLOCKED; no route wiring, production calls, telemetry sink, approval, Archive default visibility, stale Short-term default visibility, or legacy fallback/merge is claimed.

## Oracle follow-up review: memory /v3 pre-runtime readiness chain (2026-06-20)

**Session:** `compact-memory-v3-readiness-review`
...
**Local verification before consult:** clean tree at `ad01f543b`; full memory regression `676 passed, 3 warnings`; runtime readiness remained `BLOCKED` with `remaining_gate_count=10`, `existing_local_proof_count=33`; no `backend/routers/memories.py` diff.

### Verdict

Oracle: **GO to begin a default-off wiring implementation; NO-GO to enable the memory path, canary it, or describe it as runtime-ready.** The review says the local proof chain has reached diminishing returns and the next meaningful proof is composed route behavior, but several seam defects should be fixed before the first router commit.

### P0 activation blockers

1. No composed memory FastAPI behavior exists; current real-router proof is legacy-only baseline evidence.
...
6. Decide/pin pagination compatibility: non-enrolled legacy keeps current behavior; memory should use signed keyset cursors and reject `offset>0` unless product decides otherwise.
7. Replace module-import checks for memory adapter invocation with function-level call counters/spies.
...
### 2026-06-20 — memory-V3-F1 seam invariant hardening
...
1. Add a typed request-scoped `V3GetRuntimeSnapshot` with subject/control/config/account generation/projection generation/convergence/cursor/archive capability/deadline/read timestamp coherence.
2. Build a framework-independent composed service entry point before route wiring.
3. First router commit should be default-off, legacy-byte-preserving for non-enrolled callers, memory-only for enrolled callers, dependency-overrideable in TestClient, and never fallback to legacy after memory entry.
...
2. **Pagination compatibility:** recommended default is legacy offset behavior only for non-enrolled callers; memory uses signed keyset cursor, rejects `offset>0`, default limit `100`, max `500`, no `5000` first-page override.

## Oracle milestone review: F1+F2 before F3 composed service (2026-06-20)
...
According to the June 20, 2026 repository bundle, pure pre-wiring work may continue while runtime remains explicitly blocked.

## 1. Verdict

**LIMITED GO for F3 only.**

Proceed with the framework-independent composed service. Keep **NO-GO** for router wiring, canaries, or external readiness. F3 must not be presented as read-cutover evidence, especially while write/delete convergence remains a separate runtime concern.

## 2. Highest-risk gaps

1. **Snapshot-to-read TOCTOU:** define one immutable execution context binding subject, grant/config epoch, account generation, projection generation/commit, cursor-policy and secret versions, read timestamp, and deadline. The projection result must attest the same values; any mismatch should invalidate the whole page.

2. **Row-level fences:** F2’s snapshot coherence is insufficient by itself. Every returned row should match subject, account/projection generation, item revision, source version/commit, and delete/tombstone state. Subject/generation/proof mismatches should fail the page, not merely drop the row.

3. **Filtering versus cursor advancement:** specify whether the next cursor represents the last scanned or last returned row. Prefer last scanned with bounded refill, otherwise hidden Archive, expired Short-term, or tombstoned rows can cause duplicates, omissions, or endless sparse pages.

4. **Deadline enforcement:** checking a deadline only during snapshot construction is not enough. Pass remaining budget into every adapter, recheck around projection reads, and never turn timeout or partial-read results into `200 []`.

5. **Public error taxonomy:** pin exact mappings now—request/cursor/offset errors, grant denial, infrastructure/config failure, generation invalidation, and deadline exhaustion—while keeping public details non-enumerating and sanitized.

## 3. F3 acceptance criteria

* Exact call-order tests with “bomb” adapters proving no later stage runs after failure.
* Non-enrolled path calls only the legacy reader; enrolled paths can never call it.
* Missing, false, malformed, or revoked grant performs zero projection reads and zero legacy reads.
* Closed typed adapter outputs only; dictionaries, unknown enum values, contradictory status flags, exceptions, and malformed returns fail closed.
* Post-read snapshot attestation plus row-level validation occurs before lifecycle filtering or response adaptation.
* Multi-page property tests prove no duplicates or omissions across filtered Archive, expired Short-term, deleted rows, equal timestamps, and generation changes.
* Bounded scan/refill/read counts, response-byte cap, and deadline propagation; no unbounded work merely to fill a 500-item page.
* `200 []` is reserved for a successfully verified empty projection/page—not timeout, malformed output, partial convergence, or all-security-invalid rows.
* Readiness results are derived from executed spies and negative cases, not hard-coded “zero call” booleans or test-count growth. Include mutation tests that deliberately bypass a gate and verify the suite fails.
* Static import checks prohibit FastAPI/router modules, production database clients, network/provider clients, and telemetry sinks.

## 4. Product-decision concerns

**No-default-memory-grant:** the fail-closed/no-fallback decision is sound. The remaining semantic issue is scope: if this grant represents universal privacy consent for default memory, allowing non-enrolled legacy reads without it is contradictory. If it authorizes only the memory path, state that explicitly. Bind a grant/config epoch into the snapshot and cursor so revocation invalidates outstanding cursors and stale positive caches.

**Pagination:** the decision is sound. Pin `offset=0` as an accepted compatibility no-op, use a stable total order such as `(created_at DESC, memory_id DESC)`, and bind cursors to subject, generation/commit, filters, Archive intent, source/read mode, and grant/config epoch. Include expiry and bounded previous-secret support for rotation.
...
**Still NO-GO:** activation, shadow reads against real services, canary, external readiness, production control/projection reads, writes, telemetry sinks, or authoritative read cutover. The unresolved memory-read/legacy-write split-brain remains a cutover blocker.
...
1. **Mode-dependent validation:** do not put `ge/le` constraints on the shared FastAPI `limit` or `offset` parameters. That would change legacy behavior before source selection and could return 422 instead of the required memory 400. Perform memory limits inside F3.
2. **No exception-driven downgrade:** never implement `try memory; except: legacy`. Only an explicit typed `LEGACY_PRIMARY` decision may call legacy.
...
* **Production/default dependency disabled:** default request, explicit `limit`, `offset=0`, and `offset>0` exactly match current legacy calls and bodies; every memory adapter has zero calls.
...
* **Response:** body remains `List[MemoryDB]`; memory-only fields are stripped; only allowlisted additive headers are emitted; next cursor does not mutate the body.
...
* one exported `get_v3_get_runtime()` dependency returning an immutable bundle containing the composed service, source selector/control reader, legacy reader, projection reader, cursor keyring/codec, clocks/deadline, and no-op observer
...
* The production default dependency cannot activate memory.
* Full memory regression, focused real-router tests, `git diff --check`, and existing readiness scripts remain green while reporting `BLOCKED`.
...
### 2026-06-20 — memory-V3-F4 default-off router seam completion

Implemented the F4 router-boundary slice under Oracle's LIMITED GO while preserving the NO-GO activation posture:

- `GET /v3/memories` now has a default-off memory runtime seam in `backend/routers/memories.py`.
- The production/default `get_v3_get_runtime()` dependency is structurally disabled and cannot be activated by env vars, request inputs, headers, persisted control records, or rollout flags.
...
- TestClient overrides can exercise `legacy_primary` and `memory_read`; once in `memory_read`, the router maps the composed service result directly and never falls back to legacy.
...
1. **Structural-disable proof:** repository-wide assertion that production code never overrides `get_v3_get_runtime`, reads activation state, or constructs `memory_read`; malformed/unknown runtime values must fail closed rather than select legacy.
2. **Pagination branch-order proof:** memory must receive default `100`/max `500` semantics before the legacy `offset=0 → limit=5000` rewrite; prove `offset=0` no-op, `offset>0` 400, and no legacy call on grant denial, cursor failure, timeout, or service exception.
...
```bash
python3 backend/scripts/v3_f5_real_service_evidence_readiness.py --execute
```
