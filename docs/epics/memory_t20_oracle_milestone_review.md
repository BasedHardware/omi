...
**2026-06-20 local `/v3` canary/approval artifact source-selection and ownership/IAM readiness slice:** Added safe-by-default `backend/scripts/v17_p1_3_v3_canary_approval_source_readiness.py` plus `backend/tests/unit/test_v17_p1_3_v3_canary_approval_source_readiness.py`, registered it in `backend/test.sh`, and linked `canary_approval_source_readiness_proof` into observability approval, `/v3` external compatibility, and GET runtime-wiring readiness. The artifact selects the future server-owned route-scoped approval source `firestore:system/v17_v3_canary_approvals/routes/get_v3_memories`, pins bounded owner groups (`product_privacy_ops`, `memory_platform_oncall`), forbids client-supplied approval artifacts, requires direct-client-deny rules/IAM or emulator proof and backend service-principal read proof before runtime wiring, now inventories static/emulator-harness readiness for signed-in client read/create/update/delete denial plus Admin-context service-principal read fixture coverage via `backend/scripts/v17_firestore_rules_emulator_test.mjs` / `npm run test:v17-v3-canary-approval-source:emulator`, and keeps production artifact existence/read/approval as missing/not-run. No `backend/routers/memories.py` change, runtime `/v3` behavior change, production Firestore/cloud/provider/vector/network call, telemetry sink production call, rollout approval, PII/raw-content/secret/cursor logging, Archive default visibility, stale Short-term default visibility, or legacy fallback/merge is claimed.

Verification: RED `cd backend && env -u VIRTUAL_ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin python3 -m pytest tests/unit/test_v17_p1_3_v3_canary_approval_source_readiness.py -q` → `5 failed in 0.08s` (missing runner/test.sh/docs/readiness links). GREEN/focused/full/async/docs outputs are recorded in the handoff for this commit; readiness remains **BLOCKED / NO-GO**.
...
**2026-06-20 `/v3` server-owned runtime control/config source-selection readiness proof:** Added `backend/scripts/v17_p1_3_v3_runtime_config_source_readiness.py` and `backend/tests/unit/test_v17_p1_3_v3_runtime_config_source_readiness.py` as a production-safe/readiness-only source-selection proof for future backend-owned `/v3` runtime enablement decisions. The selected route-scoped server-owned config sources are `memory_control/v17_global_read_gate` and `memory_control/v17_write_convergence_gate`, with bounded config keys, backend service-principal read-only ownership, no client override, no uid/session/memory/request-payload/cursor-token dimensions, and fail-closed behavior for missing, stale, or malformed config. Default execution remains `BLOCKED`/`NOT_RUN` with no production calls; gated `--execute` is read-only and does not claim route wiring, rollout approval, telemetry sink execution, provider/vector calls, Firestore writes, user-content logging, Archive default visibility, stale Short-term default visibility, or legacy fallback/merge. Linked as `runtime_config_source_readiness_proof` from `/v3` external compatibility and GET runtime-wiring readiness; Oracle remains **BLOCKED / NO-GO**.

Verification for this slice is recorded in this handoff and in `docs/epics/v17_memory_implementation_tickets.md`. Remaining `/v3` work: execute a gated real backend service-principal config read proof only when the production-safe env gates/credentials are intentionally supplied, then proceed to the next non-canary route-readiness proof before any runtime `/v3` wiring.
...
**No-default-memory-grant:** the fail-closed/no-fallback decision is sound. The remaining semantic issue is scope: if this grant represents universal privacy consent for default memory, allowing non-enrolled legacy reads without it is contradictory. If it authorizes only the V17 path, state that explicitly. Bind a grant/config epoch into the snapshot and cursor so revocation invalidates outstanding cursors and stale positive caches.
...
## V17-V3-F5 real-service read-only evidence preparation (2026-06-20)
