...
on [Apple](https://apps.apple.com/us/app/omi-ai-wearable/id6502156163)/[Google](https://play.google.com/store/apps/details?id=com.omi.ios&hl=en_US) store
...
### 7. Setup Firebase
   ...
   - Run the following command to register the prod flavor of the app. The command will prompt you to select `configuration type`; under it, select `Target` and then `Runner`
     ```
     flutterfire config --out=lib/firebase_options_prod.dart --ios-bundle-id=com.omi-app-with-wearable.ios12 --android-app-id=com.omi.ios --android-out=android/app/src/prod/  --ios-out=ios/Config/Prod/
     ```
   - Similarly for dev environment
     ```
     flutterfire config --out=lib/firebase_options_dev.dart --ios-bundle-id=com.omi-app-with-wearable.ios12.develop --android-app-id=com.omi.ios.dev --android-out=android/app/src/dev/  --ios-out=ios/Config/Dev/
     ```
