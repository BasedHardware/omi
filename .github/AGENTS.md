...
- Backend deploy workflows may only run Firestore index readiness with `--check-only` against `RUNTIME_GCP_PROJECT_ID`; run it in an isolated job from the approved commit with `GCP_FIRESTORE_READONLY_CREDENTIALS`, and bind manual deploys to the exact checked candidate SHA. A failed gate may upload only a locally revalidated, bounded, redacted schema proposal artifact; production index writes remain outside backend deployment workflows.
