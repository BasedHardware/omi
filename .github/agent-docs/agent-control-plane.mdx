...
[agent-control-plane-invariants](./agent-control-plane-invariants.mdx).

## 1. Executive decision

Omi is the agent control plane. ACP, pi-mono, Hermes, OpenClaw, AgentVM, A2A agents, and future runtimes are execution adapters beneath it.

## Agent VM tool surface

The Agent VM REST surface is intentionally narrower than the backend's internal tool registry. `GET /v1/agent/tools` omits `fetch_url_tool`, and `POST /v1/agent/execute-tool` rejects requests naming it with `404`; clients must treat that absence and rejection as the stable removal contract and must not retry the rejection as a transient tool failure. Agent VMs must not use a cloud execution path for URLs or content that a desktop user supplied for local-only handling; URL fetching remains available only through the backend agent path when the URL appears in the latest user turn's `<user_provided_urls>` allowlist (each listed URL may be fetched at most once per turn), and a redirect target stays eligible only when it is same-origin with the fetched URL or is itself exactly listed in that turn's allowlist.
...
flowchart TB
    ...
    Future[Hermes / OpenClaw / AgentVM / A2A adapters]
...
local ACP, Hermes, and OpenClaw sessions use the token-free `refresh_owner`
