...
void main() {
  group('TypingIndicator', () {
    testWidgets('renders three animated bubbles', (tester) async {
      ...
      int bubbleCount = 0;
      final containers = find.descendant(of: typingIndicator, matching: find.byType(Container));
    ...
    testWidgets('uses SlideTransition and optimized ScaleTransition with RepaintBoundary', (tester) async {
      ...
      final slideTransitions = find.descendant(of: typingIndicator, matching: find.byType(SlideTransition));
      expect(slideTransitions, findsNWidgets(3));

      // Should have ScaleTransition (restored with optimization: 0.85-1.0 range)
      final scaleTransitions = find.descendant(of: typingIndicator, matching: find.byType(ScaleTransition));
      expect(scaleTransitions, findsNWidgets(3));

      // Should have RepaintBoundary to isolate repaints (optimization)
      final repaintBoundaries = find.descendant(of: typingIndicator, matching: find.byType(RepaintBoundary));
    ...
    testWidgets('uses AnimatedBuilder for color animation', (tester) async {
      ...
      final animatedBuilders = find.descendant(of: typingIndicator, matching: find.byType(AnimatedBuilder));
