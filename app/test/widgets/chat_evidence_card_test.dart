...
void main() {
  testWidgets('renders a supplemental evidence card without replacing the answer text', (tester) async {
    var opened = false;
        ...
        home: Scaffold(
          body: Column(
            children: [
              ...
              ChatEvidenceReferenceCard(reference: reference, onOpen: () => opened = true),
            ],
          ),
        ),
      ),
  ...
  testWidgets('renders honest non-blocking states and exposes an accessible label', (tester) async {
    const states = [
    ...
    for (final state in states) {
      ...
      final semantics = tester.getSemantics(find.byKey(ValueKey('chat-evidence-${reference.id}')));
      expect(semantics.label, contains(reference.accessibilityLabel));
    }
  });

  testWidgets('keeps the answer visible for loading and offline frame requests without actions', (tester) async {
    for (final state in [ChatEvidenceReferenceState.loading, ChatEvidenceReferenceState.offline]) {
      ...
      await tester.pumpWidget(
        MaterialApp(
          localizationsDelegates: AppLocalizations.localizationsDelegates,
          supportedLocales: AppLocalizations.supportedLocales,
          home: Scaffold(body: buildMessageWidget(message, (_) {}, false, false, null, (_) {}, setMessageNps)),
        ),
      );
      ...
      expect(find.text(reference.statusLabel), findsOneWidget);
      expect(
        find.ancestor(of: find.byKey(ValueKey('chat-evidence-${reference.id}')), matching: find.byType(InkWell)),
        findsNothing,
      );
      ...
      await tester.pumpWidget(
        MaterialApp(
          ...
          supportedLocales: AppLocalizations.supportedLocales,
          home: Scaffold(
            body: ChatEvidenceReferenceCard(reference: reference, onOpen: () => opened = true),
          ),
        ),
      );
  ...
  testWidgets('empty envelopes render no supplemental chrome', (tester) async {
    await tester.pumpWidget(
      const MaterialApp(
        home: Scaffold(
          body: ChatEvidenceReferenceList(envelope: ChatEvidenceReferenceEnvelope(references: [])),
        ),
      ),
    );
