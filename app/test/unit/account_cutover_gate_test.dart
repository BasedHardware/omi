...
void main() {
  ...
  test('force upgrade fails closed before product traffic', () {
    final control = AccountCutoverControl.fromJson(_validControlJson(
      clientAction: 'force_upgrade',
  ...
  test('drain only while product traffic remains allowed', () {
    final drainLegacy = AccountCutoverControl.fromJson(_validControlJson(
      offlineQueueInstruction: 'drain',
    ));
    expect(gate.shouldUploadOfflineQueues(drainLegacy), isTrue);

    final drainWhileBlocked = AccountCutoverControl.fromJson(_validControlJson(
      state: 'migrating',
      accountGeneration: 2,
      clientAction: 'migration_maintenance',
      offlineQueueInstruction: 'drain',
  ...
  test('unknown wire enums and non-boolean safety flags are rejected', () {
    ...
    expect(
      () => AccountCutoverControl.fromJson({
        ..._validControlJson(),
        'product_traffic_allowed': 'yes',
      }),
      throwsA(isA<AccountCutoverControlParseException>()),
    );
    expect(
      () => AccountCutoverControl.fromJson({
        ..._validControlJson()..remove('product_traffic_allowed'),
      }),
      throwsA(isA<AccountCutoverControlParseException>()),
    );
  });
  ...
  test('runtime retains generation across failed refresh and blocks product traffic', () async {
    ...
    final migrating = AccountCutoverControl.fromJson(_validControlJson(
      state: 'migrating',
      accountGeneration: 9,
      clientAction: 'migration_maintenance',
      offlineQueueInstruction: 'quarantine',
    ...
    var fetches = 0;
    final client = AccountCutoverControlClient(fetch: () async {
      fetches++;
      if (fetches == 1) {
  ...
  test('owner transition clears prior account state and ignores stale in-flight results', () async {
    ...
    final secondControl = AccountCutoverControl.fromJson(_validControlJson(
      state: 'new',
      accountGeneration: 3,
      productTrafficAllowed: false,
      legacyWritesAllowed: false,
    ...
    await runtime.bindAuthenticatedOwner(
      'owner-b',
      client: AccountCutoverControlClient(
        fetch: () async => AccountCutoverFetchResult.success(secondControl),
      ),
    );
    ...
    pendingFirst.complete(AccountCutoverFetchResult.success(AccountCutoverControl.fromJson(_validControlJson(
      accountGeneration: 99,
      state: 'legacy',
    ))));
    await firstBind;
    expect(runtime.ownerUid, 'owner-b');
