...
void main() {
  group('Meta glasses runtime regressions', () {
    ...
    test('runtime proof log can be pulled from EddyPhone app container', () {
      final script = File('scripts/pull_meta_glasses_runtime_proof.sh');

      expect(script.existsSync(), isTrue);
      final source = script.readAsStringSync();
      expect(source, contains('2649C7E8-7E64-501B-9108-8BC6038B8C2F'));
      expect(source, contains('dev.moni11811.omi'));
      expect(source, contains('appDataContainer'));
      expect(source, contains('Documents/meta_glasses_runtime_proof.log'));
      expect(source, contains('/tmp/omi-meta-glasses-runtime-proof-pulled.log'));
    });

    test('runtime proof checker requires stream and frame evidence', () {
      final script = File('scripts/check_meta_glasses_runtime_proof.sh');

      expect(script.existsSync(), isTrue);
      final source = script.readAsStringSync();
      expect(source, contains('MetaGlassStreamDiag stream-started'));
      expect(source, contains('MetaGlassStreamDiag frame-event'));
      expect(source, contains('MetaGlassStreamDiag frame-captured'));
      expect(source, contains('gestures=unsupported'));
      expect(source, contains('auto-start-skip reason=not-ready'));
    });

    test('runtime plan does not claim hardware proof complete while proof gate is pending', () {
      final plan = _read('docs/superpowers/plans/2026-07-04-meta-glasses-runtime-stabilization.md');

      expect(plan, isNot(contains('runtime — done 2026-07-04')));
      expect(plan, isNot(contains('needs on-device check: no, EddyPhone + glasses proof completed')));
      expect(plan, contains('latest_launch=blocked_locked_device'));
      expect(plan, contains('previous_proof_MetaGlassGestureDiag received=0'));
    });
