...
Memory _memory({required String id, String? band = 'current', DateTime? computedAt, bool historical = false}) {
...
void main() {
  ...
  test('server views keep useful-now, history, and all datasets disjoint', () async {
  ...
  test('history continuation keeps partial state until the server finishes', () async {
    ...
    final provider = MemoriesProvider(
      ...
      fetchLedgerHistoryRequest: ({int limit = 500, int offset = 0}) async {
        ...
        if (historyCalls == 1) {
          return GetLedgerHistoryResult([first], supported: true, truncated: true);
        }
    ...
    expect(provider.memories.map((memory) => memory.id), containsAll(<String>['old-1', 'old-2']));
  ...
  test('concurrent load-more history calls are single-flight', () async {
    ...
    final provider = MemoriesProvider(
      ...
      fetchLedgerHistoryRequest: ({int limit = 500, int offset = 0}) async {
        offsets.add(offset);
        return GetLedgerHistoryResult([_memory(id: 'old-$offset', historical: true)], supported: true, truncated: true);
      },
    );
    addTearDown(provider.dispose);
    ...
    expect(provider.memories.map((memory) => memory.id), containsAllInOrder(<String>['old-0', 'old-1', 'old-2']));
  });
  ...
  test('history follows a short server cursor page instead of claiming completion', () async {
    ...
    final provider = MemoriesProvider(
      ...
      fetchLedgerHistoryCursorRequest: ({int limit = 500, int offset = 0, String? cursor}) async {
        ...
        page++;
        if (page == 1) {
          return const GetLedgerHistoryResult([], supported: true, nextCursor: 'history-2', beliefEnabled: true);
        }
  ...
  test('suppressed rows stay available in explicit history only', () async {
    ...
    final provider = MemoriesProvider(
      fetchMemoriesRequest: ({int limit = 100, int offset = 0, bool thisDeviceOnly = false}) async =>
          GetMemoriesResult([suppressed], true, beliefEnabled: true),
      fetchLedgerHistoryRequest: ({int limit = 500, int offset = 0}) async =>
          GetLedgerHistoryResult([suppressed], supported: true, beliefEnabled: true),
    );
    addTearDown(provider.dispose);
  ...
  test('a missing capability header resets temporal filtering and preserves rows', () async {
    final current = _memory(id: 'current');
    final old = _memory(id: 'old', historical: true);
    final provider = MemoriesProvider(
      fetchMemoriesRequest: ({int limit = 100, int offset = 0, bool thisDeviceOnly = false}) async =>
    ...
    expect(provider.filteredMemories.map((memory) => memory.id), contains('old'));
  });
  ...
  test('a false capability preserves legacy rows without temporal filtering', () async {
    ...
    final provider = MemoriesProvider(
      fetchMemoriesRequest: ({int limit = 100, int offset = 0, bool thisDeviceOnly = false}) async =>
          GetMemoriesResult([current], true, beliefEnabled: true),
      fetchLedgerHistoryRequest: ({int limit = 500, int offset = 0}) async =>
          GetLedgerHistoryResult([old], supported: true, beliefEnabled: false),
    );
    addTearDown(provider.dispose);

    await provider.loadMemories();
    expect(provider.memoryBeliefEnabled, isFalse);
    expect(provider.filteredMemories.map((memory) => memory.id), containsAll(<String>['current-false', 'old-false']));
  });

  test('memory-use retries reuse one feedback id and refresh the confirmed row', () async {
    ...
    final provider = MemoriesProvider(
      ...
      fetchLedgerHistoryRequest: ({int limit = 500, int offset = 0}) async =>
          const GetLedgerHistoryResult([], supported: false, beliefEnabled: true),
      memoryUseRequest:
          ({required String memoryId, required MemoryUseAction action, required String feedbackId}) async {
    ...
    await provider.loadMemories();
    expect(await provider.setMemoryUse(memory, MemoryUseAction.suppress), isFalse);
    expect(await provider.setMemoryUse(memory, MemoryUseAction.suppress), isTrue);
  ...
  test('current list follows a server cursor after an empty page without mixing offset', () async {
    final calls = <({int offset, String? cursor, MemoryReadView? view})>[];
    final provider = MemoriesProvider(
      fetchMemoriesCursorRequest:
          ({int limit = 100, int offset = 0, bool thisDeviceOnly = false, String? cursor, MemoryReadView? view}) async {
            calls.add((offset: offset, cursor: cursor, view: view));
            if (cursor == null) {
              return const GetMemoriesResult([], true, nextCursor: 'memory-2', beliefEnabled: true);
            }
            return GetMemoriesResult([_memory(id: 'memory-2')], true, beliefEnabled: true);
          },
      fetchLedgerHistoryRequest: ({int limit = 500, int offset = 0}) async =>
          const GetLedgerHistoryResult([], supported: false),
    );
    ...
    expect(provider.memories.map((memory) => memory.id), contains('memory-2'));
  });
  test('memory-use suppress allow suppress creates three distinct acknowledged actions', () async {
    ...
    await provider.loadMemories();
    expect(await provider.setMemoryUse(memory, MemoryUseAction.suppress), isTrue);
    expect(await provider.setMemoryUse(memory, MemoryUseAction.allow), isTrue);
    expect(await provider.setMemoryUse(memory, MemoryUseAction.suppress), isTrue);
    expect(actions, [MemoryUseAction.suppress, MemoryUseAction.allow, MemoryUseAction.suppress]);
