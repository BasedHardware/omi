// Ask Omi before the UI program: starters, composing, a reply and its actions, the Chat Apps
// drawer and Clear Chat.
...
final chatScenarios = <AuditScenario>[
  ...
  AuditScenario(
    ...
    run: (a) async {
      await a.pump(const ChatPage());
      // The app bar's extension button opens the Chat Apps end drawer at this revision.
      await a.tap(find.widgetWithIcon(IconButton, Icons.extension));
      await a.shot('Open the Chat Apps drawer', step: 'drawer');
      await a.tap(find.text('Clear Chat').first);
      await a.shot('Tap Clear Chat at the top of the drawer', step: 'clear-confirm');
    },
  ),
  AuditScenario(
    id: 'chat-feedback',
