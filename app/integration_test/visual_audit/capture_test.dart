...
void main() {
  ...
  final only = (Platform.environment['OMI_AUDIT_ONLY'] ?? '')
      .split(',')
      .map((s) => s.trim())
      .where((s) => s.isNotEmpty);
  final selected = only.isEmpty
      ? auditSuite.scenarios
      : auditSuite.scenarios.where((s) => only.contains(s.id)).toList();
  ...
  File('${dir.path}/scenarios.json').writeAsStringSync(
