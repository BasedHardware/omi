...
void main() {
  ...
  testWidgets('positive: create through provider, edit through PATCH, reload persists server-side', (tester) async {
    ...
    evidence.record(
      ...
      ok: created,
      invariant: 'create completes through the production provider',
    );
    expect(created, isTrue);

    final serverSawCreate = server.countOf('POST', '/v3/memories') == 1;
    evidence.record(
      'create-reached-server',
      ok: serverSawCreate,
      invariant: 'the memory create is persisted server-side through the real API path',
    );
    ...
    evidence.record(
      'optimistic-visible',
      ok: optimistic,
      invariant: 'the created memory is visible to the user immediately',
    );
    ...
    evidence.record(
      'edit-persisted',
      ok: editOk && server.memories.firstWhere((m) => m['id'] == serverRecordId)['content'] == '$content$editedSuffix',
    ...
    final persisted = reloaded.memories.any(
      (m) =>
  ...
  testWidgets('negative: drop-memory-save must fail naming the persistence invariant', (tester) async {
    final evidence = JourneyEvidence.begin(
      journeyId: 'j3_memory_create_edit_reload.drop-memory-save',
      lane: journeyLane,
    );
    final server = await JourneyHermeticBoot.start();
    addTearDown(JourneyHermeticBoot.stop);
