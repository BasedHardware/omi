  ...
  testWidgets('persisted person id renders after reload, including an offline-synced conversation', (tester) async {
    ...
    for (final conversation in provider.conversations) {
      await JourneyHermeticBoot.pumpPage(
        tester,
        page: Scaffold(
          body: TranscriptWidget(key: ValueKey(conversation.id), segments: conversation.transcriptSegments),
        ),
      );
      ...
      evidence.record(
        conversation.id,
        ok: named,
        invariant: 'persisted person_id resolves through reloaded people to the displayed name',
      );
    ...
    await JourneyHermeticBoot.pumpPage(
      ...
      page: Scaffold(
        body: TranscriptWidget(
          ...
          segments: provider.conversations.first.transcriptSegments,
        ),
      ),
    );
    await tester.pumpAndSettle();
    ...
    evidence.record(
      ...
      ok: cleared,
      invariant: 'another account does not display the previous account person name',
    );
