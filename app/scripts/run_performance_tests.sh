...
REPORT_DIR="/tmp/omi_perf_reports"
mkdir -p "$REPORT_DIR"
...
for test_entry in "${TESTS[@]}"; do
  ...
  if flutter drive \
    ...
    --profile \
    --flavor dev \
    $DEVICE_FLAG 2>&1 | tee "$REPORT_DIR/${test_file%.dart}_$TIMESTAMP.log"; then
    echo "✓ PASS: $test_name"
    PASSED=$((PASSED + 1))
...
{
  echo "# Omi App Performance Report"
  echo ""
  ...
  # Parse JSON results from /tmp/ if available
  for json_file in /tmp/omi_perf_memory_*.json /tmp/omi_perf_cpu_*.json /tmp/omi_perf_responsiveness_*.json /tmp/omi_perf_battery_*.json; do
    if [[ -f "$json_file" ]]; then
      echo ""
      echo "*(Data from: $(basename "$json_file"))*"
      echo ""
    fi
  done
  ...
  echo "## Raw Data"
  echo ""
  echo "Performance test logs and JSON results are in: \`$REPORT_DIR/\`"
  echo ""
  echo "---"
  echo "*Generated by Omi performance test suite*"
} > "$REPORT_FILE"
