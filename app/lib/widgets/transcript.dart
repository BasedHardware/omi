...
class _TranscriptWidgetState extends State<TranscriptWidget> {
  ...
  Widget build(BuildContext context) {
    debugPrint('🔥 TranscriptWidget: Building with ${widget.segments.length} segments and ${widget.images.length} images');
    ...
    if (isExistingConversation) {
      ...
      debugPrint('🔥 TranscriptWidget: Existing conversation - showing all ${imagesToShow.length} images');
    } else {
      ...
      imagesToShow = widget.images.where((image) {
        return provider.photos.any((photo) {
          ...
          debugPrint('🔍 Filtering live image ${image.id}: matches=$matches, hasDescription=$hasDescription');
          return matches && hasDescription;
        });
      }).toList();
      debugPrint('🔥 TranscriptWidget: Live conversation - filtered to ${imagesToShow.length} images with descriptions');
    }
    
    // Debug: Log images to show
    for (int i = 0; i < imagesToShow.length; i++) {
      final image = imagesToShow[i];
      debugPrint('📸 Image to display $i: ID=${image.id}, URL=${image.thumbnailUrl.substring(0, math.min(50, image.thumbnailUrl.length))}..., CreatedAt=${image.createdAt}');
    }
    ...
    debugPrint('🔥 TranscriptWidget: Total items before sorting: ${allItems.length}');
    ...
    debugPrint('🔥 TranscriptWidget: Items after sorting: ${allItems.length}');
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, idx) {
        ...
        if (item is ImageSegment) {
          debugPrint('🖼️ Building ImageSegment at index $idx: ${item.id}');
  ...
  Widget _buildImageItem(ImageSegment image) {
    debugPrint('🖼️ _buildImageItem called for image: ${image.id}');
    debugPrint('🖼️ Thumbnail URL: ${image.thumbnailUrl}');
    
    // Check if this is a base64 data URL
    final isDataUrl = image.thumbnailUrl.startsWith('data:image/');
    debugPrint('🖼️ Is data URL: $isDataUrl');
                ...
                : Image.network(
                    ...
                    loadingBuilder: (context, child, loadingProgress) {
                      if (loadingProgress == null) {
                        debugPrint('🖼️ Network image loaded successfully: ${image.id}');
                        return child;
                      }
                      debugPrint('🖼️ Loading network image: ${image.id}, progress: ${loadingProgress.expectedTotalBytes}');
                      return Container(
                        height: 200,
                    ...
                    errorBuilder: (context, error, stackTrace) {
                      debugPrint('🖼️ Error loading network image: ${image.id}, error: $error');
                      return Container(
                        height: 200,
                        color: Colors.grey[900],
                        child: const Center(
            ...
            builder: (context, provider, child) {
              final matchingPhoto = provider.photos.where((photo) {
                String photoIdentifier = photo.photoId?.isNotEmpty == true 
                    ...
                    : photo.id.toString();
                bool matches = photoIdentifier == image.id;
                debugPrint('🔍 Matching photo: ${photo.id} (${photoIdentifier}) == ${image.id}? $matches');
                return matches;
              }).firstOrNull;
              ...
              debugPrint('🔍 Found matching photo with description: ${description.substring(0, math.min(30, description.length))}...');
