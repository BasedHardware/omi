...
class _ConversationCapturingPageState extends State<ConversationCapturingPage> {
  ...
  /// The live page's state in the shared vocabulary: muted or on a call is Paused, a dropped
  /// transcription socket is Reconnecting, everything else is Recording.
  CaptureDisplayState _displayState(CaptureProvider provider) {
    if (provider.isPaused || provider.isCallActive) return CaptureDisplayState.paused;
    if (provider.recordingState == RecordingState.interrupted) return CaptureDisplayState.reconnecting;
    return CaptureDisplayState.recording;
  }
    ...
    return Consumer2<CaptureProvider, DeviceProvider>(
      builder: (context, provider, deviceProvider, child) {
        ...
        return Scaffold(
          ...
          appBar: ConversationStateAppBar(state: _displayState(provider)),
          body: Column(
            children: [
