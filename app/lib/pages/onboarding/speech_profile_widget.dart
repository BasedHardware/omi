...
class _SpeechProfileWidgetState extends State<SpeechProfileWidget> with WidgetsBindingObserver {
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            Text(
              '${flow.promptIndex + 1} / ${GuidedVoiceController.promptCount}',
        ...
        Text(
          ...
          style: const TextStyle(color: Colors.white, fontSize: 30, height: 1.35, fontWeight: FontWeight.w600),
        ),
        ...
        if (flow.error != null) ...[
          const SizedBox(height: 12),
          Text(
            flow.error == 'microphone' ? context.l10n.microphoneAccessDescription : copy('transcriptionError'),
  ...
  Widget _review() => Column(
    ...
    children: [
      Text(
        copy('review'),
        style: const TextStyle(fontSize: 27, color: Colors.white, fontWeight: FontWeight.w600),
      ),
      const SizedBox(height: 12),
      ...
      if (flow.error == 'memories') Text(copy('memoryError'), style: const TextStyle(color: Colors.white, height: 1.5)),
    ],
  );
  ...
  List<Widget> _actions() {
    ...
    return [
      if (flow.busy)
        ...
        _button(
          copy('start'),
          flow.busy || _stoppingCapture ? null : () => unawaited(_start()),
          'speech_profile_start',
        ),
      const SizedBox(height: 8),
      Row(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (done) ...[
                          ...
                          const SizedBox(height: 24),
                          _status(
                            flow.savedMemoryCount > 0 ? copy('savedMemories') : copy('noMemories'),
                            success: true,
                          ),
                          if (flow.goalSaved) ...[
