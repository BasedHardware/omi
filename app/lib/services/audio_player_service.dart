...
class AudioPlayerService extends ChangeNotifier {
  ...
  bool isWalPlaying(String walId) => _currentPlayingWalId == walId;
  ...
  Future<void> _startPlayback(Wal wal) async {
    ...
    try {
      final wavFilePath = await _getOrCreateWavFile(wal);
      if (wavFilePath == null) {
  ...
  void _setupPositionTracking(Wal wal) {
    ...
    // Backup timer for position tracking
  ...
  Future<void> shareWalAsWav(Wal wal) async {
    ...
    try {
      final wavFilePath = await _getOrCreateWavFile(wal, forSharing: true);
      if (wavFilePath == null) {
  ...
  Future<String?> _getOrCreateWavFile(Wal wal, {bool forSharing = false}) async {
    final cacheKey = forSharing ? '${wal.id}_share' : wal.id;

    if (!forSharing && _wavFileCache.containsKey(cacheKey)) {
      final cachedPath = _wavFileCache[cacheKey]!;
    ...
    if (wal.codec.isOpusSupported()) {
      wavFilePath = await _decodeOpusToWav(wal, audioFilePath, forSharing: forSharing);
    } else {
      wavFilePath = await _convertPcmToWav(wal, audioFilePath, forSharing: forSharing);
    }

    if (wavFilePath != null && !forSharing) {
      _wavFileCache[cacheKey] = wavFilePath;
    }
  ...
  Future<String?> _decodeOpusToWav(Wal wal, String opusFilePath, {bool forSharing = false}) async {
    try {
      ...
      if (!file.existsSync()) throw Exception('Opus file not found');
      ...
      if (opusFrames.isEmpty) throw Exception('No opus frames found in file');
      ...
      if (pcmFrames.isEmpty) throw Exception('No PCM data decoded');
  ...
  Future<String?> _convertPcmToWav(Wal wal, String pcmFilePath, {bool forSharing = false}) async {
    try {
      ...
      if (!file.existsSync()) throw Exception('PCM file not found');
      ...
      if (pcmFrames.isEmpty) throw Exception('No PCM frames found in file');
  ...
  String? getCachedWavPath(String walId) => _wavFileCache[walId];

  Future<String?> ensureWavFileExists(Wal wal) async {
    final cacheKey = wal.id;

    if (_wavFileCache.containsKey(cacheKey)) {
      final cachedPath = _wavFileCache[cacheKey]!;
      if (File(cachedPath).existsSync()) {
        return cachedPath;
      }
    ...
    return await _getOrCreateWavFile(wal, forSharing: false);
  }
