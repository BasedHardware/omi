...
class DeviceProvider extends ChangeNotifier implements IDeviceServiceSubsciption {
  ...
  /// Keeps scanning for nearby devices while a device is already paired, so the
  /// picker can offer a second device (OmiGlass next to an Omi). The default
  /// discovery loop stops as soon as something is paired; this one runs until
  /// [stopDiscoveryScanning].
  ...
  void onDeviceConnectionStateChanged(String deviceId, DeviceConnectionState state) async {
    ...
    switch (state) {
      case DeviceConnectionState.connected:
        ...
        _connectDebouncerFor(deviceId).run(() => _handleDeviceConnected(deviceId));
        break;
      ...
      case DeviceConnectionState.disconnected:
        ...
        if (_connectedDevices.containsKey(deviceId) ||
          ...
          _disconnectDebouncerFor(deviceId).run(() => _handleDeviceDisconnected(deviceId));
        }
        break;
    }
  }
