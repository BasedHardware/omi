...
Stream<ServerMessageChunk> sendMessageStreamServer(String text, {String? appId, List<String>? filesId}) async* {
  ...
  try {
    ...
    final body = jsonEncode({'text': text, 'file_ids': filesId});

    // Create a client to handle the streamed response
    ...
    await for (var data in response.stream.transform(utf8.decoder)) {
      ...
      for (var line in lines.where((line) => line.isNotEmpty)) {
        // Dealing w/ the package splitting by certain byte size in http package
...
Stream<ServerMessageChunk> sendVoiceMessageStreamServer(List<XFile> files) async* {
...
Future<List<ServerMessage>> sendVoiceMessageServer(List<XFile> files) async {
  var request = http.MultipartRequest(
    'POST',
...
Future<List<MessageFile>?> uploadFilesServer(List<XFile> files, {String? appId}) async {
