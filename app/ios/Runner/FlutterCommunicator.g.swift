...
protocol WatchRecorderHostAPI {
  ...
  func sendDeviceState(isRecording: Bool, isConnected: Bool, batteryLevel: Int64) throws
}
...
class WatchRecorderHostAPISetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: WatchRecorderHostAPI?, messageChannelSuffix: String = "") {
    ...
    let sendDeviceStateChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.watch.WatchRecorderHostAPI.sendDeviceState\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      sendDeviceStateChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let isRecordingArg = args[0] as! Bool
        let isConnectedArg = args[1] as! Bool
        let batteryLevelArg = args[2] as! Int64
        do {
          try api.sendDeviceState(isRecording: isRecordingArg, isConnected: isConnectedArg, batteryLevel: batteryLevelArg)
          reply(wrapResult(nil))
        } catch {
          reply(wrapError(error))
        }
      ...
      sendDeviceStateChannel.setMessageHandler(nil)
    }
  }
}
...
protocol WatchRecorderFlutterAPIProtocol {
  ...
  func onDeviceRecordingRequested(completion: @escaping (Result<Void, PigeonError>) -> Void)
}
class WatchRecorderFlutterAPI: WatchRecorderFlutterAPIProtocol {
  ...
  func onDeviceRecordingRequested(completion: @escaping (Result<Void, PigeonError>) -> Void) {
    let channelName: String = "dev.flutter.pigeon.watch.WatchRecorderFlutterAPI.onDeviceRecordingRequested\(messageChannelSuffix)"
    let channel = FlutterBasicMessageChannel(name: channelName, binaryMessenger: binaryMessenger, codec: codec)
    channel.sendMessage(nil) { response in
      guard let listResponse = response as? [Any?] else {
        completion(.failure(createConnectionError(withChannelName: channelName)))
