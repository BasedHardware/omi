...
import {
  ...
  parseStagedDerivedGroupDreamWorkInput,
  type DerivedGroupDreamWorkInputRepository,
  type DerivedGroupDreamWorkInputStageRequest,
  type StagedDerivedGroupDreamWorkInput,
...
const contentHash = (input: StagedDerivedGroupDreamWorkInput): string => sha256CanonicalContent({
  contract_version: "derived-group-dream-work-staged-input-content-v1",
  staged_input: input,
});

const parseRow = (row: InputRow, expectedJob?: DurableMemoryWorkJob): StagedDerivedGroupDreamWorkInput => {
  exactKeys(row, INPUT_KEYS);
  let input: StagedDerivedGroupDreamWorkInput;
  try {
    input = parseStagedDerivedGroupDreamWorkInput({
...
): Promise<StagedDerivedGroupDreamWorkInput | null> => {
  const rows = await connection.query<InputRow>({
    name: "derived-group-dream.input.read",
    text: "SELECT * FROM omi_memory.read_derived_group_dream_work_input($1, $2)",
...
const stage = async (
  ...
  request: DerivedGroupDreamWorkInputStageRequest,
    ...
    return await withAuthorizedSerializableConnectionTransaction(
      ...
      async ({ authority, connection }) => {
        await connection.query({
          name: "derived-group-dream.input.lock_key",
          text: "SELECT pg_advisory_xact_lock(hashtextextended($1, 0))",
          values: [`derived-group-dream-input:${authority.account_id}:${request.pending_job.job_id}`],
        });
        const expected = materializeStagedDerivedGroupDreamWorkInput(request);
        ...
        const rows = await connection.query<{ inserted: boolean }>({
          name: "derived-group-dream.input.insert",
          text: `
SELECT omi_memory.insert_derived_group_dream_work_input(
    ...
    return await withAuthorizedSerializableConnectionTransaction(
      ...
      async ({ authority, connection, dbNowEpochSeconds }) => {
        const rows = await connection.query<LeaseRow>({
          name: "derived-group-dream.input.current_lease",
          text: `
...
export interface PostgresDerivedGroupDreamWorkInputOptions {
  readonly pool: PostgresTransactionPool;
  readonly observability?: PostgresTransactionObservability;
}

/** Inert derived-group-dream input adapter. No worker, scheduler, route, or model default. */
export const createPostgresDerivedGroupDreamWorkInputRepository = (
  options: PostgresDerivedGroupDreamWorkInputOptions,
): DerivedGroupDreamWorkInputRepository => defineDerivedGroupDreamWorkInputRepository({
