...
} from "./typesense-deletion-receipt-repository";
import type {
...
const receipt = (): TypesenseStoredDeletionReceipt => {
...
class FakeConnection implements CheckedOutPostgresConnection {
  ...
  async query<Row extends Record<string, unknown>>(
    ...
    if (this.failure !== null && statement.name !== "typesense_cleanup_receipt.set_role") {
      throw this.failure;
    }
    if (statement.name === "typesense_cleanup_receipt.set_role") return [];
    if (statement.name === "typesense_cleanup_receipt.load") return this.loadRows as readonly Row[];
    if (statement.name === "typesense_cleanup_receipt.record") return this.recordRows as readonly Row[];
...
describe("PostgreSQL Typesense deletion receipt repository", () => {
  test("loads missing/found receipts and records exact replay through fixed cleanup operations", async () => {
    ...
    expect(connection.statements.map((statement) => statement.name)).toEqual([
      "typesense_cleanup_receipt.set_role", "typesense_cleanup_receipt.load",
      "typesense_cleanup_receipt.set_role", "typesense_cleanup_receipt.load",
      "typesense_cleanup_receipt.set_role", "typesense_cleanup_receipt.record",
    ]);
    expect(connection.statements.at(-1)?.values).toEqual([
      ...
      "disposed", 2, "d".repeat(64), receipt().receipt_digest,
    ]);
  });

  test("rejects invalid input before a transaction and persisted coordinate substitution closed", async () => {
    const connection = new FakeConnection();
    const pool = new FakePool(connection);
    const repository = createPostgresTypesenseDeletionReceiptRepository(pool);
    await expect(repository.load({ ...key, operation_ref: "raw-operation" })).rejects
      .toEqual(new PostgresTypesenseDeletionReceiptError("invalid_input"));
    expect(pool.calls).toBe(0);

    connection.loadRows = [{ ...row(), account_id: "account:other" }];
    await expect(repository.load(key)).rejects
      .toEqual(new PostgresTypesenseDeletionReceiptError("persistence_failed"));
  });

  test("maps database conflicts and provider detail to content-safe outcomes", async () => {
    const connection = new FakeConnection();
    const repository = createPostgresTypesenseDeletionReceiptRepository(new FakePool(connection));
    connection.failure = Object.assign(new Error("private SQL and provider payload"), { code: "23505" });
    await expect(repository.record(receipt())).rejects
      .toEqual(new PostgresTypesenseDeletionReceiptError("receipt_conflict"));

    connection.failure = new Error("private SQL and provider payload");
    await expect(repository.load(key)).rejects
      .toEqual(new PostgresTypesenseDeletionReceiptError("persistence_failed"));
  });

  test("rejects digest-valid but semantically impossible records before database I/O", async () => {
    const connection = new FakeConnection();
    const pool = new FakePool(connection);
    const repository = createPostgresTypesenseDeletionReceiptRepository(pool);
    const core = Object.freeze({
    ...
    const invalid = Object.freeze({
      ...core,
      receipt_digest: sha256({
        contract_version: "typesense-deletion-stored-receipt-v1", receipt: core,
      }),
    });
