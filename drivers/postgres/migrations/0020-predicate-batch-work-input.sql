-- P3 exact predicate-batch input persistence. The bounded predicate snapshot is
-- staged before work acceptance so a later worker can reconstruct the exact
-- question after process loss without re-planning against a changed graph.

CREATE TABLE omi_memory.memory_predicate_batch_work_inputs (
  account_id text NOT NULL,
  staged_input_id text NOT NULL CHECK (staged_input_id ~ '^pwi1_[0-9a-f]{64}$'),
  job_id text NOT NULL CHECK (length(job_id) BETWEEN 1 AND 256),
  input_version text NOT NULL CHECK (input_version = 'predicate-batch-work-staged-input-v1'),
  ...
  snapshot_version text NOT NULL CHECK (snapshot_version = 'predicate-batch-input-snapshot-v1'),
...
CREATE FUNCTION omi_memory.require_predicate_batch_work_input()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
BEGIN
  IF NEW.work_kind = 'predicate_batch' AND NOT EXISTS (
    SELECT 1
    FROM omi_memory.memory_predicate_batch_work_inputs AS i
    ...
    RAISE EXCEPTION USING ERRCODE = '23503', MESSAGE = 'predicate batch work input missing';
  END IF;
  RETURN NULL;
END;
$$;

CREATE CONSTRAINT TRIGGER memory_predicate_batch_acceptance_requires_input
AFTER INSERT ON omi_memory.memory_work_acceptances
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW
EXECUTE FUNCTION omi_memory.require_predicate_batch_work_input();

CREATE FUNCTION omi_memory.read_predicate_batch_work_input(
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION USING ERRCODE = '42501', MESSAGE = 'predicate batch work input access denied';
  END IF;
  ...
  IF capability = 'memories.work.execute' AND NOT EXISTS (
    ...
    RAISE EXCEPTION USING ERRCODE = '42501', MESSAGE = 'predicate batch work input lease denied';
  END IF;
...
CREATE FUNCTION omi_memory.insert_predicate_batch_work_input(
  requested_account_id text,
...
BEGIN
  ...
  INSERT INTO omi_memory.memory_predicate_batch_work_inputs (
...
REVOKE ALL ON omi_memory.memory_predicate_batch_work_inputs FROM PUBLIC;
REVOKE ALL ON FUNCTION omi_memory.require_predicate_batch_work_input() FROM PUBLIC;
REVOKE ALL ON FUNCTION omi_memory.read_predicate_batch_work_input(text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION omi_memory.insert_predicate_batch_work_input(
  text, text, text, text, bigint, text, text, text, text, text, text, jsonb, text, text
) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION omi_memory.read_predicate_batch_work_input(text, text)
  TO omi_platform_application;
GRANT EXECUTE ON FUNCTION omi_memory.insert_predicate_batch_work_input(
  text, text, text, text, bigint, text, text, text, text, text, text, jsonb, text, text
) TO omi_platform_application;

-- No direct table grant and no runtime composition. The trigger protects only
-- newly inserted predicate-batch acceptances; earlier rows remain unchanged.
