...
- **Carry-forward (test harness):** strict filename-alpha ordering (`test_ws_b_*` before `test_ws_i_*`) still fails without WS-I's `firebase_admin`/heavy-import stubs pre-installed — pre-existing, separate from the legacy `database._client` leak (fixed). Coordinator/explicit 16-module list order remains the supported single-process ordering.
...
- **Test-ordering leak (legacy `test_memory_temporal_brain.py`, 2026-06-23):** module-level `ModuleType` stub for `database._client` (only `document_id_from_seed`, no `db`) leaked across collection and broke `from database._client import db` in later WS modules. **Only `test_memory_temporal_brain.py` was the culprit** (`test_memory_category_auto` / `test_memories_validation` are self-contained mocks with no `sys.modules` stubs). Harness fix: defer stub + imports to module-scoped autouse fixture with `_AutoMockModule` + teardown restore — no assertion changes, no product code.
