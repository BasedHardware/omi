...
export const invokePredicateAlignment = async (model: PredicateAlignmentPort, predicates: readonly Predicate[], frontier: string): Promise<readonly PredicateAssertion[]> => {
  const input: PredicateAlignmentRequest = { predicate_frontier: frontier, predicates: predicates.map((predicate) => ({ predicate_id: predicate.predicate_id, name: predicate.display_name || predicate.identity_name, slot_ids: predicate.slot_ids })).sort((left, right) => left.predicate_id.localeCompare(right.predicate_id)) };
