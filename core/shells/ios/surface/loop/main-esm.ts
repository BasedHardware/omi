// Ship-origin probe suite. Runs top-to-bottom (top-level await = another ESM
// feature under test) and prints one PROBE line per fact. The shell scrapes
// these lines from the console relay.
...
// --- cookies (index.html response carried Set-Cookie: plain + Secure) -------
log(`document.cookie "${document.cookie}"`);

// --- same-origin fetch (does the surface reach its own origin?) -------------
try {
  const r = await fetch('/probe/echo', { headers: { 'x-probe': 'hi' } });
  log(`fetch same-origin ${r.status} ${j(await r.json())}`, 'ok');
} catch (e) { log(`fetch same-origin FAIL ${e}`, 'fail'); }
...
// --- extra external probes to separate CORS failure from no-network ---------
try {
...
} catch (e) { log(`fetch external-https-cors2(github) FAIL ${e}`, 'fail'); }

// --- external cleartext http (ATS posture from an http loopback page) -------
try {
...
if ('serviceWorker' in navigator) {
  ...
  } catch (e) { log(`serviceWorker present but register FAIL ${e}`, 'fail'); }
} else {
  log('serviceWorker API absent', 'fail');
}

// --- streaming fetch + client-side cancellation -----------------------------
try {
  ...
  ctl.abort(); // server should observe the abort — check shell log
  log(`stream read ${chunks} chunks then aborted client-side`, 'ok');
} catch (e) { log(`stream FAIL ${e}`, 'fail'); }

// --- websocket to the loopback origin (the /listen shape) -------------------
try {
  await new Promise<void>((res, rej) => {
    ...
    ws.onmessage = (ev) => {
      ...
      if (got >= 4) { ws.close(); res(); }
    };
    ws.onerror = () => rej(new Error('ws error'));
    setTimeout(() => rej(new Error('ws timeout')), 8000);
  });
  log('websocket ok', 'ok');
} catch (e) { log(`websocket FAIL ${e}`, 'fail'); }
