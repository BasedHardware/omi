...
ChatStreamHost makeHost(
  FakeChatNativeHttpClient client,
  List<String> scripts,
  ShellCredentialCustody custody,
) {
...
void main() {
  test(
    'stateful UTF-8 streams every byte split through the real JS sink before terminal',
    () async {
      ...
      final host = makeHost(
        client,
        scripts,
        ShellCredentialCustody('native-secret'),
      );
      ...
      expect(
        request.url.path,
        '/v1/chat-generations/generation%2Freconnect/events',
      );
  ...
  test(
    'credit pauses delivery, grant resumes exactly its count, and cancel closes the socket',
    () async {
  ...
  test(
    'duplicate malformed and wrong-channel frames cannot contaminate another session',
    () async {
      ...
      await host.handleMessage(
        frame(<String, Object>{
          ...
          'channel': 'chat-generation-events',
          'params':
              '{"generationId":"generation-safe","origin":"https://caller.invalid","token":"caller-token"}',
          'credit': 1,
        }),
      );
  ...
  test(
    'exact sign-out prevents a later Chat stream request before dispatch',
    () async {
      ...
      expect(
        parseSingleArgumentFrame(scripts.single)['failure'],
        'not-authenticated',
      );
      await host.close();
    },
